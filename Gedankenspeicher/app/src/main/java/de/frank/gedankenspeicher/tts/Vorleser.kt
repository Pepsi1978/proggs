...
class Vorleser(context: Context, private val einstellungen: Einstellungen) {
    ...
    /**
     * Welcher Absatz gerade gesprochen wird — Grundlage der wandernden Hervorhebung (M-09).
     *
     * −1 heisst: es spricht niemand. Der Index zaehlt in derselben Zerlegung wie
     * [Absaetze.teile], damit die Oberflaeche denselben Absatz hervorhebt, der wirklich
     * klingt — und nicht einen, den sie sich selbst zurechtgeschnitten hat.
     */
    private val _absatzNr = MutableStateFlow(-1)
    val absatzNr: StateFlow<Int> = _absatzNr

    /** Woran gerade vorgelesen wird (Notiz-Kennung oder KI-Antwort-Kennung), sonst null. */
    private val _quelle = MutableStateFlow<String?>(null)
    val quelle: StateFlow<String?> = _quelle

    /** Setzt die Kennung fuer den naechsten Vorlesevorgang. */
    fun merkeQuelle(kennung: String?) { _quelle.value = kennung }
