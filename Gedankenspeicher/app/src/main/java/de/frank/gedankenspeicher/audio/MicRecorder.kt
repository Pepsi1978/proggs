...
class MicRecorder(context: Context) {
    ...
    /**
     * Der gemessene Pegel, 0f bis 1f — für den pulsierenden Ring am Aufnahmeknopf (M-05).
     *
     * Ohne ihn wäre das Pulsieren eine bloße Zierde. Mit ihm sieht Frank, dass das Mikrofon
     * ihn wirklich hört, und merkt sofort, wenn es das nicht tut.
     */
    ...
    /**
     * Der Spitzenpegel eines Puffers, auf 0f..1f gebracht.
     *
     * Gemessen wird der lauteste Ausschlag, nicht der Durchschnitt: der Ring soll auf das
     * gesprochene Wort reagieren, nicht auf die Stille dazwischen mitteln.
     */
    private fun pegelVon(puffer: ByteArray, gelesen: Int): Float {
        var spitze = 0
        var i = 0
        while (i + 1 < gelesen) {
            val wert = ((puffer[i + 1].toInt() shl 8) or (puffer[i].toInt() and 0xFF)).toShort().toInt()
            val betrag = if (wert < 0) -wert else wert
            if (betrag > spitze) spitze = betrag
            i += 2
        }
        return (spitze / 32768f).coerceIn(0f, 1f)
    }
