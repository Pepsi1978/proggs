...
class CodexAuthManager(context: Context) {
    ...
    // ---------------------------------------------------------------------------------
    // Die vier Aufgaben, die Gedankenspeicher an Codex gibt.
    //
    // Sie sind bewusst schmal: Kontext hinein, Text heraus. Alles, was PerfectMoment an
    // Fragenlisten, Perspektiven und Symbolen brauchte, ist hier entfallen — dieser Weg
    // trägt nur noch das, was die Notizen-App wirklich schickt.
    // ---------------------------------------------------------------------------------

    /** F-05: die Überschrift einer einzelnen Notiz. Leer, wenn nichts Brauchbares kam. */
    suspend fun ueberschriftFuer(
        notiztext: String,
        model: CodexModel,
        effort: ReasoningEffort,
    ): String = withContext(Dispatchers.IO) {
        if (notiztext.isBlank()) return@withContext ""
        einzeiler(requestCodexResponse(kurztextPayload(UEBERSCHRIFT_AUFTRAG, notiztext, model, effort), false))
    }

    /** F-12: der Titel einer Sitzung, aus ihrer ersten Notiz. */
    suspend fun sitzungstitelFuer(
        ersteNotiz: String,
        model: CodexModel,
        effort: ReasoningEffort,
    ): String = withContext(Dispatchers.IO) {
        if (ersteNotiz.isBlank()) return@withContext ""
        einzeiler(requestCodexResponse(kurztextPayload(SITZUNGSTITEL_AUFTRAG, ersteNotiz, model, effort), false))
    }
    ...
    /**
     * F-09, erster Schritt: die eine Rückfrage, die sich auf den Inhalt der Notizen bezieht.
     *
     * Kommt keine brauchbare Frage zurück, wird **nicht** heimlich eine Standardfrage
     * untergeschoben — der Aufrufer sieht den leeren Text und entscheidet selbst.
     */
    suspend fun stelleRueckfrage(
        notizen: String,
        model: CodexModel,
        effort: ReasoningEffort,
    ): String = withContext(Dispatchers.IO) {
