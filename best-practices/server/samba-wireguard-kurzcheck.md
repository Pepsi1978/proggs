...
| 9 | Grosse Dateien ueber den Tunnel schieben | **SFTP statt SMB** (1 x 48 MB: 37,1 statt 10,2 Mbit/s; 6 x 8 MB: 40,5 statt 26,7 bei 44 Mbit/s Maximum). Kleine Dateien bleiben bei SMB (18,7 statt 4,4 Dateien/s). Nach SFTP `chown` auf den Samba-User (SSH schreibt als root), Share gehoert in den rclone-PFAD, jede Messung serverseitig gegenpruefen, SMB als Rueckfall behalten. | §9 |
| 10 | Windows-Client schnell machen | Einmalig `windows/cortex-tuning.ps1` (Admin): Caches 10→60 s, `MaxCmds` 50→2048, Durchsatzbremse aus → fluessiges Blaettern, rund **2x**. Dazu `windows/cortex-menu-install.ps1`: legt "Cortex: Hier schnell einfuegen" ins Kontextmenue, damit der schnelle Weg auch der bequeme ist (Strg+C → Rechtsklick). Befehl mit `powershell.exe -STA` starten, mehrere Dateien per `--files-from` buendeln. Signing/`smb encrypt` NICHT abschalten. | §5 |
