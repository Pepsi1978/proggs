...
> **Zweite Seite der Medaille zum Bug-Almanach** `~/proggs/bugs/python-windows.md`: dort steht
...
## ⚡ TL;DR — die wichtigsten Regeln zuerst

1. **`open()` IMMER mit `encoding='utf-8'`** — sonst cp1252-Default auf Windows. Beim Lesen evtl.
   BOM-behafteter Dateien `encoding='utf-8-sig'`. (§1.1, §1.2)
2. **JSON: `json.dump(data, f, ensure_ascii=False)` PLUS `open(..., encoding='utf-8')`** — beides
   noetig; nur einer reicht nicht. JSON crash-sicher: erst `json.dumps` zu String, dann atomar. (§1.3, §2.6)
3. **Generierte Text-/JSON-Dateien `newline='\n'`**, CSV `newline=''` — sonst CRLF/Leerzeilen auf
   Windows. (§1.4)
4. **Atomar schreiben** gegen abgeschnittene Dateien: tempfile im SELBEN Verzeichnis → `write` →
   `flush` → `os.fsync` → `close` → **`os.replace`** (nicht `os.rename`). (§2)
5. **Pfade nur ueber `pathlib.Path`** — `Path.home()`/`expanduser()`, `as_posix()` fuer Tools/Configs,
   NIE `/c/Users/...` hardcoden. (§3.1–§3.6)
6. **`subprocess`: Args als Liste**, kein `shell=True`/`shlex`, `text=True, encoding='utf-8',
   errors='replace', check=True`. (§3.7)
7. **`print()`/`logging` mit Umlaut/Emoji absichern**: `sys.stdout.reconfigure(encoding='utf-8',
   errors='backslashreplace')`, `FileHandler(encoding='utf-8')`, `print(flush=True)` in Pipes/Hooks. (§1.8)
8. **venv pro Projekt (`.venv`)**, IMMER `python -m pip` (nie nacktes `pip`), `py`-Launcher fuer
   Versionswahl, `.venv` nie committen. (§4)
9. **CLI: `main()`-Pattern + `raise SystemExit(main())`**, `argparse` als Baseline, korrekte
   Exit-Codes (0/1/2/130). (§5)
10. **Moderne Typen** (`list[str]`, `str | None`), **ruff** als Linter+Formatter, beim Entwickeln
    `EncodingWarning` aktivieren (`-X warn_default_encoding`). (§6)
11. **Globale Notbremse fuer Fremdcode**: `PYTHONUTF8=1` / `python -X utf8` — aber im eigenen Code
    bleibt explizites `encoding=` robuster (reist mit dem Code mit). (§1.6)

---

## 🔗 Bezugs-Tabelle: Best-Practice ↔ Bug-Almanach

| Best-Practice (diese Datei) | Failure-Mode im Almanach `bugs/python-windows.md` |
|---|---|
