...
> **Die "richtige Seite der Medaille" zum Bug-Almanach [`~/proggs/bugs/mcp-server.md`](../../../bugs/mcp-server.md).**
...
> - **Diese Datei** = **Server BAUEN** (Server-Autor-Seite, Projekt-Code) — Gegenstueck zu `bugs/mcp-server.md`.
> - `best-practices/05-mcp/` = **Server KONFIGURIEREN/VERBINDEN in Claude Code** (Harness-Seite).
> - `bugs/claude-hooks.md` = **Hooks** (MCP nur am Rand: Matcher, MCP-Tool-als-Hook).
>
...
Best-Practices (diese Datei) ↔ Bug-Almanach [`~/proggs/bugs/mcp-server.md`](../../../bugs/mcp-server.md). Die identische Tabelle steht auch dort. Links die *richtige Arbeitsweise*, rechts die *Falle, die sie verhindert*.

| Best-Practice-Abschnitt (hier) | Zugehoeriger Bug-Almanach-Abschnitt (`bugs/mcp-server.md`) |
|--------------------------------|------------------------------------------------------------|
...
*Neue belegte Best-Practices hier ergaenzen (mit Quelle + Datum + `offiziell`/`extern`). Bei jedem neu erlebten Bug zusaetzlich `bugs/mcp-server.md` ergaenzen und diese Bezugstabelle synchron halten. Bei SDK-Versionssprung (1.30+, Spec-Revision) Re-Check von B (Schema-Verhalten), D7 (Timeout-Default) und G5 (Fix-Status).*
