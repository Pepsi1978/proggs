...
> **Zweite Seite der Medaille zum Bug-Almanach** ([`bugs/jetpack-compose.md`](../../../bugs/jetpack-compose.md)):
...
## 🔗 Bezug zum Bug-Almanach ([`bugs/jetpack-compose.md`](../../../bugs/jetpack-compose.md))
...
| Best-Practice (hier) | Verhindert Bug(s) in `bugs/jetpack-compose.md` |
|----------------------|------------------------------------------------|
...
- **Fremd-Modul-Typen** (kein Compose-Compiler, z.B. `java.time.LocalDateTime`) gelten als instabil →
  ...
  *(offiziell: /performance/stability/fix)* *(Plugin-Setup im Build → [`gradle.md`](../gradle/best-practices.md))*
...
- **IMMER im Release-Build (R8 / `minifyEnabled true`) messen** — Debug ist by-design langsam und
  maskiert echte Performance. *(offiziell: /develop/ui/compose/performance)* (Bug-Almanach §10.1) *(Build-Config → [`gradle.md`](../gradle/best-practices.md))*
...
- Gegenstueck-Bugs immer auch in [`bugs/jetpack-compose.md`](../../../bugs/jetpack-compose.md) pflegen
