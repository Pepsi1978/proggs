...
> **Die „richtige Seite der Medaille" zum Bug-Almanach** [`bugs/stream-deck.md`](../../../bugs/stream-deck.md).
...
## TL;DR — die 12 wichtigsten Best Practices

1. **Modernes Node-SDK fuer Neues, klassisches SDK pflegen.** Neuanlage immer per `streamdeck create` (Node-SDK, `SingletonAction`, TypeScript, SDKVersion 3). Das klassische HTML/JS-SDK (TVO) bleibt gueltig, ist aber die aeltere Generation.
2. **`registerAction(...)` VOR `streamDeck.connect()` — `connect()` ist die letzte Zeile** der Entry-Datei. (Node-SDK)
3. **`context` (`ev.action.id`) ist die einzige Instanz-Identitaet** — nie Koordinaten, nie extern als Langzeit-ID persistieren. Pro context ein State-Eintrag (Map oder Settings).
4. **Bei `willAppear` idempotent initialisieren, bei `willDisappear` Timer stoppen + aufraeumen.** Kein Doppel-Timer, keine Ghost-Contexts.
5. **`setSettings` ersetzt das GANZE Objekt** — immer `{...current, feld}` aus den Event-Args. Settings aus `ev.payload.settings` lesen, NICHT synchron `getSettings`.
6. **Secrets/API-Keys/Tokens NUR in `globalSettings`** (verschluesselt) — nie Action-Settings (Klartext, Profil-Export), nie ins Bundle.
7. **An den PI erst senden, wenn er offen ist** (`streamDeck.ui.current` pruefen / nach `propertyInspectorDidAppear`). Im Node-SDK fetch-like Routing mit Status+Body bevorzugen.
8. **Property Inspector mit `sdpi-components` bauen** (lokal eingebunden, `setting="key"` bindet automatisch, KEIN Save-Button).
9. **Dynamische Tasten: ganze Taste als SVG via `setImage()` + `setTitle("")`** — umgeht den Titel-Cache, volle Layout-Kontrolle. Mehrere Manifest-`States` nur fuer echte Toggles.
10. **Max 10 Updates/s, nur bei echter Aenderung rendern (Dedup/Coalescing).** Push statt Poll: SSE/`EventSource` ist nicht vom Chromium-Background-Throttling betroffen, `setInterval` im Webview schon.
11. **Node-Backend Sandbox-konform:** keine nativen `.node`-Module (`--no-addons`), alle Deps bundeln (`--no-global-search-paths`), globale `uncaughtException`/`unhandledRejection`-Handler, `streamDeck.logger` statt `console.log`.
12. **Distribution nur mit `streamdeck validate` → `streamdeck pack`** — niemals `Compress-Archive` (Backslash-ZIP-Falle). UUID nach Release einfrieren.
...
> Verknuepft jede Best-Practice mit der konkreten Bug-Loesung in [`bugs/stream-deck.md`](../../../bugs/stream-deck.md).

| Best-Practice-Abschnitt (hier) | Almanach-Bug-Abschnitt (bugs/stream-deck.md) |
|--------------------------------|----------------------------------------------|
