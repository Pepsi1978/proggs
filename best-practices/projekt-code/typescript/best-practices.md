...
> **Die "richtige Seite der Medaille" zum Bug-Almanach `~/proggs/bugs/typescript.md`.**
...
Best-Practices (diese Datei) <-> Bug-Almanach `~/proggs/bugs/typescript.md`. Die identische Tabelle
steht auch dort. So bleibt jede Best-Practice mit ihrer konkreten Bug-Loesung verlinkt (und umgekehrt).

| Best-Practice-Abschnitt (hier) | Zugehoeriger Bug-Almanach-Abschnitt (`bugs/typescript.md`) |
|--------------------------------|-----------------------------------------------------------|
...
*Neue belegte Best-Practices hier ergaenzen (mit Quelle + Datum + `offiziell`/`extern`). Bei jedem neu erlebten Bug zusaetzlich `bugs/typescript.md` ergaenzen und die Bezugstabelle synchron halten. Bei Versionssprung (Node 26, TS 7.0 "Corsa") Re-Check von A (tsconfig-Defaults) und F (Node-APIs).*
