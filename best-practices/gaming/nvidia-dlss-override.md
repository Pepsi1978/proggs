...
- Im Spiel: Grafik → Qualität → „Upscaling/Sharpening“ = NVIDIA DLSS, Stufe Quality (1440p) bzw. Balanced/Quality (4K).
- **Vor Ort verifiziert (21.09.2026, aktuelle Steam-Version):** Die Optionen stehen in `Documents\Call of Duty MWII\players\options.3.cod22.cst`. Dort gibt es `AATechniquePreferred` mit **DLAA** als Wahl (die Web-Quellen „kein DLAA in MW2“ sind veraltet), `DLSSPerfMode`, `DLSSSharpness` (-1 bis 1) und `NvidiaReflex` = Disabled/Enabled/„Enabled + boost“. Im Menü war Reflex für den Nutzer weder unter „Qualität“ noch unter „Anzeige“ zu finden. Die Datei nur bei **geschlossenem** Spiel ändern, sonst wird sie beim Beenden überschrieben.
- Mit dem Transformer-Modell `DLSSSharpness` niedrig halten (0 bis 0,2), weil das neue Modell selbst scharf zeichnet und hohe Werte überschärfen. https://www.charlieintel.com/call-of-duty-modern-warfare-2/best-modern-warfare-2-pc-settings-199966/
