...
| 13 | Plugin darf NIE aufs Terminal schreiben (KRITISCH) | Plugin läuft IM TUI-Prozess → jeder TTY-Schreibvorgang zerstört die TUI. NIE `console.*` (→ `client.app.log`); jeden `$`-Aufruf mit `.quiet()` (Bun echoed stdout sonst ans TTY). Almanach §6 #48a | §2 |
