# xAI Grok API — Best Practices (Stand 2026-06-09)
...
- `high` fuer Mathe/Mehrschritt-Logik, `none` nur bei reiner Geschwindigkeit. Bei `grok-4.20-multi-agent` steuert derselbe Parameter die Agent-ANZAHL, nicht die Denktiefe. Quelle: https://docs.x.ai/developers/model-capabilities/text/reasoning · offiziell
...
- Limits: max 2048 Zeichen/String, 256 Array-Items, 64 Properties. `additionalProperties` explizit `false`, keine zirkulaeren `$ref`, `anyOf`/`oneOf` statt komplexem `allOf`. Best-effort-Keywords (`not`, `if/then/else`) zusaetzlich client-seitig validieren. Quelle: https://docs.x.ai/developers/model-capabilities/text/structured-outputs · offiziell
...
- Bilder: `enable_image_understanding` (Bildanalyse via `view_image`) und `enable_image_search` getrennt; Zitate kommen automatisch. Verbrauch ueberwachen (Kosten pro Quelle nicht klar dokumentiert). Quelle: https://docs.x.ai/docs/guides/live-search · offiziell
...
- In TPM zaehlen Prompt-, Completion-, Reasoning- UND gecachte Prompt-Tokens (letztere zu reduziertem Preis abgerechnet, aber zaehlen mit) → Reasoning-Budget einplanen. Quelle: https://docs.x.ai/docs/key-information/consumption-and-rate-limits · offiziell
...
| 5 Live Search | 15 |
| 6 Streaming | 3, 4, 5, 6 |
| 7 Rate Limits & Token-Budget | 10, 11, 12 |
| 8 SDK & Auth | 13, 14 |
