...
VERSION = "0.58.0 (07.07.2026, 20:35 Uhr)"  # 0.58.0: Feature-Chronik "Was Cortex kann" komplett ueberarbeitet — alle Eintraege in leichtes, verstaendliches Deutsch (Fachbegriffe/Namen/Zahlen wie BM25, RRF, Qdrant, ENV-Variablen, Endpunkte ERHALTEN + je kurz erklaert), aehnliche/redundante zusammengefasst (55 -> 32 Eintraege, ~36% kleiner) — optimiert fuers Werkzeug was_kann_cortex (Hauptagent liest alle auf einmal). Dauer-Schreibregel im features.json-hinweis + DEPLOY.md verankert. 0.57.2: Sichtbarkeits-Bump zum agent-0.63.0-Deploy (neues Werkzeug was_kann_cortex — der Hauptagent kann die System-Info-Chronik live lesen). 0.57.1: FIX System-Info-Chronik zeigte neue Eintraege nicht (Frank-Bug 2026-07-07). Root Cause: /api/features liest die PERSISTENTE Kopie (dashboard-data/features.json), die nur EINMAL aus dem Image-Seed kopiert wird — seit sie am 06.07. entstand, wurde JEDER neue Deploy-Eintrag ignoriert (Turn-Logbuch 13:09 UND Werkzeugkasten 15:44 fehlten in der Anzeige). Fix: _read_features_data merged jetzt fehlende Seed-Eintraege (per id) vorne ein + persistiert einmalig (Poka-Yoke: kuenftige Eintraege erscheinen automatisch, editierte bleiben erhalten). 0.57.0: Agenten-Umbau Schritt 2 — WERKZEUGKASTEN des Hauptagenten gebaut + deployed (agent 0.62.0): die 4 Lese-/Such-Werkzeuge (Gehirn durchsuchen -> Schnipsel + gezielt Volltext laden, Web-Suche, Logbuch lesen). Isoliert am Star-Trek-Fall getestet (grün: Hauptagent fand 50 Einträge, nutzte Franks Notizen). NOCH NICHT im echten Chat aktiv — Integration in _process_turn ist der nächste Schritt. Zuvor auch der Tool-Loop-Motor (codex_generate_tools) + agent.jsonl-Persistenz. Alt: 0.56.1 (07.07.2026, 14:27 Uhr) Kategorie-Sortierung Alphabet.

VERSION = "0.62.1 (08.07.2026, 17:35 Uhr)"  # 0.62.1: Sichtbarer Bump zum agent-0.65.1-Fix (history_compress_at erscheint jetzt korrekt in /config, damit das neue Feld 'Verlauf komprimieren ab' den gespeicherten Wert anzeigt). 0.62.0 (Frank-Wunsch 2026-07-08): NEUE Einstellung 'Verlauf komprimieren ab' in den Qdrant-Limits (agent 0.65.0) — ab X Chat-Nachrichten (deine + Cortex' Antworten zusammen) verdichtet der Hauptagent den ganzen Gespraechsverlauf KUMULATIV zu einer Zusammenfassung (statt aeltere Nachrichten hart abzuschneiden), spart Kontext ohne Detailverlust; 0 = aus. Alt: 0.61.0 (Agenten-Umbau Schritt 2, Frank-Auftrag 2026-07-08): Der WERKZEUGKASTEN des Hauptagenten ist jetzt im ECHTEN Chat scharfgeschaltet (agent 0.64.0). Die generischen Antwort-Zweige (query/query_internet/internet/smalltalk) in _process_turn wurden durch EINEN Tool-Loop ersetzt: der GPT-5.5-Hauptagent durchsucht das Gedaechtnis selbst, laedt gezielt Volltexte, sucht bei Bedarf im Web, liest das Logbuch — er filtert selbst (Antwort- vs. Kontext-Treffer) statt eines separaten Leseagenten. Loest den Star-Trek-Bug (Rohsuche fand 50, Leseagent waehlte 0). Router + komplette Speicher-Sicherheit + deterministische Spezial-Antworten (Projektstand, Kategorie-Gesamt/-Zaehlung) bleiben UNANGETASTET; Verlauf wird eingespeist (Follow-ups), Quellen-Chips + Confidence bleiben. Alt: 0.60.0 (08.07.2026, 15:02 Uhr) Bibliothekar-Prompts blockieren Handy-Kategorien.
VERSION = "0.63.0 (08.07.2026, 19.37 Uhr)"  # 0.63.0 (Frank-Wunsch 2026-07-08): Embedding-Modell-Statusanzeige in Einstellungen -> Agenten (links neben 'Speichern'): read-only Schalter, links 'Modell 1', rechts 'Modell 2', markiert das LIVE aktive Modell (aus /health embed_model). Nur Anzeige — kein Umschalten (Modellwechsel erfordert Neu-Einbettung). Passt zum brain-api-1.26.0-Umstieg auf gemini-embedding-2 (3072). Alt: 0.62.1.
VERSION = "0.63.1 (08.07.2026, 19.55 Uhr)"  # 0.63.1 (Frank-Wunsch): Embedding-Statusanzeige zeigt die echten Modellnamen 'Gemini Embedding 001' (links) / 'Gemini Embedding 2' (rechts) statt 'Modell 1/2'. Alt: 0.63.0.
...
VERSION = "0.67.4 (10.07.2026, 18:45 Uhr)"  # 0.67.4: Sichtbarer Bump zum librarian-0.11.3-Fix: Kategorie-Vorschlaege des Nachtschicht-Bibliothekars ergaenzen vorhandene Kategorien, statt sie zu ersetzen. Alt: 0.67.3.
...
VERSION = "0.77.0 (12.07.2026, 18:29 Uhr)"  # 0.77.0 PERFORMANCE-Tiefendebugging (Stack-Deploy #47856): (1) Chat-Proxy-Timeout 60s -> 300s — echte Chat-Laufzeiten liegen bei 20-74s, das 60s-Cap warf fertige Antworten weg ('Antwort kommt nie an'). (2) Loesch-Aktionen im Cockpit brechen nach 30s sichtbar ab statt endlos still zu haengen. Dazu agent 0.77.0 (geloeschte Gespraeche bleiben geloescht: Live-Session-Eviction + Mirror-Tombstone; Selbstregel-Pruefung NACH der deterministischen Bereinigung, erspart ~28s Neugenerierung) + brain-api 1.33.0 (BM25-Rebuild debounced im Hintergrund, Single-Flight) + WireGuard PersistentKeepalive 25s serverseitig. Alt: 0.76.6.
VERSION = "0.78.0 (12.07.2026, 19:30 Uhr)"  # 0.78.0: GPT-5.6 Sol, Terra und Luna sind in den Agenten-Dropdowns jeweils normal und als Fast-Auswahl verfügbar; Fast sendet über agent 0.78.0 service_tier=priority an das jeweilige Basismodell. Alt: 0.77.0.
VERSION = "0.78.1 (12.07.2026, 19:54 Uhr)"  # 0.78.1 PERFORMANCE Handy<->Server: agent 0.78.1 beantwortet /config sofort (Codex-Modellliste aus 10-Min-Cache mit Stale-while-Revalidate + Warmup beim Start statt Live-Call zu chatgpt.com mit bis zu 12s); Cortex Android 0.8.1 zeigt die Agenten-Modelle beim Oeffnen der Einstellungen sofort aus dem lokalen Zwischenspeicher und aktualisiert still im Hintergrund — die Eieruhr entfaellt. Alt: 0.78.0.
VERSION = "0.79.0 (12.07.2026, 20:19 Uhr)"  # 0.79.0 CHAT-STREAMING im Web-Cockpit (Frank-Auftrag #47862): NEU POST /api/chat/stream (SSE-Durchreiche an agent /chat/stream) — die Antwort erscheint im Chat ab dem ERSTEN Absatz statt erst am Ende, Heartbeats halten die Verbindung durch lange Web-/Regel-Phasen offen (kein Timeout-/Tunnel-Risiko mehr). Vorgelesen wird ab dem ersten VOLLSTAENDIGEN Absatz (TTS-Warteschlange), ausschliesslich der kanonische regelgeprüfte Text (canonical_reply, §9.1). Faellt der Stream aus, greift automatisch der bisherige /api/chat-Weg (fail-closed). Optik unveraendert. Alt: 0.78.1.
...
CONV_CATEGORY = os.getenv("DASH_CONV_CATEGORY", "Gespräche")  # deutsche Umlaute; Altbestand mit kleinem g bleibt per casefold erkannt
...
def _read_features_data() -> dict:
    ...
    Danach werden bei jedem Lesen NEUE Seed-Eintraege (id noch nicht vorhanden) vorne ergaenzt und
    persistiert — so erscheint JEDER neue Deploy-Feature-Eintrag automatisch, ohne im Dashboard
    editierte Eintraege zu ueberschreiben. FIX 2026-07-07: Seit die persistente Datei existierte,
...
async def api_websearch_toggle_test() -> dict:
    """Kostenfreier Selbsttest der Tavily-Schalterlogik. Proxy an agent /websearch-toggle-selftest."""
    try:
        return await asyncio.to_thread(_aget, "/websearch-toggle-selftest")
    except Exception as e:  # noqa: BLE001
...
def api_limits_recommendation() -> dict:
    ...
    large_corpus = total >= 600 or category_count >= 30
    very_large_corpus = total >= 1500 or category_count >= 60
    long_docs = avg_chars >= 8000 or avg_chunks_per_doc >= 3 or largest_chars >= 40000
    very_long_docs = avg_chars >= 18000 or avg_chunks_per_doc >= 6 or largest_chars >= 90000

    # RAG-Best-Practice: normale Antworten brauchen einen fokussierten Kandidaten-Korridor.
    # "Alles lesen" bleibt expliziten Vollabrufen vorbehalten, sonst sinkt die Praezision (Lost-in-the-Middle/Rauschen).
    recall_normal = 25
    if total >= 200 or category_count >= 15:
        recall_normal = 35
    if large_corpus:
        recall_normal = 40
    if very_large_corpus or broad_topic >= 200:
        recall_normal = 50
    recall_normal = _clampi(recall_normal, 20, 50)

    multi_query = 3 if total >= 400 or category_count >= 20 else 2
    snippet = 1800 if very_long_docs else (1500 if long_docs else 1200)
    answer_hit = 12000 if very_long_docs else (9000 if long_docs else 7000)
    answer_total = 48000 if very_long_docs else (40000 if long_docs or large_corpus else 32000)
    answer_tokens = 8192 if answer_total >= 48000 else 6144
    work_threshold = 12 if very_long_docs else (15 if large_corpus or long_docs else 20)
    full_batch = 28000 if long_docs else 24000
    dedup_candidates = 5 if total >= 250 else 3
    entity_extract = 4000 if long_docs else 3000

    overfetch = 4
    if avg_chunks_per_doc >= 6:
        overfetch = 8
    elif avg_chunks_per_doc >= 3 or long_docs:
        overfetch = 6
    bm25_factor = 5 if category_count >= 20 or total >= 400 else 4
    bm25_min = 40 if large_corpus else (30 if total >= 200 or category_count >= 15 else 20)
    chunk_chars = 3200
    chunk_overlap = 400
    ...
    recommended = {
        ...
        "brain": {
            ...
            "search_date_overfetch_factor": 25 if total >= 1000 else 20,
            "bm25_candidate_factor": bm25_factor,
            "bm25_min_candidates": bm25_min,
        },
    }
    reasons = {
        "agent.recall_normal_max": "Fokussiertes Top-k: genug Recall für RAG, aber kein Rauschen durch Vollabruf.",
        "agent.recall_full_limit": "Explizite Vollfragen bleiben vollständig und laufen über Arbeitscache/Kategorie-Leser.",
        "agent.multi_query_variants": "Zwei bis drei Query-Varianten verbessern Synonyme ohne Suchrauschen zu vervielfachen.",
        "agent.entity_extract_chars": "Lange Einträge bekommen genug Fenster für Entity-Linking, kurze bleiben günstig.",
        "agent.entity_docs_limit": "Entity-Fragen dürfen vollständig sein; der Normal-Recall bleibt davon getrennt.",
        "agent.lese_snippet_chars": "Der Leseagent bekommt kompakten, aber ausreichenden Kontext pro Kandidat.",
        "agent.answer_hit_chars": "Pro Quelle bleibt genug Text, ohne einzelne Dokumente die Antwort dominieren zu lassen.",
        "agent.answer_total_chars": "Gesamtbudget schützt vor Lost-in-the-Middle und lässt 5-10 gute Quellen zu.",
        "agent.answer_max_tokens": "Ausgabe reicht für begründete Antworten, bleibt aber kein Freibrief für Endloszusammenfassungen.",
        "agent.recall_working_cache_threshold": "Große Vollabrufe werden früh blockweise verdichtet.",
        "agent.full_category_batch_chars": "Kategorie-Vollabrufe werden in stabilen Blöcken statt in einem Riesenkontext verarbeitet.",
        "agent.dedup_candidates": "Dublettenprüfung bleibt leicht, aber breit genug für wachsende Bestände.",
        "agent.history_max": "Dialogkontext bleibt verständlich, ohne Retrieval-Kontext zu verdrängen.",
        "agent.context_prompt_max_chars": "Zusatzaufträge bleiben nützlich, aber begrenzt.",
        "agent.chat_text_max_chars": "Große Eingaben bleiben möglich; stille Kürzung bleibt verboten.",
        "brain.search_overfetch_factor": "Chunk-Level-Suche holt genug Rohpunkte für Dokument-Deduplizierung.",
        "brain.search_date_overfetch_factor": "Zeitfragen behalten Reserve für Filter-Fallbacks.",
        "brain.bm25_candidate_factor": "Hybrid-Suche stärkt exakte Namen, Zahlen, IDs und Fehlercodes.",
        "brain.bm25_min_candidates": "BM25 bleibt auch bei kleinem Top-k breit genug.",
        "brain.chunk_chars": "Künftige Chunks liegen näher am RAG-Startbereich von ca. 512-800 Tokens.",
        "brain.chunk_overlap": "Overlap liegt im Best-Practice-Bereich von ca. 10-20 Prozent.",
    }
    return {
        "ok": True,
        ...
        "analysis": {
            ...
            "quality_profile": "Normale RAG-Suche = 20-50 finale Dokumentkandidaten; Vollständigkeit nur bei expliziten Vollabrufen; Hybrid für exakte Begriffe; Chunks näher an 512-800 Tokens mit 10-20% Overlap.",
...
async def api_store(request: Request) -> dict:
    ...
    if category:
        payload["category"] = category[:120]
    try:
        return await asyncio.to_thread(_bpost, "/store", payload)
    except Exception as e:  # noqa: BLE001
        _log(logging.WARNING, "Store-Proxy fehlgeschlagen", err=str(e), title=title or None, category=category or None)
        return JSONResponse(status_code=502, content={"ok": False, "detail": f"Speichern fehlgeschlagen: {type(e).__name__}"})


def _build_chat_payload(body: dict) -> "tuple[dict | None, str | None]":
    """Gemeinsamer Payload-Bau fuer /api/chat und /api/chat/stream (identische Validierung).
    Gibt (payload, None) oder (None, fehler_text) zurueck."""
    text = body.get("text") or ""
    if not text.strip():
        return None, "Leere Nachricht — schreib mir was zum Ablegen oder eine Frage."
    ...
    if len(text) > MAX_STORE_CHARS:
        return None, (f"Der Text ist mit {len(text)} Zeichen zu lang "
...
async def api_chat_stream(request: Request):
    """SSE-Durchreiche an agent /chat/stream (Frank 2026-07-12: Antwort erscheint im Cockpit ab dem
    ersten Absatz statt erst am Ende). Der Agent streamt NUR den kanonischen finalen Text (ready-Event
    ...
    def gen():
        if payload is None:
            yield ("data: " + json.dumps({"type": "error", "message": err}, ensure_ascii=False) + "\n\n").encode("utf-8")
            return
        try:
            # read=60s pro Chunk reicht: der Agent sendet spaetestens alle 15s einen Heartbeat.
            with _HTTP.stream("POST", f"{AGENT_URL}/chat/stream", json=payload, headers=HEADERS,
                              timeout=httpx.Timeout(300.0, connect=10.0, read=60.0)) as r:
                r.raise_for_status()
                for chunk in r.iter_raw():
                    yield chunk
        except Exception as e:  # noqa: BLE001 — Fehler als SSE-Event melden; der Client faellt auf /api/chat zurueck
            _log(logging.WARNING, "Chat-Stream-Proxy fehlgeschlagen", err=str(e))
            yield b'data: {"type": "error", "message": "proxy"}\n\n'
...
def api_get_entry_categories(doc_id: str) -> dict:
    ...
    if not doc_id:
        return JSONResponse(status_code=400, content={"ok": False, "detail": "doc_id erforderlich"})
    try:
        return _bget("/entry/categories", doc_id=doc_id, user_id=USER_ID)
    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            return JSONResponse(status_code=404, content={"ok": False, "detail": "Eintrag nicht gefunden"})
        _log(logging.WARNING, "Gezielter Kategorie-Abruf fehlgeschlagen", doc_id=doc_id, status=e.response.status_code)
...
async def api_improve(request: Request) -> dict:
    ...
    try:
        ...
        if extra:
            payload["extra_instruction"] = extra[:2000]
        return await asyncio.to_thread(_apost, "/improve", payload)
    except Exception as e:  # noqa: BLE001
