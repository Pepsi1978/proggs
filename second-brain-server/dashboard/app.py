...
def entries_by_parent(parent: str = "", limit: int = 200) -> dict:
    """Alle Eintraege UNTER einer Hauptkategorie (Phase 4 — Uebersicht-Baum-Filter). Proxy an brain
    /by-parent. Sync def -> Threadpool (fastapi §1, _bget ist sync httpx). Graceful: leere Liste statt
    500, damit die Uebersicht nutzbar bleibt."""
    p = parent.strip()
    if not p:
        return {"mode": "parent", "items": []}
    limit = max(1, min(limit, 500))
    try:
        d = _bget("/by-parent", parent=p, user_id=USER_ID)
        return {"mode": "parent", "parent": p, "items": d.get("items", [])[:limit]}
    except Exception as e:  # noqa: BLE001
...
async def api_chat(request: Request) -> dict:
    ...
    if len(text) > 100000:   # Eingabe-Limit (fastapi §8) gegen Missbrauch/OOM, aber GROSSZUEGIG fuer Inhalte
        text = text[:100000]   # FIX 2026-06-25: frueher 8000 -> schnitt lange Eintraege (Almanache) mitten ab. brain-api chunkt beliebig lange Texte selbst.
