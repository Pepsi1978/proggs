...
def gemini_generate_with_native_web(system: str, user: str, model: str, max_tokens: int, temperature: float) -> str:
    ...
    resp = gclient.models.generate_content(model=model, contents=user, config=config)
...
def hauptagent_answer_native_web(session: dict, question: str) -> str:
    ...
    if _is_gemini(model):
        return gemini_generate_with_native_web(
