...
# (Stand 2026-07-01). Zentrale Quelle fuer /config -> Dashboard + Android-App zeigen sie an. minimax laeuft
# ueber OpenCode-Zen-Abo, gpt-5.x ueber ChatGPT-Abo (nicht pro-Token abgerechnet) -> Preis None (= "Abo").
MODEL_PRICES = {
...
# Reihenfolge der Reasoning-Stufen fuers Deckeln (Performance): Der ROUTER-Aufruf (reine
# JSON-Klassifikation) braucht kein tiefes Denken — bei gpt-5.5 kostete allein das Routing
# mit 'medium' oft 15-40s, BEVOR die eigentliche Antwort startete (Frank-Vorfall 2026-07-02).
_EFFORT_ORDER = ["none", "minimal", "low", "medium", "high", "xhigh"]


def _cap_effort(effort: str, cap: str = "low") -> str:
    """Deckelt eine Reasoning-Stufe nach oben (z.B. medium->low), laesst niedrigere unveraendert.
    Unbekannte Werte -> cap. Antwort-Aufrufe bleiben ungedeckelt (eingestellte Stufe gilt dort)."""
    try:
        return effort if _EFFORT_ORDER.index((effort or "medium").strip().lower()) <= _EFFORT_ORDER.index(cap) else cap
    except ValueError:
        return cap
...
def hauptagent_route(session: dict, user_text: str, pending: dict | None, context_mode: str = "auto", context_prompt: str = "") -> dict:
    ...
    # PERFORMANCE (Frank 2026-07-02): Das Routing ist eine simple JSON-Klassifikation — tiefes
    # Denken bringt hier nichts, kostet bei gpt-5.5/'medium' aber 15-40s VOR der eigentlichen
    # Antwort. Deshalb wird NUR dieser Router-Aufruf auf 'low' gedeckelt; die Antwort-Aufrufe
    # (hauptagent_answer / _internet / _native_web) behalten die eingestellte Thinking-Stufe.
    raw = _extract_json(llm_generate(
        ...
        reasoning_override=_cap_effort(ROLE_REASONING.get("haupt", "medium"), "low")))
    try:
        data = json.loads(raw)
