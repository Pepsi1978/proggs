...
def _cat_key(name: str) -> str:
    ...
    Hierarchie (Phase 5, Frank-Wunsch 2026-06-25): genau 2 Ebenen 'Haupt/Unter'. Schraegstriche
    normalisieren — ' / ' und 'Haupt /Unter' werden zu 'Haupt/Unter' (Slash OHNE umgebende
    Leerzeichen), max 2 Ebenen, leere Teile fallen weg. So entstehen keine Schreibvarianten-Dubletten
    ('Programmieren / Bugs' vs. 'Programmieren/Bugs') beim Anlegen ueber Dashboard, Speicheragent oder Frank."""
    s = re.sub(r"\s+", " ", (name or "").strip())
    if "/" in s:
        parts = [p.strip() for p in s.split("/") if p.strip()]
        s = "/".join(parts[:2])   # genau 2 Ebenen; alles ab der 3. wird verworfen
    return s[:60]
...
- HIERARCHIE (2 Ebenen): Eine Kategorie kann eine Unterebene haben, geschrieben als 'Haupt/Unter' (genau ein Schrägstrich, z.B. 'Programmieren/Bugs'). Passt der Text spezifisch in eine bestehende Unterkategorie aus der Liste, nimm GENAU DIESE. Gibt es bisher nur die Hauptkategorie, der Text gehört aber klar in eine feinere Schublade darunter, darfst du EINE neue 'Haupt/Unter' vorschlagen (dann 'eskalation':true + kurze Rückfrage — Frank wird gefragt). Nie tiefer als 2 Ebenen; erfinde keinen Schrägstrich, wenn eine flache Kategorie genügt.
...
HIERARCHIE (2 Ebenen): Kategorien können eine Unterebene haben, geschrieben als "Haupt/Unter" (GENAU EIN Schrägstrich, z.B. "Programmieren/Bugs"). Passt der Text spezifisch in eine bestehende Unterkategorie aus der Liste, wähle GENAU DIESE ("Haupt/Unter", zeichengenau, "eskalation":false). Gibt es bisher nur die Hauptkategorie, der Text gehört aber klar in eine feinere Schublade darunter, darfst du EINE neue "Haupt/Unter" vorschlagen — dann "eskalation":true mit einer kurzen Rückfrage (Frank wird vor dem Anlegen gefragt). Nie tiefer als 2 Ebenen; erfinde keinen Schrägstrich, wo eine flache Kategorie genügt.
...
def _do_store(quote: str, categories: list[str], override_category: str = "", override_title: str = "") -> dict:
    ...
    if override_key and ot:
        ...
        return _store_final(quote, override_key, ot, "")
...
def leseagent_select(question: str, hits: list[dict], categories: list[str]) -> tuple[list[dict], str | None]:
    ...
    hits_txt = "\n".join(
        ...
        f"{(h.get('text') or h.get('match') or '').strip()}"
...
def hauptagent_answer(session: dict, question: str, selected: list[dict]) -> str:
    ...
    text = llm_generate(build_hauptagent_answer_prompt(), user_block, model=ROLE_MODELS["haupt"],
...
def _process_turn(session: dict, user_text: str, pending: dict | None, category: str = "", title: str = "") -> dict:
    ...
    if intent == "save":
        ...
        if t and cat_key:
            reply = f"Soll ich das als „{t}“ unter „{cat_key}“ ablegen: „{quote}“?"
        elif t:
            reply = f"Soll ich das als „{t}“ ablegen: „{quote}“?"
        elif cat_key:
    ...
    if intent == "query":
        ...
        try:
            ...
            answer = hauptagent_answer(session, user_text, selected)           # Hauptagent: formuliert aus den ORIGINAL-Treffern
...
async def chat(req: ChatReq) -> dict:
    ...
    outcome = await asyncio.to_thread(_process_turn, session, req.text, pending, (req.category or "").strip(), (req.title or "").strip())
