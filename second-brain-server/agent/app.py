...
def _gemini_thinking_config(model: str) -> "tuple[Any, int]":
    ...
    if genai_types is None or not hasattr(genai_types, "ThinkingConfig"):
        return None, 0
    budget = _GEMINI_THINKING_BUDGET.get(_gemini_effort_for_model(model), 4096)
...
def _gemini_generate(*, model: str, contents: str, base_kwargs: dict, base_max_tokens: int):
    ...
    tc, budget = _gemini_thinking_config(model)
    kw = dict(base_kwargs)
    if tc is not None:
...
def _llm_generate_once(system: str, user: str, *, model: str, json_mode: bool, max_tokens: int, temperature: float) -> str:
    ...
    resp = _gemini_generate(model=model, contents=user, base_kwargs=kwargs, base_max_tokens=max_tokens)
    text = (resp.text or "").strip() if getattr(resp, "text", None) else ""
    finish = None
    try:
        finish = getattr((resp.candidates or [None])[0], "finish_reason", None)
