...
def tavily_search(query: str, response_size: str = "m") -> dict:
    ...
    # Treffer-Staffel von Frank festgelegt (2026-07-02): S=5, M=8, XL=15 — detailliertere Antworten.
    max_results = {"s": 5, "m": 8, "xl": 15}[rs]
...
async def chat(req: ChatReq) -> dict:
    ...
    outcome = await asyncio.to_thread(
        ...
        _norm_response_size(req.response_size),
    )
...
async def chat_stream(req: ChatReq) -> StreamingResponse:
    ...
    async def gen():
        turn = asyncio.ensure_future(asyncio.to_thread(
            ...
            _norm_context_mode(req.context_mode), (req.context_prompt or "").strip(),
            _norm_response_size(req.response_size), on_delta,
        ))
        try:
            while True:
