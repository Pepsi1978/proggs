...
def brain_entities_docs(name: str, limit: int) -> dict:
...
_ENTITY_QUERY_RE = re.compile(
    r"(?:alles (?:über|ueber|zu)|was (?:weißt|weisst) du (?:über|ueber|zu)|was weiß ich (?:über|ueber|zu)|"
    r"was wissen wir (?:über|ueber|zu)|zeig mir alles (?:zu|über|ueber))\s+(?:de[mnr]\s+|die\s+|das\s+|meine[mnr]?\s+)?(.{2,60}?)\s*[?.!]*\s*$",
    re.IGNORECASE)
...
def smart_recall(user_text: str, query: str) -> "tuple[list[dict], dict]":
    ...
    def _one(q: str) -> list[dict]:
        try:
            return brain_search(q, RECALL_LIMIT, date_from=d_from, date_to=d_to)
    ...
    if len(queries) == 1:
        lists = [brain_search(queries[0], RECALL_LIMIT, date_from=d_from, date_to=d_to)]
    else:
    ...
    if d_label and not hits:
        try:
            hits = brain_search(query, RECALL_LIMIT)
    ...
    if ENTITY_ENABLED:
        ...
        if ent_name:
            ent = brain_entities_docs(ent_name, ENTITY_DOCS_LIMIT)
...
def entity_extract_and_link(text: str, title: str, doc_id: str) -> None:
    ...
    try:
        ...
        for e in ents[:6]:
            ...
            brain_entities_upsert(name, (e.get("typ") or "").strip(), aliases, [doc_id])
            linked += 1
...
def _norm_response_size(size: "str | None") -> str:
    s = (size or "m").strip().lower()
    return s if s in {"s", "m", "xl"} else "m"
...
def tavily_search(query: str, response_size: str = "m") -> dict:
    ...
    SUCHTIEFE folgt dem S/M/XL-Profil der App (Frank-Wunsch 2026-07-02): S = flach + wenige Treffer,
    M = Standard, XL = Tavily-Tiefensuche 'advanced' + mehr Treffer (staerkere Recherche, mehr Zeit)."""
...
class ChatReq(BaseModel):
    ...
    response_size: str | None = Field(default=None, max_length=4, description="Antwortlaengen-Profil s|m|xl — steuert zusaetzlich die Tavily-Suchtiefe (xl = advanced)")
...
def put_config(req: ConfigReq) -> dict:
    ...
    return {"status": "ok", "models": ROLE_MODELS, "reasoning": ROLE_REASONING,
            "router_model": ROUTER_MODEL, "router_reasoning": ROUTER_REASONING, "tavily_enabled": TAVILY_ENABLED}
