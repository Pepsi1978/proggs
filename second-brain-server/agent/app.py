...
_eval_state: dict = {"running": False, "done": 0, "total": len(EVAL_CASES), "started": None,
                     "finished": None, "passed": None, "log": None, "error": None}
...
def _run_eval_background() -> None:
    try:
        ...
        _eval_state.update({"running": False, "finished": _now_local().strftime("%d.%m.%Y, %H:%M:%S")})
...
def eval_run() -> dict:
    ...
    if _eval_state.get("running"):
        return {"ok": True, "started": False, **_eval_state}
    _eval_state.update({"running": True, "done": 0, "total": len(EVAL_CASES), "passed": None,
                        "log": None, "error": None, "finished": None,
                        "started": _now_local().strftime("%d.%m.%Y, %H:%M:%S")})
    ...
    return {"ok": True, "started": True, **_eval_state}
