...
- **Automatische Sichtbarkeit**: Das Overlay erscheint nur, wenn Terminal.app aktiv ist, und versteckt sich automatisch
- **Zeile loeschen**: X-Button loescht die aktuelle Eingabezeile im Terminal (Ctrl+U)
...
3. In `~/.config/TerminalVoiceOverlay/.env`
...
> Das Build-Skript kompiliert die Swift-Quelldateien und erstellt `build/TerminalVoiceOverlay.app`.
...
2. **Accessibility**: Fuer Tastatureingabe (Cmd+V, Ctrl+U, Return) — unter Systemeinstellungen > Datenschutz & Sicherheit > Bedienungshilfen die App hinzufuegen
...
| Button | Funktion |
|---|---|
| **X** | Loescht die aktuelle Eingabezeile im Terminal (Ctrl+U) |
...
TerminalVoiceOverlay-macOS/
  ...
  TerminalVoiceOverlay/
    ...
    TerminalController.swift — Tastatureingabe-Simulation (Cmd+V, Ctrl+U, Return)
    ...
    AppWatcher.swift        — Erkennt aktives Terminal.app-Fenster
```
...
| Overlay erscheint nicht | Pruefen ob Terminal.app im Vordergrund ist |
...
- **CGEvent** — Tastatureingabe-Simulation (Cmd+V, Ctrl+U, Return)
