 ...
 * - Ein `KEYCODE_HEADSETHOOK` an diese Session BEENDET die Voice-Session
 *   (Audio faellt danach auf `MODE_NORMAL`). `PAUSE`/`STOP`/`PLAY_PAUSE` ignoriert ChatGPT.
 * - ChatGPTs Kugel-Overlay hat KEINEN Beenden-Knopf; selbst der In-App-Beenden-Knopf
 *   stoppt die Aufnahme nicht zuverlaessig. Deshalb der Headsethook-Weg.
 *
 * Der Abbau dauert 1-3 s (HAL-/UI-Teardown), daher wird verzoegert verifiziert und bei
 * Bedarf ein zweiter Druck gesendet.
 */
object AssistantStopper {
     ...
     * Tastendruecke (Abbau braucht Zeit) und verifiziert ueber den Audio-Modus, ob die
     * Session wirklich aus ist. Tut nichts, wenn keine Session aktiv ist (kein versehentlicher
     * Start einer neuen Session).
     */
    fun endVoiceSession(context: Context, reason: String) {
        val am = audio(context)
        // MODE_IN_CALL = echtes Telefonat -> niemals anfassen. Nur MODE_IN_COMMUNICATION (ChatGPT/VoIP).
        ...
        if (ending) {
            Obs.d("AssistantStopper", "endVoiceSession", "Lauf laeuft bereits", mapOf("ausloeser" to reason))
            return
        }
        ending = true
        Obs.i("AssistantStopper", "endVoiceSession", "Headsethook gesendet", mapOf("ausloeser" to reason))
        sendHeadsetHook(am)
    ...
    private fun verifyOrRetry(am: AudioManager, attempt: Int) {
        ...
        Obs.w("AssistantStopper", "verifyOrRetry", "Noch aktiv — erneuter Headsethook", mapOf("versuch" to attempt + 1))
        sendHeadsetHook(am)
        handler.postDelayed({ verifyOrRetry(am, attempt + 1) }, VERIFY_DELAY_MS)
    }
