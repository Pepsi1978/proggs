...
import de.frank.genialeideen.network.shutdownSafely
...
class EdgeTtsPlayer(context: Context) {
    ...
    fun speak(
        ...
        /** Wecker-Erweiterung: fertiges Audio abholen, ohne es jetzt abzuspielen. */
        ...
        val listener = object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                ...
                val escaped = text.map { if (it.code < 32 && it !in "\t\n\r") ' ' else it }.joinToString("")
                    .replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                val ratePercent = ((speechRate.coerceIn(0.5f, 2.0f) - 1f) * 100).roundToInt()
                val formattedRate = if (ratePercent >= 0) "+$ratePercent%" else "$ratePercent%"
                val boundedPitch = pitchPercent.coerceIn(-20, 20)
                val formattedPitch = if (boundedPitch >= 0) "+${boundedPitch}Hz" else "${boundedPitch}Hz"
                // Edge erlaubt nur voice + prosody. Ein zusätzlicher lang-Knoten führt zu
                // stummen Timeouts (Bug-Almanach tts-provider ET1/ET3). Die Landesstimme und
                // xml:lang am speak-Knoten legen Deutsch fest; Multilingual bleibt anbietergesteuert.
                val sprache = if (erzwingeDeutsch) "de-DE" else voice.substringBeforeLast('-')
                val ssml = "X-RequestId:$requestId\r\n" +
                    "Content-Type:application/ssml+xml\r\n" +
                    "Path:ssml\r\n\r\n" +
                    "<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' " +
                    "xml:lang='$sprache'><voice name='$voice'>" +
                    "<prosody rate='$formattedRate' pitch='$formattedPitch' volume='+0%'>" +
            ...
            override fun onMessage(webSocket: WebSocket, text: String) {
                ...
                scope.launch {
                    ...
                    if (onAudioReady == null) playFile(audioFile, requestGeneration, callbacks)
                    else try {
