...
- **Sonderfall neues/Stealth-Modell fehlt komplett in der Auswahl** (z.B. `openrouter/owl-alpha`, 2026-04-28, gratis, 1M Kontext): models.dev kennt frische Stealth-Modelle verzögert/nicht → tauchen in `/models` gar nicht auf. **FIX:** manuell in `opencode.json(c)` unter `provider.openrouter.models.<slug>` definieren (`name` + optional `limit`), dann ist es wählbar. Wählbarer String = doppelt: `openrouter/openrouter/owl-alpha` (Provider `openrouter` + Autor-Slug `openrouter/owl-alpha`). OpenCode danach neu starten. **Datenschutz:** Owl Alpha & viele Gratis-Stealth-Modelle loggen Prompts/Completions fürs Training → nicht für sensible/private App-Daten. **Quelle:** https://openrouter.ai/openrouter/owl-alpha · https://opencode.ai/docs/providers (custom models).
