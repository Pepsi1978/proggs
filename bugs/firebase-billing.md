...
1. **acknowledgePurchase() innerhalb von 3 Tagen** nach jedem `PURCHASED`-Kauf — sonst **automatischer
   Refund + Entzug** (Bug 14). Der teuerste, haeufigste Billing-Bug ueberhaupt.
2. **`PurchaseState.PENDING` nie sofort freischalten** — erst bei `PURCHASED` (Bug 18). `enablePendingPurchases(PendingPurchasesParams…)` ist ab Billing 7 Pflicht (Bug 1).
3. **google-services.json**: nach Konsolen-Aenderung `./gradlew clean` (sonst stale, Bug 44); bei
   `applicationIdSuffix` (`.debug`) eigene JSON pro Variante (Bug 42); **App-Signing-Key-SHA** in Firebase
...
4. **R8/Release ist aktiv** → fehlende Keep-Regeln fuer Firebase/Billing/Gemini lassen die App **nur im
   ...
   `best-practices/projekt-code/gradle/best-practices.md §4`).
...
6. **App Check + Enforcement**: erst Client-Rollout, dann Enforcement (Bug 90); Debug-Token in der Konsole
   eintragen (Bug 89); Play-Integrity-403 auf zertifizierten Geraeten ist ein **offener** SDK-Bug (Bug 88).
...
> manifestieren sich NUR im Release-Build (Debug laeuft). Querverweis: `best-practices/projekt-code/gradle/best-practices.md §4`.
