...
1. **Whisper halluziniert bei STILLE Floskeln** ("Vielen Dank", "Thank you", "Untertitel des ZDF").
   Das ist inhaerentes Modellverhalten (Trainingsdaten = YouTube-/TV-Untertitel), **kein API-Bug
   und NICHT abschaltbar** — `large-v3` ist sogar schlimmer als aeltere Modelle. (§1)
2. **Keine Einzelmassnahme reicht.** Funktionserhaltende KETTE: (a) Sprachgehalt-/VAD-Vorfilter VOR
   dem Senden → (b) Mindest-Sprechdauer → (c) `verbose_json`-Confidence-Gate → (d) Floskel-Blocklist. (§2)
3. **`response_format=verbose_json` kostet bei Groq KEINE Mehrlatenz/Mehrkosten** vs. `text` (nur
   `word`-Timestamps kosten extra) — und liefert `no_speech_prob`/`avg_logprob`/`compression_ratio`
   zum Filtern. Es gibt keinen Grund, bei `text` zu bleiben. (§3, §2.3)
4. **Confidence-Gate mit UND, nie ODER:** verwerfen nur wenn `no_speech_prob > 0.6` **UND**
   `avg_logprob < -1.0`. Die AND-Verknuepfung schuetzt echte leise Sprache. (§2.3)
5. **Min-Abrechnung 10 s pro Clip; RPM 20 (free).** Viele winzige Clips sind teuer und laufen schnell
   ins 429. `retry-after`-Header lesen, nicht blind sofort retryen. (§4)
