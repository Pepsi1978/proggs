...
| 27 | App startet in der falschen Erscheinung / ein Schalter steht anders als vorbelegt | Startbefehl war `monkey -p ... 1` — die `1` sind **zufaellige Ereignisse**, nicht Startversuche: der Monkey tippt nach dem Start einmal blind auf den Bildschirm → zum Starten `am start -a MAIN -c LAUNCHER -n <paket>/<activity>` nehmen; `monkey` nur, wenn `am start` scheitert (#26) |
