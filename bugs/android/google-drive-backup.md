...
- **Was ein zweiter und dritter Durchgang über dieselbe Ecke noch zutage brachte** (eigener Vorfall, Modul M1.1 v4→v7 — jede Runde fand einen Fehler in der Runde davor):
  1. **Der Merker darf erst gelöscht werden, wenn geschrieben UND geprüft ist — nie vorher.** Die erste Fassung löschte ihn vor dem Schreiben und setzte ihn bei Fehlschlag zurück. Damit stand ausgerechnet während des Schreibens „nichts offen“ in der Ablage: genau in der Phase, um derentwillen es den Merker gibt. Derselbe Fehler in anderer Gestalt.
  2. **Jeder Fehlschlagweg stempelt, nicht nur der beim Zurücklesen.** Scheitert schon das Schreiben (Freigabe weg, Anbieter legt keine Datei an, Ordner nicht auflistbar), blieb die tote Sicherung unsichtbar und die Anzeige nannte weiter die letzte geglückte von vor Wochen. Siehe INC1/INC2.
  3. **Stempel und Merker mit `commit()` schreiben, nicht `apply()`.** Beides fällt am Ende eines Laufs an, der typischerweise beim Verlassen der App stattfindet — also kurz bevor der Vorgang stirbt.
  4. **Entprellung nie mit der Wanduhr rechnen.** `System.currentTimeMillis()` springt (Funkzeit, Zeitumstellung, von Hand gestellt); ein Rückwärtssprung verschiebt die Sicherung um den Sprung. `SystemClock.elapsedRealtime()`.
  5. **Eine Sicherung von Hand muss den Merker mitquittieren.** Sonst schreibt die selbsttätige zwei Minuten später dieselbe Datei ein zweites Mal, und der nächste Start holt etwas nach, das längst dasteht. Der Dienst meldet jeden geglückten Lauf samt seinem Beginnzeitpunkt; abgehakt wird nur, was bis dahin gemeldet war.
  6. **Merker prüfen und setzen gehören unter einen Riegel.** Ohne ihn passt zwischen „seither kam nichts“ und dem Schreiben genau eine Änderung: Im Speicher steht dann „offen“, in der Ablage „nichts offen“.
  7. **Eine frisch geschriebene, nicht zurücklesbare Datei sofort wegwerfen.** Ihr Name trägt den jüngsten Zeitpunkt, sie gilt also ab sofort als „die aktuelle“ — der nächste geglückte Lauf räumt die letzte GUTE Sicherung als „die überzählige“ weg und behält die unlesbare als Rückfallebene.
  8. **`runCatching` um den Sicherungslauf fängt auch den Abbruch von aussen.** `CancellationException` gesondert weiterreichen, sonst läuft die abgebrochene Coroutine weiter, als wäre nichts gewesen.
  9. **Das Zurücklesen ist kein Vorschau-Lauf.** Lief es als solcher, lud die App bei JEDER selbsttätigen Sicherung ihren gesamten Bestand ein zweites Mal, nur um „wie viel davon fehlt mir“ zu beantworten — eine Zahl, die beim Nachrechnen niemand ansieht.
  10. **Nullbare Werte gehören nie in die Prüfsumme.** Das Schreiben rechnet mit `"null"`, ein Leser, der JSON-`null` zu `""` macht, mit `""` — jedes Zurücklesen scheitert mit „Datei beschädigt“, obwohl sie heil ist.
