...
**FIX (funktionserhaltend für Copy/Paste):** In `~/.config/opencode/tui.json` `"mouse": false` setzen. Laut offizieller OpenCode-Dokumentation bleibt damit die native Mausauswahl des Terminals erhalten. Zusätzlich in Windows-Terminal-`settings.json` `"copyOnSelect": true`, `"experimental.rightClickContextMenu": false` und optional `"copyFormatting": "none"` setzen. Dann kopiert Mouse-Up die native Auswahl und Rechtsklick fügt ein. Ist die native Auswahl optisch kaum erkennbar, unter `profiles.defaults` `"selectionBackground": "#808080"` setzen; das erzwingt für alle Profile einen deutlich grauen Markierungshintergrund. Trade-off: OpenCodes eigene Mausfunktionen wie internes Scrollen und anklickbare TUI-Elemente sind deaktiviert; Tastatur und natives Terminal-Scrollen bleiben.
