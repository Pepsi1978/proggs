...
| 1a | Windows Terminal: Markieren/Rechtsklick wird von OpenCode abgefangen | OpenCode-`tui.json`: `"mouse": false`; zusätzlich im Terminal `"copyOnSelect": true` und `"experimental.rightClickContextMenu": false`. | §2 (#10a) |
...
### 10a. Windows Terminal: Linksauswahl kopiert nicht automatisch, Rechtsklick wird abgefangen
**Symptom:** Mit der linken Maustaste markierter OpenCode-Text landet erst nach einem Rechtsklick oder trotz „Copied to clipboard" gar nicht in der Zwischenablage; Rechtsklick im Eingabefeld fügt nichts ein.
**Ursache:** OpenCode aktiviert standardmäßig `"mouse": true` und übernimmt damit die Mausereignisse selbst. Windows Terminal erhält deshalb weder das Loslassen der linken Taste noch den Rechtsklick zuverlässig. Die alleinige Terminalkonfiguration reicht nicht. OpenCodes eigener Copy-on-select-Pfad ist zudem als offener Fehler #17796 dokumentiert.
**Versionen:** OpenCode 1.17.18 und Windows Terminal 1.24.11321.0 bestätigt; OpenCode-Fehler #17796 offen.
**FIX (funktionserhaltend für Copy/Paste):** In `~/.config/opencode/tui.json` `"mouse": false` setzen. Laut offizieller OpenCode-Dokumentation bleibt damit die native Mausauswahl des Terminals erhalten. Zusätzlich in Windows-Terminal-`settings.json` `"copyOnSelect": true`, `"experimental.rightClickContextMenu": false` und optional `"copyFormatting": "none"` setzen. Dann kopiert Mouse-Up die native Auswahl und Rechtsklick fügt ein. Trade-off: OpenCodes eigene Mausfunktionen wie internes Scrollen und anklickbare TUI-Elemente sind deaktiviert; Tastatur und natives Terminal-Scrollen bleiben.
**Quelle:** https://opencode.ai/docs/tui/#options · https://github.com/anomalyco/opencode/issues/17796 · https://learn.microsoft.com/windows/terminal/customize-settings/interaction#automatically-copy-selection-to-clipboard · https://learn.microsoft.com/windows/terminal/customize-settings/interaction#context-menu
