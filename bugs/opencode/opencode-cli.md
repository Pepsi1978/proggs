...
**Updatefester Betrieb:** Der Launcher prüft höchstens täglich die stabile npm-Version. Das Buildskript klont den exakten Git-Tag, wendet den Patch mit Windows-Zeilenendungstoleranz an, prüft Full-Repaint-Recovery, führt vier Typechecks, fokussierte Tests, Build und Versions-Smoke-Test aus und installiert unveränderlich nach `~/.local/share/opencode-mousefix/versions/<customVersion>/opencode.exe`. Erst danach wird `current.json` atomar aktiviert. Bei Konflikt bleibt der letzte grüne Pointer erhalten; Fallback-Reihenfolge: `active`, `previous`, Pointer-Backup, Legacy-Binary, offizielles `opencode` im PATH. Laufende Sitzungen werden nie überschrieben.
