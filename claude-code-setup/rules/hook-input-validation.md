...
- Claude Code sendet bei jedem Stop-Event eine Hook-Invokation — auch ohne echten Subagent
...
- ❌ Guard als `exit 1` / `exit 2` formulieren (blockiert Claude Code)
...
Bei jedem `/self-improve` Stufe 6: Alle Hooks in `~/.claude/hooks/` pruefen die auf
