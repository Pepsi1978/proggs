...
vom `session-scorer.sh`-Hook am SessionEnd erfasst und in `~/.claude/session-scores.jsonl`
gespeichert.
...
|-----------|---------|------------------|
| **Intent-Treue** | 25% | Hat Claude das urspruengliche Ziel erreicht? |
...
| "Session-Trend zeigen" | Liest `~/.claude/session-scores.jsonl` und zeigt Trend |
