# Gemini Launcher with random Windows Terminal tab color.
# Mirrors start-claude-wt.ps1 / start-codex-wt.ps1 but invokes the Gemini CLI.
#
# Gemini has no inner /color slash command, so we only set the WT tab color and
...
#
# Logging: every invocation appends to start-gemini-wt.log for diagnosis.
#
...
$logFile = 'C:\Users\barwa\start-gemini-wt.log'
...
Write-Log "=== gemini launcher started, PID $PID ==="

# Same palette as start-claude-wt.ps1 / start-codex-wt.ps1 so the tab strip
# stays visually consistent across all CLIs.
$colors = @(
...
# The inner pwsh runs `gemini -y` from the GeminiCLI workspace, matching the
# previous shortcut command (`cd C:\Users\barwa\GeminiCLI; gemini -y`).
$tabArgs = @(
    'new-tab'
    '--tabColor';        $pick.Hex
    '--title';           "Gemini-$($pick.Name)"
    '--startingDirectory'; 'C:\Users\barwa\GeminiCLI'
    'pwsh.exe'
    '-NoExit'
...
# Fallback-Argumente: gemini direkt in pwsh (klassisches Konsolenfenster)
$fallbackArgs = @(
    '-NoExit'
    '-ExecutionPolicy'; 'Bypass'
    '-Command';         'gemini -y'
...
$ok = Start-WtCliRobust -LogFile $logFile -WtPath $wtPath -TabArgs $tabArgs `
    -InnerMatch 'gemini -y' -FallbackPwshArgs $fallbackArgs `
    -FallbackWorkDir 'C:\Users\barwa\GeminiCLI'
...
Write-Log "gemini launcher exiting normally"
