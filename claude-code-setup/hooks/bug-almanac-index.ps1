...
try {
    ...
    $ctx = "BUG-ALMANACH-SYSTEM aktiv (Stufe 2 = ERZWINGUNG). Vorhandene Almanache in bugs/: $list. " +
        ...
        "bugs/README.md pruefen und den passenden Almanach ZUERST mit dem Read-Tool lesen, DANN die zugehoerige " +
        "best-practices/projekt-code/<kategorie>/best-practices-<bereich>.md (zwei Seiten einer Medaille: Almanach = was schiefgeht, " +
        "Best Practices = wie man es richtig macht). WICHTIG: Existiert ein Almanach fuer den Bereich, BLOCKIERT der bug-almanac-guard " +
        "Edit/Write so lange, bis du in dieser Session ZUERST die Almanach-Datei UND DANACH (falls vorhanden) die Best-Practices-Datei per Read " +
        "geoeffnet hast (jedes Lesen gibt frei, gilt pro Bereich 1x/Session; Reihenfolge erst Almanach, dann Best Practices). " +
