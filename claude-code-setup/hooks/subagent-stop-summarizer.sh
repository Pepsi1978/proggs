...
#
# HOW IT WORKS:
#   Claude Code passes subagent info via stdin as JSON.
#   We read stdin, check for subagent_id. If empty -> silent exit 0.
#   If real subagent -> output prompt text to stdout (Claude reads it as injected prompt).
#
# POKA-YOKE: type:"command" hooks that output text inject that text as a prompt.
#   This only fires when Claude Code actually calls SubagentStop with a subagent_id.
#
# EXIT CODES:
#   0 = success (with or without output)
#   Non-0 = error (Claude Code may log but continues)
...
# Read stdin (subagent context passed by Claude Code)
INPUT_JSON=""
if [ -t 0 ]; then
    # No stdin available
    INPUT_JSON=""
