# claude-mem-worker-launcher.ps1 — Windows counterpart for claude-mem worker startup
#
# On macOS this is called by launchd. On Windows, claude-mem starts
...
if (Test-Path $settingsPath) {
    try {
        ...
        if ($settings.CLAUDE_MEM_WORKER_PORT) {
            $cmemPort = [int]$settings.CLAUDE_MEM_WORKER_PORT
        }
    } catch { }
}
...
try {
    ...
    Write-Host "[claude-mem] Worker already running on port $cmemPort"
    exit 0
} catch { }

# --- Find plugin directory ---
$pluginDir = Join-Path $env:USERPROFILE ".claude\plugins\cache\thedotmack\claude-mem"
if (-not (Test-Path $pluginDir)) {
    Write-Host "[claude-mem] Plugin not installed at $pluginDir"
    exit 0
}

$latestVersion = Get-ChildItem -Path $pluginDir -Directory | Sort-Object Name | Select-Object -Last 1
if (-not $latestVersion) {
    Write-Host "[claude-mem] No plugin versions found"
    exit 0
}

$workerScript = Join-Path $latestVersion.FullName "scripts\worker-service.cjs"
if (-not (Test-Path $workerScript)) {
    Write-Host "[claude-mem] worker-service.cjs not found at $workerScript"
    exit 0
}
...
if (-not $runtime) {
    Write-Host "[claude-mem] Neither bun nor node found"
    exit 0
}

Write-Host "[claude-mem] Starting worker: $runtime $workerScript --daemon"
