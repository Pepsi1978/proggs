...
if ($envData) {
    ...
    if ($null -ne $subModel -and $subModel -ne '' -and $subModel -notin @('sonnet','opus','opus[1m]','inherit')) {
        $blocks += "CLAUDE_CODE_SUBAGENT_MODEL=$subModel (erlaubt: sonnet, opus, opus[1m], inherit)"
    }
