...
$whiteboard = Join-Path $proggsDir ".claude\agent-memory\shared\MEMORY.md"
...
function Notify-Critical([string]$msg) {
    ...
    try {
        ...
        $textNodes.Item(0).AppendChild($template.CreateTextNode("Claude Heartbeat KRITISCH")) | Out-Null
...
function Check-KeyDirectories {
    ...
    foreach ($dir in @(
        ...
        (Join-Path $proggsDir ".claude\agent-memory\shared")
    )) {
...
try {
    ...
    # Print summary for SessionStart hook output (visible to Claude)
    $warnings = @()
