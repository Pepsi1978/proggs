...
- Weicht ein Projekt von diesen Mustern ab, steht die Regel in dessen eigener CLAUDE.md. Findest du dort nichts und passt kein Muster: frag nach, rate nicht.
...
20. APK-Update nach jeder Android-Änderung: Jede Android-App, die neu gebaut, programmiert oder geändert wird, kommt am Ende zusätzlich über den Skill `apk-update` in den Drive-Ordner `Meine Ablage/Dokumente/Updates/<Projekt>` — ohne Rückfrage, als letzter Schritt nach Bauen, Versions-Bump, Commit, Push und Installation. Nur so steht jede App, die auf meinem Handy installiert ist, mit ihrer aktuellen Version in UpdateStation, und ein neues Update wird dort erkannt. Das gilt auch für neue Apps: Die erste Veröffentlichung legt ihren Ordner an. Meldet der Skill `vorbereitet`, die genannten Dateien committen, pushen und den Skill erneut starten; bei `fehler` die Ursache beheben und neu starten. Nie eine APK von Hand in den Ordner kopieren (ohne passende `update.json` sieht UpdateStation sie nicht). In der Abschlussmeldung zusätzlich melden: APK-Update ja/nein, mit versionCode.
