# Windows-Fensterweg ohne tmux

Nur laden, wenn die bestätigte Windows-Sitzung nicht in tmux läuft. Die verbindlichen Grundregeln stehen in ../SKILL.md. Das Skill-Verzeichnis liegt eine Ebene über dieser Referenz; Helferpfade darauf beziehen.
...
Für den vorbereiteten Standardfall den kurzen Helfer [scripts/terminal-fast-path.mjs](../scripts/terminal-fast-path.mjs) bevorzugen. Die ausführliche Anleitung unter „Eingeben und prüfen“ erklärt dieselben Prüfschritte und dient zur Initialisierung, Fokuskorrektur oder Wiederaufnahme nach Fehlern; sie ist kein zusätzlich auszuführender zweiter Ablauf. Vor dem ersten Import den Helfer lesen, seinen absoluten Dateipfad aus diesem Skill-Verzeichnis bestimmen und `createTerminalFastPath(sky, terminalInsertWindow)` einmal als `tt` in der vorhandenen Node-Sitzung anlegen. Import und Konstruktion erzeugen keine Eingabe. Danach drei **getrennte** Werkzeugzellen verwenden und jedes Ergebnis vor der nächsten Aktion auswerten:
...
`tt.invalidate()` verwirft den lokalen Zustand, löscht aber keinen sichtbaren Text. Nach Fehler oder verlorenem Runtime-Zustand kann eine Eingabe bereits angekommen sein: zuerst beobachten, niemals durch Neuinitialisierung blind erneut senden. Der Helfer sperrt parallele Aktionen und wiederholtes Enter; nach Fehler und erneutem Beobachten kann jedoch wieder `type()` aufgerufen werden. Die Entscheidung, ob schon Text vorhanden ist, bleibt daher beim prüfenden Assistenten. Eine Meldung über neue Benutzereingaben respektieren und den Zustand neu lesen; bei wiederholter Intervention die Fenstersteuerung pausieren. Die vier automatisierten Fehlertests liegen in `scripts/terminal-fast-path.test.mjs`; sie ersetzen keinen Test des echten Fokusverhaltens.

Der konfigurierbare Standardwert der Renderwartezeit steht im Helfer als `renderDelayMs`. Bei einem nötigen direkten API-Ablauf denselben Wert als Ausgangspunkt verwenden; keine unabhängige zweite Vorgabe pflegen. Die Messnotizen dokumentieren historische Versuche, keine zusätzlichen Sollwerte.

### Lange Texte und echte Zeilenumbrüche

Länge, visueller Zeilenumbruch und ein Paste-Platzhalter bedeuten nicht, dass der übergebene Text echte Zeilenumbrüche enthält. Auch lange einzeilige Aufträge funktionieren im Helfer. Selbst formulierte Gesprächsaufträge können von Anfang an als einzeiliger Fließtext erstellt werden, sofern Inhalt und Verständlichkeit erhalten bleiben.

Der Helfer weist Steuerzeichen einschließlich echter Zeilenumbrüche absichtlich ab; sie dürfen nicht versehentlich als Absenden wirken. Mehrzeilige Originaltexte und Code nicht stillschweigend abflachen. Wenn Format erhalten bleiben muss, den exakten Text in einer temporären UTF-8-Datei in einem für beide Agenten zugänglichen Arbeitsordner ablegen und einen kurzen einzeiligen Auftrag mit eindeutigem Dateipfad übergeben. Vorhandene Dateien nicht überschreiben, die Ablage außerhalb des Produktcodes halten und nicht ungefragt committen. Die CLI soll die Datei als Auftragsinhalt lesen; erst ihre tatsächliche Lektüre bestätigt die Inhaltsübergabe. Dies ist eine Dateiübergabe, keine behauptete mehrzeilige Terminaleingabe. Fehlt ein gemeinsamer Dateizugriff, nur einen für die konkrete CLI dokumentierten und überprüften Mehrzeilen-Eingabeweg verwenden; andernfalls die Einschränkung nennen. Keine ungeprüften Tastenkombinationen oder eingebetteten Enter-Zeichen als Ersatz ausprobieren.

## Antworten zügig mitlesen

Bei begleitender Codeprüfung bereits während der Arbeit die gespeicherten Änderungen ansehen; nicht grundsätzlich bis zum Antwortende warten. Terminalausgabe liefert Hinweise, der bestätigte Arbeitsordner liefert den tatsächlichen Dateistand. Für den sparsamen Vergleich und optionale Sitzungslogs [references/live-mitlesen.md](live-mitlesen.md) lesen. Das ist eine Folge von Momentaufnahmen, kein lückenloser Livestream.
...
- Bei in dieser Sitzung wiederholt nachgewiesenem verzögertem Rendering eine kurze Renderwartezeit innerhalb der Aktionszelle vor dem Zustandsabruf verwenden, statt routinemäßig eine zusätzliche Modellrunde zu benötigen. Den Ausgangswert aus dem Helfer übernehmen, wie oben beschrieben. Danach genau einen Zustand erfassen und auswerten; keine blinde Wiederholung von Eingaben und keine unbeschränkte Screenshot-Schleife. Dies ist eine Anpassung an beobachtetes Rendering, keine pauschale Wartepflicht bei jeder Installation.
...
Die ausführlichen Versionsbeobachtungen und Zeitmessungen stehen in [references/beobachtungen.md](beobachtungen.md). Nur bei Bedarf nachlesen, nicht vor jeder Eingabe.
