#!/usr/bin/env bash
# update-changelog.sh — best-practices Skill (Teil 1)
#
# macOS/Linux/Git-Bash-Gegenstueck zu update-changelog.ps1.
...
#
# Aufruf:
#   bash update-changelog.sh                    # inkrementell
#   bash update-changelog.sh --first-run         # komplette Neuanlage
#   bash update-changelog.sh --data-dir <pfad>   # anderer Zielordner

set -euo pipefail

DATA_DIR="${HOME}/proggs/best-practices"
FIRST_RUN=0
while [ $# -gt 0 ]; do
  case "$1" in
