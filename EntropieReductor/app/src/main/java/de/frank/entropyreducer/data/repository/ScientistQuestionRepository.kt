...
/**
 * Eigener DataStore fuer die proaktive Forscher-Frage. Vom Aufgaben-DataStore
 * (KiQuestionRepository) getrennt, damit beide Fragen unabhaengig leben koennen.
 */
private val Context.scientistQuestionStore by preferencesDataStore(name = "scientist_question_state")

@Singleton
class ScientistQuestionRepository @Inject constructor(
    @ApplicationContext private val context: Context,
) {

    val currentQuestion: Flow<KiQuestion?> = context.scientistQuestionStore.data.map { prefs ->
    ...
    val dismissedUntilMs: Flow<Long> =
        context.scientistQuestionStore.data.map { it[KEY_DISMISSED_UNTIL] ?: 0L }

    suspend fun setCurrent(question: KiQuestion?) {
        context.scientistQuestionStore.edit { prefs ->
            if (question == null) {
