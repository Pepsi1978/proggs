...
/**
 * Vollbild-Detail-Screen eines Ideen-/Entropie-Eintrags (Frank-Wunsch 2026-05-20).
 *
 ...
 */
@Composable
fun IdeenEntryDetailScreen(
    onBack: () -> Unit,
    viewModel: IdeenEntryDetailViewModel = hiltViewModel(),
) {
    ...
    // Standard ist false; wird unten in der Column auf true gesetzt wenn entry.improvedText existiert.
    ...
    Box(modifier = Modifier.fillMaxSize().background(MaterialTheme.colorScheme.background)) {
        Column(modifier = Modifier.fillMaxSize().padding(top = topInset)) {
            ...
            Column(
                ...
                // Hier setzen wir den Wert sobald entry geladen ist.
...
private fun ImprovableEntryCard(
    ...
    GlassCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth()) {
            ...
            Text(
                text = formatIdeenTimestamp(timestampMs),
            ...
            BasicTextField(
                ...
                cursorBrush = SolidColor(IdeenAccent),
            )
            Spacer(modifier = Modifier.height(8.dp))
...
private fun TabChip(label: String, active: Boolean, onClick: () -> Unit) {
    val cosmos = LocalCosmos.current
    val bg = if (active) IdeenAccent.copy(alpha = 0.18f) else Color.Transparent
    val fg = if (active) IdeenAccent else cosmos.textSecondary
    Box(
        modifier =
            Modifier
...
private fun FollowupCard(
    ...
    onDelete: () -> Unit,
    improveVm: IdeenImproveViewModel,
    ...
    GlassCard(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth()) {
            ...
            Text(
                text = formatIdeenTimestamp(followup.createdAtMs),
                style = MaterialTheme.typography.labelSmall,
                color = cosmos.textSecondary,
