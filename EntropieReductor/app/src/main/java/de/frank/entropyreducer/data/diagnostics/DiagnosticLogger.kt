 ...
 * Claude. Wenn Tage spaeter ein Sync-/API-Fehler untersucht werden soll, muss nachvollziehbar
 * sein WAS WANN in welchem Bereich (Strava, Whoop, Oura, Drive, Kalender, Groq, Gemini, Claude,
 * TTS, Health Connect) schiefging — inklusive HTTP-Codes und Stacktraces. Sekundaer dient der
 ...
 *     (`<externalFilesDir>/diagnostics/diag-YYYY-MM-DD.jsonl`) → von Claude per `adb pull`
 *     auch Tage spaeter auslesbar und grep-/parsebar. DAS ist die Schicht fuer das
 *     Nachhinein-Debuggen. Pfad auf dem Geraet:
 *     /sdcard/Android/data/<package>/files/diagnostics/
 *
 * Robustheit (Direktive #3):
 *  - Fire-and-forget: Methoden sind NICHT suspend, blockieren den Aufrufer nie.
 *  - Jede Schicht in eigenem runCatching — ein Fehler in einer Schicht stoppt die anderen nicht
 *    und beeintraechtigt NIE die eigentliche Funktion (Sync, Backup).
 *  - Auto-Pruning: DB (14 Tage / max 2000) und alte JSONL-Dateien (>14 Tage) werden geraeumt.
 */
@Singleton
