...
class CodexClient(context: Context) {
     ...
     *
     * [werkzeuge] schaltet die Werkzeuge des Dienstes frei — für die Nachrichten sind das die
     * Websuche und die Bilderzeugung. Ohne Werkzeuge antwortet das Modell nur aus dem Gedächtnis.
     */
    suspend fun frage(
        anweisung: String,
        eingabe: String,
    ...
    suspend fun ladeModelle(): List<CodexModell> = withContext(Dispatchers.IO) {
        ...
        val anfrage = Request.Builder()
            ...
            .header("User-Agent", "codex_cli_rs/$CLIENT_VERSION (${AppProfil.PRODUKT})")
            .header("ChatGPT-Account-ID", konto)
            .build()
        val ergebnis = fuehreAus(anmeldeClient, anfrage)
        pruefeHttp(ergebnis.code, ergebnis.rumpf)
        val liste = leseJson(ergebnis.rumpf, "Die Modellliste war unlesbar.").optJSONArray("models")
            ?: return@withContext emptyList()
        (0 until liste.length()).mapNotNull { index ->
            val modell = liste.optJSONObject(index) ?: return@mapNotNull null
            if (modell.optString("visibility") != "list") return@mapNotNull null
            val slug = modell.optString("slug").takeIf(String::isNotBlank) ?: return@mapNotNull null
            val stufen = modell.optJSONArray("supported_reasoning_levels")
            CodexModell(
                id = slug,
                name = modell.optString("display_name").ifBlank { slug },
                stufen = (0 until (stufen?.length() ?: 0)).mapNotNull {
                    stufen?.optJSONObject(it)?.optString("effort")?.takeIf(String::isNotBlank)
                },
                standardStufe = modell.optString("default_reasoning_level").ifBlank { "medium" },
            )
        }
    }
    ...
    private fun baueNutzlast(
        ...
        if (werkzeuge != null && werkzeuge.length() > 0) {
            nutzlast.put("tools", werkzeuge)
    ...
    private suspend fun frageEinmal(
        ...
        return CodexAntwort(sammler.ergebnis(), sammler.quellen.toList(), sammler.bilder.toList())
    }
...
internal class StromSammler {
    ...
    /** Adressen, die die Websuche benutzt oder die Antwort zitiert hat. */
    val quellen = linkedSetOf<String>()

    /** Base64-Bilder aus der Bilderzeugung, in der Reihenfolge ihres Eintreffens. */
    val bilder = mutableListOf<String>()
    ...
    fun nimm(daten: String): String {
        ...
        return when (ereignis.optString("type")) {
            ...
            "response.output_item.done" -> {
                ereignis.optJSONObject("item")?.let(::sammle)
                ""
            }
            "response.completed" -> {
                fertig = true
                val antwort = ereignis.optJSONObject("response")
                antwort?.optJSONArray("output")?.let { ausgabe ->
                    for (index in 0 until ausgabe.length()) ausgabe.optJSONObject(index)?.let(::sammle)
                }
                abgeschlossenerText = antwort?.let(::holeText)
                ""
            }
    ...
    /** Merkt sich Quellen und Bilder eines fertigen Ausgabe-Elements; jedes nur einmal. */
    private fun sammle(element: JSONObject) {
        val kennung = element.optString("id").ifBlank { element.toString().hashCode().toString() }
        if (!gesehen.add(kennung)) return
        when (element.optString("type")) {
            "web_search_call" -> {
                val funde = element.optJSONObject("action")?.optJSONArray("sources") ?: return
                for (index in 0 until funde.length()) {
                    funde.optJSONObject(index)?.optString("url")?.takeIf(String::isNotBlank)?.let(quellen::add)
                }
            }
            "image_generation_call" -> element.optString("result").takeIf(String::isNotBlank)?.let(bilder::add)
            "message" -> {
                val inhalt = element.optJSONArray("content") ?: return
                for (index in 0 until inhalt.length()) {
                    val hinweise = inhalt.optJSONObject(index)?.optJSONArray("annotations") ?: continue
                    for (stelle in 0 until hinweise.length()) {
                        hinweise.optJSONObject(stelle)?.optString("url")?.takeIf(String::isNotBlank)?.let(quellen::add)
                    }
                }
            }
        }
    ...
    fun ergebnis(): String {
        ...
        return abgeschlossenerText?.takeIf(String::isNotBlank)
            ...
            ?: (if (bilder.isNotEmpty()) "" else null)
    ...
    private fun holeText(antwort: JSONObject): String? {
        ...
        return buildString {
            for (aussen in 0 until ausgabe.length()) {
                val element = ausgabe.optJSONObject(aussen) ?: continue
                // Zwischenbemerkungen während der Suche gehören nicht in die Antwort.
                if (element.optString("phase") == "commentary") continue
                val inhalt = element.optJSONArray("content") ?: continue
