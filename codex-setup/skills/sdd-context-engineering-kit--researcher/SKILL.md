...
**Skills** are reusable knowledge artifacts stored in `.codex/skills/<skill-name>/prompts:skill.md`. They capture expertise about specific technologies, libraries, patterns, or techniques that multiple tasks and agents can leverage.
...
- AGENTS.md, constitution.md, README.md if present for project context
- **CRITICAL**: Check `.codex/skills/` directory for existing related skills (see Stage 0)
...
1. List all directories in `.codex/skills/`:

   ```bash
   ls -la .codex/skills/ 2>/dev/null || echo "No skills directory yet"
   ```
...
1. Run the scratchpad creation script `bash ${CODEX_PLUGIN_ROOT}/scripts/create-scratchpad.sh` - it will create the file: `.specs/scratchpad/<hex-id>.md`
...
- Project Patterns: [From AGENTS.md, constitution.md]
...
1. Read the complete skill file: `.codex/skills/<skill-name>/prompts:skill.md`
...
```bash
# Skill path (existing)
.codex/skills/<existing-skill-name>/prompts:skill.md
```
...
1. **Write to**: `.codex/skills/<skill-name>/prompts:skill.md`
...
> 
> Skill location: `.codex/skills/<skill-name>/prompts:skill.md`
```
...
> 
> Skill location: `.codex/skills/jwt-authentication/SKILL.md`
...
- **Skip skill discovery**: ALWAYS check `.codex/skills/` first
...
- [ ] Checked for existing skills in `.codex/skills/`
...
- [ ] Skill file created/updated at `.codex/skills/<skill-name>/prompts:skill.md`
...
```
Skill Complete: .codex/skills/<skill-name>/prompts:skill.md
