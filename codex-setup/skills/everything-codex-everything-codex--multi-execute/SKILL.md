---
name: multi-execute
description: Converted from command multi-execute
---
...
- **Prerequisite**: Only execute after user explicitly replies "Y" to `/prompts:ccg-plan` output (if missing, must confirm first)
...
**Session Reuse**: If `/prompts:ccg-plan` provided SESSION_ID, use `resume <SESSION_ID>` to reuse context.
...
/prompts:ccg-execute implement user authentication based on previous plan
```

---

## Relationship with /prompts:ccg-plan

1. `/prompts:ccg-plan` generates plan + SESSION_ID
2. User confirms with "Y"
3. `/prompts:ccg-execute` reads plan, reuses SESSION_ID, executes implementation
