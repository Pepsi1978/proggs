...
```swift
enum ModelTier {
    case fast      // codex-3-haiku: Quick, cheap, simple tasks
    case balanced  // codex-sonnet: Good balance for most tasks
    case powerful  // codex-opus: Complex reasoning, synthesis

    var modelId: String {
        switch self {
        case .fast: return "codex-3-haiku-20240307"
        case .balanced: return "codex-sonnet-4-20250514"
        case .powerful: return "codex-opus-4-20250514"
        }
    }
}
...
class AgentOrchestrator {
    ...
    func run(config: AgentConfig, userMessage: String) async -> AgentResult {
        ...
        while iteration < config.maxIterations {
            // Get agent response
            let response = await codex.message(
