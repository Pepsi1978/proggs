...
Hooks automatically run commands in response to Codex events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when Codex sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when Codex requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when Codex is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when Codex needs attention
**Value**: Know when Codex is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Codex is waiting\" with title \"Codex\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | Codex needs permission for a tool |
| `idle_prompt` | Codex waiting for input (60+ seconds) |
...
Hooks go in `.codex/settings.json`:
...
Recommend creating the `.codex/` directory if it doesn't exist.
