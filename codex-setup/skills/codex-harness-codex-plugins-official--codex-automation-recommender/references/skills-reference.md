...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.codex/skills/<name>/SKILL.md`. Skills can be invoked by Codex automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `.codex/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only Codex can invoke (for background knowledge)
...
3. Run validation: `bash ~/.codex/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (Codex-only)

Background knowledge Codex applies automatically:
...
The command output replaces the placeholder before Codex sees the skill content.
