---
name: resume-session
description: Load the most recent session file from ~/.codex/sessions/ and resume work with full context from where the last session ended.
---

# Resume Session Command

Load the last saved session state and orient fully before doing any work.
This command is the counterpart to `/prompts:everything-codex-everything-codex--save-session`.
...
```
/prompts:everything-codex-everything-codex--resume-session                                                      # loads most recent file in ~/.codex/sessions/
/prompts:everything-codex-everything-codex--resume-session 2024-01-15                                           # loads most recent session for that date
/prompts:everything-codex-everything-codex--resume-session ~/.codex/sessions/2024-01-15-session.tmp           # loads a specific legacy-format file
/prompts:everything-codex-everything-codex--resume-session ~/.codex/sessions/2024-01-15-abc123de-session.tmp  # loads a current short-id session file
```
...
3. If the folder does not exist or has no matching files, tell the user:
   ```
   No session files found in ~/.codex/sessions/
   Run /prompts:everything-codex-everything-codex--save-session at the end of a session to create one.
   ```
   Then stop.

If an argument is provided:

- If it looks like a date (`YYYY-MM-DD`), search `~/.codex/sessions/` for files matching
...
Report: "Session file found but appears empty or unreadable. You may need to create a new one with /prompts:everything-codex-everything-codex--save-session."
...
```
SESSION LOADED: /Users/you/.codex/sessions/2024-01-15-abc123de-session.tmp
════════════════════════════════════════════════
...
- After resuming, the user may want to run `/prompts:everything-codex-everything-codex--save-session` again at the end of the new session to create a new dated file
