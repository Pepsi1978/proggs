...
- [ ] Note which AI tools were used for initial exploration (Codex, Copilot, etc.)
...
6. **Start `/ce:work` on remote** - Begin implementing in Codex on the web (use `&` to run in background)
...
- **`/ce:work` on remote** → Run `/ce:work docs/plans/<plan_filename>.md &` to start work in background for Codex web
