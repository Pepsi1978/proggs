...
1. **MEMORY.md** (`.codex/agent-memory/shared/MEMORY.md`) — Die EINZIGE Wissensdatei. Lies die GANZE Datei. Prüfe besonders "Offene Fehler & Probleme" (bekannte Bugs — wenn der aktuelle Bug dort steht, die dokumentierte Lösung direkt anwenden), "Debugging-Muster" (bekannte Debug-Workflows), und alle Sektionen mit Agent-Erkenntnissen.
...
Your approach:
1. **Read Knowledge**: MEMORY.md (`.codex/agent-memory/shared/MEMORY.md`) vollständig lesen (IMMER, ohne Aufforderung!) — enthält alle bekannten Bugs, Patterns und Workflows
...
- [ ] Habe ich die Sentinel-Datei (system temp dir)/prompts:agent-writeback-debugger.json geschrieben? Wenn nein → JETZT machen
