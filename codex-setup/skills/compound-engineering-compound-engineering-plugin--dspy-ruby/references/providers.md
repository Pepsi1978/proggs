...
Provides the Codex adapter. Install it for any `anthropic/*` model id.

**SDK dependency:** `anthropic ~> 1.12`

```ruby
lm = DSPy::LM.new('anthropic/codex-sonnet-4-20250514',
...
lm = DSPy::LM.new('ruby_llm/codex-sonnet-4-20250514')
...
lm = DSPy::LM.new('ruby_llm/anthropic/codex-3-opus',
  api_key: ENV['OPENROUTER_API_KEY'],
  provider: 'openrouter'
)
```
...
lm = DSPy::LM.new('ruby_llm/anthropic.codex-3-5-sonnet', provider: 'bedrock')
...
| Anthropic   | `ruby_llm/codex-sonnet-4-20250514`       | Auto-detected from registry     |
...
| AWS Bedrock | `ruby_llm/anthropic.codex-3-5-sonnet`    | Configure RubyLLM globally      |
| VertexAI    | `ruby_llm/gemini-pro`                     | Configure RubyLLM globally      |
| OpenRouter  | `ruby_llm/anthropic/codex-3-opus`        | Use `provider: 'openrouter'`    |
...
powerful = DSPy::LM.new('anthropic/codex-sonnet-4-20250514', api_key: ENV['ANTHROPIC_API_KEY'])
...
DSPy.with_lm(powerful) do
  result = classifier.call(text: "Hello")   # uses codex-sonnet-4
end
```
...
```ruby
classifier = Classifier.new
classifier.configure { |c| c.lm = DSPy::LM.new('anthropic/codex-sonnet-4-20250514', api_key: ENV['ANTHROPIC_API_KEY']) }

fast = DSPy::LM.new('openai/gpt-4o-mini', api_key: ENV['OPENAI_API_KEY'])

DSPy.with_lm(fast) do
  classifier.call(text: "Test")  # still uses codex-sonnet-4 (instance-level wins)
end
```
...
agent.configure_predictor('thought_generator') do |c|
  c.lm = DSPy::LM.new('anthropic/codex-sonnet-4-20250514', api_key: ENV['ANTHROPIC_API_KEY'])
end
...
# Override the classifier to use Codex
DSPY_MODEL_CLASSIFIER=anthropic/codex-sonnet-4-20250514
...
|-------------------------------------------|--------------------------------|
| Single provider (OpenAI, Codex, or Gemini) | Dedicated gem (`dspy-openai`, `dspy-anthropic`, `dspy-gemini`) |
...
| Anthropic | `anthropic/codex-sonnet-4-20250514` | Balanced reasoning    |
