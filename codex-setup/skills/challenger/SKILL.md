...
**Before challenging**: Read `.codex/agent-memory/shared/MEMORY.md` (the whole file) for existing architecture decisions, patterns, and known failure patterns ("Offene Fehler & Probleme") — use these as ammunition for your challenges. If a plan repeats a known failure pattern, call it out immediately.
