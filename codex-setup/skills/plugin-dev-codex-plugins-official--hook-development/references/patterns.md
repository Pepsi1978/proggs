...
This reference provides common, proven patterns for implementing Codex hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CODEX_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${CODEX_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$CODEX_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart Codex after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$CODEX_PROJECT_DIR/.codex/my-plugin.local.json"
