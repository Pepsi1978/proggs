---
name: command--self-improve
description: Systematic self-improvement of the Codex development environment (~10-30 min, token-intensive). ONLY use when the user explicitly says "/self-improve", "verbessere dich", "optimiere deine Umgebung", "check dein Setup", "update alles", "mach mich besser", "aktualisiere alles", "pruef mein System", "System-Check", or "Umgebung pruefen". NEVER run this proactively or automatically — only on manual user request.
---
...
Das Whiteboard (`.codex/agent-memory/shared/MEMORY.md`) ist die EINZIGE zentrale Wissensdatei
fuer ALLE Komponenten des Codex Systems: Agents, Skills, Hooks, Plugins.
...
3. **Benutzer-Feedback lesen**: `~/.codex/projects/*/memory/feedback_*.md` enthaelt
   direkte Korrekturen und Praeferenzen des Benutzers. Diese MUESSEN gelesen werden, weil:
   - `~/.codex/projects/*/memory/project_*.md` — aktiver Projektstatus und Blocker (vor Verbesserungen lesen!)
   - `~/.codex/projects/*/memory/MEMORY.md` — Memory-Index (zeigt alle verfuegbaren Memory-Dateien)
...
4. **Codex-Mem Observations lesen**: `~/.codex/homunculus/projects/*/prompts:observations.jsonl`
...
7. **Keine separaten Wissens-Dateien**: Es gibt NUR dieses eine Whiteboard fuer System-Wissen.
   ...
   angeschlossen (siehe Header) und werden von /prompts:self-improve mitgelesen.
...
- Default: `effortLevel: high` (permanenter Standard laut AGENTS.md). Protected: `model: codex-opus-4-6`, `SUBAGENT_MODEL: sonnet`
...
- **SHELL/TERMINAL-UPDATES IMMER ZULETZT (KRITISCH — v5.8):**
  ...
  **Betroffene Tools**: PowerShell (pwsh), Git, Git Bash, Node.js, npm, Bun, Deno, Python, Codex CLI
...
- **Standard-Arbeitsverzeichnis**: `C:\Users\barwa\Codex` (Windows) / `~/prompts:proggs` (macOS).
  Self-Improve arbeitet AUSSCHLIESSLICH in diesem Repository. Alle Dateien, Commits, Pushes
  gehen nach `Pepsi1978/proggs`.
- **VERBOTENES Verzeichnis**: `C:\Users\barwa\Codex` / `~/prompts:codex` — dieses Verzeichnis gehoert
  AUSSCHLIESSLICH dem Codex-CLI-Tool. Codex darf dort NIEMALS lesen, schreiben oder
  in irgendeiner Form darauf zugreifen. Jeder Versuch ist ein schwerer Regelverstoss.
  Grund: Codex und Codex arbeiten parallel aber in getrennten Arbeitsbereichen.
  Jede Ueberschneidung kann zu Datenverlust und Konflikten fuehren.

## Thoroughness Detection

- **Standard**: "/prompts:self-improve" without extras → Stufe 1 quick, R1+R5+R6+R8 only (cached R2-R4)
...
**0a) Whiteboard lesen:**
Lese `.codex/agent-memory/shared/MEMORY.md` vollstaendig und erstelle eine kompakte Zusammenfassung:
...
Lese `~/Codex/Forschung.md` vollstaendig. Extrahiere:
...
- **Compound Effect Tracker (PFLICHT seit v5.20):**
  ...
  - Pruefen ob Direktive #1 (Superintelligenz) und #2 (Selbstbeobachtung) noch in allen
    Speicherorten vorhanden sind (rules/, AGENTS.md, Whiteboard, Memory)
...
The evolution-analyst reads `~/.codex/session-scores.jsonl` and `~/.codex/agent-memory/shared/MEMORY.md`
...
4. If BOTH fail: **WARNING** — "Session-Scorer liefert Dummy-Daten!" Then:
   - Load newest non-agent transcript from `~/.codex/projects/*/` (UUID filename, not `agent-*`)
...
Wenn `~/Codex/mcp-code-search/` existiert: Pruefe ob der Index aktuell ist.
- Pruefe `~/Codex/.code-search/.last-index-time` — wenn aelter als 24h oder nicht vorhanden: Neu indexieren.
**Indexierung**: Wenn noetig, ausfuehren:
  ```bash
  bash ~/.codex/scripts/semantic-search-healthcheck.sh --reindex
  ```
...
```bash
bash ~/.codex/scripts/semantic-search-healthcheck.sh   # macOS/Linux
pwsh ~/.codex/scripts/semantic-search-healthcheck.ps1  # Windows
```
...
- **Smart-Cache (v5.17 — git-diff-basiert, ersetzt TTL)**:
  Cache location: `~/.codex/self-improve-cache/` (NOT in shared memory — prevents contamination).
  **Invalidierung**: Pruefen ob sich Dateien unter `~/.codex/agents/`, `~/.codex/hooks/`,
  `~/.codex/rules/` oder `settings.json` seit dem letzten Cache-Zeitstempel geaendert haben.
  Wenn ja → betroffene Researcher (R2, R3, R4) neu starten. Wenn nein → gecachte Ergebnisse nutzen.
  Zeitstempel: `~/.codex/self-improve-cache/.last-cache-time`
  | Researcher | Cache-Regel | Grund |
  |------------|-------------|-------|
  | R1 Updates | IMMER frisch | Codex Releases aendern sich schnell |
...
4. **Fehler dokumentieren**: Jeden Absturz sofort ins Whiteboard eintragen (Sektion "Offene Fehler & Probleme" in `.codex/agent-memory/shared/MEMORY.md`):
   ```
...
`.codex/agent-memory/shared/MEMORY.md` — contains ALL knowledge: errors, system state,
...
- **KEINE Abkuerzungen oder Fachbegriffe** ohne Erklaerung. Wenn ein Begriff wie "MCP-Server" oder "Hook" unvermeidbar ist, in Klammern erklaeren: "MCP-Server (ein Zusatz-Werkzeug das Codex schlauer macht)"
...
**Ziel: Codex zum BESTEN AI-Programmierer der Welt machen. SUPER INTELLIGENZ.**
...
1. **Researcher spawnen** _(Nutzt R8-Ergebnisse statt eigenem Researcher-Spawn — siehe 5C)_: "Welche neuen MCP-Server, Codex-Code-Plugins oder CLI-Tools wurden in den letzten 30 Tagen veroeffentlicht die das DENKEN verbessern?" (Wissensgraphen, Verifikation, Semantic Search, Code-Analyse, Formal Verification, Codebase-Embeddings)
2. **Bewerten**: Wuerde dieses Tool Codex bei ECHTEN Aufgaben schlauer machen? (Nicht nur cool, sondern nuetzlich)
...
Recherchiere und implementiere neue Denkstrategien die Codex FUNDAMENTAL schlauer machen:
...
- Bewertung mit dem 4-Kriterien-Framework: Umsetzbarkeit, Intelligenz-Hebel, Codex-Code-Fit, Haltbarkeit
...
1. **Codebase-Semantik**: Gibt es einen MCP-Server der die Codebase semantisch indexiert? (z.B. Zilliz codex-context, CodeGraphContext) → Testen und ggf. installieren
...
durch /prompts:self-improve). Formel: `quality * efficiency * (1 + intel_bonus)`. /prompts:self-improve LIEST den Score
...
- **NUR Thorough-Modus:** 6C (Korrektur-Analyse), 6D (Regel-/prompts:agent-audit), 6F (Lern-Extraktion)
...
1. **Whiteboard-Scanner**: Lies `.codex/agent-memory/shared/MEMORY.md` komplett → Finde ALLE unfixten Probleme (Status: OFFEN), veraltete Eintraege, Widersprueche. Jedes Problem MUSS in diesem Lauf gefixt werden.
...
1. Lies ALLE Dateien in `~/.codex/rules/` — zaehle Regeln
...
Alle Aenderungen die /prompts:self-improve macht — neue Hooks, Agents, Skills, Configs, Settings —
...
1. Hook-Scripts (*.ps1 + *.sh + *.ts) → `~/Codex/codex-harness/hooks/`
2. Agent-Definitionen (*.md) → `~/Codex/codex-harness/agents/`
3. Commands + Unterordner → `~/Codex/codex-harness/commands/` (inkl. self-improve-ref/)
4. Rules → `~/Codex/codex-harness/rules/`
5. AGENTS.md → `~/.codex/AGENTS.md`
6. **settings-reference.json** → `~/Codex/codex-harness/settings-reference.json`
   (Hooks, Plugins, Marketplaces, Env — wird von auto-sync in settings.json gemerged)
7. **Knowledge-Dateien** → `~/Codex/codex-harness/agent-memory/shared/`
...
- Plugin-Caches (~/.codex/plugins/cache/)
- Session-Transcripts, session-scores.jsonl

**Sync ausfuehren:**
```bash
bash ~/.codex/scripts/self-improve-sync.sh "#NNN - Self-improve sync"   # macOS/Linux
pwsh ~/.codex/scripts/self-improve-sync.ps1 "#NNN - Self-improve sync"  # Windows
```
...
**Session-Score-Trend (NEW v5.1):** Read `~/.codex/session-scores.jsonl`, compute 5-session
