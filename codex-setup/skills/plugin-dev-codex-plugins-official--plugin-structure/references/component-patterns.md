...
4. **Register components**: Make available to Codex
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Codex initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Codex looks up → Executes
**Agents**: Task arrives → Codex evaluates capabilities → Selects agent
**Skills**: Task context matches description → Codex loads skill
**Hooks**: Event occurs → Codex calls matching hooks
...
**Note**: Codex doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Codex doesn't support file references.
