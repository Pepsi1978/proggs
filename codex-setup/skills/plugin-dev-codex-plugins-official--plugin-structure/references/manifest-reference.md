...
**Required path**: `.codex-plugin/plugin.json`

The manifest MUST be in the `.codex-plugin/` directory at the plugin root. Codex will not recognize plugins without this file in the correct location.
...
- Plugin identification in Codex
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${CODEX_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
Codex validates the manifest on plugin load:
