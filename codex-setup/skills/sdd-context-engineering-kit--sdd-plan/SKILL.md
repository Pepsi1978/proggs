...
7. **Ensure directories exist**:
   ...
   ```bash
   bash ${CODEX_PLUGIN_ROOT}/scripts/create-folders.sh
   ```
   ...
   - `.codex/skills/` - Reusable skill documents
...
3. **Provide the value of `${CODEX_PLUGIN_ROOT}` so agents can resolve paths like `@${CODEX_PLUGIN_ROOT}/scripts/create-scratchpad.sh`**
...
- Skill file path (e.g., `.codex/skills/<skill-name>/SKILL.md`)
...
- **Prompt**:
  ...
  Read ${CODEX_PLUGIN_ROOT}/skills/plan/analyse-business-requirements.md and execute it exactly as is!
...
- **Prompt**:
  ...
  Read @${CODEX_PLUGIN_ROOT}/prompts/judge.md for evaluation methodology and execute.
