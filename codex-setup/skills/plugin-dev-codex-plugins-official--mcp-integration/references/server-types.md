...
Complete reference for all MCP server types supported in Codex plugins.
...
{
  "my-server": {
    "command": "${CODEX_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: Codex spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Codex session
4. **Shutdown**: Process terminated when Codex exits
...
{
  "custom": {
    "command": "${CODEX_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${CODEX_PLUGIN_ROOT}**
