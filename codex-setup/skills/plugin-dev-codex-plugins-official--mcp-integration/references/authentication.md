...
Complete guide to authentication methods for MCP servers in Codex plugins.
...
Codex automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. Codex detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by Codex
...
No additional auth configuration needed! Codex handles everything.
...
Tokens are stored securely by Codex:
...
```json
{
  "api": {
    ...
    "headersHelper": "${CODEX_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${CODEX_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${CODEX_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
