---
name: command--claudeception
description: Continuous learning command that extracts reusable knowledge from work sessions and improves the Codex workspace over time.
---
...
codifies it into new Codex skills. This enables autonomous improvement over time.
...
**List all skills**: Use Glob tool with pattern `**/prompts:skill.md` in each skill directory.
...
- "Codex skill description semantic matching 2026"
...
author: [original-author or "Codex"]
version: 1.0.0
date: [YYYY-MM-DD]
---
...
- **Project-specific skills**: `.codex/skills/[skill-name]/prompts:skill.md`
- **User-wide skills**: `~/.codex/skills/[skill-name]/prompts:skill.md`

**IMPORTANT**: Use the /prompts:skill-creator-skill-creator skill via the Skill tool for creating new skills. Do not write skill files directly with the Write tool — the skill-creator handles formatting, validation, and registration.
...
When `/prompts:claudeception` is invoked at the end of a session:
...
**Whiteboard**: `.codex/agent-memory/shared/MEMORY.md` (EINZIGE zentrale Wissensdatei)
...
Also invoke when:
- User runs `/prompts:claudeception` to review the session
