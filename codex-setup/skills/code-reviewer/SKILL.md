...
**Before reviewing**: Read `.codex/agent-memory/shared/MEMORY.md` (the whole file) to see what all agents have learned about the environment (tools, conventions, recent changes, known failures). Use this context to inform your review.

**After reviewing**: If you discover recurring patterns, common mistakes, or project conventions not yet documented, update `.codex/agent-memory/shared/MEMORY.md` — add entries under "Erkenntnisse aus Code Reviews". Keep entries concise (1 line per pattern). This helps the self-improve skill create better rules and hooks based on real code review findings.
...
3. **Design**: Apple HIG (Swift), Fluent Design (C#/prompts:wpf), idiomatic patterns per language.
