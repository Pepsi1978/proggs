...
Lies **zuerst** `.codex/agent-memory/shared/MEMORY.md`, besonders:
...
1. Test-Befehl und Build-Befehl aus dem Projekt ermitteln (z.B. `./prompts:gradlew test`, `cargo test`, `npm test`, `go test ./...`)
...
`./prompts:gradlew test` ergibt 3 Fehler:
...
**Verifikation:** `./prompts:gradlew test` → GRUEN. Alle 3 Tests bestehen.
