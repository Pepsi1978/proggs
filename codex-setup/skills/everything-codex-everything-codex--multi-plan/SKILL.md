---
name: multi-plan
description: Converted from command multi-plan
---
...
**Session Reuse**: Each call returns `SESSION_ID: xxx` (typically output by wrapper), **MUST save** for subsequent `/prompts:ccg-execute` use.
...
**If ace-tool MCP is NOT available**, use Codex built-in tools as fallback:
...
### SESSION_ID (for /prompts:ccg-execute use)
...
**`/prompts:ccg-plan` responsibilities end here, MUST execute the following actions**:
...
3. Output prompt in **bold text** (MUST use actual saved file path):
   ...
   **Please review the plan above. You can:**
   ...
   ```
   /prompts:ccg-execute .codex/plan/actual-feature-name.md
   ```
   ---
...
- Ask user "Y/N" then auto-execute (execution is `/prompts:ccg-execute`'s responsibility)
- Any write operations to production code
- Automatically call `/prompts:ccg-execute` or any implementation actions
...
```bash
/prompts:ccg-execute .codex/plan/<feature-name>.md
```
...
5. **SESSION_ID Handoff** – Plan must include `CODEX_SESSION` / `GEMINI_SESSION` at end (for `/prompts:ccg-execute resume <SESSION_ID>` use)
