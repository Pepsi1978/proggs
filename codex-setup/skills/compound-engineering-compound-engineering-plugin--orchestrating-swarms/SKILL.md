...
```bash
CODEX_CODE_TEAM_NAME="my-project"
CODEX_CODE_AGENT_ID="worker-1@my-project"
CODEX_CODE_AGENT_NAME="worker-1"
CODEX_CODE_AGENT_TYPE="Explore"
CODEX_CODE_AGENT_COLOR="#4A90D9"
CODEX_CODE_PLAN_MODE_REQUIRED="false"
CODEX_CODE_PARENT_SESSION_ID="session-xyz"
```
...
Task({
  ...
  name: "worker",
  subagent_type: "general-purpose",
  prompt: "Your name is $CODEX_CODE_AGENT_NAME. Use it when sending messages to team-lead."
})
```
...
- Explicitly set via `CODEX_CODE_SPAWN_BACKEND=in-process`
...
// export CODEX_CODE_SPAWN_BACKEND=in-process
```
...
- Explicitly set via `CODEX_CODE_SPAWN_BACKEND=tmux`
...
export CODEX_CODE_SPAWN_BACKEND=tmux
```
...
# Auto-detect (default)
unset CODEX_CODE_SPAWN_BACKEND
```
...
Replace YOUR_NAME with your actual agent name from $CODEX_CODE_AGENT_NAME.
`
