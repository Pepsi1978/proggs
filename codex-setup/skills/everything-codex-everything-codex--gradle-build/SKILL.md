...
|-----------|---------------|
| `build.gradle.kts` + `composeApp/` (KMP) | `./prompts:gradlew composeApp:compileKotlinMetadata 2>&1` |
| `build.gradle.kts` + `app/` (Android) | `./prompts:gradlew app:compileDebugKotlin 2>&1` |
| `settings.gradle.kts` with modules | `./prompts:gradlew assemble 2>&1` |
| Detekt configured | `./prompts:gradlew detekt 2>&1` |
...
| Duplicate class | Check for conflicting dependencies with `./prompts:gradlew dependencies` |
| KSP error | Run `./prompts:gradlew kspCommonMainKotlinMetadata` to regenerate |
