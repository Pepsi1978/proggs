...
4. **NIEMALS** Dateien AUSSERHALB von `~/Codex/` anfassen
5. **NIEMALS** `~/.codex/settings.json` oder `~/.codex/settings.local.json` veraendern
6. Bei JEDEM Fehler: Sofort Backup wiederherstellen und abbrechen

## Ablauf

### Phase 1: Komponenten-Inventar
Lies das Whiteboard (`.codex/agent-memory/shared/MEMORY.md`) und identifiziere
...
**BACKUP-PFAD**: Immer `~/Codex/.chaos-backup/` (absolut, im Repo).

Fuer jedes Szenario ATOMARER Ablauf:
1. **Recovery-Datei schreiben**: `~/Codex/.chaos-backup/RECOVERY.txt` mit:
...
2. **Backup erstellen**: Datei kopieren nach `~/Codex/.chaos-backup/`
...
**CRASH-RECOVERY**: Falls der Agent abstuerzt und `~/Codex/.chaos-backup/RECOVERY.txt`
existiert, kann jeder andere Agent oder der Benutzer wiederherstellen:
`cp ~/Codex/.chaos-backup/[BACKUP] [ORIGINAL]`

**Erlaubte Test-Dateien**: MEMORY.md, bug-cases.jsonl, error-antigens.jsonl, superintelligenz.md
**VERBOTENE Dateien**: settings.json, settings.local.json, .gitattributes, .mcp.json, AGENTS.md
...
Bericht in `.codex/agent-memory/shared/MEMORY.md` unter "Chaos-Test Ergebnisse" eintragen.
