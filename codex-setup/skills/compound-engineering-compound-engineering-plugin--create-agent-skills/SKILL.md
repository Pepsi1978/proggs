...
description: Expert guidance for creating Codex skills and slash commands. Use when working with SKILL.md files, authoring new skills, improving existing skills, creating slash commands, or understanding skill structure and best practices.
---

# Creating Skills & Commands

This skill teaches how to create effective Codex skills following the official specification from [code.claude.com/docs/en/skills](https://code.claude.com/docs/en/skills).

## Commands and Skills Are Now The Same Thing

Custom slash commands have been merged into skills. A file at `.codex/commands/review.md` and a skill at `.codex/skills/review/SKILL.md` both create `/review` and work the same way. Existing `.codex/commands/` files keep working. Skills add optional features: a directory for supporting files, frontmatter to control invocation, and automatic context loading.
...
- Background knowledge Codex should auto-load
...
| `description` | Recommended | What it does AND when to use it. Codex uses this for auto-discovery. Max 1024 chars. |
| `argument-hint` | No | Hint shown during autocomplete. Example: `[issue-number]` |
| `disable-model-invocation` | No | Set `true` to prevent Codex auto-loading. Use for manual workflows like `/deploy`, `/commit`. Default: `false`. |
| `user-invocable` | No | Set `false` to hide from `/` menu. Use for background knowledge. Default: `true`. |
| `allowed-tools` | No | Tools Codex can use without permission prompts. Example: `Read, Bash(git *)` |
...
| Frontmatter | User can invoke | Codex can invoke | When loaded |
|-------------|----------------|-------------------|-------------|
...
**Use `disable-model-invocation: true`** for workflows with side effects: `/deploy`, `/commit`, `/triage-prs`, `/send-slack-message`. You don't want Codex deciding to deploy because your code looks ready.
...
The `` !`command` `` syntax runs shell commands before content is sent to Codex:
...
- **Punting to Codex** - Scripts should handle errors explicitly
...
- [Extend Codex with skills - Official Docs](https://code.claude.com/docs/en/skills)
