...
```text
1. ./prompts:gradlew build        -> Parse error message
2. Read affected file     -> Understand context
3. Apply minimal fix      -> Only what's needed
4. ./prompts:gradlew build        -> Verify fix
5. ./prompts:gradlew test         -> Ensure nothing broke
```
...
./prompts:gradlew dependencies --configuration runtimeClasspath

# Force refresh dependencies
./prompts:gradlew build --refresh-dependencies

# Clear project-local Gradle build cache
./prompts:gradlew clean && rm -rf .gradle/build-cache/

# Check Gradle version compatibility
./prompts:gradlew --version

# Run with debug output
./prompts:gradlew build --debug 2>&1 | tail -50

# Check for dependency conflicts
./prompts:gradlew dependencyInsight --dependency <name> --configuration runtimeClasspath
```
...
- **Always** run `./prompts:gradlew build` after each fix to verify
