...
Guide the user through creating a complete, high-quality Codex plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
2. Analyze plugin requirements and determine needed components:
   ...
   > **Note:** The `commands/` directory is a legacy format. For new plugins, user-invoked slash commands should be created as skills in `skills/<name>/prompts:skill.md`. Both are loaded identically — the only difference is file layout. `commands/` remains an acceptable legacy alternative.
...
2. Choose plugin location:
   ...
   - Offer options: current directory, ../prompts:new-plugin-name, custom path
...
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR Codex (not TO user)
...
> Prefer `skills/<name>/prompts:skill.md` for new plugins. Use `commands/` only when maintaining an existing plugin that already uses this layout.
...
**Goal**: Test that plugin works correctly in Codex
...
2. **Verification checklist** for user to perform:
   ...
   - [ ] User-invoked skills appear in `/prompts:help` and execute correctly
...
3. **Testing recommendations**:
   ...
   - For user-invoked skills: Run `/prompts:plugin-name-skill-name` with various arguments
   - For agents: Create scenarios matching agent examples
   - For hooks: Use `codex --debug` to see hook execution
   - For MCP: Use `/prompts:mcp` to verify servers and tools
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR Codex (not TO user)
