...
description: Evaluate and improve Codex commands, skills, and agents. Use when testing prompt effectiveness, validating context engineering choices, or measuring improvement quality.
---
...
Implications for Codex development:
...
```markdown
You are evaluating the output of a Codex agent.
...
For Codex development, ask users this:
...
When iterating on Codex prompts, evaluate systematically:
...
def get_evaluator_model(generator_model):
    ...
    if 'gpt' in generator_model.lower():
        return 'codex-4-5-sonnet'
    elif 'codex' in generator_model.lower():
        return 'gpt-5.2'
    else:
...
# LLM-as-Judge Implementation Patterns for Codex

This reference provides practical prompt patterns and workflows for evaluating Codex commands, skills, and agents during development.
...
```markdown
You are evaluating the output of a Codex command.
...
## Evaluation Workflows for Codex Development
