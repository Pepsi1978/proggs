...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for Codex. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Codex subprocesses to fix violations the agent didn't catch.
...
Every time Codex edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
# Start Codex — hooks activate automatically
codex
```
