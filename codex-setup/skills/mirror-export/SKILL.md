...
You are the **export** half of the Universal Mirror Bridge. Your job is to scan everything that changed in this session and write it into the shared mirror ledger so that OTHER platforms (macOS/Windows) and OTHER CLIs (Codex, Codex CLI, Gemini CLI) can automatically import and apply these changes.
...
**Before starting**: Read `.codex/agent-memory/shared/MEMORY.md` to understand current system state and recent issues.
...
Detect the CLI from environment: Codex is the default. Check for `$CODEX_CLI` or `$GEMINI_CLI` env vars if they exist.
...
git -C ~/prompts:proggs log --oneline -10
git -C ~/prompts:proggs diff --name-status HEAD~10..HEAD

# Uncommitted changes
git -C ~/prompts:proggs diff --name-only HEAD
git -C ~/prompts:proggs status --short
```

Also check:
- Whiteboard (`~/.codex/agent-memory/shared/MEMORY.md`) for today's entries
- `~/.codex/agents/` for new or modified agents
- `~/.codex/rules/` for new or modified rules
- `~/.codex/hooks/` for new or modified hooks
- `~/.codex/settings.json` and `~/.codex/settings.local.json` for changes
- `~/Codex/.mcp.json` for MCP changes
...
| `directive` | Changes to AGENTS.md |
| `plugin` | New Codex plugin installations |
| `whiteboard` | Changes to MEMORY.md / shared knowledge |

### Schritt 3 — Deduplizierung

Read the existing ledger: `~/Codex/codex-harness/mirror-ledger.md`
...
- `directive`: The exact section added/changed in AGENTS.md, with before/after if modified.
...
- NEVER create entries for files in other CLI's setup directories (e.g., don't export Codex-specific files when running in Codex).
...
It is shared across ALL platforms (macOS, Windows) and ALL CLIs (Codex, Codex CLI, Gemini CLI)
