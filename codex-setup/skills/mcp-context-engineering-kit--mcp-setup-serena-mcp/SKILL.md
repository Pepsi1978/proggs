...
2. **Project level (personal preferences)** - Configuration stays local, not tracked in git
   - AGENTS.md updates go to: `./CODEX.local.md`
...
3. **User level (global)** - Configuration applies to all projects for this user
   - AGENTS.md updates go to: `~/.codex/AGENTS.md`
...
2. **Identify client type**: Determine which MCP client the user is using (Codex, Codex Desktop, Cursor, VSCode, etc.)
...
6. If MCP was just setup, ask user to restart Codex to load the new MCP server, write to user explisit instructions, including "exit codex code console, then run 'codex --continue' and then write "continue" to continue setup process"
...
```markdown
You must restart Codex to load the new MCP server:

  1. Exit Codex console (type exit or press Ctrl+C)
  2. Run codex --continue
