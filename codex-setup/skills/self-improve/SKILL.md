...
Mandatory whenever the user explicitly asks about Claude Code or Claude Code setup changes.
...
- `codex-setup/scripts/validate-codex-setup.*`
...
- local and repo `SKILL.md` files still pass loader-safe frontmatter checks (`name`, `description`, YAML-safe inline syntax, `description <= 1024`)
- no Windows Bash-to-Python heredoc one-offs remain in Codex control files
- the active Codex launcher still invokes `~/.codex/hooks/session-scorer.ts`, and the scorer remains side-effect free apart from `~/.codex/session-scores.jsonl`
...
- include bridge-relevant addresses or adoption guidance when the implementation affects Claude Code, Gemini CLI, or future CLI bridges
...
Before closing the run, execute `node codex-setup/scripts/validate-codex-setup.mjs` and treat any failure as blocking.
