...
Use `${CODEX_PLUGIN_ROOT}` for portable paths within plugin:

```markdown
Read ${CODEX_PLUGIN_ROOT}/tasks/step-1-workflow-name.md and execute.
```
...
mkdir -p ${CODEX_PLUGIN_ROOT}/tasks

# Optional: Create agents directory (if using custom agents)
mkdir -p ${CODEX_PLUGIN_ROOT}/agents
```
...
- **Prompt**:
  \`\`\`
  Read ${CODEX_PLUGIN_ROOT}/tasks/step-2-<workflow>-<name>.md and execute.
...
| `model` | Specific Codex model (sonnet, opus, haiku) | Inherits from conversation |
...
1. Agent 1: Security analysis → Read ${CODEX_PLUGIN_ROOT}/tasks/step-1a-security.md
2. Agent 2: Performance analysis → Read ${CODEX_PLUGIN_ROOT}/tasks/step-1b-performance.md
3. Agent 3: Code quality analysis → Read ${CODEX_PLUGIN_ROOT}/tasks/step-1c-quality.md
...
- **Prompt**:
  \`\`\`
  Read ${CODEX_PLUGIN_ROOT}/tasks/step-1-feature-impl-research.md
...
- **Prompt**:
  \`\`\`
  Read ${CODEX_PLUGIN_ROOT}/tasks/step-2-feature-impl-architecture.md
...
- **Prompt**:
  \`\`\`
  Read ${CODEX_PLUGIN_ROOT}/tasks/step-3-feature-impl-implement.md
...
- [ ] File paths use `${CODEX_PLUGIN_ROOT}` for portability
...
1. **Directories**:
   - `${CODEX_PLUGIN_ROOT}/tasks/` - All task files directly here
   - `${CODEX_PLUGIN_ROOT}/agents/` - (Optional) Custom agent definitions
