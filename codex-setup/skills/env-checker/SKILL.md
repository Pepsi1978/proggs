...
**Before checking**: Read `.codex/agent-memory/shared/MEMORY.md` (the whole file) — especially "Systemzustand" for previous environment snapshots and known configuration issues.
...
```bash
echo "=== VERSIONS ===" && codex --version 2>&1 && echo "---" && node --version && echo "---" && npm --version && echo "---" && bun --version 2>/dev/null && echo "---" && deno --version 2>&1 | head -1 && echo "---" && rustc --version && echo "---" && cargo --version && echo "---" && go version && echo "---" && dotnet --version && echo "---" && java --version 2>&1 | head -1 && echo "---" && kotlinc -version 2>&1 && echo "---" && gradle --version 2>&1 | grep "Gradle " && echo "---" && git --version && echo "---" && gh --version 2>&1 | head -1 && echo "---" && cmake --version 2>&1 | head -1 && echo "---" && biome --version 2>&1 && echo "---" && adb --version 2>&1 | head -1
```

Also check npm registry for Codex latest: `npm view @anthropic-ai/codex version`

### 2. Settings Verification
Read `~/.codex/settings.json` and verify:
- `model` = "codex-opus-4-6"
- `effortLevel` = "high"
- `env.CODEX_CODE_EFFORT_LEVEL` = "high"
- `env.CODEX_CODE_SUBAGENT_MODEL` = "sonnet"
- `env.CODEX_AUTOCOMPACT_PCT_OVERRIDE` = "95"
- `env.CODEX_CODE_MAX_OUTPUT_TOKENS` = "64000"
- `alwaysThinkingEnabled` = true
- Count enabledPlugins

### 3. Agent Model Tiers
Check all files in `~/.codex/agents/*.md` — verify model assignments:
...
### 7. Backup Drift
Compare `~/.codex/{agents,rules,commands,hooks}` with `~/Codex/codex-harness/{agents,rules,commands,hooks}`
...
For each rule file in `~/.codex/rules/*.md`, check it mentions format, lint, and test commands.
