...
- Bash tool disabled by default; commands must be explicitly allowed via `--allowedTools` in `codex_args`
...
| Configuration | Risk |
|--------------|------|
| `codex_args: "--allowedTools Bash(*)"` | Unrestricted shell access; any prompt injection achieves full RCE |
...
```yaml
codex_args: '--allowedTools "Bash(npm test:*) Bash(git diff:*)"'
```
...
| Remediation Need | Codex Action | OpenAI Codex | Gemini CLI | GitHub AI Inference |
|-----------------|-------------------|--------------|------------|-------------------|
...
| Disable dangerous mode | Remove `Bash(*)` from `codex_args` | Remove `danger-full-access` from `sandbox` | Remove `--yolo` from CLI args | N/A |
