...
| Codex Action | Medium confidence | `Bash(echo:*)` in `--allowedTools` is structurally similar -- allows the `echo` command through Bash, which may evaluate subshell expansion. Unconfirmed at runtime. |
...
**Confidence note:** This vector is CONFIRMED for Gemini CLI (PoCs 1-2 achieved arbitrary command execution via `echo $(env)` and `echo $(whoami)`). For Codex Action and OpenAI Codex, the attack is structurally similar but behavior under subshell expansion needs runtime testing to confirm exploitability.
...
2. **Codex Action:** `with.codex_args` containing `--allowedTools` with `Bash(echo:*)`, `Bash(cat:*)`, `Bash(printf:*)`, or similar restricted-but-expandable command patterns
...
2. `with.codex_args` (Codex Action) -- look for `--allowedTools` flags with `Bash(command:*)` patterns
