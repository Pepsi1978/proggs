...
|--------|-----------|-------|
| Codex Action | Yes | `codex_args` with `--allowedTools Bash(*)` disables tool restrictions |
...
**Codex Action (`anthropics/codex-action`):**

- `with.codex_args` containing `--allowedTools Bash(*)` or `--allowedTools "Bash(*)"` -- unrestricted shell access, the AI can execute any command
- `with.codex_args` with broad tool patterns combining multiple unrestricted categories (e.g., `Bash(npm:*) Bash(git:*) Bash(curl:*)`)
...
- **Codex:** Parse `with.codex_args` string for `--allowedTools` patterns. Also check `with.settings` for external config file path
...
```yaml
# Codex Action -- unrestricted shell
- uses: anthropics/codex-action@v1
  with:
    codex_args: "--allowedTools Bash(*)"
...
- **Specific restricted tool patterns** in Codex: `--allowedTools "Bash(npm test:*)"` or `--allowedTools "Bash(echo:*)"` -- these are restrictive, not dangerous (though they may be exploitable via Vector F for subshell expansion)
...
- **Default configurations:** Actions without explicit sandbox/safety config fields -- defaults are generally safe (Codex defaults to restricted tools, Codex defaults to `sandbox: workspace-write`, Gemini defaults to sandbox enabled)
- **Codex `--allowedTools` with narrow patterns:** e.g., `--allowedTools "Read(*) Grep(*)"` -- read-only tools pose minimal risk
