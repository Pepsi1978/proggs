...
description: "Audits GitHub Actions workflows for security vulnerabilities in AI agent integrations including Codex Action, Gemini CLI, OpenAI Codex, and GitHub AI Inference. Detects attack vectors where attacker-controlled input reaches AI agents running in CI/CD pipelines, including env var intermediary patterns, direct expression injection, dangerous sandbox configurations, and wildcard user allowlists. Use when reviewing workflow files that invoke AI coding agents, auditing CI/CD pipeline security for prompt injection risks, or evaluating agentic action configurations."
allowed-tools:
  - Read
  - Grep
  - Glob
...
- Reviewing CI/CD configurations that invoke Codex Action, Gemini CLI, or OpenAI Codex
...
|-----------------|-------------|
| `anthropics/codex-action` | Codex Action |
...
- `codex_args` -- CLI arguments passed to Codex (may contain `--allowedTools`, `--disallowedTools`)
...
- `settings` -- path to Codex settings file (may configure tool permissions)
...
"Found N AI action instances across M workflow files: X Codex Action, Y Gemini CLI, Z OpenAI Codex, W GitHub AI Inference"
...
4. **Name the specific consequence** in the final step (e.g., "Codex executes with tainted prompt -- attacker achieves arbitrary code execution"), not just the YAML element.
