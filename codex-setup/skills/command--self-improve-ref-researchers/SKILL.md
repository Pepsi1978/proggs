---
name: command--self-improve-ref-researchers
description: "Converted from command self-improve-ref:researchers"
---
...
```bash
# Pruefe ob sich ~/.codex/ seit dem letzten Cache-Zeitstempel geaendert hat:
...
if [ -f "$CACHE_STAMP" ]; then
  ...
  CHANGES=$(find "$HOME/.codex/agents" "$HOME/.codex/hooks" "$HOME/.codex/rules" \
    -newer "$CACHE_STAMP" -type f 2>/dev/null | wc -l)
  SETTINGS_CHANGED=$(find "$HOME/.codex/settings.json" -newer "$CACHE_STAMP" 2>/dev/null | wc -l)
...
When a run completes, save a snapshot to the cache directory (`~/.codex/self-improve-cache/`):
- Write R2, R3, R4 summaries as markdown files (e.g., `R2_plugins.md`, `R3_parallel.md`, `R4_versions.md`)
- Update the timestamp: `touch ~/.codex/self-improve-cache/.last-cache-time`
...
werden nicht akzeptiert. Der Haupt-Codex parst das JSON und baut daraus automatisch die
...
**Fuer den Haupt-Codex (Parsing):** Nach Rueckkehr jedes Researchers den ```json-Block
...
### R1 — Codex Updates (ALWAYS run)
```
"Research Codex CLI updates and new features. Current version: [version].
...
### R2 — Plugins & Marketplace (skip if no env changes since last cache — git-diff check against ~/.codex/agents/, ~/.codex/hooks/, ~/.codex/rules/; run unconditionally if > 14 days since last run)

**Before spawning R2**: Read `~/.codex/settings.json`, extract all `enabledPlugins` keys, and substitute them into the R2 prompt below in place of `[INSTALLED_PLUGINS_LIST]`.

```
"Research new Codex plugins available in [month+year].
Search: (1) official plugins (anthropics/codex-plugins-official),
...
### R3 — Parallelization & Automation (skip if no env changes since last cache — git-diff check against ~/.codex/agents/, ~/.codex/hooks/, ~/.codex/rules/; run unconditionally if > 14 days since last run)
```
"Research Codex agent teams, parallelization, automation best practices as of [today].
...
### R4 — Tool Versions (skip if no env changes since last cache — git-diff check against ~/.codex/agents/, ~/.codex/hooks/, ~/.codex/rules/; run unconditionally if > 14 days since last run)
```
...
(3) Codex security advisories, (4) supply chain alerts.
...
Your job is to discover NEW IDEAS for improving a Codex development environment
on macOS and Windows.

Think OUTSIDE the box:
1. Search for blog posts, GitHub repos, and discussions about creative Codex setups
...
1. **Reasoning-Durchbrueche** (akademische Forschung):
   ...
   Fuer jedes relevante Paper: Kernidee in 2 Saetzen + wie es KONKRET in Codex umsetzbar ist.

2. **Kognitive Werkzeuge** (sofort einsetzbar):
   Suche nach MCP-Servern, Codex-Code-Plugins, CLI-Tools die das DENKEN verbessern:
   ...
   Fuer jedes Tool: Name, Link, was es tut, wie es Codex SCHLAUER macht (nicht nur schneller).
...
3. **Kompetitive Analyse** (Was macht die Konkurrenz?):
   ...
   - Welche Techniken nutzen sie die Codex NICHT hat?
...
```
"Security review of Codex plugin '[owner/repo]'.
