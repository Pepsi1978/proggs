...
- You want Codex's own review (just ask Codex directly)
...
**Question 3 — Project context** (skip if neither AGENTS.md nor AGENTS.md exists):

Check for AGENTS.md first, then AGENTS.md in the repo root.
...
```
User: /second-opinion
Codex: [asks 4 questions: tool, scope, context, focus]
User: picks "Both", "Branch diff", "Yes include AGENTS.md", "Security"
Codex: [detects default branch = main]
Codex: [shows diff --stat: 6 files, +103 -15]
Codex: [assembles prompt with review instructions + AGENTS.md + security focus + diff]
Codex: [runs codex exec and gemini in parallel]
Codex: [reads codex output file, parses structured findings]
Codex: [presents both reviews, highlights agreements/differences]
```

**Codex only with inline args:**
```
User: /second-opinion check uncommitted changes for bugs
Codex: [scope known: uncommitted, focus known: custom]
Codex: [asks 2 questions: tool, project context]
User: picks "Codex only", "No context"
Codex: [shows diff --stat: 3 files, +45 -10]
Codex: [writes prompt file with review instructions + diff]
Codex: [runs codex exec, reads structured JSON output]
Codex: [presents findings by priority with file:line refs]
```

**Gemini only:**
```
User: /second-opinion
Codex: [asks 4 questions]
User: picks "Gemini only", "Uncommitted", "No", "General"
Codex: [shows diff --stat: 2 files, +20 -5]
Codex: [runs gemini -p "/code-review" --yolo -e code-review]
Codex: [presents review]
```

**Large diff warning:**
```
User: /second-opinion
Codex: [asks questions] → user picks "Both", "Uncommitted", "General"
Codex: [shows diff --stat: 45 files, +3200 -890]
Codex: "Large diff (3200+ lines). Proceed, or narrow the scope?"
User: "proceed"
Codex: [runs both reviews]
```
