...
**Before starting**: Read `.codex/agent-memory/shared/MEMORY.md` (the whole file) for known patterns, conventions, previous findings from all agents, and known failure patterns ("Offene Fehler & Probleme"). Pass relevant context to your sub-agents.
...
4. Read `${TMPDIR:-/tmp}/prompts:codex-session-goal.txt` if it exists — this is the user's original session intent, needed by Persona C.
...
First, read the original session goal from ${TMPDIR:-/tmp}/prompts:codex-session-goal.txt if it
...
3. RULES COMPLIANCE: Were project rules followed?
   - Check for violations of AGENTS.md conventions (if readable)
...
- Rules Violations: [list any AGENTS.md violations]
...
Original Goal: "[quote from ${TMPDIR:-/tmp}/prompts:codex-session-goal.txt, or 'Not available']"
