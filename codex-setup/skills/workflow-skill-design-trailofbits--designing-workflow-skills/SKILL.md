...
description: >-
  Guides the design and structuring of workflow-based Codex skills
...
- Non-skill Codex development — this is specifically for skill architecture
...
description: "Third-person description with trigger keywords — this is how Codex decides to activate the skill"
allowed-tools:
  - [minimum tools needed]
# Optional fields — see tool-assignment-guide.md for full reference:
# disable-model-invocation: true    # Only user can invoke (not Codex)
# user-invocable: false             # Only Codex can invoke (hidden from / menu)
...
Skills support three types of string substitutions: dollar-prefixed variables for arguments and session ID, and exclamation-backtick syntax for shell preprocessing. The skill loader processes these before Codex sees the file — even inside code fences — so never use the raw syntax in documentation text. See [tool-assignment-guide.md](references/tool-assignment-guide.md) for the full variable reference and usage guidance.
