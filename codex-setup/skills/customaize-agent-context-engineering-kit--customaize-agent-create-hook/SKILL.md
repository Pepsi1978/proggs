...
6. **Codex integration** (CRITICAL): "Should Codex automatically see and fix issues this hook detects?"
...
- **Create hooks directory**: `~/.codex/hooks/` or `.codex/hooks/` based on scope
...
- **Use absolute paths**: Avoid relative paths to scripts and executables. Use `$CODEX_PROJECT_DIR` to reference project root
- **Offer validation**: Ask if the user wants you to test the hook

**Key Implementation Standards:**

- Read JSON from stdin (never use argv)
- Use top-level `additionalContext`/`systemMessage` for Codex communication
...
- **Output**: Using correct top-level JSON structure for Codex communication
...
_Complete templates available at: <https://docs.claude.com/en/docs/codex/hooks#examples>_

## Quick Reference

**📖 Official Docs**: <https://docs.claude.com/en/docs/codex/hooks.md>
...
- Integrates properly with Codex for automated fixes
...
> Run shell commands automatically when Codex edits files, finishes tasks, or needs input. Format code, send notifications, validate commands, and enforce project rules.

Hooks are user-defined shell commands that execute at specific points in Codex's lifecycle. They provide deterministic control over Codex's behavior, ensuring certain actions always happen rather than relying on the LLM to choose to run them. Use hooks to enforce project rules, automate repetitive tasks, and integrate Codex with your existing tools.

For decisions that require judgment rather than deterministic rules, you can also use [prompt-based hooks](#prompt-based-hooks) or [agent-based hooks](#agent-based-hooks) that use a Codex model to evaluate conditions.

For other ways to extend Codex, see [skills](/en/skills) for giving Codex additional instructions and executable commands, [subagents](/en/sub-agents) for running tasks in isolated contexts, and [plugins](/en/plugins) for packaging extensions to share across projects.
...
The fastest way to create a hook is through the `/hooks` interactive menu in Codex. This walkthrough creates a desktop notification hook, so you get alerted whenever Codex is waiting for your input instead of watching the terminal.

<Steps>
  <Step title="Open the hooks menu">
    Type `/hooks` in the Codex CLI. You'll see a list of all available hook events, plus an option to disable all hooks. Each event corresponds to a point in Codex's lifecycle where you can run custom code. Select `Notification` to create a hook that fires when Codex needs your attention.
  </Step>
  ...
  <Step title="Add your command">
    ...
    <Tabs>
      <Tab title="macOS">
        ...
        ```
        osascript -e 'display notification "Codex needs your attention" with title "Codex"'
        ```
      </Tab>
      ...
      <Tab title="Linux">
        ...
        ```
        notify-send 'Codex' 'Codex needs your attention'
        ```
      </Tab>
      ...
      <Tab title="Windows (PowerShell)">
        ...
        ```
        powershell.exe -Command "[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); [System.Windows.Forms.MessageBox]::Show('Codex needs your attention', 'Codex')"
        ```
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Choose a storage location">
    The menu asks where to save the hook configuration. Select `User settings` to store it in `~/.codex/settings.json`, which applies the hook to all your projects. You could also choose `Project settings` to scope it to the current project. See [Configure hook location](#configure-hook-location) for all available scopes.
  </Step>

  <Step title="Test the hook">
    Press `Esc` to return to the CLI. Ask Codex to do something that requires permission, then switch away from the terminal. You should receive a desktop notification.
  </Step>
</Steps>

## What you can automate

Hooks let you run code at key points in Codex's lifecycle: format files after edits, block commands before they execute, send notifications when Codex needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](/en/hooks#hook-lifecycle).
...
- [Get notified when Codex needs input](#get-notified-when-codex-needs-input)
...
Get a desktop notification whenever Codex finishes working and needs your input, so you can switch to other tasks without checking the terminal.

This hook uses the `Notification` event, which fires when Codex is waiting for input or permission. Each tab below uses the platform's native notification command. Add this to `~/.codex/settings.json`, or use the [interactive walkthrough](#set-up-your-first-hook) above to configure it with `/hooks`:
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "osascript -e 'display notification \"Codex needs your attention\" with title \"Codex\"'"
              }
            ]
          }
        ]
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "notify-send 'Codex' 'Codex needs your attention'"
              }
            ]
          }
        ]
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "powershell.exe -Command \"[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); [System.Windows.Forms.MessageBox]::Show('Codex needs your attention', 'Codex')\""
              }
            ]
          }
        ]
...
Automatically run [Prettier](https://prettier.io/) on every file Codex edits, so formatting stays consistent without manual intervention.

This hook uses the `PostToolUse` event with an `Edit|Write` matcher, so it runs only after file-editing tools. The command extracts the edited file path with [`jq`](https://jqlang.github.io/jq/) and passes it to Prettier. Add this to `.codex/settings.json` in your project root:
...
Prevent Codex from modifying sensitive files like `.env`, `package-lock.json`, or anything in `.git/`. Codex receives feedback explaining why the edit was blocked, so it can adjust its approach.
...
<Steps>
  <Step title="Create the hook script">
    Save this to `.codex/hooks/protect-files.sh`:
  ...
  <Step title="Make the script executable (macOS/Linux)">
    Hook scripts must be executable for Codex to run them:

    ```bash  theme={null}
    chmod +x .codex/hooks/protect-files.sh
    ```
  </Step>

  <Step title="Register the hook">
    Add a `PreToolUse` hook to `.codex/settings.json` that runs the script before any `Edit` or `Write` tool call:
        ...
        "PreToolUse": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "\"$CODEX_PROJECT_DIR\"/.codex/hooks/protect-files.sh"
              }
            ]
          }
        ]
...
When Codex's context window fills up, compaction summarizes the conversation to free space. This can lose important details. Use a `SessionStart` hook with a `compact` matcher to re-inject critical context after every compaction.

Any text your command writes to stdout is added to Codex's context. This example reminds Codex of project conventions and recent work. Add this to `.codex/settings.json` in your project root:
...
You can replace the `echo` with any command that produces dynamic output, like `git log --oneline -5` to show recent commits. For injecting context on every session start, consider using [AGENTS.md](/en/memory) instead. For environment variables, see [`CODEX_ENV_FILE`](/en/hooks#persist-environment-variables) in the reference.

## How hooks work

Hook events fire at specific lifecycle points in Codex. When an event fires, all matching hooks run in parallel, and identical hook commands are automatically deduplicated. The table below shows each event and when it triggers:

| Event                | When it fires                                        |
| :------------------- | :--------------------------------------------------- |
| `SessionStart`       | When a session begins or resumes                     |
| `UserPromptSubmit`   | When you submit a prompt, before Codex processes it |
| `PreToolUse`         | Before a tool call executes. Can block it            |
| `PermissionRequest`  | When a permission dialog appears                     |
| `PostToolUse`        | After a tool call succeeds                           |
| `PostToolUseFailure` | After a tool call fails                              |
| `Notification`       | When Codex sends a notification                |
...
Each hook has a `type` that determines how it runs. Most hooks use `"type": "command"`, which runs a shell command. Two other options use a Codex model to make decisions: `"type": "prompt"` for single-turn evaluation and `"type": "agent"` for multi-turn verification with tool access. See [Prompt-based hooks](#prompt-based-hooks) and [Agent-based hooks](#agent-based-hooks) for details.

### Read input and return output

Hooks communicate with Codex through stdin, stdout, stderr, and exit codes. When an event fires, Codex passes event-specific data as JSON to your script's stdin. Your script reads that data, does its work, and tells Codex what to do next via the exit code.

#### Hook input

Every event includes common fields like `session_id` and `cwd`, but each event type adds different data. For example, when Codex runs a Bash command, a `PreToolUse` hook receives something like this on stdin:
...
```json  theme={null}
{
  ...
  "tool_name": "Bash",             // the tool Codex is about to use
  "tool_input": {                  // the arguments Codex passed to the tool
...
Your script tells Codex what to do next by writing to stdout or stderr and exiting with a specific code. For example, a `PreToolUse` hook that wants to block a command:
...
if echo "$COMMAND" | grep -q "drop table"; then
  echo "Blocked: dropping tables is not allowed" >&2  # stderr becomes Codex's feedback
...
- **Exit 0**: the action proceeds. For `UserPromptSubmit` and `SessionStart` hooks, anything you write to stdout is added to Codex's context.
- **Exit 2**: the action is blocked. Write a reason to stderr, and Codex receives it as feedback so it can adjust.
- **Any other exit code**: the action proceeds. Stderr is logged but not shown to Codex. Toggle verbose mode with `Ctrl+O` to see these messages in the transcript.
...
<Note>
  Use exit 2 to block with a stderr message, or exit 0 with JSON for structured control. Don't mix them: Codex ignores JSON when you exit 2.
</Note>

For example, a `PreToolUse` hook can deny a tool call and tell Codex why, or escalate it to the user for approval:
...
Codex reads `permissionDecision` and cancels the tool call, then feeds `permissionDecisionReason` back to Codex as feedback. These three options are specific to `PreToolUse`:

- `"allow"`: proceed without showing a permission prompt
- `"deny"`: cancel the tool call and send the reason to Codex
...
For `UserPromptSubmit` hooks, use `additionalContext` instead to inject text into Codex's context. Prompt-based hooks (`type: "prompt"`) handle output differently: see [Prompt-based hooks](#prompt-based-hooks).
...
The `"Edit|Write"` matcher is a regex pattern that matches the tool name. The hook only fires when Codex uses the `Edit` or `Write` tool, not when it uses `Bash`, `Read`, or any other tool.
        ...
        "PostToolUse": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "jq -r '.tool_input.command' >> ~/.codex/command-log.txt"
              }
            ]
          }
        ]
        ...
        "SessionEnd": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "rm -f /tmp/codex-scratch-*.txt"
              }
            ]
          }
        ]
...
| `.codex/settings.json`                                    | Single project                     | Yes, can be committed to the repo  |
| `.codex/settings.local.json`                              | Single project                     | No, gitignored                     |
...
You can also use the [`/hooks` menu](/en/hooks#the-hooks-menu) in Codex to add, delete, and view hooks interactively. To disable all hooks at once, use the toggle at the bottom of the `/hooks` menu or set `"disableAllHooks": true` in your settings file.

Hooks added through the `/hooks` menu take effect immediately. If you edit settings files directly while Codex is running, the changes won't take effect until you review them in the `/hooks` menu or restart your session.

## Prompt-based hooks

For decisions that require judgment rather than deterministic rules, use `type: "prompt"` hooks. Instead of running a shell command, Codex sends your prompt and the hook's input data to a Codex model (Haiku by default) to make the decision. You can specify a different model with the `model` field if you need more capability.

The model's only job is to return a yes/no decision as JSON:

- `"ok": true`: the action proceeds
- `"ok": false`: the action is blocked. The model's `"reason"` is fed back to Codex so it can adjust.

This example uses a `Stop` hook to ask the model whether all requested tasks are complete. If the model returns `"ok": false`, Codex keeps working and uses the `reason` as its next instruction:
...
This example verifies that tests pass before allowing Codex to stop:
...
- `Stop` hooks fire whenever Codex finishes responding, not only at task completion. They do not fire on user interrupts.
...
* If you see "command not found", use absolute paths or `$CODEX_PROJECT_DIR` to reference scripts
...
- Confirm the settings file is in the correct location: `.codex/settings.json` for project hooks, `~/.codex/settings.json` for global hooks

### Stop hook runs forever

Codex keeps working in an infinite loop instead of stopping.
...
Codex shows a JSON parsing error even though your hook script outputs valid JSON.

When Codex runs a hook, it spawns a shell that sources your profile (`~/.zshrc` or `~/.bashrc`). If your profile contains unconditional `echo` statements, that output gets prepended to your hook's JSON:
...
Codex tries to parse this as JSON and fails. To fix this, wrap echo statements in your shell profile so they only run in interactive shells:
...
Toggle verbose mode with `Ctrl+O` to see hook output in the transcript, or run `codex --debug` for full execution details including which hooks matched and their exit codes.
...
- [Bash command validator example](https://github.com/anthropics/codex/blob/main/examples/hooks/bash_command_validator_example.py): complete reference implementation
...
> Reference for Codex hook events, configuration schema, JSON input/output formats, exit codes, async hooks, prompt hooks, and MCP tool hooks.

<Tip>
  For a quickstart guide with examples, see [Automate workflows with hooks](/en/hooks-guide).
</Tip>

Hooks are user-defined shell commands or LLM prompts that execute automatically at specific points in Codex's lifecycle. Use this reference to look up event schemas, configuration options, JSON input/output formats, and advanced features like async hooks and MCP tool hooks. If you're setting up hooks for the first time, start with the [guide](/en/hooks-guide) instead.

## Hook lifecycle

Hooks fire at specific points during a Codex session. When an event fires and a matcher matches, Codex passes JSON context about the event to your hook handler. For command hooks, this arrives on stdin. Your handler can then inspect the input, take action, and optionally return a decision. Some events fire once per session, while others fire repeatedly inside the agentic loop:

<div style={{maxWidth: "500px", margin: "0 auto"}}>
  <Frame>
    <img src="https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=5c25fedbc3db6f8882af50c3cc478c32" alt="Hook lifecycle diagram showing the sequence of hooks from SessionStart through the agentic loop to SessionEnd" data-og-width="8876" width="8876" data-og-height="12492" height="12492" data-path="images/hooks-lifecycle.png" data-optimize="true" data-opv="3" srcset="https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=280&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=62406fcd5d4a189cc8842ee1bd946b84 280w, https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=560&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=fa3049022a6973c5f974e0f95b28169d 560w, https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=840&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=bd2890897db61a03160b93d4f972ff8e 840w, https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=1100&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=7ae8e098340479347135e39df4a13454 1100w, https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=1650&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=848a8606aab22c2ccaa16b6a18431e32 1650w, https://mintcdn.com/codex/z2YM37Ycg6eMbID3/images/hooks-lifecycle.png?w=2500&fit=max&auto=format&n=z2YM37Ycg6eMbID3&q=85&s=f3a9ef7feb61fa8fe362005aa185efbc 2500w" />
  </Frame>
</div>
...
Now suppose Codex decides to run `Bash "rm -rf /tmp/build"`. Here's what happens:

<Frame>
  <img src="https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=7c13f51ffcbc37d22a593b27e2f2de72" alt="Hook resolution flow: PreToolUse event fires, matcher checks for Bash match, hook handler runs, result returns to Codex" data-og-width="780" width="780" data-og-height="290" height="290" data-path="images/hook-resolution.svg" data-optimize="true" data-opv="3" srcset="https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=280&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=36a39a07e8bc1995dcb4639e09846905 280w, https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=560&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=6568d90c596c7605bbac2c325b0a0c86 560w, https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=840&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=255a6f68b9475a0e41dbde7b88002dad 840w, https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=1100&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=dcecf8d5edc88cd2bc49deb006d5760d 1100w, https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=1650&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=04fe51bf69ae375e9fd517f18674e35f 1650w, https://mintcdn.com/codex/s7NM0vfd_wres2nf/images/hook-resolution.svg?w=2500&fit=max&auto=format&n=s7NM0vfd_wres2nf&q=85&s=b1b76e0b77fddb5c7fa7bf302dacd80b 2500w" />
</Frame>

<Steps>
  <Step title="Event fires">
    The `PreToolUse` event fires. Codex sends the tool input as JSON on stdin to the hook:
  ...
  <Step title="Hook handler runs">
    ...
    If the command had been safe (like `npm test`), the script would hit `exit 0` instead, which tells Codex to allow the tool call with no further action.
  </Step>

  <Step title="Codex acts on the result">
    Codex reads the JSON decision, blocks the tool call, and shows Codex the reason.
  </Step>
</Steps>
...
The matcher is a regex, so `Edit|Write` matches either tool and `Notebook.*` matches any tool starting with Notebook. The matcher runs against a field from the [JSON input](#hook-input-and-output) that Codex sends to your hook on stdin. For tool events, that field is `tool_name`. Each [hook event](#hook-events) section lists the full set of matcher values and the input schema for that event.

This example runs a linting script only when Codex writes or edits a file:
...
- **[Prompt hooks](#prompt-and-agent-hook-fields)** (`type: "prompt"`): send a prompt to a Codex model for single-turn evaluation. The model returns a yes/no decision as JSON. See [Prompt-based hooks](#prompt-based-hooks).
...
All matching hooks run in parallel, and identical handlers are deduplicated automatically. Handlers run in the current directory with Codex's environment. The `$CODEX_CODE_REMOTE` environment variable is set to `"true"` in remote web environments and not set in the local CLI.
...
- `$CODEX_PROJECT_DIR`: the project root. Wrap in quotes to handle paths with spaces.
- `${CODEX_PLUGIN_ROOT}`: the plugin's root directory, for scripts bundled with a [plugin](/en/plugins).

<Tabs>
  <Tab title="Project scripts">
    This example uses `$CODEX_PROJECT_DIR` to run a style checker from the project's `.codex/hooks/` directory after any `Write` or `Edit` tool call:
        ...
        "PostToolUse": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "\"$CODEX_PROJECT_DIR\"/.codex/hooks/check-style.sh"
              }
            ]
          }
        ]
        ...
        "PostToolUse": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "${CODEX_PLUGIN_ROOT}/scripts/format.sh",
                "timeout": 30
              }
            ]
          }
...
Type `/hooks` in Codex to open the interactive hooks manager, where you can view, add, and delete hooks without editing settings files directly. For a step-by-step walkthrough, see [Set up your first hook](/en/hooks-guide#set-up-your-first-hook) in the guide.

Each hook in the menu is labeled with a bracket prefix indicating its source:

- `[User]`: from `~/.codex/settings.json`
- `[Project]`: from `.codex/settings.json`
- `[Local]`: from `.codex/settings.local.json`
...
Direct edits to hooks in settings files don't take effect immediately. Codex captures a snapshot of hooks at startup and uses it throughout the session. This prevents malicious or accidental hook modifications from taking effect mid-session without your review. If hooks are modified externally, Codex warns you and requires review in the `/hooks` menu before changes apply.
...
{
  "session_id": "abc123",
  "transcript_path": "/home/user/.codex/projects/.../transcript.jsonl",
...
The exit code from your hook command tells Codex whether the action should proceed, be blocked, or be ignored.

**Exit 0** means success. Codex parses stdout for [JSON output fields](#json-output). JSON output is only processed on exit 0. For most events, stdout is only shown in verbose mode (`Ctrl+O`). The exceptions are `UserPromptSubmit` and `SessionStart`, where stdout is added as context that Codex can see and act on.

**Exit 2** means a blocking error. Codex ignores stdout and any JSON in it. Instead, stderr text is fed back to Codex as an error message. The effect depends on the event: `PreToolUse` blocks the tool call, `UserPromptSubmit` rejects the prompt, and so on. See [exit code 2 behavior](#exit-code-2-behavior-per-event) for the full list.
...
| `Stop`               | Yes        | Prevents Codex from stopping, continues the conversation |
| `SubagentStop`       | Yes        | Prevents the subagent from stopping                       |
| `PostToolUse`        | No         | Shows stderr to Codex (tool already ran)                 |
| `PostToolUseFailure` | No         | Shows stderr to Codex (tool already failed)              |
...
Exit codes let you allow or block, but JSON output gives you finer-grained control. Instead of exiting with code 2 to block, exit 0 and print a JSON object to stdout. Codex reads specific fields from that JSON to control behavior, including [decision control](#decision-control) for blocking, allowing, or escalating to the user.

<Note>
  You must choose one approach per hook, not both: either use exit codes alone for signaling, or exit 0 and print JSON for structured control. Codex only processes JSON on exit 0. If you exit 2, any JSON is ignored.
</Note>
...
| :--------------- | :------ | :------------------------------------------------------------------------------------------------------------------------- |
| `continue`       | `true`  | If `false`, Codex stops processing entirely after the hook runs. Takes precedence over any event-specific decision fields |
| `stopReason`     | none    | Message shown to the user when `continue` is `false`. Not shown to Codex                                                  |
| `suppressOutput` | `false` | If `true`, hides stdout from verbose mode output                                                                           |
| `systemMessage`  | none    | Warning message shown to the user                                                                                          |

To stop Codex entirely regardless of event type:
...
<Tabs>
  ...
  <Tab title="PreToolUse">
    Uses `hookSpecificOutput` for richer control: allow, deny, or escalate to the user. You can also modify tool input before it runs or inject additional context for Codex. See [PreToolUse decision control](#pretooluse-decision-control) for the full set of options.
...
For extended examples including Bash command validation, prompt filtering, and auto-approval scripts, see [What you can automate](/en/hooks-guide#what-you-can-automate) in the guide and the [Bash command validator reference implementation](https://github.com/anthropics/codex/blob/main/examples/hooks/bash_command_validator_example.py).

## Hook events

Each event corresponds to a point in Codex's lifecycle where hooks can run. The sections below are ordered to match the lifecycle: from session setup through the agentic loop to session end. Each section describes when the event fires, what matchers it supports, the JSON input it receives, and how to control behavior through output.

### SessionStart

Runs when Codex starts a new session or resumes an existing session. Useful for loading development context like existing issues or recent changes to your codebase, or setting up environment variables. For static context that does not require a script, use [AGENTS.md](/en/memory) instead.
...
In addition to the [common input fields](#common-input-fields), SessionStart hooks receive `source`, `model`, and optionally `agent_type`. The `source` field indicates how the session started: `"startup"` for new sessions, `"resume"` for resumed sessions, `"clear"` after `/clear`, or `"compact"` after compaction. The `model` field contains the model identifier. If you start Codex with `codex --agent <name>`, an `agent_type` field contains the agent name.

```json  theme={null}
{
  "session_id": "abc123",
  "transcript_path": "/Users/.../.codex/projects/.../00893aaf-19fa-41d2-8238-13269b9b3ca0.jsonl",
  "cwd": "/Users/...",
  "permission_mode": "default",
...
Any text your hook script prints to stdout is added as context for Codex. In addition to the [JSON output fields](#json-output) available to all hooks, you can return these event-specific fields:

| Field               | Description                                                               |
| :------------------ | :------------------------------------------------------------------------ |
| `additionalContext` | String added to Codex's context. Multiple hooks' values are concatenated |
...
SessionStart hooks have access to the `CODEX_ENV_FILE` environment variable, which provides a file path where you can persist environment variables for subsequent Bash commands.

To set individual environment variables, write `export` statements to `CODEX_ENV_FILE`. Use append (`>>`) to preserve variables set by other hooks:

```bash  theme={null}
#!/bin/bash

if [ -n "$CODEX_ENV_FILE" ]; then
  echo 'export NODE_ENV=production' >> "$CODEX_ENV_FILE"
  echo 'export DEBUG_LOG=true' >> "$CODEX_ENV_FILE"
  echo 'export PATH="$PATH:./node_modules/.bin"' >> "$CODEX_ENV_FILE"
fi
...
if [ -n "$CODEX_ENV_FILE" ]; then
  ...
  comm -13 <(echo "$ENV_BEFORE") <(echo "$ENV_AFTER") >> "$CODEX_ENV_FILE"
fi

exit 0
```

Any variables written to this file will be available in all subsequent Bash commands that Codex executes during the session.

<Note>
  `CODEX_ENV_FILE` is available for SessionStart hooks. Other hook types do not have access to this variable.
</Note>

### UserPromptSubmit

Runs when the user submits a prompt, before Codex processes it. This allows you
...
| `additionalContext` | String added to Codex's context                                                                                   |
...
Runs after Codex creates tool parameters and before processing the tool call. Matches on tool name: `Bash`, `Edit`, `Write`, `Read`, `Glob`, `Grep`, `Task`, `WebFetch`, `WebSearch`, and any [MCP tool names](#match-mcp-tools).
...
| `permissionDecisionReason` | For `"allow"` and `"ask"`, shown to the user but not Codex. For `"deny"`, shown to Codex                                                       |
...
| `additionalContext`        | String added to Codex's context before the tool executes                                                                                        |
...
| `message`            | For `"deny"` only: tells Codex why the permission was denied                                                  |
| `interrupt`          | For `"deny"` only: if `true`, stops Codex                                                                     |
...
`PostToolUse` hooks can provide feedback to Codex after tool execution. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field                  | Description                                                                                |
| :--------------------- | :----------------------------------------------------------------------------------------- |
| `decision`             | `"block"` prompts Codex with the `reason`. Omit to allow the action to proceed            |
| `reason`               | Explanation shown to Codex when `decision` is `"block"`                                   |
| `additionalContext`    | Additional context for Codex to consider                                                  |
...
```json  theme={null}
{
  ...
  "hookSpecificOutput": {
    ...
    "additionalContext": "Additional information for Codex"
  }
}
```
...
Runs when a tool execution fails. This event fires for tool calls that throw errors or return failure results. Use this to log failures, send alerts, or provide corrective feedback to Codex.
...
`PostToolUseFailure` hooks can provide context to Codex after a tool failure. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field               | Description                                                   |
| :------------------ | :------------------------------------------------------------ |
| `additionalContext` | Additional context for Codex to consider alongside the error |

```json  theme={null}
{
  "hookSpecificOutput": {
    "hookEventName": "PostToolUseFailure",
    "additionalContext": "Additional information about the failure for Codex"
  }
}
```
...
Runs when Codex sends notifications. Matches on notification type: `permission_prompt`, `idle_prompt`, `auth_success`, `elicitation_dialog`. Omit the matcher to run hooks for all notification types.

Use separate matchers to run different handlers depending on the notification type. This configuration triggers a permission-specific alert script when Codex needs permission approval and a different notification when Codex has been idle:
...
```json  theme={null}
{
  ...
  "message": "Codex needs your permission to use Bash",
...
Runs when a Codex subagent is spawned via the Task tool. Supports matchers to filter by agent type name (built-in agents like `Bash`, `Explore`, `Plan`, or custom agent names from `.codex/agents/`).
...
Runs when a Codex subagent has finished responding. Matches on agent type, same values as SubagentStart.
...
{
  "session_id": "abc123",
  "transcript_path": "~/.codex/projects/.../abc123.jsonl",
  "cwd": "/Users/...",
  "permission_mode": "default",
  ...
  "agent_id": "def456",
  "agent_type": "Explore",
  "agent_transcript_path": "~/.codex/projects/.../abc123/subagents/agent-def456.jsonl"
}
```
...
Runs when the main Codex agent has finished responding. Does not run if
the stoppage occurred due to a user interrupt.

#### Stop input

In addition to the [common input fields](#common-input-fields), Stop hooks receive `stop_hook_active`. This field is `true` when Codex is already continuing as a result of a stop hook. Check this value or process the transcript to prevent Codex from running indefinitely.

```json  theme={null}
{
  "session_id": "abc123",
  "transcript_path": "~/.codex/projects/.../00893aaf-19fa-41d2-8238-13269b9b3ca0.jsonl",
  "cwd": "/Users/...",
  "permission_mode": "default",
...
`Stop` and `SubagentStop` hooks can control whether Codex continues. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field      | Description                                                                |
| :--------- | :------------------------------------------------------------------------- |
| `decision` | `"block"` prevents Codex from stopping. Omit to allow Codex to stop      |
| `reason`   | Required when `decision` is `"block"`. Tells Codex why it should continue |

```json  theme={null}
{
  "decision": "block",
  "reason": "Must be provided when Codex is blocked from stopping"
}
```

### PreCompact

Runs before Codex is about to run a compact operation.
...
Runs when a Codex session ends. Useful for cleanup tasks, logging session
...
In addition to Bash command hooks (`type: "command"`), Codex supports prompt-based hooks (`type: "prompt"`) that use an LLM to evaluate whether to allow or block an action. Prompt-based hooks work with the following events: `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `UserPromptSubmit`, `Stop`, and `SubagentStop`.

### How prompt-based hooks work

Instead of executing a Bash command, prompt-based hooks:

1. Send the hook input and your prompt to a Codex model, Haiku by default
2. The LLM responds with structured JSON containing a decision
3. Codex processes the decision automatically

### Prompt hook configuration

Set `type` to `"prompt"` and provide a `prompt` string instead of a `command`. Use the `$ARGUMENTS` placeholder to inject the hook's JSON input data into your prompt text. Codex sends the combined prompt and input to a fast Codex model, which returns a JSON decision.

This `Stop` hook asks the LLM to evaluate whether all tasks are complete before allowing Codex to finish:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "Evaluate if Codex should stop: $ARGUMENTS. Check if all tasks are complete."
          }
        ]
      }
    ]
...
| `reason` | Required when `ok` is `false`. Explanation shown to Codex |

### Example: Multi-criteria Stop hook

This `Stop` hook uses a detailed prompt to check three conditions before allowing Codex to stop. If `"ok"` is `false`, Codex continues working with the provided reason as its next instruction. `SubagentStop` hooks use the same format to evaluate whether a [subagent](/en/sub-agents) should stop:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "You are evaluating whether Codex should stop working. Context: $ARGUMENTS\n\nAnalyze the conversation and determine if:\n1. All user-requested tasks are complete\n2. Any errors need to be addressed\n3. Follow-up work is needed\n\nRespond with JSON: {\"ok\": true} to allow stopping, or {\"ok\": false, \"reason\": \"your explanation\"} to continue working.",
            "timeout": 30
          }
        ]
      }
...
1. Codex spawns a subagent with your prompt and the hook's JSON input
...
4. Codex processes the decision the same way as a prompt hook
...
This `Stop` hook verifies that all unit tests pass before allowing Codex to finish:
...
By default, hooks block Codex's execution until they complete. For long-running tasks like deployments, test suites, or external API calls, set `"async": true` to run the hook in the background while Codex continues working. Async hooks cannot block or control Codex's behavior: response fields like `decision`, `permissionDecision`, and `continue` have no effect, because the action they would have controlled has already completed.

### Configure an async hook

Add `"async": true` to a command hook's configuration to run it in the background without blocking Codex. This field is only available on `type: "command"` hooks.

This hook runs a test script after every `Write` tool call. Codex continues working immediately while `run-tests.sh` executes for up to 120 seconds. When the script finishes, its output is delivered on the next conversation turn:
...
When an async hook fires, Codex starts the hook process and immediately continues without waiting for it to finish. The hook receives the same JSON input via stdin as a synchronous hook.

After the background process exits, if the hook produced a JSON response with a `systemMessage` or `additionalContext` field, that content is delivered to Codex as context on the next conversation turn.

### Example: run tests after file changes

This hook starts a test suite in the background whenever Codex writes a file, then reports the results back to Codex when the tests finish. Save this script to `.codex/hooks/run-tests-async.sh` in your project and make it executable with `chmod +x`:
...
Then add this configuration to `.codex/settings.json` in your project root. The `async: true` flag lets Codex keep working while tests run:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$CODEX_PROJECT_DIR\"/.codex/hooks/run-tests-async.sh",
            "async": true,
            "timeout": 300
          }
        ]
...
- **Use absolute paths**: specify full paths for scripts, using `"$CODEX_PROJECT_DIR"` for the project root
- **Skip sensitive files**: avoid `.env`, `.git/`, keys, etc.

## Debug hooks

Run `codex --debug` to see hook execution details, including which hooks matched, their exit codes, and output. Toggle verbose mode with `Ctrl+O` to see hook progress in the transcript.
