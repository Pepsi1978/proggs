...
**Before starting**: Read `.codex/agent-memory/shared/MEMORY.md` to understand current system state and avoid known issues.
...
uname -s
```
- `Darwin` → macOS, platform key: `macos/codex`
- `MINGW`/`MSYS`/`Windows` → Windows, platform key: `windows/codex`
- Codex CLI → platform key: `{os}/prompts:codex`
- Gemini CLI → platform key: `{os}/prompts:gemini`

Then pull the latest changes:
```bash
cd ~/prompts:proggs && git pull --rebase --quiet
```
...
Read the full file: `~/Codex/codex-harness/mirror-ledger.md`
...
|---|-----|-----|--------|-------|-------------|
| 1 | MIRROR-2026-03-25-MAC-001 | rule | macOS/codex | 2026-03-25 | ... |

### Review noetig:
| # | ID | Typ | Quelle | Datum | Beschreibung |
|---|-----|-----|--------|-------|-------------|
| 1 | MIRROR-2026-03-25-MAC-002 | hook | macOS/codex | 2026-03-25 | ... |
```
...
2. Write it as a `.md` file in `~/.codex/rules/auto-learned/` (create directory if needed)
3. Copy to `~/Codex/codex-harness/rules/auto-learned/`
→ **AUTO** (no user confirmation needed)

#### Type: `rule`
1. Extract the complete rule content from the entry
2. Write to `~/.codex/rules/{filename}.md`
3. Copy to `~/Codex/codex-harness/rules/{filename}.md`
→ **AUTO**

#### Type: `directive`
1. Extract the AGENTS.md change from the entry
2. Apply to BOTH copies: `~/prompts:codex.md` AND `~/.codex/AGENTS.md`
...
2. Insert into `~/.codex/agent-memory/shared/MEMORY.md` under the correct section header
→ **AUTO**

#### Type: `hook`
1. Extract the correct platform variant (.sh for macOS, .ps1 for Windows) from the entry
2. Write the file to `~/.codex/hooks/{hookname}.{ext}`
3. On macOS: `chmod +x` the .sh file
4. Extract the settings registration JSON from the entry
5. Add the hook stanza to `~/.codex/settings.json`
6. Apply the **3-file rule**: Also update:
   - `~/Codex/codex-harness/settings-reference.json` (Windows reference)
   - `~/Codex/codex-harness/settings.json` (macOS reference)
   - Hook file copy to `~/Codex/codex-harness/hooks/`
7. Validate JSON: `python3 -m json.tool ~/.codex/settings.json > /dev/null`
→ **SHOW DIFF** before applying

#### Type: `settings`
1. Extract the JSON stanza from the entry
2. Insert into `~/.codex/settings.json` at the correct location
3. Apply the **3-file rule** (update all 3 settings files in setup-repo)
4. Validate JSON: `python3 -m json.tool ~/.codex/settings.json > /dev/null`
→ **SHOW DIFF** before applying

#### Type: `agent`
1. Extract the complete agent file (frontmatter + prompt)
2. Write to `~/.codex/agents/{agentname}.md`
3. Copy to `~/Codex/codex-harness/agents/{agentname}.md`
...
```bash
cd ~/prompts:proggs
git add codex-harness/mirror-ledger.md
git add -A codex-harness/  # Include any new files added to setup-repo
...
| `/tmp/` | `$env:TEMP\` |
| `bash ~/.codex/hooks/X.sh` | `pwsh -NoProfile -File "$USERPROFILE/.codex/hooks/X.ps1"` |
