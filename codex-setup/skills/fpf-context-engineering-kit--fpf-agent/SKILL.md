...
- **Validity**: Evidence expires — check with `/prompts:q-decay`
- **Scope**: Knowledge applies within specified conditions only

**State Location:** `.fpf/` directory (git-tracked)

**Key Principle:** You (Codex) generate options with evidence. Human decides. This is the Transformer Mandate — a system cannot transform itself.
...
**Epistemic Debt** — Accumulated staleness when evidence expires. Managed via `/prompts:q-decay`.
...
* **Informs:** `U.ServiceClause` definitional pattern (external promises derive from capabilities); `U.Dynamics` definitional pattern (models used as evidence or predictors); Γ/prompts:aggregation (capability of composites is stated at the whole).
...
— Do **not** name the characteristic “applicability/envelope/generality/capability envelope/**publication applicability**/prompts:validity.” Those words are permitted only as explanatory aliases in notes.
...
For each `U.RoleAssignment` and `U.Work`, the following fields are REQUIRED: `{authority?, justification?, provenance?}` where `justification: U.Episteme` and `provenance: U.Method`/prompts:process evidence. This aligns with the kernel’s governance and B‑cluster lineage practices.
...
Where transformers are *agentic*, A.3’s policy–plan–action split and boundary‑centred accounting dovetail with **active inference** and **free‑energy** formulations of self‑organising systems (Markov blankets; Bayesian mechanics). This legitimises `U.Objective`/prompts:cost function links and makes design–run duality natural (prior vs posterior policies). ([MIT Press Direct][4], [PubMed][5], [arXiv][6])
...
`U.Method` is the **semantic “way of doing”**. It is **not** a `U.MethodDescription` (artifact on a carrier), **not** a `U.Work` (dated execution), **not** a `U.Role`/prompts:assignment, and **not** a `U.ServiceClause`/prompts:promise.
