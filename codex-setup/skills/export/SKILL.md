...
self-contained entries to `~/Codex/codex-harness/mirror-ledger.md` so that
other platforms (macOS, Windows) and other CLIs (Codex, Codex CLI, Gemini CLI)
...
```bash
grep -q "^## \\[BASELINE-" ~/Codex/codex-harness/mirror-ledger.md 2>/dev/null
```
...
git -C ~/prompts:proggs diff --name-status HEAD~10..HEAD 2>/dev/null || git -C ~/prompts:proggs diff --name-status HEAD
...
| `rule` | `rules/*.md` |
| `agent` | `agents/*.md` |
| `skill` | `commands/*.md`, `skills/**/prompts:skill.md` |
...
| `directive` | `AGENTS.md`, `GEMINI.md`, `AGENTS.md`, equivalent top-level directives |
...
Target rules:
- Prefer exact target keys such as `windows/codex`, `macos/codex`, `windows/gemini`.
...
It is shared across macOS, Windows, Codex, Codex CLI, and Gemini CLI via git.
