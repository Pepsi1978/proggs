...
$whiteboardPath = Join-Path $env:USERPROFILE "proggs\.codex\agent-memory\shared\MEMORY.md"
$content = $null
...
# --- Invariant 5: AGENTS.md Sync ---
$claudeRepo = Join-Path $env:USERPROFILE "proggs\AGENTS.md"
$claudeHome = Join-Path $env:USERPROFILE "AGENTS.md"
if ((Test-Path $claudeRepo) -and (Test-Path $claudeHome)) {
    ...
    if ($hashRepo -ne $hashHome) {
        $violations += "CODEX.MD: Repo-Version und Home-Version sind NICHT synchron!"
    }
}

# --- Invariant 6: Heartbeat-Status ---
$heartbeatStatus = Join-Path $env:USERPROFILE ".codex\heartbeat-status.json"
...
$settingsMain = Join-Path $env:USERPROFILE ".codex\settings.json"
