...
function findProjectsDir(): string {
	const claudeDir = join(HOME, ".codex", "projects");
...
const LEARNING_STATE_FILE = join(HOME, ".codex", "learning-loop-state.json");
const AUTO_RULES_DIR = join(HOME, ".codex", "rules", "auto-learned");
...
function insertViaWhiteboardInsert(section: string, entry: string): void {
	const hookDir = join(homedir(), ".codex", "hooks");
