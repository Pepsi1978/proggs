#!/usr/bin/env bash
# codex-mem-worker-guard.sh — Universal service readiness & plugin hooks hardener
#
# PRIMARY PURPOSE: Prevent SessionStart hook errors from race conditions between
# MCP servers and plugin hooks. Currently handles codex-mem's worker daemon,
...
#
# ROBUSTNESS FEATURES:
#   - Dynamic port detection from ~/.codex-mem/settings.json
...
# Read codex-mem port dynamically
CMEM_PORT=37777
CMEM_HOST="127.0.0.1"
...
if [ -f "$CMEM_SETTINGS" ]; then
    ...
    [ -n "$_p" ] && CMEM_PORT="$_p"
    _h=$(grep -o '"CODEX_MEM_WORKER_HOST"[[:space:]]*:[[:space:]]*"[^"]*"' "$CMEM_SETTINGS" 2>/dev/null | sed 's/.*"\([^"]*\)"$/\1/')
    [ -n "$_h" ] && CMEM_HOST="$_h"
fi
...
SERVICES=(
    "codex-mem-worker|${CMEM_HOST}|${CMEM_PORT}"
...
find_plugin_root() {
    ...
    local mkt_dir="$HOME/.codex/plugins/marketplaces/$vendor_short/plugin"
...
patch_plugin_hooks() {
    ...
    sed \
        ...
        -e "s|worker-service\.cjs\" hook codex context\"|worker-service.cjs\" hook codex context ${fallback}\"|" \
...
for service_entry in "${SERVICES[@]}"; do
    ...
    case "$svc_name" in
        codex-mem-worker)
            PLUGIN_ROOT=$(find_plugin_root "thedotmack/codex-mem" "scripts/worker-service.cjs")
