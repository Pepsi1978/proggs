...
#
# IMPORTANT: NEVER kill codex.exe processes — they manage their own lifecycle.
# Killing older codex.exe processes DESTROYS active sessions in other tabs.
# (Root cause of "session destruction" bug, fixed 2026-04-12)

$ErrorActionPreference = 'SilentlyContinue'

try {
    # Get all running codex.exe PIDs — these are ACTIVE session owners
    $activeClaudes = @{}
    ...
    if ($activeClaudes.Count -eq 0) {
        # No codex processes at all — nothing to clean
        exit 0
    }

    # Find orphaned node.exe processes:
    # A node.exe is an orphan if its parent PID is NOT an active codex.exe process
    ...
    if ($nodeProcs) {
        foreach ($node in $nodeProcs) {
            try {
                ...
                # If parent is alive (whether codex or something else), leave it alone
            } catch {
