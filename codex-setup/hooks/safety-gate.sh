...
for pattern in "${SHELL_UPDATE_PATTERNS[@]}"; do
    if printf '%s' "$cmd" | grep -Eq "$pattern"; then
        ...
        printf '%s\n' "WARNING (safety-gate): Shell/Tool-Update erkannt. Laut AGENTS.md-Regel muessen Shell-Updates NACH Abschluss aller Aufgaben erfolgen — sie koennen alle offenen Terminals killen. Bitte erst alle Aufgaben beenden und committen, dann updaten."
