...
$heartbeatDir = Join-Path $env:USERPROFILE ".codex"
...
$whiteboard = Join-Path $proggsDir ".codex\agent-memory\shared\MEMORY.md"
$settingsJson = Join-Path $env:USERPROFILE ".codex\settings.json"
$settingsLocal = Join-Path $env:USERPROFILE ".codex\settings.local.json"
...
function Notify-Critical([string]$msg) {
    ...
    try {
        ...
        $textNodes.Item(0).AppendChild($template.CreateTextNode("Codex Heartbeat KRITISCH")) | Out-Null
...
function Check-KeyDirectories {
    ...
    $details = ""
    foreach ($dir in @(
        (Join-Path $env:USERPROFILE ".codex\hooks"),
        (Join-Path $env:USERPROFILE ".codex\agents"),
        (Join-Path $env:USERPROFILE ".codex\rules"),
        (Join-Path $proggsDir ".codex\agent-memory\shared")
    )) {
...
try {
    ...
    # Print summary for SessionStart hook output (visible to Codex)
    $warnings = @()
