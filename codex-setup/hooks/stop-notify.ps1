# stop-notify.ps1 — Fires immediately when Codex finishes work
# Hook event: Stop
...
$toastCmd = 'try{[Windows.UI.Notifications.ToastNotificationManager,Windows.UI.Notifications,ContentType=WindowsRuntime]|Out-Null;$x=[Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent(0);$x.GetElementsByTagName(\"text\").Item(0).AppendChild($x.CreateTextNode(\"Codex ist fertig\"))|Out-Null;[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier(\"Codex\").Show([Windows.UI.Notifications.ToastNotification]::new($x))}catch{}'
