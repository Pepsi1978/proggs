#!/bin/bash
# Dynamic notification for macOS: extracts the actual message from Codex
...
if command -v osascript &>/dev/null; then
    ...
    safe_msg="${msg//\\/\\\\}"
    safe_msg="${safe_msg//\"/\\\"}"
    osascript -e "display notification \"${safe_msg}\" with title \"Codex\"" 2>/dev/null
    if [ $? -ne 0 ]; then
        hook_log_warn "osascript notification failed"
    fi
elif command -v notify-send &>/dev/null; then
    # Linux fallback (requires libnotify-bin)
    notify-send "Codex" "$msg" 2>/dev/null
    if [ $? -ne 0 ]; then
