...
# Event: PostToolUse (Edit|Write auf .ps1 oder .sh Dateien in ~/.codex/hooks/)
# Zweck: Sofortmassnahme 2 — Automatische Cross-Platform-Pruefung
#
# Was dieses Skript tut:
# 1. Erkennt wenn eine .ps1-Datei in ~/.codex/hooks/ bearbeitet wurde
# 2. Prueft ob ein .sh-Gegenstueck existiert
# 3. Prueft ob das Gegenstueck aelter ist als die geaenderte Datei
# 4. Warnt Codex per additionalContext — blockiert NICHT (Stufe 1)
#
# Defense in Depth:
#   Schicht 1: Diese automatische Warnung (Poka-Yoke Stufe 1)
#   Schicht 2: Die Cross-Platform-Checkliste in AGENTS.md (Text-Regel)
...
# Nur Hook-Dateien in ~/.codex/hooks/ pruefen
if ($filePath -notmatch '\.codex/hooks/') { exit 0 }
...
if ($warnings.Count -gt 0) {
    ...
    # Logging
    $logDir = Join-Path $env:USERPROFILE ".codex" "logs" "hooks"
