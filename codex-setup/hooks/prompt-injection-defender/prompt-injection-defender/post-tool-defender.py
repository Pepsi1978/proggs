...
# ///
"""
Codex Prompt Injection Defender - PostToolUse Hook
=========================================================

Scans tool outputs for prompt injection attempts and warns Codex.
Detects: instruction overrides, role-playing/DAN, encoding/obfuscation, context manipulation.

This hook runs AFTER tool execution and provides warnings to Codex about
...
JSON output for warnings:
  {"decision": "block", "reason": "Warning message for Codex"}

Note: In PostToolUse, "block" doesn't prevent execution (tool already ran),
but sends the reason message to Codex as a warning.
"""
    ...
    if isinstance(tool_result, dict):
        ...
        if "content" in tool_result:
            ...
            if isinstance(content, str):
                ...
                # Handle array of content blocks (common in Codex responses)
                texts = []
...
def format_warning(
    ...
    """Format detections into a warning message for Codex.
...
def _get_trusted_prefixes() -> list:
    ...
    if not home:
        return []
    return [
        os.path.join(home, ".codex").replace("\\", "/").lower(),
...
def _is_trusted_bash_command(command: str, trusted_prefixes: list) -> bool:
    ...
    Reduces false positives from commands like 'cat ~/.codex/settings.json'
...
def main() -> None:
    ...
    # Codex uses "tool_response", not "tool_result"
    ...
    if detections:
        ...
        # Output JSON to provide warning to Codex
        # Using "block" decision sends the reason to Codex as feedback
