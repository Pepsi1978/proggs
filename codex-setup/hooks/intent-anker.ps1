...
# Read JSON input from stdin (Codex sends hook context)
$json = $null
try {
...
# This prevents the counter from carrying over across separate Codex sessions
$goalFile = Join-Path $env:TEMP "codex-session-goal.txt"
...
if (($turn % 5) -eq 0 -and (Test-Path $GoalFile)) {
    $reminderFile = Join-Path $env:TEMP "codex-intent-reminder.txt"
