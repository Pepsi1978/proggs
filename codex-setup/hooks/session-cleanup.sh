...
# Cleans up temp files when Codex exits.
...
CODEX_TEMP="${TMPDIR:-/tmp}/claude"
if [ -d "$CODEX_TEMP" ]; then
    # Remove files older than 2 hours (120 minutes)
    while IFS= read -r -d '' file; do
        rm -f "$file" 2>/dev/null && CLEANED=$((CLEANED + 1)) || true
    done < <(find "$CODEX_TEMP" -type f -mmin +120 -print0 2>/dev/null)

    # Remove empty directories (bottom-up)
    find "$CODEX_TEMP" -type d -empty -delete 2>/dev/null || true
fi
...
# Clean /tmp/codex-* files — but preserve intent-anker state files
while IFS= read -r -d '' file; do
    rm -f "$file" 2>/dev/null && CLEANED=$((CLEANED + 1)) || true
done < <(find "${TMPDIR:-/tmp}" -maxdepth 1 -name "codex-*" -type f \
    ! -name "codex-session-goal.txt" \
    ! -name "codex-turn-counter.txt" \
    ! -name "codex-intent-reminder.txt" \
    -print0 2>/dev/null)
