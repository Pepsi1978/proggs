...
# 1. Turn count
COUNTER_FILE="${TMPDIR:-/tmp}/codex-turn-counter.txt"
turns=0
...
GOAL_FILE="${TMPDIR:-/tmp}/codex-session-goal.txt"
goal="unknown"
if [ -f "$GOAL_FILE" ]; then
...
# 3. Hook errors today
LOG_DATE=$(date +%Y-%m-%d)
LOG_FILE="$HOME/.codex/logs/hooks/$LOG_DATE.log"
hook_errors=0
if [ -f "$LOG_FILE" ]; then
...
goal_path = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'codex-session-goal.txt')
if not os.path.exists(goal_path):
    goal_path = '/tmp/codex-session-goal.txt'
goal = 'unknown'
...
SUMMARY_FILE="${TMPDIR:-/tmp}/codex-session-summary.txt"
...
# for f in codex-session-goal.txt codex-turn-counter.txt codex-intent-reminder.txt; do
...
HOOK_LOGS="$HOME/.codex/logs/hooks"
if [ -d "$HOOK_LOGS" ]; then
