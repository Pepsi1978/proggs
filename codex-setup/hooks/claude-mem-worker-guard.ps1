# codex-mem-worker-guard.ps1 — Universal service readiness & plugin hooks hardener
#
...
$cmemPort = 37777
$cmemHost = "127.0.0.1"
$cmemSettings = Join-Path $env:USERPROFILE ".codex-mem\settings.json"

if (Test-Path $cmemSettings) {
    try {
        $settings = Get-Content $cmemSettings -Raw | ConvertFrom-Json
        if ($settings.CODEX_MEM_WORKER_PORT) { $cmemPort = [int]$settings.CODEX_MEM_WORKER_PORT }
        if ($settings.CODEX_MEM_WORKER_HOST) { $cmemHost = $settings.CODEX_MEM_WORKER_HOST }
    } catch { }
}

# Service registry — extend for future plugins
$services = @(
    @{ Name = "codex-mem-worker"; Host = $cmemHost; Port = $cmemPort }
...
function Find-PluginRoot {
    ...
    $cacheDir = Join-Path $env:USERPROFILE ".codex\plugins\cache\$Vendor"
    ...
    $mktPath = Join-Path $env:USERPROFILE ".codex\plugins\marketplaces\$vendorShort\plugin"
