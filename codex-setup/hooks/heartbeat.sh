#!/usr/bin/env bash
# heartbeat.sh — Periodic health check for the Codex development environment
...
#   5. codex-mem worker running
#   6. Key directories exist
#
# Output: Writes status to ~/.codex/heartbeat-status.json (atomic write)
# Notifications: Only for CRITICAL issues via osascript
# Logging: ~/.codex/logs/heartbeat/YYYY-MM-DD.log
#
...
# Configuration
# ─────────────────────────────────────────────────────────────────────
HEARTBEAT_DIR="$HOME/.codex"
STATUS_FILE="$HEARTBEAT_DIR/heartbeat-status.json"
LOG_DIR="$HEARTBEAT_DIR/logs/heartbeat"
LOG_FILE="$LOG_DIR/$(date '+%Y-%m-%d').log"
PROGGS_DIR="$HOME/proggs"
WHITEBOARD="$PROGGS_DIR/.codex/agent-memory/shared/MEMORY.md"
SETTINGS_JSON="$HOME/.codex/settings.json"
SETTINGS_LOCAL="$HOME/.codex/settings.local.json"
...
notify_critical() {
    ...
    if command -v osascript &>/dev/null; then
        local safe_msg="${msg//\\/\\\\}"
        safe_msg="${safe_msg//\"/\\\"}"
        osascript -e "display notification \"${safe_msg}\" with title \"Codex Heartbeat\" subtitle \"KRITISCH\"" 2>/dev/null || true
    fi
}
...
check_config_integrity() {
    ...
    if [ -f "$SETTINGS_JSON" ]; then
        if ! $PYTHON3 -c "import json; json.load(open('$SETTINGS_JSON'))" 2>/dev/null; then
            ...
            notify_critical "settings.json ist beschaedigt! Codex funktioniert nicht korrekt."
        fi
    else
        issues=$((issues + 1))
...
check_claude_mem_worker() {
    local loaded
    loaded=$(launchctl list 2>/dev/null | grep -c "com.codex-mem.worker" || echo "0")

    if [ "$loaded" -gt 0 ]; then
        echo '"codex_mem_worker": {"status": "OK", "detail": "Worker loaded in launchd"}'
    else
        echo '"codex_mem_worker": {"status": "WARNING", "detail": "Worker not loaded — run: launchctl load ~/Library/LaunchAgents/com.codex-mem.worker.plist"}'
    fi
}
...
check_key_directories() {
    ...
    for dir in "$HOME/.codex/hooks" "$HOME/.codex/agents" "$HOME/.codex/rules" "$PROGGS_DIR/.codex/agent-memory/shared"; do
        if [ ! -d "$dir" ]; then
            missing=$((missing + 1))
...
# Codex was operating below acceptable quality and corrections occurred.
# CRITICAL (notification) if 2+ sessions below threshold.
# ─────────────────────────────────────────────────────────────────────
check_validator_compliance() {
    local scores_file="$HOME/.codex/session-scores.jsonl"
...
# field. More than 3 corrections in one session means Codex made too
