...
SENSITIVE=false
if echo "$FILE_PATH" | grep -qE 'settings-reference\.json$|settings\.json$|settings\.local\.json$|codex-harness/.*\.json$'; then
    SENSITIVE=true
fi
...
Alternativ: python3 ~/.codex/scripts/redact-secrets.py \"$FILE_PATH\"'''
