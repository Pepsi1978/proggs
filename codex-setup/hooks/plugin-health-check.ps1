...
# --- Hook logging ---
$logDir = "$env:USERPROFILE\.codex\logs\hooks"
...
# --- CHECK 1: codex-mem worker ---
$cmemPort = 37777
$cmemHost = "127.0.0.1"
$cmemSettings = "$env:USERPROFILE\.codex-mem\settings.json"
if (Test-Path $cmemSettings) {
    try {
        $s = Get-Content $cmemSettings -Raw | ConvertFrom-Json
        if ($s.CODEX_MEM_WORKER_PORT) { $cmemPort = $s.CODEX_MEM_WORKER_PORT }
    } catch { }
}
...
try {
    ...
    Hook-Log "PASS codex-mem-worker (${cmemHost}:${cmemPort})"
} catch {
    Hook-LogError "FAIL codex-mem-worker (${cmemHost}:${cmemPort}) — not responding"
    $errors++
}
...
$pluginCache = "$env:USERPROFILE\.codex\plugins\cache\thedotmack\codex-mem"
    ...
    if ($latestVer) {
        ...
        if (Test-Path $hooksFile) {
            ...
            if ($content -match "sleep.*curl.*api/health.*exit") {
                Hook-Log "PASS codex-mem hooks.json (patched with fallback)"
            } else {
                Hook-LogWarn "codex-mem hooks.json exists but fallback patch missing"
            }
        } else {
            Hook-LogError "FAIL codex-mem hooks.json missing at $hooksFile"
            $errors++
        }
    }
}
