#!/usr/bin/env bash
# codex-mem-worker-launcher.sh — Launchd wrapper for codex-mem worker
#
# Dynamically finds the latest plugin version and starts the worker.
# Used by com.codex-mem.worker.plist (launchd agent).
#
# RESILIENCE FEATURES:
...
# - Logs to /tmp/codex-mem-worker.log for debugging
...
PLUGIN_DIR="$HOME/.codex/plugins/cache/thedotmack/codex-mem"
MAX_WAIT=60
waited=0
...
echo "$(LOG_PREFIX) Starting codex-mem worker: $BUN $WORKER_SCRIPT --daemon"
