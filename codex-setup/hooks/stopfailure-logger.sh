...
# Triggered by StopFailure hook event (Codex v2.1.78+)
...
RATE_LIMIT_FILE="$TMPDIR/codex-stopfailure-last.txt"
if [ ! -d "$TMPDIR" ]; then
    RATE_LIMIT_FILE="/tmp/codex-stopfailure-last.txt"
fi
...
# Read stdin for error details (Codex pipes error info)
ERROR_INPUT=""
if [ -t 0 ]; then
