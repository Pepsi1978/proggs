...
WHITEBOARD="$HOME/proggs/.codex/agent-memory/shared/MEMORY.md"
...
CODEX_REPO="$HOME/proggs/AGENTS.md"
CODEX_HOME="$HOME/AGENTS.md"
if [ -f "$CODEX_REPO" ] && [ -f "$CODEX_HOME" ]; then
    hash_repo=$(shasum -a 256 "$CODEX_REPO" | cut -d' ' -f1)
    hash_home=$(shasum -a 256 "$CODEX_HOME" | cut -d' ' -f1)
