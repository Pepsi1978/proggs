...
if [[ "$normalized" != *".codex/settings"*".json" ]]; then
    exit 0
fi
...
# ---------------------------------------------------------------------------
# Codex auto-creates allow entries when the user approves a tool.
...
allow_fixed=false
result=$(remove_allow_list_silent "$HOME/.codex/settings.json")
[ "$result" = "removed" ] && allow_fixed=true && hook_log "AUTO-FIX: removed allow list from settings.json" 2>/dev/null || true

result=$(remove_allow_list_silent "$HOME/.codex/settings.local.json")
...
if [ -d "$HOME/.codex/projects" ]; then
    for pd in "$HOME/.codex/projects"/*/; do
        [ -d "$pd" ] || continue
...
# effortLevel: MUST be "high" — BLOCK anything else (AGENTS.md requirement)
eff = data.get('effortLevel')
if eff and eff != 'high':
    blocks.append(f"effortLevel={eff} (MUSS 'high' sein — AGENTS.md-Regel)")

env_data = data.get('env', {})
if env_data:
    # CODEX_CODE_EFFORT_LEVEL in env
    env_eff = env_data.get('CODEX_CODE_EFFORT_LEVEL')
    if env_eff and env_eff != 'high':
        blocks.append(f"CODEX_CODE_EFFORT_LEVEL={env_eff} (MUSS 'high' sein)")

    # SUBAGENT_MODEL: BLOCK if changed from sonnet (critical for cost/quality)
    sub_model = env_data.get('CODEX_CODE_SUBAGENT_MODEL')
    if sub_model and sub_model != 'sonnet':
        blocks.append(f"CODEX_CODE_SUBAGENT_MODEL={sub_model} (erwartet: sonnet)")

    # AUTOCOMPACT: BLOCK if below 85 (85 is the correct standard, NOT 95)
    acp = env_data.get('CODEX_AUTOCOMPACT_PCT_OVERRIDE')
    if acp is not None:
        try:
            if int(acp) < 85:
