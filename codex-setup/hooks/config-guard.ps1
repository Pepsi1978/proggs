...
if ($normalizedPath -notmatch '\.codex/settings(\.local)?\.json$') {
    exit 0
}
...
$settingsPath2 = Join-Path $env:USERPROFILE ".codex" "settings.json"
$localSettingsPath2 = Join-Path $env:USERPROFILE ".codex" "settings.local.json"
...
try {
    $projectsDir2 = Join-Path $env:USERPROFILE ".codex" "projects"
...
$Settings = Join-Path $env:USERPROFILE ".codex" "settings.json"
...
$envData = $data.env
if ($envData) {
    # CODEX_CODE_EFFORT_LEVEL in env — same as above, user-controlled
    $envEff = $envData.CODEX_CODE_EFFORT_LEVEL
    if ($null -ne $envEff -and $envEff -ne '' -and $envEff -notin $validEfforts) {
        $blocks += "CODEX_CODE_EFFORT_LEVEL=$envEff (ungueltiger Wert)"
    }

    # SUBAGENT_MODEL: BLOCK if changed from sonnet (critical for cost/quality)
    $subModel = $envData.CODEX_CODE_SUBAGENT_MODEL
    if ($null -ne $subModel -and $subModel -ne '' -and $subModel -ne 'sonnet') {
        $blocks += "CODEX_CODE_SUBAGENT_MODEL=$subModel (erwartet: sonnet)"
    }

    # AUTOCOMPACT: BLOCK if below 85 (85 is the correct standard, NOT 95)
    $acp = $envData.CODEX_AUTOCOMPACT_PCT_OVERRIDE
