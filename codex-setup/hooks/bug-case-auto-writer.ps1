...
# 3. Wenn JA: Gibt den alten Fix als Kontext an Codex zurueck (RAG-Effekt)
...
# --- Pfade ---
$bugCasesPath = Join-Path $env:USERPROFILE "proggs" ".codex" "agent-memory" "shared" "bug-cases.jsonl"
...
if ($matchedCase -and $matchedScore -ge 0.5) {
    ...
    # Ausgabe als additionalContext fuer Codex
    $output = @{
        hookSpecificOutput = @{
    ...
    if (-not $isDuplicate) {
        ...
        # Codex informieren dass ein neuer Fall erfasst wurde
...
if ($outputJson) {
    ...
    try {
        ...
        $logDir2 = Join-Path $env:USERPROFILE ".codex" "logs" "hooks"
