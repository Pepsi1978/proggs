...
try:
    ...
    env = data.get('env', {})
    acp = env.get('CODEX_AUTOCOMPACT_PCT_OVERRIDE', '')
    if acp and int(acp) < 85:
        violations.append(f'AUTOCOMPACT={acp} (Minimum: 85)')

    sub = env.get('CODEX_CODE_SUBAGENT_MODEL', '')
...
check_settings_file "$HOME/.codex/settings.json"
check_settings_file "$HOME/.codex/settings.local.json"
