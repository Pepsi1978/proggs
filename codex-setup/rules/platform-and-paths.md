...
## 1. Arbeitsverzeichnis: IMMER ~/Codex/
...
`Pepsi1978/proggs` liegt in `~/Codex/`. Alle Projekte, Git-Operationen → dorthin.

**Was beim Session-Start passieren muss:**
1. Pruefen: Ist das aktuelle Verzeichnis `~/Codex/`?
2. Wenn NEIN: Sofort melden und alle Operationen auf `~/Codex/` beziehen

**Technische Absicherung:** Windows Terminal ist konfiguriert mit `startingDirectory: C:\Users\barwa\Codex`.
...
```python
# FALSCH:
with open('/c/Users/barwa/.codex/settings.json') as f: ...
...
- Nach `npm update -g` (besonders codex CLI)
...
```bash
# macOS/Linux:
bash ~/.codex/hooks/path-verify.sh --fix

# Windows:
pwsh ~/.codex/hooks/path-verify.ps1 -Fix
```

### Was geprueft wird

**Tools:** git, node, npm, bun, python3, rustc, cargo, go, codex, swift (macOS),
kotlin, gradle, deno, ollama (macOS), codexwatch
...
| Datei | Beschreibung |
|-------|-------------|
| `~/Codex/.mcp.json` | Projekt-MCP (plattformspezifisch!) |
| `codex-harness/mcp-macos.json` | macOS-Referenz |
| `codex-harness/mcp-windows.json` | Windows-Referenz |
| `~/.codex/hooks/reindex-codebase.sh/.ps1` | Plattform-spezifische Hooks |
| `~/Codex/.code-search/` | Lokaler Index (nie synchronisieren) |
...
> JSON zerstoert → Codex startete nicht richtig.
...
Bei Problemen: `find ~/.codex/plugins -name "*.sh" ! -perm -u+x` → `chmod +x`
...
```bash
# macOS/Linux:
echo "test" | codex -p --debug-file /tmp/hook-debug.txt --system-prompt "Reply with just OK"
grep "Hook.*error" /tmp/hook-debug.txt

# Windows:
echo "test" | codex -p --debug-file $env:TEMP\hook-debug.txt --system-prompt "Reply with just OK"
...
1. `grep -rn 'dateiname.ps1' ~/.codex/hooks/` — wird die Datei per Dot-Source geladen?
