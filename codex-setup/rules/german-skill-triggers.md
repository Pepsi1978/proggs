...
| "Kotlin testen", "Kotlin Tests" | `everything-codex:kotlin-test` | Kotlin-spezifisch: Kotest + MockK + Kover Coverage |
| "E2E Test", "Browser testen", "Playwright" | `everything-codex:e2e` | End-to-End Tests mit Playwright, Screenshots/Videos/Traces |
...
| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "ist mein Code sicher?", "Sicherheit beim Coden" | `everything-codex:security-review` | **Passives Wissen**: gibt Codex Sicherheitsregeln (OWASP, Secrets, Injection) als Kontext — fuehrt KEINEN Scan durch |
...
| "ist meine Codex-Config sicher?", "AgentShield" | `everything-codex:security-scan` | Scannt die **Codex-Code-Konfiguration** (~/.codex/), nicht den Quellcode — prueft AGENTS.md, settings.json, hooks auf Fehlkonfiguration |
...
**Entscheidungshilfe**: Quellcode scannen → `aikido:scan`. Codex-Konfiguration scannen → `security-scan`. Beim Coden Sicherheit beachten → `security-review`.
...
| "was lief gut?", "AGENTS.md aktualisieren" | `codex-reflect:reflect` | Sammelt Korrekturen aus der Session und schreibt sie in AGENTS.md — **langfristiges Lernen** |
...
| "Tastenkürzel ändern", "Keybinding" | `keybindings-help` | Hilft bei ~/.codex/keybindings.json Anpassungen |
...
**NICHT verwechseln**: `codex-reflect` = langfristig lernen (AGENTS.md). `reflexion:reflect` = aktuelle Antwort bewerten. `ace-curator` = datenbasierte Regeloptimierung (Session-Scores + Bug-DB).
...
| "was wissen wir über X?", "Erinnerung suchen" | `codex-mem:mem-search` | Durchsucht das **codex-mem Observations-System** (Tree-sitter-geparst, Timeline, Token-Kosten) — umfassender als episodic-memory |
| "wie haben wir das Problem gelöst?", "früherer Fix" | Beide nacheinander: erst `codex-mem:mem-search`, dann `episodic-memory` falls nichts gefunden |
...
| "Präsentation bauen", "Slides erstellen" | `everything-codex:frontend-slides` | Animationsreiche HTML-Praesentation oder PowerPoint-Konvertierung |
...
| "Bild generieren", "Bild erstellen" | `everything-codex:fal-ai-media` | Text-to-Image via fal.ai (Nano Banana), auch Video und Audio |
...
| "Go Code reviewen" | `everything-codex:go-review` | Idiomatic Go, Concurrency, Error Handling Review |
| "Python Code reviewen" | `everything-codex:python-review` | PEP 8, Type Hints, Pythonic Idioms Review |
| "Kotlin Code reviewen" | `everything-codex:kotlin-review` | Null Safety, Coroutines, Compose Review |
| "Build-Fehler Go" | `everything-codex:go-build` | Fixt Go Build/Vet Fehler mit minimalen Aenderungen |
| "Build-Fehler Kotlin/Gradle" | `everything-codex:kotlin-build` | Fixt Kotlin/Gradle Build-Fehler mit minimalen Aenderungen |
| "Build-Fehler TypeScript" | `everything-codex:build-error-resolver` Agent | Fixt Build/Type-Errors, fokussiert auf gruenes Build |

## 13. Skill-Erstellung & System-Erweiterung

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "erstelle einen Skill", "neuer Skill" | `skill-creator:skill-creator` | Gefuehrte Skill-Erstellung mit Qualitaetspruefung (PFLICHT laut AGENTS.md) |
...
| "AGENTS.md verbessern", "Regeln prüfen" | `codex-md-management:codex-md-improver` | Auditiert und verbessert AGENTS.md-Dateien |
...
- **Neues CLI gestartet (Codex/Gemini)** → `import` (portiert Aenderungen von Codex)

**Automatische Benachrichtigung:** Bei SessionStart zeigt `mirror-check` Hook an wenn Eintraege ausstehen.

**Technischer Hinweis (Custom Agents starten):** Custom Agents aus `~/.codex/agents/` sind
...
Vollstaendige Regel: `~/.codex/rules/notebooklm-cli.md`
...
Bei fehlendem Tool: `pwsh ~/.codex/hooks/path-verify.ps1 -Fix` repariert automatisch.
...
| "Reflektion" / "Reflect" | `codex-reflect:reflect` (Lernen) oder `reflexion:reflect` (Bewertung) — nachfragen! |
...
| "wie habe ich mich verbessert?", "Session-Trend", "Fortschritt" | Session-Scores lesen (`~/.codex/session-scores.jsonl`) | Quantitative Trend-Analyse ueber die letzten N Sessions |
