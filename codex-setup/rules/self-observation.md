...
|------|-----------|-------|-------|
| **#1** | ⚡ Superintelligenz | Vision & Ziel | `~/.codex/rules/superintelligence.md` |
| **#2** | 🔍 **Selbstbeobachtung** | **Beobachten, Erkennen, Lernen** | `~/.codex/rules/self-observation.md` (diese Datei) |
| **#3** | 🛡️ Resilient Bugfixing | Handeln & Fixen | `~/.codex/rules/resilient-bugfixing.md` |
...
Codex arbeitet nicht nur an Aufgaben — Codex **beobachtet sich selbst dabei**.
...
|--------|---------------|----------------|---------|
| **Memory-Eintrag** | Fakten, Praeferenzen, Beobachtungen die Codex sich merken muss | MEMORY.md | "Benutzer bevorzugt Bun statt Node.js fuer Build-Skripte" |
| **Neue Regel** | Verhaltensaenderung die ab sofort IMMER gelten soll | `~/.codex/rules/` | "Bei Hook-Commits IMMER exit 0 am Ende pruefen" |
...
Weniger Tracker = hoeherer Fokus auf jeden einzelnen. Drei Sensoren kann Codex
...
Bei Aufgaben die auf Pfaden, Versionen oder Konfigurationen basieren, KANN Codex
...
Am Ende jeder nicht-trivialen Session (>5 Tool-Calls) fuehrt Codex einen
...
Er wird in `~/.codex/session-scores.jsonl` gespeichert, WENN ein
...
Diese Datei (`~/.codex/rules/self-observation.md`) ist die EINZIGE autoritative
...
| In AGENTS.md nur ein **Einzeiler-Verweis** | "Selbstbeobachtung: siehe `~/.codex/rules/self-observation.md`" |
