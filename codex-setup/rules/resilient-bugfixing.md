...
|------|-----------|-------|
| **#1** | Superintelligenz | `~/.codex/rules/superintelligence.md` |
| **#2** | Selbstbeobachtung | `~/.codex/rules/self-observation.md` |
| **#3** | **Resilient Bugfixing** | `~/.codex/rules/resilient-bugfixing.md` (diese Datei) |
| #4+ | Alle anderen Regeln | `~/.codex/rules/*` |
...
Dies ist eines der wichtigsten Prinzipien dieser Direktive. Codex neigt dazu, bei
...
`additionalContext` injiziert (erkennbar am Text `BEKANNTER FEHLER`), MUSS Codex
...
(erkennbar am Text `CROSS-PLATFORM`), MUSS Codex das dem Benutzer **sofort ansagen**:
...
Wenn der `config-guard-preemptive` Hook eine Aenderung blockiert hat, MUSS Codex
...
|-----------|-------|---------|
| **Gleiche Klasse** | Kann der gleiche Fehlertyp an anderer Stelle auftreten? | Race Condition bei codex-mem → gibt es andere Plugins mit dem gleichen Muster? |
...
Codex muss **waehrend der Arbeit** kontinuierlich nach Verbesserungen suchen und dem Benutzer
...
| Hook-Ausfuehrung | `bash ~/.codex/hooks/*.sh` | `pwsh -File "$USERPROFILE/.codex/hooks/*.ps1"` |
...
### Beispiel 1: codex-mem Hook-Fehler (2026-03-22)
