...
1. **Global**: `~/.codex/settings.json` → `permissions.defaultMode: "bypassPermissions"`
2. **Global lokal**: `~/.codex/settings.local.json` → `permissions.defaultMode: "bypassPermissions"`
3. **Jedes Projekt**: `~/.codex/projects/*/settings.local.json` → `permissions.defaultMode: "bypassPermissions"`
...
| 4 | Diese Regel | Codex weiss: Nie den Benutzer nach Permissions fragen, nie allow-Liste erstellen |
| 5 | Memory | Feedback-Memory als zusaetzliche Erinnerung |
| 6 | `.bashrc` Auto-cd | Workspace-Korrektur bevor Codex startet |
...
- Neue Repos erstellen (verboten laut AGENTS.md)
