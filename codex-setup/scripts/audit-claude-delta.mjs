...
function classifyPath(sourcePath) {
	if (sourcePath === "CLAUDE.md") {
		return {
			category: "root-rules",
			target_hints: ["AGENTS.md", "codex-setup/rules/"],
		};
	}
	...
	if (
		sourcePath === "claude-code-setup/state/implemented-intelligence-suggestions.json"
	) {
		return {
	...
	if (/^claude-code-setup\/skills\/self-improve/u.test(sourcePath)) {
		return {
			category: "self-improve",
			target_hints: [
				"codex-setup/commands/self-improve.md",
				"codex-setup/skills/",
			],
		};
	}
	...
	if (/^claude-code-setup\/commands\//u.test(sourcePath)) {
		return {
			...
			target_hints: [
				...
				"codex-setup/rules/german-skill-triggers.md",
			],
		};
	}
	...
	return {
		category: "setup-doc",
		target_hints: ["codex-setup/README.md", "codex-setup/skills/"],
	};
}
...
function buildRecommendation(strategy) {
	...
	if (strategy === "REPLACE") {
		return "Vor Ersetzung warnen und ausdrückliche Freigabe einholen.";
	}
	return "Für Codex übersetzen und additive Integration bevorzugen.";
}
...
function buildReason({
	...
	if (strategy === "ADD") {
		const base =
			"Die Änderung hat keinen direkten Codex-Zielkonflikt und wirkt additiv.";
		return bugfixSignal
			? `${base} Zusätzlich ist robuste Bugfix- oder Härtungslogik erkennbar.`
			: base;
	}
	if (strategy === "REPLACE") {
		const base = `Die Änderung berührt bestehende Codex-Ziele (${overlappingTargets.join(", ")}).`;
		return bugfixSignal
			? `${base} Gleichzeitig steckt darin bestehende Fixlogik, die vor einem Ersatz separat bewertet werden soll.`
			: base;
	}
	const base = `Die Änderung betrifft vorhandene Codex-Bereiche (${categories.join(", ")}), braucht aber Übersetzung statt blindes Spiegeln.`;
	return bugfixSignal
		? `${base} Die Quelle enthält ausserdem übernehmbare Bugfix- oder Härtungsmuster.`
		: base;
}
	...
	for (const commit of commits) {
		...
		candidates.push({
			...
			bugfix_recommendation: bugfixSignal
				? "Fixlogik und Regression-Schutz für Codex mitprüfen und nach Möglichkeit übernehmen."
				: "",
			reason: buildReason({
				strategy,
				categories,
...
function formatTextReport(result) {
	const lines = [
		...
		"Hinweis: Alle gefundenen Delta-Kandidaten sind nur Vorschläge. Nichts autonom ersetzen; additive Codex-Integration bevorzugen und REPLACE-Fälle vorher bestätigen lassen.",
	];

	if (result.base_commit_status === "missing") {
		lines.push(
			"Warnung: Der gespeicherte Basis-Commit ist nicht mehr gültig. Der Audit hat deshalb die gesamte relevante Historie betrachtet.",
		);
	}
	...
	for (const group of groups) {
		...
		items.forEach((candidate, index) => {
			...
			lines.push(
				`  Bugfix-/Härtungssignal: ${candidate.bugfix_signal ? "ja" : "nein"}`,
			);
			...
			if (candidate.approval_required) {
				lines.push(
					"  Warnung: Diese Änderung würde bestehende Codex-Logik berühren. Vor Ersetzung erst bestätigen lassen.",
				);
			}
		});
	}
