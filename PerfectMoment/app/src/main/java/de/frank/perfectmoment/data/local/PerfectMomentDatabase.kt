...
abstract class PerfectMomentDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val Migration8To9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                insertAssumptionReinforcementSkill(db, System.currentTimeMillis())
            }
        }
        ...
        private fun insertAssumptionReinforcementSkill(db: SupportSQLiteDatabase, createdAt: Long) {
            db.insert(
                "skills",
                SQLiteDatabase.CONFLICT_ABORT,
                ContentValues().apply {
                    put("name", PreinstalledContent.ASSUMPTION_REINFORCEMENT_SKILL_NAME)
                    put("text", PreinstalledContent.assumptionReinforcementSkillText)
                    put("createdAt", createdAt)
                },
            )
        }
    ...
    private object SeedCallback : Callback() {
        override fun onCreate(db: SupportSQLiteDatabase) {
            ...
            insertAssumptionReinforcementSkill(db, now + 3)
