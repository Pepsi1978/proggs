        ...
        .put(
            ...
            JSONObject().put(
                ...
                JSONObject()
                    ...
                    .put("name", "experimente_textfassung")
                    .put("strict", true)
                    .put("schema", schema),
            ),
        )
...
class CodexAuthManager(context: Context) {
    ...
    /**
     * Der allgemeine Weg zur KI: schickt eine fertige Nutzlast und liefert den Antworttext.
     *
     * Die Nutzlasten für Vorschläge, Auswertung, Gespräch, Logbuch, Verdichtung und
     * Erkenntnisse werden in `de.frank.experimente.ai.Aufgaben` gebaut — dieser Baustein
     * kennt nur das Anfragen, nicht den Zweck.
     */
    suspend fun frage(payload: JSONObject): String = withContext(Dispatchers.IO) {
        try {
            requestCodexResponse(payload, decodeQuestions = false)
        } catch (error: IOException) {
