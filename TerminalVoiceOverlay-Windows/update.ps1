...
for ($i = 0; $i -lt $maxAttempts; $i++) {
    $procs = Get-Process -Name "TerminalVoiceOverlay" -ErrorAction SilentlyContinue
    if (-not $procs) { break }
...
$remaining = Get-Process -Name "TerminalVoiceOverlay" -ErrorAction SilentlyContinue
if ($remaining) {
...
for ($i = 0; $i -lt $maxAttempts; $i++) {
    ...
    Stop-Process -Name "TerminalVoiceOverlay" -Force -ErrorAction SilentlyContinue
...
$running = Get-Process -Name "TerminalVoiceOverlay" -ErrorAction SilentlyContinue
if ($running) {
