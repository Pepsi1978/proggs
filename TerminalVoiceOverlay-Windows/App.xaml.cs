...
namespace TerminalVoiceOverlay
{
    public partial class App : Application
    {
        ...
        private static void ApplyRealtimePriority()
        {
            ...
            {
                try { Console.Error.WriteLine($"priority_realtime_failed | {ex.Message}"); } catch { }
                try
                {
                ...
                {
                    try { Console.Error.WriteLine($"priority_high_fallback_failed | {fallbackEx.Message}"); } catch { }
                }
            }
        ...
        // "TerminalVoiceOverlay" traegt: eine reine Namenssuche koennte den
        ...
        private static string OverlayPidFile => Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
            "TerminalVoiceOverlay", "overlay.pid");
        ...
        private Process? FindOverlayProcess()
        {
            ...
            try
            {
                    ...
                    pid != myPid)
                {
                    ...
                    if (string.Equals(byPid.ProcessName, "TerminalVoiceOverlay", StringComparison.OrdinalIgnoreCase))
                        return byPid;
                }
            }
            catch
