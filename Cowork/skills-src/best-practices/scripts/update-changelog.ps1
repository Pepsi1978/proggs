#!/usr/bin/env pwsh
# update-changelog.ps1 — best-practices Skill (Cowork-Fassung)
#
...
#
# Aufruf:
#   pwsh scripts/update-changelog.ps1                 # inkrementell (Default)
#   pwsh scripts/update-changelog.ps1 -FirstRun       # komplette Neuanlage
#   pwsh scripts/update-changelog.ps1 -DataDir <pfad> # anderer Zielordner

[CmdletBinding()]
param(
  [string]$DataDir = (Join-Path (Get-Location) 'best-practices'),  # Cowork: relativ zum Arbeitsordner (CWD)
  [switch]$FirstRun
)
