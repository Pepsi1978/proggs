...
class Sicherungsrahmen(
    ...
    private suspend fun lies(quelle: Reader, einspielen: Boolean): SicherungsVorschau {
            ...
            while (leser.hasNext()) {
                ...
                when (val feld = inhalt.kopfAliase[gelesenesFeld] ?: gelesenesFeld) {
                    ...
                    // wird es übergangen — eine Datei aus einer neueren Fassung bleibt so lesbar.
                        ...
                        // ueberhaupt zu dieser App gehoert. Erst pruefen, dann einspielen —
                        // sonst landen Saetze aus einer fremden App in der Datenbank, bevor
                        // die Ablehnung greift.
                        if (!kopfGeprueft) {
        ...
        // Bei einer Datei ohne Nutzlast kam die Pruefung oben nicht zum Zug.
        if (!kopfGeprueft) pruefeKopf(schema, app, datenmodellVersion)
