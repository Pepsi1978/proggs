...
Die Trigger zeigen auf den Skill der WIRKLICH das tut was der Benutzer will ÔÇö nicht
...
| "mach das r├╝ckg├ñngig", "undo", "revert" | `undo-changes` | Revert per `git revert` (nie force-push), lokal UND auf GitHub |
| "Branches aufr├ñumen", "alte Branches l├Âschen" | `commit-commands:clean_gone` | Loescht lokale Branches die remote schon geloescht sind |
...
**NICHT verwechseln**: "pushe das" (ohne PR) ÔåÆ kein Skill noetig, direkt `git push` ausfuehren.

## 2. Code Review (3 verschiedene Tools ÔÇö richtig waehlen!)

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "reviewe den PR", "PR Review" | `code-review:code-review` | Analysiert einen **GitHub-PR** mit 5 parallelen Agenten + Scoring (nur Issues ÔëÑ80/100 werden als GitHub-Kommentar gepostet) |
| "pr├╝fe meine ├änderungen", "schau dir das an" | `pr-review-toolkit:review-pr` | Prueft **lokale Aenderungen** (kein PR noetig) mit 6 Spezialisten (Kommentare, Tests, stille Fehler, Typen, Code-Qualitaet, Vereinfachung) |
| "CodeRabbit", "externer Review" | `coderabbit:review` | Ruft den **externen CodeRabbit-Cloud-Dienst** auf ÔÇö die Analyse macht NICHT Claude sondern CodeRabbit.ai |
| "zweite Meinung", "was sagt Codex?", "Gemini fragen" | `second-opinion:second-opinion` | Holt Review von einem **anderen LLM** (OpenAI Codex oder Google Gemini CLI) ÔÇö nicht von Claude |

**Entscheidungshilfe**: PR auf GitHub? ÔåÆ `code-review`. Lokale Aenderungen? ÔåÆ `pr-review-toolkit`. Externes Tool? ÔåÆ `coderabbit` oder `second-opinion`.

## 3. Debugging & Fehler

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "finde den Bug", "warum geht das nicht?", "funktioniert nicht" | `superpowers:systematic-debugging` | 4-Phasen-Methode: Root Cause ÔåÆ Pattern ÔåÆ Hypothese ÔåÆ Fix. Eisernes Gesetz: KEIN Fix ohne vorherige Root-Cause-Analyse |
...
| "was ist kaputt?", "Fehler suchen", "Analyse" | `tool-check` | Systematischer Code-Scanner: 4 Analyse-Schleifen (Oberflaeche ÔåÆ Tief ÔåÆ Architektur ÔåÆ UI), MANUELL getriggert |

**NICHT verwechseln**: "Fehler fixen" bei bekanntem Bug ÔåÆ `systematic-debugging`. "Code pr├╝fen" ohne bekannten Bug ÔåÆ `tool-check`.
...
| "wie sollen wir das machen?", "Ideen sammeln" | `superpowers:brainstorming` | Gleich ÔÇö immer VOR der Planung, nie ueberspringen |
...
| "Spezifikation schreiben", "Anforderungen kl├ñren" | `sdd:brainstorm` | Verfeinert rohe Ideen zu vollstaendigen Designs ÔÇö aehnlich wie superpowers:brainstorming aber aus dem SDD-Framework |

**Reihenfolge**: Brainstorming ÔåÆ Plan ÔåÆ Implementation. Nie direkt zum Plan springen ohne Brainstorming.

## 5. Testen (3 verschiedene Ansaetze ÔÇö richtig waehlen!)

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "schreib Tests f├╝r meinen Code", "Testabdeckung erh├Âhen" | `tdd:write-tests` | Schreibt **nachtraeglich** Tests fuer existierenden Code via Agenten-Schwarm (Haiku bewertet Komplexitaet, Agenten schreiben Tests parallel) |
...
**Entscheidungshilfe**: Code existiert, Tests fehlen? ÔåÆ `tdd:write-tests`. Neuer Code wird geschrieben? ÔåÆ `superpowers:tdd`. Tests sind rot nach Aenderung? ÔåÆ `tdd:fix-tests`.

## 6. Sicherheit (3 komplett verschiedene Scopes!)

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "ist mein Code sicher?", "Sicherheit beim Coden" | `everything-claude-code:security-review` | **Passives Wissen**: gibt Claude Sicherheitsregeln (OWASP, Secrets, Injection) als Kontext ÔÇö fuehrt KEINEN Scan durch |
...
| "ist meine Claude-Config sicher?", "AgentShield" | `everything-claude-code:security-scan` | Scannt die **Claude-Code-Konfiguration** (~/.claude/), nicht den Quellcode ÔÇö prueft CLAUDE.md, settings.json, hooks auf Fehlkonfiguration |
...
| "Prompt Injection pr├╝fen" | Parry-Scanner (automatisch via Hook) | Lauft bereits automatisch bei jedem UserPromptSubmit |

**Entscheidungshilfe**: Quellcode scannen ÔåÆ `aikido:scan`. Claude-Konfiguration scannen ÔåÆ `security-scan`. Beim Coden Sicherheit beachten ÔåÆ `security-review`.
...
| "was lief gut?", "CLAUDE.md aktualisieren" | `claude-reflect:reflect` | Sammelt Korrekturen aus der Session und schreibt sie in CLAUDE.md ÔÇö **langfristiges Lernen** |
| "bewerte deine Antwort", "war das gut?" | `reflexion:reflect` | Bewertet die **aktuelle Ausgabe** mit strengem Scoring (1-5) ÔÇö **sofortige Qualitaetspruefung** |
| "pr├╝fe die Umgebung", "Environment Check" | `env-checker` Agent | Prueft installierte Tools, Versionen, Hooks, Plugins, Security-Patches |
| "konfiguriere", "Settings ├ñndern", "Hook einrichten" | `update-config` | Aendert settings.json: Permissions, Hooks, Env-Vars |
| "Tastenk├╝rzel ├ñndern", "Keybinding" | `keybindings-help` | Hilft bei ~/.claude/keybindings.json Anpassungen |
...
| "was wissen wir ├╝ber X?", "Erinnerung suchen" | `claude-mem:mem-search` | Durchsucht das **claude-mem Observations-System** (Tree-sitter-geparst, Timeline, Token-Kosten) ÔÇö umfassender als episodic-memory |
| "wie haben wir das Problem gel├Âst?", "fr├╝herer Fix" | Beide nacheinander: erst `claude-mem:mem-search`, dann `episodic-memory` falls nichts gefunden |
...
| "was sagt die Doku?", "offizielle Dokumentation" | context7 MCP (`resolve-library-id` ÔåÆ `query-docs`) | Laedt aktuelle Dokumentation fuer beliebige Bibliothek |
...
| "Pr├ñsentation bauen", "Slides erstellen" | `everything-claude-code:frontend-slides` | Animationsreiche HTML-Praesentation oder PowerPoint-Konvertierung |
...
| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "Swift bauen", "iOS/macOS App" | `apple-platform-build-tools:building-apple-platform-products` | Baut, testet, archiviert mit xcodebuild/swift ÔÇö absorbiert Build-Logs |
...
| "CLAUDE.md verbessern", "Regeln pr├╝fen" | `claude-md-management:claude-md-improver` | Auditiert und verbessert CLAUDE.md-Dateien |
...
**Entscheidungshilfe**: Session beenden ÔåÆ `mirror-export`. Neue Plattform starten ÔåÆ `mirror-import`. Beide Agenten zusammen bilden die **Universal Mirror Bridge**.
...
| "Reflektion" / "Reflect" | `claude-reflect:reflect` (Lernen) oder `reflexion:reflect` (Bewertung) ÔÇö nachfragen! |
| "Code Rabbit" | `coderabbit:review` |

## 14. Nemo ÔÇö Kostenloser Wissens-Worker (Nemotron 3 Super 120B)
...
| "Quizfragen generieren", "500 Fragen erstellen" | `nemo` Agent ÔåÆ `nemo_quiz` | Parallele Quiz-Generierung via Nemotron (QuizVerse-Format) |
| "recherchiere mit Nemo", "Nemo soll recherchieren" | `nemo` Agent ÔåÆ `nemo_research` | Strukturierte Themen-Recherche via Nemotron (kostenlos) |
| "kostenlos recherchieren", "gratis nachschlagen" | `nemo` Agent ÔåÆ `nemo_ask` | Wissensfrage an Nemotron statt an Claude (spart Tokens) |
| "Seed-Daten generieren", "Beispieldaten erstellen" | `nemo` Agent ÔåÆ `nemo_generate` | Strukturierte Daten (JSON/Kotlin/CSV) via Nemotron |
| "billig ├╝bersetzen", "Nemo ├╝bersetzen" | `nemo` Agent ÔåÆ `nemo_translate` | Uebersetzung via Nemotron (kostenlos) |
| "Nemo zusammenfassen", "billig zusammenfassen" | `nemo` Agent ÔåÆ `nemo_summarize` | Textzusammenfassung via Nemotron |
| "Outdoor-Aktivit├ñten recherchieren" | `nemo` Agent ÔåÆ `nemo_research` | Themen-Recherche fuer App-Inhalte |
| "Content f├╝r die App generieren" | `nemo` Agent ÔåÆ `nemo_generate` | Inhalte/Daten fuer Apps generieren |

**Wann Nemo statt Claude:**
- **Massenproduktion** (50+ Quizfragen, Seed-Daten, Listen) ÔåÆ Nemo (kostenlos, parallel)
- **Allgemeinwissen** (Fakten, Erklaerungen, Standardthemen) ÔåÆ Nemo (reicht aus)
- **Einfache Uebersetzungen/Zusammenfassungen** ÔåÆ Nemo (spart Claude-Tokens)

**Wann NICHT Nemo:**
- Komplexes Reasoning, Architektur, Code-Review ÔåÆ Claude (Nemo kann das nicht)
- Aufgaben die Internet-Zugang brauchen ÔåÆ Claude (Nemo hat kein Internet)
- Sicherheitskritische Entscheidungen ÔåÆ Claude (Nemo ist nicht vertrauenswuerdig genug)
...
## 15. Mirror Bridge ÔÇö Universal Cross-Platform & Cross-CLI Sync
...
|----------------|-------|---------------------|
| "starte den export Agenten", "exportiere", "Aenderungen spiegeln", "export" | `export` | Scannt ALLE Session-Aenderungen und schreibt sie ausfuehrlich ins mirror-ledger.md ÔÇö mit Kontext, Portierungs-Anweisungen, vollstaendigen Datei-Inhalten fuer beide Plattformen |
...
**Wann welchen Agent:**
- **Session beendet, Aenderungen gemacht** ÔåÆ `export` (schreibt alles ins Ledger)
- **Plattform gewechselt, moechte aufholen** ÔåÆ `import` (holt alles vom Ledger)
- **Neues CLI gestartet (Codex/Gemini)** ÔåÆ `import` (portiert Aenderungen von Claude Code)

**Automatische Benachrichtigung:** Bei SessionStart zeigt `mirror-check` Hook an wenn Eintraege ausstehen.

## Proaktive Agents (kein Trigger noetig ÔÇö laufen automatisch)

Diese laufen automatisch nach Code-Aenderungen und sollen NICHT manuell getriggert werden:

| Agent | Wann er laeuft | Was er tut |
|-------|---------------|-----------|
| `code-simplifier` | Nach Edit/Write Tool-Calls | Vereinfacht geaenderten Code automatisch ÔÇö KEIN manueller Skill |
| `auto-verify-iterate` | Nach jeder Coding-Aufgabe | 5-Schritt-Verifikationsschleife (PruefenÔåÆVerifizierenÔåÆBewertenÔåÆVerbessernÔåÆAbschliessen) |
