...
- `codex-setup/` und `claude-code-setup/` sind reine Lesequellen (Br├╝cken).
...
Aktion:
- `node Gemini-Setup/scripts/audit-codex-delta.mjs` ausf├╝hren.
- Kandidaten als `ADD`, `ADAPT` oder `REPLACE` klassifizieren.
- Alles sind nur Vorschl├ñge (­ƒÆí), keine autonome Umsetzung.

### Claude-Code-Delta (Br├╝cke)
...
Aktion:
- `node Gemini-Setup/scripts/audit-claude-delta.mjs` ausf├╝hren.
- Kandidaten als `ADD`, `ADAPT` oder `REPLACE` klassifizieren.
- Alles sind nur Vorschl├ñge (­ƒÆí), keine autonome Umsetzung.
...
- `node Gemini-Setup/scripts/bootstrap-report.mjs` f├╝r den Report.
- `node Gemini-Setup/scripts/check-code-search-health.mjs` f├╝r die Suche.
