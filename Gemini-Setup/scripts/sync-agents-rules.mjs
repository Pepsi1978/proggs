...
function warn(msg) { console.error(`ÔÜá´©Å [agents-rules-sync] WARNUNG: ${msg}`); }
...
for (const [name, filename] of Object.entries(CORE_RULES)) {
  ...
  // Wir pr├╝fen hier nur auf Existenz der Erw├ñhnung in der AGENTS.md
  // Eine tiefere Inhalts-Pr├╝fung k├Ânnte zu restriktiv sein, 
...
if (issues === 0) {
  log("Konsistenz zwischen AGENTS.md und Kern-Regeln ist gew├ñhrleistet.");
  process.exit(0);
} else {
