...
function warn(msg) { console.error(`ÔÜá´©Å [mcp-path-check] WARNUNG: ${msg}`); }

if (!fs.existsSync(MCP_CONFIG_PATH)) {
  log("Keine .mcp.json gefunden. ├£berspringe Check.");
  process.exit(0);
}
      ...
      server.args.forEach(arg => {
        if (typeof arg === "string" && arg.startsWith("/") && arg.includes("/Users/")) {
          if (!arg.startsWith(REPO_ROOT)) {
            warn(`Server "${name}": Absoluter Pfad au├ƒerhalb des aktuellen Workspace gefunden: ${arg}`);
            issues++;
      ...
      for (const [key, value] of Object.entries(server.env)) {
        if (typeof value === "string" && value.startsWith("/") && value.includes("/Users/")) {
          if (!value.startsWith(REPO_ROOT)) {
            warn(`Server "${name}": Env-Var ${key} zeigt au├ƒerhalb des Workspace: ${value}`);
            issues++;
