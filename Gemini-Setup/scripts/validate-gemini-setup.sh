...
for file in "${required_files[@]}"; do
  if [[ ! -f "$file" ]]; then
    ...
    echo "Ô£à Vorhanden: $file"
  fi
done

echo "[5/6] F├╝hre AGENTS.md Konsistenz-Check aus..."
node Gemini-Setup/scripts/sync-agents-rules.mjs

if [[ $missing -eq 0 ]]; then
  echo "---"
  echo "­ƒÜÇ Alle Gemini-Setup-Komponenten sind vorhanden."
  exit 0
else
  echo "---"
  echo "ÔÜá´©Å $missing Komponenten fehlen."
  exit 1
fi
