#!/usr/bin/env bun
/**
 * Session Scorer ÔÇö Meta-Evolution Artifact #1 (Cross-Platform)
 *
 ...
 * Trend warnings go to stderr (hook log) ÔÇö evolution-analyst reads session-scores.jsonl.
...
interface SessionMetrics {
	...
	had_intelligence_suggestion: boolean; // true if ­ƒÆí pattern found in assistant output
	intelligence_suggestion_count: number; // v5: count of ­ƒÆí suggestions (self-observation metric)
...
function analyzeTranscript(path: string): SessionMetrics {
	...
	// v5: Self-observation patterns ÔÇö detect when Gemini reflected on its own work process
	...
	// v4: Meta-intelligence tracking ÔÇö did this session improve the system itself?
	...
	const correctionPatterns = [
		...
		/\bnicht das\b/i,
		/\bstop\b/i,
		/\br[u├╝]ckg[a├ñ]ngig\b/i, // handles both r├╝ckg├ñngig and rueckgaengig
		/\bundo\b/i,
		/\bwrong\b/i,
		/\bthat's not\b/i,
		/\bdon't do\b/i,
	...
	// v3: More precise error detection ÔÇö exclude common non-error patterns
	const errorPattern =
	...
	for (const line of lines) {
		try {
			...
			if (entryType === "assistant" && msg) {
				...
				// v4: Check assistant text for intelligence suggestions (­ƒÆí pattern)
				for (const block of content) {
					if (block.type === "text" && typeof block.text === "string") {
						// v5: Count individual intelligence suggestions (­ƒÆí or "Intelligenz-Vorschlag N")
						const bulbMatches = block.text.match(/­ƒÆí/g);
						if (bulbMatches) {
...
function insertViaWhiteboardInsert(section: string, entry: string): void {
	...
	if (process.platform === "win32") {
		...
		try {
			...
			// Non-fatal ÔÇö stderr warning is sufficient
		}
	}
 else {
...
function checkIntelligenceSuggestion(metrics: SessionMetrics): void {
	...
	if (metrics.total_turns >= 10 && !metrics.had_intelligence_suggestion) {
		console.error(
			`INTELLIGENCE WARNING: Session ${sid} had ${metrics.total_turns} turns but no intelligence suggestion (­ƒÆí). ` +
	...
	// Check #2: Self-observation (second directive) ÔÇö sessions with >15 turns should show self-observation
	...
	if (metrics.intelligence_suggestion_count >= 3) {
		console.error(
			`EXCELLENT: Session ${sid} produced ${metrics.intelligence_suggestion_count} intelligence suggestions ÔÇö compound effect in action.`,
		);
	}
	...
	if (metrics.meta_intelligence_score >= 2) {
		console.error(
			`META-INTELLIGENCE: Session improved the system itself (score: ${metrics.meta_intelligence_score}) ÔÇö ` +
...
function detectTrends(currentMetrics: SessionMetrics): void {
	...
	if (allScores.length >= 10) {
		...
		if (avg < prevAvg - 0.5) {
			...
			console.error(
				`TREND WARNING: Quality declining: ${prevAvg.toFixed(1)} ÔåÆ ${avg.toFixed(1)}`,
			);
		}
	}
...
function main(): void {
	...
	try {
		...
		// M1/B4: Deduplication ÔÇö only write if session_id changed or turns increased by >5
			...
			if (lastLine) {
				try {
					...
					if (
						...
						// Same session, turns barely changed ÔÇö skip duplicate write
						process.exit(0);
					}
				} catch {
					/* parse error ÔÇö write anyway */
				}
			}
		}
