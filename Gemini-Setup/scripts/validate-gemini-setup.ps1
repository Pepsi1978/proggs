# validate-gemini-setup.ps1 (Gemini) ÔÇö Gesamt-Validierung der Umgebung
$RepoRoot = $env:GEMINI_WORKSPACE
if (-not $RepoRoot) { $RepoRoot = "C:\Users\barwa\GeminiCLI" }
cd $RepoRoot

Write-Host "=== ­ƒÆí Gemini Setup Validierung (PS7) ===" -ForegroundColor Cyan
...
foreach ($file in $requiredFiles) {
    if (-not (Test-Path $file)) {
        Write-Host "ÔØî Fehlt: $file" -ForegroundColor Red
        $missing++
    } else {
        Write-Host "Ô£à Vorhanden: $file" -ForegroundColor Green
    }
}
...
if ($parityExitCode -ne 0) {
    Write-Host "`n­ƒÆí Inkonsistenzen in den Regeln gefunden!" -ForegroundColor Yellow
...
if ($missing -eq 0 -and $parityExitCode -eq 0) {
    Write-Host "`n­ƒÜÇ Alle Gemini-Setup-Komponenten sind vorhanden und konsistent." -ForegroundColor Green
    exit 0
} else {
    Write-Host "`nÔÜá´©Å Validierung fehlgeschlagen ($missing Dateien fehlen, Parity-Status: $parityExitCode)." -ForegroundColor Red
    exit 1
}
