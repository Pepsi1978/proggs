...
Write-Host "Pr├╝fe Code-Search MCP..."
node "$RepoRoot/Gemini-Setup/scripts/check-code-search-health.mjs" | Out-File -FilePath $LogFile -Append
if ($LASTEXITCODE -eq 0) {
    Write-Host "Ô£à Code-Search MCP ist gesund." -ForegroundColor Green
} else {
    Write-Host "ÔÜá´©Å Code-Search MCP hat Probleme." -ForegroundColor Yellow
}

# CHECK 2: Ollama
try {
    $response = Invoke-RestMethod -Uri "http://localhost:11434/api/tags" -Method Get -TimeoutSec 2
    Write-Host "Ô£à Ollama ist erreichbar." -ForegroundColor Green
} catch {
    Write-Host "ÔÜá´©Å Ollama ist nicht aktiv." -ForegroundColor Yellow
}
