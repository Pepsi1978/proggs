...
function extractFixesAndIntelligence(content) {
  ...
  const intelligenceRegex = /- \*\*\[(.*?)\] (.*?)\*\* ÔÇö Status: (UMGESETZT|GEFIXT) .*? \| Quelle: (.*?) \| Empfehlung: (.*?)\n\s+(.*)/g;
  ...
  // 3. Ausf├╝hrliche GEFIXT-Eintr├ñge in der Offene-Fehler-Sektion
  const detailedFixRegex = /### (.*?) ÔÇö Source: (.*?) ÔÇö (.*?)\n\*\*Context:\*\* (.*?)\n\*\*Source:\*\* (.*?)\n\*\*Symptom:\*\* (.*?)\n\*\*Ursache:\*\* (.*?)\n\*\*Betroffene Dateien:\*\* (.*?)\n\*\*Reproduktion:\*\* (.*?)\n\*\*Fix-Details & Begruendung:\*\* (.*?)\n\*\*Status:\*\* GEFIXT/gs;
...
async function main() {
  console.log("Gemini CLI Delta Bruecke ÔÇö Lerne von Gemini und Codex\n");
