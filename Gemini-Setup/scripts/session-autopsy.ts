#!/usr/bin/env bun
/**
 * Session Autopsy ÔÇö Meta-Evolution Artifact #2
 *
...
// v3: Closed-Loop Learning ÔÇö auto-generate rules from recurring correction patterns
const workspaceRoot = process.env.GEMINI_WORKSPACE || join(HOME, "GeminiCLI");
...
function findLatestTranscript(): string | null {
	...
	try {
		...
		for (const entry of entries) {
			...
			// Skip agent transcripts ÔÇö only process main sessions (UUID format)
...
// Rules are tested in order; first match wins (more specific ÔåÆ less specific)
...
function analyzeTranscript(transcriptPath: string): TranscriptStats {
	...
	for (const line of lines) {
		...
		if (entryType === "user") {
			...
			// Reset after consuming ÔÇö next assistant turn replaces it
			lastAssistantText = "";
		}
	}
...
const RULE_SUGGESTIONS: Record<CorrectionType, string> = {
	scope_drift:
		"Vor jeder Aktion pr├╝fen: Ist das Teil der urspr├╝nglichen Anfrage?",
	wrong_file: "Datei-Pfade explizit best├ñtigen bevor ├änderungen gemacht werden",
	wrong_approach:
		"L├Âsungsansatz kurz erkl├ñren bevor mit der Implementierung begonnen wird",
	...
	other: "├ûfter beim Benutzer r├╝ckfragen ob der Ansatz korrekt ist",
};
...
function insertViaWhiteboardInsert(section: string, entry: string): void {
	...
	if (!existsSync(hookDir)) {
		process.stderr.write(
			`[session-autopsy] hooks directory not found at ${hookDir} ÔÇö entry NOT written.\n`,
		);
		return;
	}
	...
	if (process.platform === "win32") {
		...
		if (!existsSync(script)) {
			process.stderr.write(
				`[session-autopsy] whiteboard-insert.ps1 not found ÔÇö entry NOT written.\n`,
			);
			return;
		}
		...
		if (!existsSync(script)) {
			process.stderr.write(
				`[session-autopsy] whiteboard-insert.sh not found ÔÇö entry NOT written.\n`,
			);
			return;
		}
...
function formatAutopsyEntry(stats: TranscriptStats): string {
	...
	return (
		`### [${date}] Session Autopsy ÔÇö ${shortId}\n` +
		`- **Korrekturen gesamt**: ${corrections.length} (Rate: ${rate}%)\n` +
		`- **H├ñufigstes Muster**: ${dominantType} (${dominantCount} von ${corrections.length})\n` +
		`- **Details**:\n` +
...
// Rule content templates per correction type ÔÇö these become actual rule files
...
3. If the answer is not a clear YES ÔÇö STOP and ask the user
...
- "While I'm here, let me also..." ÔÇö NO, do NOT
- "This could be improved by..." ÔÇö only if the user asked for improvements
- "I noticed this other issue..." ÔÇö mention it, don't fix it
...
3. If the user says "nein", "anders", or "so nicht" ÔÇö STOP and ask for guidance
...
function readLearningState(): LearningState {
	try {
		if (existsSync(LEARNING_STATE_FILE)) {
			...
			// Validate structure ÔÇö if corrupt, return fresh state
			if (
		...
		// JSON parse error or file read error ÔÇö start fresh
		process.stderr.write(
			"[session-autopsy] learning state corrupt or missing ÔÇö starting fresh\n",
		);
	}
...
function detectAndGenerateRules(state: LearningState): string[] {
	...
	for (const [type, count] of Object.entries(typeCounts)) {
		...
		if (existingRuleIdx >= 0) {
			...
			// v3.1: Verify the rule file still exists on disk ÔÇö if deleted (e.g. by
			...
			if (existsSync(existingFilePath)) {
				// Rule still exists ÔÇö just update trigger count
				existingRule.trigger_count = count;
				existingRule.date = new Date().toISOString().split("T")[0]!;
				continue;
			} else {
				// Rule was deleted ÔÇö remove from state to allow regeneration
				state.auto_rules_generated.splice(existingRuleIdx, 1);
				process.stderr.write(
					`[session-autopsy] auto-rule ${existingRule.file} was deleted ÔÇö removing from state for regeneration\n`,
				);
		...
		if (state.auto_rules_generated.length >= MAX_AUTO_RULES) {
			process.stderr.write(
				`[session-autopsy] max auto-rules (${MAX_AUTO_RULES}) reached ÔÇö skipping ${type}\n`,
			);
			continue;
		}
		...
		try {
			...
			const header =
				...
				`<!-- DO NOT DELETE ÔÇö this rule will be regenerated if the pattern persists -->\n\n`;
...
function main(): void {
	try {
		...
		if (stats.corrections.length >= MIN_CORRECTIONS_TO_WRITE) {
			...
			const summary = `- **[${date}] Autopsy ${shortId}**: ${stats.corrections.length} Korrekturen (${rate}%), Hauptmuster: ${dominant[0]} (${dominant[1]}x) ÔÇö Empfehlung: ${rule}`;
			...
			// v3: CLOSED-LOOP ÔÇö detect recurring patterns and auto-generate rules
		...
		// Always save state (even if no corrections ÔÇö state file tracks history)
