...
**After reviewing**: If you discover recurring patterns, common mistakes, or project conventions not yet documented, update `.claude/agent-memory/shared/MEMORY.md` ÔÇö add entries under "Erkenntnisse aus Code Reviews". Keep entries concise (1 line per pattern). This helps the self-improve skill create better rules and hooks based on real code review findings.
...
```
ÔåÆ Spawn 3 review agents simultaneously:
...
- **CRITICAL** ÔÇö must fix before commit (security holes, crashes, data loss)
- **WARNING** ÔÇö should fix soon (bad patterns, potential bugs)
- **SUGGESTION** ÔÇö nice to have (style, minor optimizations)
...
2. **Write a sentinel file (see Mandatory Write-Back below).** If you found a bug pattern that could recur, prefix your sentinel findings with [BUG:] ÔÇö the writeback-enforcer will route these to "Offene Fehler & Probleme".
...
- LSP nicht verfuegbar ÔåÆ Ohne LSP reviewen, Ergebnis als "ohne LSP-Validierung" markieren.
...
- Zweiter Fehlschlag ÔåÆ Im Ergebnis dokumentieren: "ÔÜá´©Å [Name] ausgefallen ÔÇö Review basiert auf [N-1]/[N] Perspektiven."
- IMMER ein Review-Ergebnis liefern, auch wenn nur 2 von 3 Sub-Agents erfolgreich waren.
- Wenn ALLE Sub-Agents fehlschlagen ÔåÆ Direkt selbst reviewen (ohne Sub-Agents) als Fallback.

### Selbst-Terminierung
- 5 Turns ohne messbaren Fortschritt ÔåÆ SOFORT Review mit vorhandenen Findings abschliessen.
- Keine geaenderten Dateien gefunden ÔåÆ "NO CHANGES ÔÇö Nichts zu reviewen" zurueckgeben.
- NIEMALS still haengen bleiben ÔÇö es muss IMMER ein Review-Ergebnis kommen.

### Eingabe-Validierung
- Wurden Dateien oder ein Diff angegeben? Wenn nicht ÔåÆ `git diff HEAD` als Fallback versuchen.
- Existieren die referenzierten Dateien? Wenn nicht ÔåÆ Sofort melden.
