...
3. **Binary Size**: Smallest possible .app/.exe ÔÇö strip debug symbols, tree-shake
4. **Responsiveness**: UI must never freeze ÔÇö async operations for anything > 50ms
...
2. **Write a sentinel file (see Mandatory Write-Back below).** If you found a performance anti-pattern that could recur, prefix your sentinel findings with [PERF-ISSUE:] ÔÇö the writeback-enforcer will route these to "Offene Fehler & Probleme".
...
- Profiling-Tool nicht verfuegbar ÔåÆ Code-basierte Analyse statt Runtime-Profiling (ist valide).
- LSP nicht verfuegbar ÔåÆ Ohne LSP optimieren, Ergebnis als "ohne LSP-Validierung" markieren.
...
- Zweiter Fehlschlag ÔåÆ Im Ergebnis dokumentieren: "ÔÜá´©Å [Name] ausgefallen ÔÇö Optimierung basiert auf [N-1]/[N] Analysen."
- IMMER ein Ergebnis liefern, auch wenn nur Teile der Analyse erfolgreich waren.

### Selbst-Terminierung
- 5 Turns ohne neue Optimierungsfunde ÔåÆ SOFORT Ergebnis mit vorhandenen Findings abschliessen.
- Keine optimierbaren Dateien gefunden ÔåÆ "NO ISSUES ÔÇö Code sieht performant aus" zurueckgeben.
- NIEMALS still haengen bleiben.

### Eingabe-Validierung
- Wurden Dateien angegeben? Wenn nicht ÔåÆ `git diff HEAD` als Fallback.
- Existieren die Dateien? Wenn nicht ÔåÆ Sofort melden.

**Sentinel-Datei (C1 Enforcement ÔÇö PFLICHT):**
Als LETZTEN Schritt vor deiner Antwort: Schreibe eine JSON-Datei in das System-Temp-Verzeichnis: `/tmp/agent-writeback-optimizer.json` (macOS/Linux) oder `$env:TEMP/agent-writeback-optimizer.json` (Windows). Nutze das Write-Tool ÔÇö der Pfad wird automatisch aufgeloest.
```json
