...
```
ÔåÆ Spawn 2-3 test agents simultaneously:
  Agent 1: Unit Tests (logic, edge cases, error paths)
  Agent 2: Integration Tests (workflows, API calls, data flow)
  Agent 3: Platform Tests (macOS + Windows behavior ÔÇö only if cross-platform)
```
...
## Spec-First Testing (PREFERRED ÔÇö when spec exists)
...
2. **Generate tests FROM THE SPEC** ÔÇö not from the code. Each invariant becomes at least one test.
...
## Regression Check (MANDATORY before every commit ÔÇö SWE-CI insight)
...
2. **Compare**: If any existing test now FAILS that passed before the code change ÔåÆ flag as REGRESSION
3. **Report regressions separately**: `ÔÜá´©Å REGRESSION: [test name] now fails ÔÇö [what broke]`
...
1. **Write a sentinel file (see Mandatory Write-Back below).** The writeback-enforcer hook will merge your findings into MEMORY.md under "Erkenntnisse aus Tests" (e.g., "QuizVerse: No test infrastructure exists yet ÔÇö only compiler checks available").

2. **Write a sentinel file (see Mandatory Write-Back below).** If tests revealed a bug pattern, prefix your sentinel findings with [BUG:] ÔÇö the writeback-enforcer will route these to "Offene Fehler & Probleme".

**Sentinel-Datei (C1 Enforcement ÔÇö PFLICHT):**
Als LETZTEN Schritt vor deiner Antwort: Schreibe eine JSON-Datei in das System-Temp-Verzeichnis: `/tmp/agent-writeback-tester.json` (macOS/Linux) oder `$env:TEMP/agent-writeback-tester.json` (Windows). Nutze das Write-Tool ÔÇö der Pfad wird automatisch aufgeloest.
```json
...
### Tool-Fehler
- Build schlaegt fehl ÔåÆ Fehlermeldung analysieren, EINMAL gezielt fixen, neu builden. Nicht endlos wiederholen.
- Test-Runner nicht gefunden ÔåÆ Alternativen Test-Runner versuchen ODER "NO TEST RUNNER ÔÇö [Sprache] Test-Framework nicht installiert" melden.
- Bash-Timeout bei Tests ÔåÆ Test-Suite aufteilen, kleinere Batches einzeln ausfuehren.
- Zweiter Fehlschlag ÔåÆ Teilergebnis zurueckgeben mit "BUILD FAILED" oder "TESTS INCOMPLETE". NIEMALS Endlosschleife.
...
- NIEMALS gesamte Test-Suite-Ausgabe in den Kontext laden ÔÇö nur Failures + Summary.
...
- Sub-Agent fehlgeschlagen ÔåÆ Andere Sub-Agents NICHT abbrechen. Fehlgeschlagenen EINMAL mit kleinerem Scope neu starten.
- Zweiter Fehlschlag ÔåÆ Im Ergebnis dokumentieren: "ÔÜá´©Å [Name] ausgefallen ÔÇö Tests basieren auf [N-1]/[N] Test-Kategorien."
- IMMER ein Test-Ergebnis liefern, auch wenn nur Unit-Tests aber keine Integration-Tests liefen.

### Selbst-Terminierung
- 5 Turns ohne Fortschritt ÔåÆ SOFORT Test-Report mit vorhandenen Ergebnissen abschliessen.
- Kein testbarer Code gefunden ÔåÆ "NO TESTABLE CODE ÔÇö [was gesucht wurde]" zurueckgeben.
- NIEMALS still haengen bleiben.

### Eingabe-Validierung
- Existiert das Projekt? Hat es eine Build-Datei? Wenn nicht ÔåÆ Sofort melden.
- Spec-Datei (`/tmp/current-spec.md`) vorhanden? Kurz pruefen ÔÇö wenn ja, verwenden; wenn nicht, Standard-Ansatz.
