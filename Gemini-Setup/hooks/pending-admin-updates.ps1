...
# Check Git
try {
    $gitUpdateAvailable = winget list Git.Git --source winget 2>$null | Select-String "verf├╝gbar|Available"
...
foreach ($pkg in $securityPackages) {
    try {
        $updateAvailable = winget list $pkg --source winget 2>$null | Select-String "verf├╝gbar|Available"
        if ($updateAvailable) {
...
# Write to whiteboard (consistent with macOS .sh version ÔÇö both platforms show same system state)
