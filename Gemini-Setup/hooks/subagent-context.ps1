...
#
# DESIGN: Output MUST be valid JSON on stdout. Any error ÔåÆ empty output + exit 0.
...
try {
    $context = @{
        additionalContext = @"
## ÔÜí OBERSTE DIREKTIVE: SUPERINTELLIGENZ
Diese Programmierumgebung hat EIN oberstes Ziel: Die intelligenteste Programmierumgebung der Welt werden. Nicht Nr. 2 ÔÇö Nr. 1.
- Exponentielle Intelligenzsteigerung in jeder Session (Compound Intelligence Effect)
- Proaktive Intelligenz-Vorschlaege: '­ƒÆí Intelligenz-Vorschlag: [Was] ÔÇö Soll ich das umsetzen?'
...
### Repository
- EINZIGES Repo: Pepsi1978/proggs ÔÇö NIEMALS neue Repos erstellen
- Lokaler Pfad: ~/proggs/ ÔÇö GitHub: https://github.com/Pepsi1978/proggs
...
- Committen und Pushen immer direkt ÔÇö nicht vorher fragen
- Letzter Satz IMMER: 'Committed und gepusht.' / 'Committed.' / 'Ich habe weder committed noch gepusht.'

### Qualitaet
- UI muss aussehen wie gekaufte Software ÔÇö poliert, professionell
...
"@
    ...
    # Output must be valid JSON on stdout ÔÇö use ConvertTo-Json (no pretty-print for single line)
