# plugin-health-check.ps1  SessionStart diagnostic for plugin services
#
...
try {
    ...
    Hook-LogError "FAIL claude-mem-worker (${cmemHost}:${cmemPort})  not responding"
    $errors++
}
...
try {
    ...
    Hook-LogWarn "SKIP ollama (localhost:11434)  not running"
}
...
if (Test-Path $mcpDir) {
    ...
    if ($missing.Count -eq 0) {
        ...
        Hook-LogError "FAIL code-search MCP  missing: $($missing -join ', ')"
        $errors++
    }
}

# --- CHECK 5: Plugin hook script permissions (auto-repair / verification) ---
# Adapted from MIRROR-2026-03-25-MAC-015
# Note: Windows doesn't use +x bits, but we verify script existence and readability.
$pluginDirs = @("$env:USERPROFILE\.claude\plugins\marketplaces", "$env:USERPROFILE\.claude\plugins\cache")
foreach ($pdir in $pluginDirs) {
    if (Test-Path $pdir) {
        $checks++
        $scripts = Get-ChildItem -Path $pdir -Filter "*.sh" -Recurse -ErrorAction SilentlyContinue
        if ($scripts) {
            Hook-Log "found $($scripts.Count) shell scripts in plugins - verification only on Windows"
        }
    }
}
...
if ($errors -eq 0) {
    ...
    Hook-LogError "health check complete: $errors/$checks FAILED  check log above for details"
}
