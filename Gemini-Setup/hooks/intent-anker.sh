#!/usr/bin/env bash
# Intent-Anker  Saves the original session goal for drift prevention
...
# Read JSON input from stdin (Gemini CLI sends hook context as JSON)
...
if [ $((turn % 5)) -eq 0 ] && [ -f "$GOAL_FILE" ]; then
    reminder_file="${TMPDIR:-/tmp}/Gemini-intent-reminder.txt"
