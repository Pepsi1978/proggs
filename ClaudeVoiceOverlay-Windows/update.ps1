...
#
# Das Overlay und sein externer Watcher koennen sich gegenseitig neu starten.
# Deshalb baut dieses Skript zuerst separat, stoppt nur Prozesse am kanonischen
# Zielpfad, tauscht die EXE transaktional und behaelt das Backup bis zum
# erfolgreichen Stabilitaetscheck der neuen Version.

$ErrorActionPreference = "Stop"
$publishDir = Join-Path $PSScriptRoot "publish"
$tempDir = Join-Path ([System.IO.Path]::GetTempPath()) "ClaudeVoiceOverlay-update-$PID-$([Guid]::NewGuid().ToString('N'))"
...
try {
    ...
    & $dotnet publish (Join-Path $PSScriptRoot "ClaudeVoiceOverlay.csproj") -c Release -r win-x64 --self-contained true `
