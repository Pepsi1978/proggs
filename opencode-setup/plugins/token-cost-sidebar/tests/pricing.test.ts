...
describe("models.dev pricing", () => {
  ...
  test("renders separate dollar input and output model prices with an explicit unit", async () => {
    ...
    expect(source).toMatch(/label="Cachepreis" value=\{cacheRateValue\(\)\} muted/)
    ...
    expect(source).toContain('return `R/$0.05 W/${compactUsd(write)}`')
    expect(source).not.toContain(" R / ")
    expect(source).not.toContain(" W / 1M")
