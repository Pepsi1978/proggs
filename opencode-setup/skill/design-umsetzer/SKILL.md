...
description: >-
  ...
  sind und in der App noch fehlen, mit ein. Findet der Skill den passenden Design-Ordner
...
3. **IST-Analyse** der bestehenden App (falls vorhanden)
4. **Mapping** Design-Tokens → Compose-Theme + Screens planen
5. **1:1-Umsetzung** in Jetpack Compose (Code schreiben)
6. **Neue Funktionen** aus dem Design identifizieren und mit-implementieren
7. **Verifikation** (Build + visueller Abgleich gegen das Design)
...
- **JEDE** Theme-Variante als eigenes Set erfassen (Basis-Theme, `*-dark`, sowie
  ...
  Themes umgesetzt (siehe Phase 4, Multi-Theme).
...
- Welche Screens/Funktionen zeigt der Entwurf (z.B. Dashboard, Detail, Einstellungen,
  Onboarding, Timer, Statistik …)? Liste sie auf — Basis fuer Phase 6 (neue Funktionen).
...
3. **Kein passender App-Ordner gefunden:** Fragen, ob eine **neue App** angelegt werden
   soll und unter welchem Ordner-/Package-Namen. Fuer neue Apps ein sauberes
   Compose-Grundgeruest anlegen (siehe unten) und dann den Entwurf hineinbauen.
...
- Feststellen: **Welche Screens/Funktionen aus dem Entwurf existieren schon**,
  welche fehlen (→ Phase 6). Welche Datei ist fuer welchen Design-Screen zustaendig.

Dokumentiere das Datei-Mapping: Design-Screen → zustaendige(r) Kotlin-Datei(en).

---

## Phase 4 — Mapping planen (Design → Compose)
...
## Phase 5 — 1:1-Umsetzung in Jetpack Compose (Code schreiben)
...
4. Navigation/Struktur verdrahten (inkl. neuer Screens aus Phase 6).
...
## Phase 6 — Neue Funktionen aus dem Design mit-implementieren

**Sehr wichtig (ausdruecklicher Wunsch des Benutzers):** Wenn der Entwurf Funktionen
oder Screens enthaelt, die es in der bestehenden App **noch gar nicht gibt**, werden
diese **mit-programmiert** — nicht nur optisch als Attrappe, sondern funktional.
