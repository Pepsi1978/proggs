# Auto-Sync: Syncs Gemini CLI config from GitHub on every session start
...
$SetupDir = Join-Path $RepoDir "gemini-setup"
...
# Guard 2026-05-15: Pruefen ob MEMORY.md Konflikt-Marker enthaelt VOR dem Pull
# Das verhindert, dass wir einen bestehenden Konflikt stillschweigend mit einem Pull ueberlagern.
$whiteboard = Join-Path $RepoDir ".claude/agent-memory/shared/MEMORY.md"
if (Test-Path $whiteboard) {
    $content = Get-Content $whiteboard -Raw -ErrorAction SilentlyContinue
    if ($content -match "<<<<<<<" -or $content -match "=======" -or $content -match ">>>>>>>") {
        Write-Status "Auto-Sync: ABBRUCH! MEMORY.md enthaelt Merge-Konflikt-Marker."
        Write-Status "Auto-Sync: Bitte Konflikt manuell loesen: cd ~/proggs; git status"
        Hook-LogError "MEMORY.md conflict markers detected — sync blocked"
        exit 0
    }
}
...
# --- Sync config files from setup backup to active Gemini CLI config ---
...
if (Test-Path $rulesDir) {
    New-Item -ItemType Directory -Force -Path (Join-Path $GeminiDir "rules") | Out-Null
    $rules = @(Get-ChildItem "$rulesDir\*.md" -ErrorAction SilentlyContinue)
    if ($rules.Count -gt 0) {
        Copy-Item "$rulesDir\*.md" (Join-Path $GeminiDir "rules") -Force
...
if (Test-Path $agentsDir) {
    ...
    if ($agents.Count -gt 0) {
        Copy-Item "$agentsDir\*.md" (Join-Path $GeminiDir "agents") -Force
...
if (Test-Path $commandsDir) {
    $destCommands = Join-Path $GeminiDir "commands"
...
if (Test-Path $hooksDir) {
    $destHooks = Join-Path $GeminiDir "hooks"
    ...
    $changedHooks = @(git diff --name-only "$local" "$remote" -- gemini-setup/hooks/ 2>$null | ForEach-Object { Split-Path $_ -Leaf })
...
# Gemini.md — use the REPO version (~/proggs/Gemini.md) as authoritative source,
...
$repoGemini = Join-Path $RepoDir "Gemini.md"
if (Test-Path $repoGemini) {
    Copy-Item $repoGemini (Join-Path $env:USERPROFILE "Gemini.md") -Force
    $synced += "Gemini.md(from-repo)"
}

# Skills
$skillsDir = Join-Path $SetupDir "skills"
if (Test-Path $skillsDir) {
    $destSkills = Join-Path $GeminiDir "skills"
...
try {
    ...
    if ($LASTEXITCODE -eq 0 -and $ghToken) {
        $localSettings = Join-Path $GeminiDir "settings.local.json"
        $localData = @{}
...
Write-Status "Auto-Sync: Hinweis -- Gemini.md und Rules werden erst nach Neustart von Gemini CLI wirksam."
