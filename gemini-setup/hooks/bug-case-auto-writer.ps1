...
# 3. Wenn JA: Gibt den alten Fix als Kontext an Gemini zurueck (RAG-Effekt)
...
if ($matchedCase -and $matchedScore -ge 0.5) {
    ...
    # Ausgabe als additionalContext fuer Gemini
    $output = @{
        hookSpecificOutput = @{
    ...
    if (-not $isDuplicate) {
        ...
        # Gemini informieren dass ein neuer Fall erfasst wurde
...
if ($outputJson) {
    ...
    try {
        ...
        $logDir2 = Join-Path $env:USERPROFILE ".Gemini" "logs" "hooks"
