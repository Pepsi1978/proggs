...
check_disk_space() {
    ...
    if [ "$usage_pct" -ge 95 ] 2>/dev/null; then
        ...
        #   99%+ → bei jedem Lauf
        cooldown_file="$HOME/.Gemini/.disk-warn-last"
        should_log=1
...
check_doctor_lite() {
    ...
    local Gemini_json="$HOME/.Gemini.json"
    if [ -f "$Gemini_json" ]; then
        # Cloud MCP connectors
        if grep -q '"tengu_Geminiai_mcp_connectors": true' "$Gemini_json" 2>/dev/null; then
            if ! grep -q '"Gemini\.ai' "$Gemini_json" 2>/dev/null; then
                warnings+=("Cloud-MCP-Connectoren aktiv und NICHT blockiert (~21K Token)")
                local_warnings=$((local_warnings + 1))
            fi
        fi
        # sequential-thinking
        if grep -q '"sequential-thinking"' "$Gemini_json" 2>/dev/null; then
...
# ============================================================
check_mirror_ledger() {
    local ledger="$HOME/proggs/gemini-setup/mirror-ledger.md"
    [ ! -f "$ledger" ] && return

    local platform="macos"
    [ "$(uname)" != "Darwin" ] && platform="linux"

    local pattern="APPLIED: ${platform}/gemini-setup=PENDING"
    local count
...
check_semantic_search
check_mirror_ledger
check_mcp_auth  # LETZTER Check — ruft Gemini mcp list auf (kann bis zu 5s dauern)
