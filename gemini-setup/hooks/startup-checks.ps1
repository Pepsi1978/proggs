...
function Check-DiskSpace {
    try {
        ...
        if ($usagePct -ge 95) {
            ...
            #   99%+ → bei jedem Lauf
            $cooldownFile = Join-Path $env:USERPROFILE ".Gemini\.disk-warn-last"
            $shouldLog = $true
            ...
            if ($shouldLog) {
                try {
                    # 2026-05-15: Zeitstempel SOFORT aktualisieren (vor dem langsamen Whiteboard-Eintrag)
                    # um Race-Conditions bei parallelen Session-Starts zu minimieren.
...
function Check-DoctorLite {
    $localWarnings = 0
    $GeminiJson = "$env:USERPROFILE\.Gemini.json"
    if (Test-Path $GeminiJson) {
        $raw = Get-Content $GeminiJson -Raw -ErrorAction SilentlyContinue

        # Cloud MCP connectors active but not blocked?
        $flagActive = $raw -match '"tengu_Geminiai_mcp_connectors":\s*true'
        $hasBlocker = $raw -match '"disabledMcpjsonServers":\s*\[' -and $raw -match '"Gemini\.ai'
...
# ============================================================
# REMOVED: "Gemini mcp list" spawns orphan MCP server processes on every call.
...
# See observation #4072: "MCP Server Duplication Root Cause: Gemini mcp list Spawns Orphans"
...
function Check-MirrorLedger {
    ...
    $pattern = "APPLIED: windows/gemini-setup=PENDING"
...
Check-MirrorLedger
Check-McpAuth  # LETZTER Check — ruft Gemini mcp list auf (kann bis zu 5s dauern)
