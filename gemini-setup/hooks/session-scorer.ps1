...
$LogFile = Join-Path $env:USERPROFILE ".Gemini" "logs" "hooks" "$LogDate.log"
$hookErrors = 0
...
goal_path = os.path.join(os.environ.get('TEMP', '/tmp'), 'Gemini-session-goal.txt')
goal = 'unknown'
...
$summaryPath = Join-Path $env:TEMP "Gemini-session-summary.txt"
try {
...
$GeminiTemp = Join-Path $env:TEMP "Gemini"
if (Test-Path $GeminiTemp) {
    Get-ChildItem $GeminiTemp -Recurse -File -ErrorAction SilentlyContinue |
    ...
    Get-ChildItem $GeminiTemp -Directory -Recurse -ErrorAction SilentlyContinue |
...
# @("Gemini-session-goal.txt", "Gemini-turn-counter.txt", "Gemini-intent-reminder.txt")
...
$hookLogs = Join-Path $env:USERPROFILE ".Gemini" "logs" "hooks"
