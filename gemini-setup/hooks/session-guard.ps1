...
try {
    $sourceLogPath = Join-Path $env:USERPROFILE ".Gemini" "logs" "session-source.log"
...
$localSettingsPath = Join-Path $env:USERPROFILE ".Gemini" "settings.local.json"
...
try {
    $projectsDir = Join-Path $env:USERPROFILE ".Gemini" "projects"
...
# echten neuen Gemini-Start kommt "high" als Default zurueck.
...
# defaultet Gemini CLI auf 200k Kontext.
...
$cwdCheckFile = Join-Path $env:TEMP "Gemini-cwd-check.txt"
