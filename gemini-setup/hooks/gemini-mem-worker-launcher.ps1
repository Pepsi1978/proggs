# Gemini-mem-worker-launcher.ps1 — Windows counterpart for Gemini-mem worker startup
#
# On macOS this is called by launchd. On Windows, Gemini-mem starts
...
$cmemPort = 37777
$settingsPath = Join-Path $env:USERPROFILE ".Gemini-mem\settings.json"
if (Test-Path $settingsPath) {
    try {
        $settings = Get-Content $settingsPath -Raw | ConvertFrom-Json
        if ($settings.Gemini_MEM_WORKER_PORT) {
            $cmemPort = [int]$settings.Gemini_MEM_WORKER_PORT
        }
    } catch { }
}
...
try {
    ...
    Write-Host "[Gemini-mem] Worker already running on port $cmemPort"
    exit 0
} catch { }

# --- Find plugin directory ---
$pluginDir = Join-Path $env:USERPROFILE ".Gemini\plugins\cache\thedotmack\Gemini-mem"
if (-not (Test-Path $pluginDir)) {
    Write-Host "[Gemini-mem] Plugin not installed at $pluginDir"
    exit 0
}

$latestVersion = Get-ChildItem -Path $pluginDir -Directory | Sort-Object Name | Select-Object -Last 1
if (-not $latestVersion) {
    Write-Host "[Gemini-mem] No plugin versions found"
    exit 0
}

$workerScript = Join-Path $latestVersion.FullName "scripts\worker-service.cjs"
if (-not (Test-Path $workerScript)) {
    Write-Host "[Gemini-mem] worker-service.cjs not found at $workerScript"
    exit 0
}
...
if (-not $runtime) {
    Write-Host "[Gemini-mem] Neither bun nor node found"
    exit 0
}

Write-Host "[Gemini-mem] Starting worker: $runtime $workerScript --daemon"
