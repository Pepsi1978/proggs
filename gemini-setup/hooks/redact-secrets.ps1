...
#      Gemini sees the block message and must redact before retrying.
#
...
# and that end up in the git repo (gemini-setup/ files).
$sensitivePatterns = @(
...
# Output a JSON block response that Gemini sees as a hook error.
# This prevents the Write/Edit from executing.
$output = @{
    decision = 'block'
    reason   = "WARNUNG: Secret erkannt in '$filename' — Write blockiert.`nGefundene Muster: $preview`nBitte redacte die Secrets vor dem Schreiben mit: '<REDACTED — set locally>'`nAlternativ: python `"`$USERPROFILE/.Gemini/scripts/redact-secrets.py`" `"$filePath`""
