...
try:
    ...
    print('Doctor-Lite: .Gemini.json nicht lesbar')
    sys.exit(0)

changed = False

# 1. Check top-level cloud MCP flag (not the cached server flag)
if data.get('tengu_Geminiai_mcp_connectors') == True:
    data['tengu_Geminiai_mcp_connectors'] = False
    changed = True
