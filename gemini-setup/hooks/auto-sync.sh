...
SETUP_DIR="$REPO_DIR/gemini-setup"
...
if [ -d "$rules_dir" ]; then
    ...
    if [ "$rules_count" -gt 0 ]; then
        cp "$rules_dir"/*.md "$Gemini_DIR/rules/" 2>/dev/null || true
...
if [ -d "$agents_dir" ]; then
    ...
    if [ "$agents_count" -gt 0 ]; then
        cp "$agents_dir"/*.md "$Gemini_DIR/agents/" 2>/dev/null || true
...
if [ -d "$commands_dir" ]; then
    dest_commands="$Gemini_DIR/commands"
...
if [ -d "$hooks_dir" ]; then
    dest_hooks="$Gemini_DIR/hooks"
    mkdir -p "$dest_hooks"

    # Build list of hooks that actually changed in the incoming commits
    changed_hooks_file=$(mktemp)
    git diff --name-only "$local_sha" "$remote_sha" -- gemini-setup/hooks/ 2>/dev/null | \
...
repo_Gemini="$REPO_DIR/Gemini.md"
if [ -f "$repo_Gemini" ]; then
    cp "$repo_Gemini" "$HOME/Gemini.md"
    synced="$synced Gemini.md(from-repo)"
fi

# Skills
skills_dir="$SETUP_DIR/skills"
if [ -d "$skills_dir" ]; then
    dest_skills="$Gemini_DIR/skills"
    mkdir -p "$dest_skills"
...
if [ -n "$gh_token" ]; then
    local_settings="$Gemini_DIR/settings.local.json"
...
write_status "Auto-Sync: Hinweis -- Gemini.md und Rules werden erst nach Neustart von Gemini CLI wirksam."
