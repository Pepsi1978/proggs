...
if [[ "$normalized" != *".Gemini/settings"*".json" ]]; then
    exit 0
fi
...
# ---------------------------------------------------------------------------
# Gemini CLI auto-creates allow entries when the user approves a tool.
...
allow_fixed=false
result=$(remove_allow_list_silent "$HOME/.Gemini/settings.json")
[ "$result" = "removed" ] && allow_fixed=true && hook_log "AUTO-FIX: removed allow list from settings.json" 2>/dev/null || true

result=$(remove_allow_list_silent "$HOME/.Gemini/settings.local.json")
...
if [ -d "$HOME/.Gemini/projects" ]; then
    for pd in "$HOME/.Gemini/projects"/*/; do
        [ -d "$pd" ] || continue
...
# effortLevel: MUST be "high" — BLOCK anything else (Gemini.md requirement)
eff = data.get('effortLevel')
if eff and eff != 'high':
    blocks.append(f"effortLevel={eff} (MUSS 'high' sein — Gemini.md-Regel)")

env_data = data.get('env', {})
if env_data:
    # Gemini_CODE_EFFORT_LEVEL in env
    env_eff = env_data.get('Gemini_CODE_EFFORT_LEVEL')
    if env_eff and env_eff != 'high':
        blocks.append(f"Gemini_CODE_EFFORT_LEVEL={env_eff} (MUSS 'high' sein)")

    # SUBAGENT_MODEL: BLOCK if changed from sonnet (critical for cost/quality)
    sub_model = env_data.get('Gemini_CODE_SUBAGENT_MODEL')
    if sub_model and sub_model != 'sonnet':
        blocks.append(f"Gemini_CODE_SUBAGENT_MODEL={sub_model} (erwartet: sonnet)")

    # AUTOCOMPACT: BLOCK if below 85 (85 is the correct standard, NOT 95)
    acp = env_data.get('Gemini_AUTOCOMPACT_PCT_OVERRIDE')
    if acp is not None:
        try:
            if int(acp) < 85:
