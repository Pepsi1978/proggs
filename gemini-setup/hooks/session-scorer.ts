...
const SCORES_FILE = join(HOME, ".Gemini", "session-scores.jsonl");

function findProjectsDir(): string {
	const GeminiDir = join(HOME, ".Gemini", "projects");
...
function analyzeTranscript(path: string): SessionMetrics {
	...
	// Paths that indicate system self-improvement — restricted to .Gemini/ paths
