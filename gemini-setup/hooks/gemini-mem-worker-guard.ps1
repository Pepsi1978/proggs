# Gemini-mem-worker-guard.ps1 — Universal service readiness & plugin hooks hardener
#
...
$cmemPort = 37777
$cmemHost = "127.0.0.1"
$cmemSettings = Join-Path $env:USERPROFILE ".Gemini-mem\settings.json"

if (Test-Path $cmemSettings) {
    try {
        $settings = Get-Content $cmemSettings -Raw | ConvertFrom-Json
        if ($settings.Gemini_MEM_WORKER_PORT) { $cmemPort = [int]$settings.Gemini_MEM_WORKER_PORT }
        if ($settings.Gemini_MEM_WORKER_HOST) { $cmemHost = $settings.Gemini_MEM_WORKER_HOST }
    } catch { }
}

# Service registry — extend for future plugins
$services = @(
    @{ Name = "Gemini-mem-worker"; Host = $cmemHost; Port = $cmemPort }
...
function Find-PluginRoot {
    ...
    $cacheDir = Join-Path $env:USERPROFILE ".Gemini\plugins\cache\$Vendor"
    ...
    $mktPath = Join-Path $env:USERPROFILE ".Gemini\plugins\marketplaces\$vendorShort\plugin"
...
function Patch-PluginHooks {
    ...
    $patched = $content `
        ...
        -replace '(worker-service\.cjs" hook gemini-setup context)"', "`$1 $fallback"""
...
foreach ($svc in $services) {
    ...
    switch ($svc.Name) {
        "Gemini-mem-worker" {
            $pluginRoot = Find-PluginRoot -Vendor "thedotmack/Gemini-mem" -Marker "scripts\worker-service.cjs"
