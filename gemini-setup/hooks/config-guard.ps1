...
if ($normalizedPath -notmatch '\.Gemini/settings(\.local)?\.json$') {
    exit 0
}
...
$settingsPath2 = Join-Path $env:USERPROFILE ".Gemini" "settings.json"
$localSettingsPath2 = Join-Path $env:USERPROFILE ".Gemini" "settings.local.json"
...
try {
    $projectsDir2 = Join-Path $env:USERPROFILE ".Gemini" "projects"
...
$envData = $data.env
if ($envData) {
    # Gemini_CODE_EFFORT_LEVEL in env — same as above, user-controlled
    $envEff = $envData.Gemini_CODE_EFFORT_LEVEL
    if ($null -ne $envEff -and $envEff -ne '' -and $envEff -notin $validEfforts) {
        $blocks += "Gemini_CODE_EFFORT_LEVEL=$envEff (ungueltiger Wert)"
    }

    # SUBAGENT_MODEL: BLOCK if changed from sonnet (critical for cost/quality)
    $subModel = $envData.Gemini_CODE_SUBAGENT_MODEL
    if ($null -ne $subModel -and $subModel -ne '' -and $subModel -ne 'sonnet') {
        $blocks += "Gemini_CODE_SUBAGENT_MODEL=$subModel (erwartet: sonnet)"
    }

    # AUTOCOMPACT: BLOCK if below 85 (85 is the correct standard, NOT 95)
    $acp = $envData.Gemini_AUTOCOMPACT_PCT_OVERRIDE
