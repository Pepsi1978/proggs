...
try {
    ...
    # Pfade
    $experiencePath = Join-Path $env:USERPROFILE "proggs\.Gemini\agent-memory\shared\experience-store.jsonl"
    $trajectoryPath = Join-Path $env:USERPROFILE "proggs\.Gemini\agent-memory\shared\trajectories.jsonl"
    $scoresPath = Join-Path $env:USERPROFILE ".Gemini\session-scores.jsonl"
