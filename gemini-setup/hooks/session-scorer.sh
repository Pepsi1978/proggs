...
# 1. Turn count
COUNTER_FILE="${TMPDIR:-/tmp}/Gemini-turn-counter.txt"
turns=0
...
GOAL_FILE="${TMPDIR:-/tmp}/Gemini-session-goal.txt"
goal="unknown"
if [ -f "$GOAL_FILE" ]; then
...
# 3. Hook errors today
LOG_DATE=$(date +%Y-%m-%d)
LOG_FILE="$HOME/.Gemini/logs/hooks/$LOG_DATE.log"
hook_errors=0
if [ -f "$LOG_FILE" ]; then
...
goal_path = os.path.join(os.environ.get('TMPDIR', '/tmp'), 'Gemini-session-goal.txt')
if not os.path.exists(goal_path):
    goal_path = '/tmp/Gemini-session-goal.txt'
goal = 'unknown'
...
SUMMARY_FILE="${TMPDIR:-/tmp}/Gemini-session-summary.txt"
...
# Clean Gemini temp directory (files older than 2 hours)
if [ -d "$TMPDIR_REAL/Gemini" ]; then
    find "$TMPDIR_REAL/Gemini" -type f -mmin +120 -delete 2>/dev/null
    find "$TMPDIR_REAL/Gemini" -type d -empty -delete 2>/dev/null
fi
...
# for f in Gemini-session-goal.txt Gemini-turn-counter.txt Gemini-intent-reminder.txt; do
...
HOOK_LOGS="$HOME/.Gemini/logs/hooks"
if [ -d "$HOOK_LOGS" ]; then
