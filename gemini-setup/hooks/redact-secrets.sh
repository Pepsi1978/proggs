...
SENSITIVE=false
if echo "$FILE_PATH" | grep -qE 'settings-reference\.json$|settings\.json$|settings\.local\.json$|gemini-setup/.*\.json$'; then
    SENSITIVE=true
fi
...
Alternativ: python3 ~/.Gemini/scripts/redact-secrets.py \"$FILE_PATH\"'''
