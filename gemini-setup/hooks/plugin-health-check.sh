...
# When Gemini CLI shows "startup hook error" without details, this log
...
source "$HOOKS_DIR/hook-log.sh" 2>/dev/null || {
    ...
    _HOOK_LOG_DIR="$HOME/.Gemini/logs/hooks"
...
# ============================================================================
# CHECK 1: Gemini-mem worker (HTTP service on port 37777)
# ============================================================================
CMEM_PORT=37777
CMEM_HOST="127.0.0.1"
CMEM_SETTINGS="$HOME/.Gemini-mem/settings.json"
if [ -f "$CMEM_SETTINGS" ]; then
    _p=$(grep -o '"Gemini_MEM_WORKER_PORT"[[:space:]]*:[[:space:]]*"[0-9]*"' "$CMEM_SETTINGS" 2>/dev/null | grep -o '[0-9]*')
    [ -n "$_p" ] && CMEM_PORT="$_p"
fi

checks=$((checks + 1))
if curl -sf --connect-timeout 2 --max-time 3 "http://${CMEM_HOST}:${CMEM_PORT}/api/health" >/dev/null 2>&1; then
    hook_log "PASS Gemini-mem-worker (${CMEM_HOST}:${CMEM_PORT})"
else
    hook_log_error "FAIL Gemini-mem-worker (${CMEM_HOST}:${CMEM_PORT}) — not responding"
    errors=$((errors + 1))
fi
...
# ============================================================================
PLUGIN_CACHE="$HOME/.Gemini/plugins/cache"

# Gemini-mem plugin hooks
CMEM_HOOKS="$PLUGIN_CACHE/thedotmack/Gemini-mem"
    ...
    if [ -n "$LATEST_VER" ]; then
        ...
        if [ -f "$HOOKS_FILE" ]; then
            ...
            if grep -q "sleep.*curl.*api/health.*exit" "$HOOKS_FILE" 2>/dev/null; then
                hook_log "PASS Gemini-mem hooks.json (patched with fallback)"
            else
                hook_log_warn "Gemini-mem hooks.json exists but fallback patch missing — next plugin update will re-patch"
            fi
        else
            hook_log_error "FAIL Gemini-mem hooks.json missing at $HOOKS_FILE"
            errors=$((errors + 1))
        fi
    fi
fi
...
if [ "$(uname)" = "Darwin" ]; then
    for plist_label in "com.Gemini-mem.worker" "com.parry.daemon"; do
        checks=$((checks + 1))
