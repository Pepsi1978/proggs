#!/usr/bin/env bash
# heartbeat.sh — Periodic health check for the Gemini CLI development environment
...
# Logging: ~/.Gemini/logs/heartbeat/YYYY-MM-DD.log
#
...
notify_critical() {
    ...
    if command -v osascript &>/dev/null; then
        local safe_msg="${msg//\\/\\\\}"
        safe_msg="${safe_msg//\"/\\\"}"
        osascript -e "display notification \"${safe_msg}\" with title \"Gemini Heartbeat\" subtitle \"KRITISCH\"" 2>/dev/null || true
    fi
}
...
check_config_integrity() {
    ...
    if [ -f "$SETTINGS_JSON" ]; then
        if ! $PYTHON3 -c "import json; json.load(open('$SETTINGS_JSON'))" 2>/dev/null; then
            ...
            notify_critical "settings.json ist beschaedigt! Gemini CLI funktioniert nicht korrekt."
        fi
    else
        issues=$((issues + 1))
...
check_Gemini_mem_worker() {
    local loaded
    loaded=$(launchctl list 2>/dev/null | grep -c "com.Gemini-mem.worker" || echo "0")

    if [ "$loaded" -gt 0 ]; then
        echo '"Gemini_mem_worker": {"status": "OK", "detail": "Worker loaded in launchd"}'
    else
        echo '"Gemini_mem_worker": {"status": "WARNING", "detail": "Worker not loaded — run: launchctl load ~/Library/LaunchAgents/com.Gemini-mem.worker.plist"}'
    fi
}
...
check_key_directories() {
    ...
    for dir in "$HOME/.Gemini/hooks" "$HOME/.Gemini/agents" "$HOME/.Gemini/rules" "$PROGGS_DIR/.Gemini/agent-memory/shared"; do
        if [ ! -d "$dir" ]; then
            missing=$((missing + 1))
...
# Gemini was operating below acceptable quality and corrections occurred.
# CRITICAL (notification) if 2+ sessions below threshold.
# ─────────────────────────────────────────────────────────────────────
check_validator_compliance() {
    local scores_file="$HOME/.Gemini/session-scores.jsonl"
...
# field. More than 3 corrections in one session means Gemini made too
...
c_worker=$(check_Gemini_mem_worker)
