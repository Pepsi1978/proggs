...
# --- Hook logging ---
$logDir = "$env:USERPROFILE\.Gemini\logs\hooks"
...
# --- CHECK 1: Gemini-mem worker ---
$cmemPort = 37777
$cmemHost = "127.0.0.1"
$cmemSettings = "$env:USERPROFILE\.Gemini-mem\settings.json"
if (Test-Path $cmemSettings) {
    try {
        $s = Get-Content $cmemSettings -Raw | ConvertFrom-Json
        if ($s.Gemini_MEM_WORKER_PORT) { $cmemPort = $s.Gemini_MEM_WORKER_PORT }
    } catch { }
}
...
try {
    ...
    Hook-Log "PASS Gemini-mem-worker (${cmemHost}:${cmemPort})"
} catch {
    Hook-LogError "FAIL Gemini-mem-worker (${cmemHost}:${cmemPort}) — not responding"
    $errors++
}
...
$pluginCache = "$env:USERPROFILE\.Gemini\plugins\cache\thedotmack\Gemini-mem"
    ...
    if ($latestVer) {
        ...
        if (Test-Path $hooksFile) {
            ...
            if ($content -match "sleep.*curl.*api/health.*exit") {
                Hook-Log "PASS Gemini-mem hooks.json (patched with fallback)"
            } else {
                Hook-LogWarn "Gemini-mem hooks.json exists but fallback patch missing"
            }
        } else {
            Hook-LogError "FAIL Gemini-mem hooks.json missing at $hooksFile"
            $errors++
        }
    }
}
