#!/usr/bin/env bash
# Gemini-mem-worker-launcher.sh — Launchd wrapper for Gemini-mem worker
#
# Dynamically finds the latest plugin version and starts the worker.
# Used by com.Gemini-mem.worker.plist (launchd agent).
#
# RESILIENCE FEATURES:
...
# - Logs to /tmp/Gemini-mem-worker.log for debugging
...
PLUGIN_DIR="$HOME/.Gemini/plugins/cache/thedotmack/Gemini-mem"
MAX_WAIT=60
waited=0
...
echo "$(LOG_PREFIX) Starting Gemini-mem worker: $BUN $WORKER_SCRIPT --daemon"
