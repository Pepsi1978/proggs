...
#
# IMPORTANT: NEVER kill Gemini.exe processes — they manage their own lifecycle.
# Killing older Gemini.exe processes DESTROYS active sessions in other tabs.
# (Root cause of "session destruction" bug, fixed 2026-04-12)

$ErrorActionPreference = 'SilentlyContinue'

try {
    # Get all running Gemini.exe PIDs — these are ACTIVE session owners
    $activeGeminis = @{}
    Get-Process -Name 'Gemini' -ErrorAction SilentlyContinue | ForEach-Object {
        $activeGeminis[$_.Id] = $true
    }

    if ($activeGeminis.Count -eq 0) {
        # No Gemini processes at all — nothing to clean
        exit 0
    }

    # Find orphaned node.exe processes:
    # A node.exe is an orphan if its parent PID is NOT an active Gemini.exe process
    ...
    if ($nodeProcs) {
        foreach ($node in $nodeProcs) {
            try {
                ...
                # If parent is alive (whether Gemini or something else), leave it alone
            } catch {
...
try {
    $pluginCache = Join-Path $env:USERPROFILE ".Gemini\plugins\cache"
