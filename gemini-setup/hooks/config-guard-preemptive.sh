...
LOG_DIR="$HOME/.Gemini/logs/hooks"
...
try:
    ...
    env = data.get('env', {})
    acp = env.get('Gemini_AUTOCOMPACT_PCT_OVERRIDE', '')
    if acp and int(acp) < 85:
        violations.append(f'AUTOCOMPACT={acp} (Minimum: 85)')

    sub = env.get('Gemini_CODE_SUBAGENT_MODEL', '')
...
check_settings_file "$HOME/.Gemini/settings.json"
check_settings_file "$HOME/.Gemini/settings.local.json"
