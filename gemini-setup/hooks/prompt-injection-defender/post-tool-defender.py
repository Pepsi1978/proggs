...
# ///
"""
Gemini CLI Prompt Injection Defender - PostToolUse Hook
=========================================================

Scans tool outputs for prompt injection attempts and warns Gemini.
...
TRUSTED_PATH_PATTERNS = [
    ...
    # Hook files
    re.compile(r"\.Gemini[/\\]hooks[/\\]", re.IGNORECASE),
    # Rule files
    re.compile(r"\.Gemini[/\\]rules[/\\]", re.IGNORECASE),
    # Agent definitions
    re.compile(r"\.Gemini[/\\]agents[/\\]", re.IGNORECASE),
    # Command/skill definitions
    re.compile(r"\.Gemini[/\\]commands[/\\]", re.IGNORECASE),
    # Memory files
...
def load_config() -> Dict[str, Any]:
    ...
    if project_dir:
        project_config = (
            Path(project_dir) / ".Gemini" / "hooks" / "prompt-injection-defender" / "patterns.yaml"
        )
...
def format_warning(
    ...
    """Format detections into a warning message for Gemini."""
