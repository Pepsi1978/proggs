...
# Event: PostToolUse (Edit|Write auf .ps1 oder .sh Dateien in ~/.Gemini/hooks/)
# Zweck: Sofortmassnahme 2 — Automatische Cross-Platform-Pruefung
#
# Was dieses Skript tut:
# 1. Erkennt wenn eine .ps1-Datei in ~/.Gemini/hooks/ bearbeitet wurde
# 2. Prueft ob ein .sh-Gegenstueck existiert
# 3. Prueft ob das Gegenstueck aelter ist als die geaenderte Datei
# 4. Warnt Gemini per additionalContext — blockiert NICHT (Stufe 1)
#
# Defense in Depth:
#   Schicht 1: Diese automatische Warnung (Poka-Yoke Stufe 1)
#   Schicht 2: Die Cross-Platform-Checkliste in Gemini.md (Text-Regel)
...
# Nur Hook-Dateien in ~/.Gemini/hooks/ pruefen
if ($filePath -notmatch '\.Gemini/hooks/') { exit 0 }
...
if ($warnings.Count -gt 0) {
    ...
    # Logging
    $logDir = Join-Path $env:USERPROFILE ".Gemini" "logs" "hooks"
