...
**Fuer**: Gemini CLI, Codex CLI, Gemini CLI und jedes andere AI-CLI
...
- macOS: `gemini-setup/mcp-macos.json`
- Windows: `gemini-setup/mcp-windows.json`
...
Konfiguration in `~/.Gemini/settings.json`:
```json
{
  "hooks": {
    "SessionStart": [
...
| `gemini-setup/mcp-macos.json` | MCP-Config Referenz fuer macOS |
| `gemini-setup/mcp-windows.json` | MCP-Config Referenz fuer Windows |
| `gemini-setup/hooks/reindex-codebase.sh` | Reindex-Hook fuer macOS |
| `gemini-setup/hooks/reindex-codebase.ps1` | Reindex-Hook fuer Windows |
