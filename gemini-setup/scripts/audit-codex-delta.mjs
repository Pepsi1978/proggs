...
const REPLACE_SUBJECT_RE =
	/\b(rewrite|replace|remove|delete|drop|rename|overhaul)\b/i;
...
const DEFAULT_STATE = {
	...
	notes: "Tracks only Codex programming-environment deltas for Gemini CLI.",
};
...
function getLocalGeminiConfigPath() {
	return toSlash(path.join(os.homedir(), ".Gemini", "settings.json"));
}
...
function isPortableSourcePath(sourcePath) {
	if (sourcePath === "AGENTS.md") return true;
	if (!sourcePath.startsWith("codex-setup/")) return false;

	return (
		sourcePath === "codex-setup/agent-memory/shared/MEMORY.md" ||
		sourcePath ===
			"codex-setup/state/implemented-intelligence-suggestions.json" ||
		/^codex-setup\/rules\/.+\.md$/u.test(sourcePath) ||
		/^codex-setup\/agents\/.+\.md$/u.test(sourcePath) ||
		/^codex-setup\/commands\/.+/u.test(sourcePath) ||
		/^codex-setup\/hooks\/.+/u.test(sourcePath) ||
		/^codex-setup\/skills\/.+/u.test(sourcePath) ||
		/^codex-setup\/scripts\/.+/u.test(sourcePath) ||
		/^codex-setup\/mcp-.+\.json$/u.test(sourcePath) ||
		/^codex-setup\/hooks-.+\.json$/u.test(sourcePath) ||
		/^codex-setup\/settings.*\.json$/u.test(sourcePath) ||
		sourcePath === "codex-setup/manifest.json" ||
		sourcePath === "codex-setup/README.md"
	);
}
...
function classifyPath(sourcePath) {
	if (sourcePath === "AGENTS.md") {
		return {
			category: "root-rules",
			target_hints: ["Gemini.md", "gemini-setup/rules/"],
		};
	}
	...
	if (/^codex-setup\/rules\/.+\.md$/u.test(sourcePath)) {
		const mappedName =
			basename === "german-trigger-routing"
				? "german-skill-triggers"
				: basename === "global"
					? "superintelligence"
					: basename;
		return {
			category: "rule",
			target_hints: [`gemini-setup/rules/${mappedName}.md`],
		};
	}

	if (sourcePath === "codex-setup/agent-memory/shared/MEMORY.md") {
		return {
	...
	if (
		sourcePath === "codex-setup/state/implemented-intelligence-suggestions.json"
	) {
		return {
	...
	if (/^codex-setup\/agents\/.+\.md$/u.test(sourcePath)) {
		return {
			category: "agent",
			target_hints: [`gemini-setup/agents/${filename}`],
		};
	}

	if (/^codex-setup\/skills\/self-improve/u.test(sourcePath)) {
		return {
			category: "self-improve",
			target_hints: [
				"gemini-setup/commands/self-improve.md",
				"gemini-setup/skills/",
			],
		};
	}

	if (/^codex-setup\/commands\//u.test(sourcePath)) {
		return {
			category: "command",
			target_hints: [
				"gemini-setup/commands/",
				"gemini-setup/rules/german-skill-triggers.md",
			],
		};
	}

	if (/^codex-setup\/hooks\//u.test(sourcePath)) {
		return {
			category: "hook",
			target_hints: ["gemini-setup/scripts/", "Gemini.md"],
		};
	}

	if (/^codex-setup\/skills\//u.test(sourcePath)) {
		return {
			category: "skill",
			target_hints: ["gemini-setup/skills/", "Gemini.md"],
		};
	}

	if (/^codex-setup\/scripts\//u.test(sourcePath)) {
		return {
			category: "script",
			target_hints: ["gemini-setup/scripts/"],
		};
	}
	...
	if (
		...
		return {
			...
			target_hints: [
				getLocalGeminiConfigPath(),
				"Gemini.md",
				"gemini-setup/README.md",
			],
		};
	}
	...
	return {
		category: "setup-doc",
		target_hints: ["gemini-setup/README.md", "gemini-setup/skills/"],
	};
}
...
function buildRecommendation(strategy) {
	if (strategy === "ADD") {
		return "Als additive Gemini CLI-Erweiterung portieren.";
	}
	if (strategy === "REPLACE") {
		return "Vor Ersetzung warnen und ausdrueckliche Freigabe einholen.";
	}
	return "Fuer Gemini CLI uebersetzen und additive Integration bevorzugen.";
}
...
function buildReason({
	...
	if (strategy === "ADD") {
		const base =
			"Die Aenderung hat keinen direkten Gemini CLI-Zielkonflikt und wirkt additiv.";
		return bugfixSignal
			? `${base} Zusaetzlich ist robuste Bugfix- oder Haertungslogik erkennbar.`
			: base;
	}
	if (strategy === "REPLACE") {
		const base = `Die Aenderung beruehrt bestehende Gemini CLI-Ziele (${overlappingTargets.join(", ")}).`;
		return bugfixSignal
			? `${base} Gleichzeitig steckt darin bestehende Fixlogik, die vor einem Ersatz separat bewertet werden soll.`
			: base;
	}
	const base = `Die Aenderung betrifft vorhandene Gemini CLI-Bereiche (${categories.join(", ")}), braucht aber Uebersetzung statt blindes Spiegeln.`;
	return bugfixSignal
...
function buildCandidates(commits, state, limit) {
	...
	for (const commit of commits) {
		...
		candidates.push({
			...
			source_statuses: unique(
				relevantChanges.map((change) => change.status),
			).sort(),
			categories,
			...
			bugfix_recommendation: bugfixSignal
				? "Fixlogik und Regression-Schutz fuer Gemini CLI mitpruefen und nach Moeglichkeit uebernehmen."
				: "",
			reason: buildReason({
				strategy,
				categories,
...
function scanDeltas(args, statePath) {
	...
	const baseCommitRaw =
		args.since || (args.all ? null : state.last_reviewed_commit);
	const baseCommit = isValidCommit(baseCommitRaw) ? baseCommitRaw : null;
	const baseStatus = baseCommitRaw ? (baseCommit ? "ok" : "missing") : "none";
	...
	return {
		...
		approval_required_count: candidates.filter((item) => item.approval_required)
			.length,
		candidates,
		state,
	};
...
function formatTextReport(result) {
	...
	for (const group of groups) {
		...
		items.forEach((candidate, index) => {
			...
			lines.push(
				`${label} [${candidate.strategy}] ${candidate.source_commit.slice(0, 7)} ${candidate.source_subject}`,
			);
			lines.push(`  Kategorien: ${candidate.categories.join(", ")}`);
			lines.push(`  Quellen: ${candidate.source_paths.join(", ")}`);
			lines.push(`  Gemini CLI-Ziele: ${candidate.target_hints.join(", ")}`);
			lines.push(
				`  Bugfix-/Haertungssignal: ${candidate.bugfix_signal ? "ja" : "nein"}`,
			);
			...
			if (candidate.approval_required) {
				lines.push(
					"  Warnung: Diese Aenderung wuerde bestehende Gemini CLI-Logik beruehren. Vor Ersetzung erst bestaetigen lassen.",
				);
			}
		});
	}
...
function markState(args, statePath, fieldPrefix) {
	...
	if (!commit) {
		throw new Error(
			`No ${BRIDGE_CONTEXT.sourceLabel} delta commit available to store.`,
		);
	}
...
function main() {
	...
	throw new Error(
		"Usage: audit-codex-delta.mjs [scan|status|mark-reviewed|mark-applied] [--json] [--all] [--since REF] [--commit SHA] [--state PATH] [--limit N]",
	);
}
