...
SETUP_DIR="$PROGGS_DIR/gemini-setup"
...
if [[ ! -d "$Gemini_DIR" ]]; then
    echo "ERROR: $Gemini_DIR does not exist" >&2
    exit 1
fi

echo "==> Syncing Gemini CLI configuration to repo..."
...
cp "$Gemini_DIR/rules/"*.md "$SETUP_DIR/rules/" 2>/dev/null || echo "    (no .md files in rules/)"

# Agents
echo "--> Copying agents..."
mkdir -p "$SETUP_DIR/agents"
cp "$Gemini_DIR/agents/"*.md "$SETUP_DIR/agents/" 2>/dev/null || echo "    (no .md files in agents/)"

# Commands (self-improve.md + reference directory)
echo "--> Copying commands..."
mkdir -p "$SETUP_DIR/commands"
cp "$Gemini_DIR/commands/self-improve.md" "$SETUP_DIR/commands/" 2>/dev/null || echo "    (self-improve.md not found)"
if [[ -d "$Gemini_DIR/commands/self-improve-ref" ]]; then
    cp -r "$Gemini_DIR/commands/self-improve-ref/" "$SETUP_DIR/commands/self-improve-ref/" 2>/dev/null || true
fi

# Hooks (all variants: .sh, .ps1, .ts + subdirectories)
echo "--> Copying hooks..."
mkdir -p "$SETUP_DIR/hooks"
cp "$Gemini_DIR/hooks/"*.sh  "$SETUP_DIR/hooks/" 2>/dev/null || true
cp "$Gemini_DIR/hooks/"*.ps1 "$SETUP_DIR/hooks/" 2>/dev/null || true
cp "$Gemini_DIR/hooks/"*.ts  "$SETUP_DIR/hooks/" 2>/dev/null || true
# Subdirectory: prompt-injection-defender
if [[ -d "$Gemini_DIR/hooks/prompt-injection-defender" ]]; then
    cp -r "$Gemini_DIR/hooks/prompt-injection-defender/" "$SETUP_DIR/hooks/prompt-injection-defender/" 2>/dev/null || true
fi

# Skills
echo "--> Copying skills..."
if [[ -d "$Gemini_DIR/skills" ]]; then
    cp -r "$Gemini_DIR/skills/" "$SETUP_DIR/skills/" 2>/dev/null || echo "    (skills copy failed)"
fi

# Gemini.md (root of repo)
echo "--> Copying Gemini.md..."
cp "$HOME/Gemini.md" "$PROGGS_DIR/Gemini.md" 2>/dev/null || echo "    (~/Gemini.md not found)"

# Shared knowledge hub (MEMORY.md only — no separate files)
echo "--> Copying MEMORY.md..."
mkdir -p "$SETUP_DIR/agent-memory/shared"
cp "$Gemini_DIR/agent-memory/shared/MEMORY.md" "$SETUP_DIR/agent-memory/shared/" 2>/dev/null || echo "    (MEMORY.md not found)"
...
git add "$SETUP_DIR/" "$PROGGS_DIR/Gemini.md"
...
if ! git status --porcelain "$SETUP_DIR/" "$PROGGS_DIR/Gemini.md" | grep -q .; then
