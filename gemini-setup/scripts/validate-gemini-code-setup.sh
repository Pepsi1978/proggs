#!/usr/bin/env bash
# validate-gemini-setup.sh
# Validates that the gemini-setup directory is complete and correct.
# Inspired by codex-setup/scripts/validate-codex-setup.sh — adapted for Gemini CLI.
...
check_dir() {
  ...
  if [[ -d "$full" ]]; then
    pass "Directory exists: gemini-setup/$rel"
  else
    fail "Directory missing: gemini-setup/$rel"
  fi
}
...
check_file() {
  ...
  local full="$SETUP_DIR/$rel"
  if [[ -f "$full" ]]; then
    pass "File exists: gemini-setup/$rel"
  else
    fail "File missing: gemini-setup/$rel"
  fi
}
...
check_executable() {
  ...
  local full="$SETUP_DIR/$rel"
  if [[ ! -f "$full" ]]; then
    fail "Script missing (cannot check +x): gemini-setup/$rel"
    return
  fi
  if [[ -x "$full" ]]; then
    pass "Script is executable: gemini-setup/$rel"
  else
    fail "Script not executable (chmod +x needed): gemini-setup/$rel"
  fi
}
...
check_json_valid() {
  ...
  local full="$SETUP_DIR/$rel"
  if [[ ! -f "$full" ]]; then
    fail "JSON file missing (cannot validate): gemini-setup/$rel"
    return
  fi
  if node -e "JSON.parse(require('fs').readFileSync('$full','utf8'))" 2>/dev/null; then
    pass "Valid JSON: gemini-setup/$rel"
  else
    fail "Invalid JSON: gemini-setup/$rel"
  fi
}
...
check_content() {
  ...
  local description="$3"
  if [[ ! -f "$file" ]]; then
    fail "Cannot check content (file missing): gemini-setup/$1"
    return
  fi
  if grep -qF -- "$pattern" "$file" 2>/dev/null; then
    pass "Content found [$description]: gemini-setup/$1"
  else
    fail "Content missing [$description]: gemini-setup/$1"
  fi
}
...
# ════════════════════════════════════════════════════════════════════════════
echo "=== Gemini CLI Setup Validation ==="
...
if [[ -d "$SETUP_DIR/agent-memory/shared" ]]; then
  pass "Directory exists: gemini-setup/agent-memory/shared"
else
  fail "Directory missing: gemini-setup/agent-memory"
fi
...
if [[ -f "$SETUP_DIR/bridges/directive-2-self-observation-universal.md" ]]; then
  pass "File exists: gemini-setup/bridges/directive-2-self-observation-universal.md"
else
  warn "File missing: gemini-setup/bridges/directive-2-self-observation-universal.md (optional but recommended)"
fi
...
while IFS= read -r -d '' script; do
  ...
  if [[ -x "$script" ]]; then
    pass "Executable: gemini-setup/$rel"
  else
    fail "Not executable: gemini-setup/$rel"
  fi
...
if [[ -f "$SETUP_DIR/agent-memory/shared/MEMORY.md" ]]; then
  pass "Whiteboard exists: gemini-setup/agent-memory/shared/MEMORY.md"
else
  fail "Whiteboard missing: gemini-setup/agent-memory/shared/MEMORY.md"
fi

# ── 10. Gemini.md in repo root ───────────────────────────────────────────────
section "10. Gemini.md"
if [[ -f "$REPO_ROOT/Gemini.md" ]]; then
  pass "Gemini.md exists in repo root"
else
  fail "Gemini.md missing from repo root"
fi
...
while IFS= read -r -d '' script; do
  check_syntax_sh "$script"
         ...
         ! -name "validate-gemini-setup.sh" -print0 2>/dev/null)
...
do
  if [[ -f "$SETUP_DIR/$f" ]]; then
    pass "Recommended file exists: gemini-setup/$f"
  else
    warn "Recommended file missing: gemini-setup/$f"
  fi
done
