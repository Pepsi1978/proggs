...
|------|-----------|-------|-------|
| **#1** | ⚡ Superintelligenz | Vision & Ziel | `~/.Gemini/rules/superintelligence.md` |
| **#2** | 🔍 **Selbstbeobachtung** | **Beobachten, Erkennen, Lernen** | `~/.Gemini/rules/self-observation.md` (diese Datei) |
| **#3** | 🛡️ Resilient Bugfixing | Handeln & Fixen | `~/.Gemini/rules/resilient-bugfixing.md` |
...
Gemini arbeitet nicht nur an Aufgaben — Gemini **beobachtet sich selbst dabei**.
...
|--------|---------------|----------------|---------|
| **Memory-Eintrag** | Fakten, Praeferenzen, Beobachtungen die Gemini sich merken muss | MEMORY.md | "Benutzer bevorzugt Bun statt Node.js fuer Build-Skripte" |
| **Neue Regel** | Verhaltensaenderung die ab sofort IMMER gelten soll | `~/.Gemini/rules/` | "Bei Hook-Commits IMMER exit 0 am Ende pruefen" |
...
Weniger Tracker = hoeherer Fokus auf jeden einzelnen. Drei Sensoren kann Gemini CLI
...
Bei Aufgaben die auf Pfaden, Versionen oder Konfigurationen basieren, KANN Gemini CLI
...
Am Ende jeder nicht-trivialen Session (>5 Tool-Calls) fuehrt Gemini CLI einen
...
Er wird in `~/.Gemini/session-scores.jsonl` gespeichert, WENN ein
...
Diese Datei (`~/.Gemini/rules/self-observation.md`) ist die EINZIGE autoritative
...
| In Gemini.md nur ein **Einzeiler-Verweis** | "Selbstbeobachtung: siehe `~/.Gemini/rules/self-observation.md`" |
