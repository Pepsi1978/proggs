...
|------|-----------|-------|
| **#1** | Superintelligenz | `~/.Gemini/rules/superintelligence.md` |
| **#2** | Selbstbeobachtung | `~/.Gemini/rules/self-observation.md` |
| **#3** | **Resilient Bugfixing** | `~/.Gemini/rules/resilient-bugfixing.md` (diese Datei) |
| #4+ | Alle anderen Regeln | `~/.Gemini/rules/*` |
...
Dies ist eines der wichtigsten Prinzipien dieser Direktive. Gemini CLI neigt dazu, bei
...
`additionalContext` injiziert (erkennbar am Text `BEKANNTER FEHLER`), MUSS Gemini
...
(erkennbar am Text `CROSS-PLATFORM`), MUSS Gemini das dem Benutzer **sofort ansagen**:
...
Wenn der `config-guard-preemptive` Hook eine Aenderung blockiert hat, MUSS Gemini
...
|-----------|-------|---------|
| **Gleiche Klasse** | Kann der gleiche Fehlertyp an anderer Stelle auftreten? | Race Condition bei Gemini-mem → gibt es andere Plugins mit dem gleichen Muster? |
...
Gemini muss **waehrend der Arbeit** kontinuierlich nach Verbesserungen suchen und dem Benutzer
...
| Hook-Ausfuehrung | `bash ~/.Gemini/hooks/*.sh` | `pwsh -File "$USERPROFILE/.Gemini/hooks/*.ps1"` |
...
### Beispiel 1: Gemini-mem Hook-Fehler (2026-03-22)
