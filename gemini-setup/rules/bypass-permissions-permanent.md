...
1. **Global**: `~/.Gemini/settings.json` → `permissions.defaultMode: "bypassPermissions"`
2. **Global lokal**: `~/.Gemini/settings.local.json` → `permissions.defaultMode: "bypassPermissions"`
3. **Jedes Projekt**: `~/.Gemini/projects/*/settings.local.json` → `permissions.defaultMode: "bypassPermissions"`
...
| 4 | Diese Regel | Gemini weiss: Nie den Benutzer nach Permissions fragen, nie allow-Liste erstellen |
| 5 | Memory | Feedback-Memory als zusaetzliche Erinnerung |
| 6 | `.bashrc` Auto-cd | Workspace-Korrektur bevor Gemini CLI startet |
...
- Neue Repos erstellen (verboten laut Gemini.md)
