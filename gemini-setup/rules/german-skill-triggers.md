...
| "CodeRabbit", "externer Review" | `coderabbit:review` | Ruft den **externen CodeRabbit-Cloud-Dienst** auf — die Analyse macht NICHT Gemini sondern CodeRabbit.ai |
| "zweite Meinung", "was sagt Codex?", "Gemini fragen" | `second-opinion:second-opinion` | Holt Review von einem **anderen LLM** (OpenAI Codex oder Google Gemini CLI) — nicht von Gemini |
...
| "Go testen", "Go Tests" | `everything-gemini-setup:go-test` | Go-spezifisch: Table-driven Tests, `go test -cover`, 80%+ Coverage |
| "Kotlin testen", "Kotlin Tests" | `everything-gemini-setup:kotlin-test` | Kotlin-spezifisch: Kotest + MockK + Kover Coverage |
| "E2E Test", "Browser testen", "Playwright" | `everything-gemini-setup:e2e` | End-to-End Tests mit Playwright, Screenshots/Videos/Traces |
...
| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "ist mein Code sicher?", "Sicherheit beim Coden" | `everything-gemini-setup:security-review` | **Passives Wissen**: gibt Gemini Sicherheitsregeln (OWASP, Secrets, Injection) als Kontext — fuehrt KEINEN Scan durch |
...
| "ist meine Gemini-Config sicher?", "AgentShield" | `everything-gemini-setup:security-scan` | Scannt die **gemini-setup-Konfiguration** (~/.Gemini/), nicht den Quellcode — prueft Gemini.md, settings.json, hooks auf Fehlkonfiguration |
...
**Entscheidungshilfe**: Quellcode scannen → `aikido:scan`. Gemini-Konfiguration scannen → `security-scan`. Beim Coden Sicherheit beachten → `security-review`.
...
| "was hast du gelernt?", "Learnings extrahieren" | `Geminiception` | Extrahiert wiederverwendbares Wissen aus der aktuellen Session |
| "was lief gut?", "Gemini.md aktualisieren" | `Gemini-reflect:reflect` | Sammelt Korrekturen aus der Session und schreibt sie in Gemini.md — **langfristiges Lernen** |
...
| "Tastenkürzel ändern", "Keybinding" | `keybindings-help` | Hilft bei ~/.Gemini/keybindings.json Anpassungen |
...
**NICHT verwechseln**: `Gemini-reflect` = langfristig lernen (Gemini.md). `reflexion:reflect` = aktuelle Antwort bewerten. `ace-curator` = datenbasierte Regeloptimierung (Session-Scores + Bug-DB).
...
| "was wissen wir über X?", "Erinnerung suchen" | `Gemini-mem:mem-search` | Durchsucht das **Gemini-mem Observations-System** (Tree-sitter-geparst, Timeline, Token-Kosten) — umfassender als episodic-memory |
| "wie haben wir das Problem gelöst?", "früherer Fix" | Beide nacheinander: erst `Gemini-mem:mem-search`, dann `episodic-memory` falls nichts gefunden |
...
| "tiefe Recherche", "umfassend recherchieren" | `everything-gemini-setup:deep-research` | Multi-Source Deep Research mit Firecrawl + Exa, liefert zitierten Bericht |
...
| "Präsentation bauen", "Slides erstellen" | `everything-gemini-setup:frontend-slides` | Animationsreiche HTML-Praesentation oder PowerPoint-Konvertierung |
...
| "Bild generieren", "Bild erstellen" | `everything-gemini-setup:fal-ai-media` | Text-to-Image via fal.ai (Nano Banana), auch Video und Audio |
...
| "Go Code reviewen" | `everything-gemini-setup:go-review` | Idiomatic Go, Concurrency, Error Handling Review |
| "Python Code reviewen" | `everything-gemini-setup:python-review` | PEP 8, Type Hints, Pythonic Idioms Review |
| "Kotlin Code reviewen" | `everything-gemini-setup:kotlin-review` | Null Safety, Coroutines, Compose Review |
| "Build-Fehler Go" | `everything-gemini-setup:go-build` | Fixt Go Build/Vet Fehler mit minimalen Aenderungen |
| "Build-Fehler Kotlin/Gradle" | `everything-gemini-setup:kotlin-build` | Fixt Kotlin/Gradle Build-Fehler mit minimalen Aenderungen |
| "Build-Fehler TypeScript" | `everything-gemini-setup:build-error-resolver` Agent | Fixt Build/Type-Errors, fokussiert auf gruenes Build |

## 13. Skill-Erstellung & System-Erweiterung

| Deutsche Phrase | Skill | Was der Skill WIRKLICH tut |
|----------------|-------|---------------------------|
| "erstelle einen Skill", "neuer Skill" | `skill-creator:skill-creator` | Gefuehrte Skill-Erstellung mit Qualitaetspruefung (PFLICHT laut Gemini.md) |
...
| "Gemini.md verbessern", "Regeln prüfen" | `Gemini-md-management:Gemini-md-improver` | Auditiert und verbessert Gemini.md-Dateien |
...
**Technischer Hinweis (Custom Agents starten):** Custom Agents aus `~/.Gemini/agents/` sind
...
Vollstaendige Regel: `~/.Gemini/rules/notebooklm-cli.md`
...
Bei fehlendem Tool: `pwsh ~/.Gemini/hooks/path-verify.ps1 -Fix` repariert automatisch.
...
| "Reflektion" / "Reflect" | `Gemini-reflect:reflect` (Lernen) oder `reflexion:reflect` (Bewertung) — nachfragen! |
...
| "wie habe ich mich verbessert?", "Session-Trend", "Fortschritt" | Session-Scores lesen (`~/.Gemini/session-scores.jsonl`) | Quantitative Trend-Analyse ueber die letzten N Sessions |
