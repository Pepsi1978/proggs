...
├── VoiceOverlays/
│   └── .env                               Keys fuer Gemini- und Terminal-Overlay (Windows + macOS)
└── <weitere Projekte>/
```
...
Diese Datei (`~/.Gemini/rules/secrets-in-sk-folder.md`) ist die autoritative Quelle.
Kopie im Setup-Repo unter `~/proggs/gemini-setup/rules/secrets-in-sk-folder.md`.
