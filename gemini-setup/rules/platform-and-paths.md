...
```python
# FALSCH:
with open('/c/Users/barwa/.Gemini/settings.json') as f: ...
...
- Nach `npm update -g` (besonders gemini-setup CLI)
...
```bash
# macOS/Linux:
bash ~/.Gemini/hooks/path-verify.sh --fix

# Windows:
pwsh ~/.Gemini/hooks/path-verify.ps1 -Fix
```

### Was geprueft wird

**Tools:** git, node, npm, bun, python3, rustc, cargo, go, Gemini, swift (macOS),
kotlin, gradle, deno, ollama (macOS), Geminiwatch
...
| `gemini-setup/mcp-macos.json` | macOS-Referenz |
| `gemini-setup/mcp-windows.json` | Windows-Referenz |
| `~/.Gemini/hooks/reindex-codebase.sh/.ps1` | Plattform-spezifische Hooks |
...
> JSON zerstoert → Gemini CLI startete nicht richtig.
...
Bei Problemen: `find ~/.Gemini/plugins -name "*.sh" ! -perm -u+x` → `chmod +x`
...
1. `grep -rn 'dateiname.ps1' ~/.Gemini/hooks/` — wird die Datei per Dot-Source geladen?
