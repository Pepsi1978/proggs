...
> damit Gemini CLI und andere CLIs die Gemini-Umgebung vollstaendig importieren koennen.
...
Danach koennen andere CLIs (Gemini CLI, Codex) die Gemini-Umgebung importieren.
...
grep "BASELINE-.*-GEM" ~/proggs/gemini-setup/mirror-ledger.md
...
### 3e. Bereits importierte gemini-setup-Komponenten
...
Oeffne ~/proggs/gemini-setup/mirror-ledger.md und haenge am Ende folgenden Eintrag an:
...
### 5. Bereits importierte gemini-setup-Komponenten
...
### 8. Benutzer-Profil (falls abweichend von gemini-setup-BASELINE)
[Nur Abweichungen notieren — das gemini-setup-BASELINE hat das Hauptprofil]

### Portierung auf Gemini CLI
1. Lies diesen BASELINE um zu verstehen was Gemini kann
2. Gemini-spezifische MCP-Server/Extensions: pruefen ob gemini-setup-Aequivalente existieren
3. GEMINI.md Regeln: in Gemini.md oder rules/ uebernehmen wenn sinnvoll
...
<!-- APPLIED: macos/gemini-setup=PENDING -->
<!-- APPLIED: windows/gemini-setup=PENDING -->
...
Gleiche Struktur wie die gemini-setup-Eintraege im Ledger.
...
- Schreibe NUR in ~/proggs/gemini-setup/mirror-ledger.md
- Aendere NIEMALS Dateien in ~/.Gemini/ (das ist Gemini CLI's Workspace)
