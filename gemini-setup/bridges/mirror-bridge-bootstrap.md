...
> **Diesen Prompt in das neue CLI (Gemini, Codex, oder ein frisches Gemini CLI) einfuegen.**
...
Es geht NICHT darum Gemini CLI zu klonen — es geht darum die INTELLIGENZ und FAEHIGKEITEN
...
Lies die Datei `~/proggs/gemini-setup/mirror-ledger.md` vollstaendig.
...
- Lies `~/proggs/gemini-setup/rules/superintelligence.md`
- Lies `~/proggs/gemini-setup/rules/self-observation.md`
- Lies `~/proggs/gemini-setup/rules/resilient-bugfixing.md`
- Schreibe die Kernprinzipien in die native Konfigurationsdatei dieses CLIs
  (GEMINI.md fuer Gemini, CODEX.md fuer Codex, Gemini.md fuer Gemini CLI)

### Agent-Faehigkeiten → Vollstaendig uebernehmen
- Lies jeden Agent in `~/proggs/gemini-setup/agents/`
...
- Lies jeden Hook in `~/proggs/gemini-setup/hooks/`
...
1. Merke dir: Das Ledger liegt in `~/proggs/gemini-setup/mirror-ledger.md`
...
- Uebernimm die Intelligenz/das Wissen (nicht die gemini-setup-Dateien)
...
- Aber IMMER im EIGENEN Workspace dieses CLIs, nie in Gemini CLI's Dateien
...
### Frisches Gemini CLI
- Kann den vollstaendigen Setup nutzen: `bash gemini-setup/setup-macos.sh`
