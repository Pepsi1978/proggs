...
Aenderungen in die Gemini CLI Umgebung.

## Ablauf

1. Lies `~/.Gemini/state/gemini-delta-state.json` fuer den letzten geprueften Commit
...
- `ADD`: Neue Idee die Gemini CLI bisher nicht hat → als Empfehlung vorschlagen
- `ADAPT`: Sinnvoll, muss aber fuer Gemini CLI angepasst werden → Portierung erklaeren
...
- Nach Benutzer-Approval: `gemini-delta-state.json` und `gemini-setup/state/gemini-delta-state.json` aktualisieren
