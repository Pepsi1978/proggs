---
name: self-improve
description: Systematic self-improvement of the Gemini CLI development environment (~10-30 min, token-intensive). ONLY use when the user explicitly says "/self-improve", "verbessere dich", "optimiere deine Umgebung", "check dein Setup", "update alles", "mach mich besser", "aktualisiere alles", "pruef mein System", "System-Check", or "Umgebung pruefen". NEVER run this proactively or automatically — only on manual user request.
---
...
Das Whiteboard (`.Gemini/agent-memory/shared/MEMORY.md`) ist die EINZIGE zentrale Wissensdatei
fuer ALLE Komponenten des Gemini CLI Systems: Agents, Skills, Hooks, Plugins.
...
3. **Benutzer-Feedback lesen**: `~/.Gemini/projects/*/memory/feedback_*.md` enthaelt
   direkte Korrekturen und Praeferenzen des Benutzers. Diese MUESSEN gelesen werden, weil:
   - `~/.Gemini/projects/*/memory/project_*.md` — aktiver Projektstatus und Blocker (vor Verbesserungen lesen!)
   - `~/.Gemini/projects/*/memory/MEMORY.md` — Memory-Index (zeigt alle verfuegbaren Memory-Dateien)
...
4. **Gemini-Mem Observations lesen**: `~/.Gemini/homunculus/projects/*/observations.jsonl`
...
**I6 — SQLite-Memory fuer bug-cases** (`~/.Gemini/scripts/bug-cases-db.py`):
- `python3 ~/.Gemini/scripts/bug-cases-db.py sync` — Synchronisiert bug-cases.jsonl → bug-cases.db
- `python3 ~/.Gemini/scripts/bug-cases-db.py lookup "<text>"` — Volltext-Suche (FTS5)
- `python3 ~/.Gemini/scripts/bug-cases-db.py hash-lookup "<text>"` — Exact symptom-hash match
- DB liegt unter `~/proggs/.Gemini/agent-memory/shared/bug-cases.db` — wird ins Repo committed
...
**I7 — Strategie-Tracking fuer DGM-Pattern** (`~/.Gemini/scripts/strategy-tracker.py`):
- `python3 ~/.Gemini/scripts/strategy-tracker.py log <date> <strategy> <total> <accepted> <implemented>` — Logged Lauf
- `python3 ~/.Gemini/scripts/strategy-tracker.py history` — Strategie-Statistik aller Laeufe
- `python3 ~/.Gemini/scripts/strategy-tracker.py best-strategy` — Empfehlung fuer naechsten Lauf
- Daten in `~/.Gemini/self-improve-cache/strategies.json` (lokal, nicht ueber Git)
...
- **SHELL/TERMINAL-UPDATES IMMER ZULETZT (KRITISCH — v5.8):**
  ...
  **Betroffene Tools**: PowerShell (pwsh), Git, Git Bash, Node.js, npm, Bun, Deno, Python, Gemini CLI CLI
...
- **VERBOTENES Verzeichnis**: `C:\Users\barwa\Codex` / `~/Codex` — dieses Verzeichnis gehoert
  AUSSCHLIESSLICH dem Codex-CLI-Tool. Gemini CLI darf dort NIEMALS lesen, schreiben oder
  in irgendeiner Form darauf zugreifen. Jeder Versuch ist ein schwerer Regelverstoss.
  Grund: Codex und Gemini CLI arbeiten parallel aber in getrennten Arbeitsbereichen.
...
grep -lE '<<<<<<< (Updated upstream|HEAD)|>>>>>>> Stashed changes|\|\|\|\|\|\|\| Stash base' \
  ~/proggs/.Gemini/agent-memory/shared/MEMORY.md \
  ~/proggs/Gemini.md \
...
**0a) Whiteboard lesen:**
Lese `.Gemini/agent-memory/shared/MEMORY.md` vollstaendig und erstelle eine kompakte Zusammenfassung:
...
- **Compound Effect Tracker (PFLICHT seit v5.20):**
  ...
  - Pruefen ob Direktive #1 (Superintelligenz) und #2 (Selbstbeobachtung) noch in allen
    Speicherorten vorhanden sind (rules/, Gemini.md, Whiteboard, Memory)
...
The evolution-analyst reads `~/.Gemini/session-scores.jsonl` and `~/.Gemini/agent-memory/shared/MEMORY.md`
...
4. If BOTH fail: **WARNING** — "Session-Scorer liefert Dummy-Daten!" Then:
   - Load newest non-agent transcript from `~/.Gemini/projects/*/` (UUID filename, not `agent-*`)
...
**Indexierung**: Wenn noetig, ausfuehren:
  ```bash
  bash ~/.Gemini/scripts/semantic-search-healthcheck.sh --reindex
  ```
...
```bash
bash ~/.Gemini/scripts/semantic-search-healthcheck.sh   # macOS/Linux
pwsh ~/.Gemini/scripts/semantic-search-healthcheck.ps1  # Windows
```
...
- **Smart-Cache (v5.17 — git-diff-basiert, ersetzt TTL)**:
  Cache location: `~/.Gemini/self-improve-cache/` (NOT in shared memory — prevents contamination).
  **Invalidierung**: Pruefen ob sich Dateien unter `~/.Gemini/agents/`, `~/.Gemini/hooks/`,
  `~/.Gemini/rules/` oder `settings.json` seit dem letzten Cache-Zeitstempel geaendert haben.
  Wenn ja → betroffene Researcher (R2, R3, R4) neu starten. Wenn nein → gecachte Ergebnisse nutzen.
  Zeitstempel: `~/.Gemini/self-improve-cache/.last-cache-time`
  | Researcher | Cache-Regel | Grund |
  |------------|-------------|-------|
  | R1 Updates | IMMER frisch | Gemini CLI Releases aendern sich schnell |
...
4. **Fehler dokumentieren**: Jeden Absturz sofort ins Whiteboard eintragen (Sektion "Offene Fehler & Probleme" in `.Gemini/agent-memory/shared/MEMORY.md`):
   ```
...
`.Gemini/agent-memory/shared/MEMORY.md` — contains ALL knowledge: errors, system state,
...
- **KEINE Abkuerzungen oder Fachbegriffe** ohne Erklaerung. Wenn ein Begriff wie "MCP-Server" oder "Hook" unvermeidbar ist, in Klammern erklaeren: "MCP-Server (ein Zusatz-Werkzeug das Gemini schlauer macht)"
...
**Ziel: Gemini zum BESTEN AI-Programmierer der Welt machen. SUPER INTELLIGENZ.**
...
1. **Researcher spawnen** _(Nutzt R8-Ergebnisse statt eigenem Researcher-Spawn — siehe 5C)_: "Welche neuen MCP-Server, gemini-setup-Plugins oder CLI-Tools wurden in den letzten 30 Tagen veroeffentlicht die das DENKEN verbessern?" (Wissensgraphen, Verifikation, Semantic Search, Code-Analyse, Formal Verification, Codebase-Embeddings)
2. **Bewerten**: Wuerde dieses Tool Gemini bei ECHTEN Aufgaben schlauer machen? (Nicht nur cool, sondern nuetzlich)
...
Recherchiere und implementiere neue Denkstrategien die Gemini FUNDAMENTAL schlauer machen:
...
- Bewertung mit dem 4-Kriterien-Framework: Umsetzbarkeit, Intelligenz-Hebel, gemini-setup-Fit, Haltbarkeit
...
1. **Codebase-Semantik**: Gibt es einen MCP-Server der die Codebase semantisch indexiert? (z.B. Zilliz Gemini-context, CodeGraphContext) → Testen und ggf. installieren
...
1. **Whiteboard-Scanner**: Lies `.Gemini/agent-memory/shared/MEMORY.md` komplett → Finde ALLE unfixten Probleme (Status: OFFEN), veraltete Eintraege, Widersprueche. Jedes Problem MUSS in diesem Lauf gefixt werden.
...
1. Lies ALLE Dateien in `~/.Gemini/rules/` — zaehle Regeln
...
1. Hook-Scripts (*.ps1 + *.sh + *.ts) → `~/proggs/gemini-setup/hooks/`
2. Agent-Definitionen (*.md) → `~/proggs/gemini-setup/agents/`
3. Commands + Unterordner → `~/proggs/gemini-setup/commands/` (inkl. self-improve-ref/)
4. Rules → `~/proggs/gemini-setup/rules/`
5. Gemini.md → `~/proggs/Gemini.md`
6. **settings-reference.json** → `~/proggs/gemini-setup/settings-reference.json`
   (Hooks, Plugins, Marketplaces, Env — wird von auto-sync in settings.json gemerged)
7. **Knowledge-Dateien** → `~/proggs/gemini-setup/agent-memory/shared/`
...
- Plugin-Caches (~/.Gemini/plugins/cache/)
- Session-Transcripts, session-scores.jsonl

**Sync ausfuehren:**
```bash
bash ~/.Gemini/scripts/self-improve-sync.sh "#NNN - Self-improve sync"   # macOS/Linux
pwsh ~/.Gemini/scripts/self-improve-sync.ps1 "#NNN - Self-improve sync"  # Windows
```
...
**Session-Score-Trend (NEW v5.1):** Read `~/.Gemini/session-scores.jsonl`, compute 5-session
