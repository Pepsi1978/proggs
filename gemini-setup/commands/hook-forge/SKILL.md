---
name: hook-forge
description: "Nutze diesen Skill IMMER wenn ein neuer Gemini CLI Hook erstellt oder ein bestehender Hook grundlegend umgeschrieben wird. Auch wenn Gemini selbst einen Hook erstellt (z.B. via /hookify, /self-improve, oder manuell) MUSS dieser Skill VORHER geladen werden. Trigger: 'Hook erstellen', 'neuer Hook', 'Hook bauen', 'Hook fixen' (bei Rewrite), oder wenn Gemini intern Write/Edit auf eine .ps1/.sh Hook-Datei ausfuehrt."
---
...
1. **PowerShell-Version erstellen**: `~/.Gemini/hooks/[name].ps1`
...
1. **Setup-Repo kopieren**: Beide Versionen nach `~/proggs/gemini-setup/hooks/` kopieren.
2. **settings.json registrieren** (wenn noetig):
   - Hook in `~/.Gemini/settings.json` unter dem richtigen Event eintragen
   - Format: `{"hooks": {"[Event]": [{"type": "command", "command": "pwsh -NoProfile -File ..."}]}}`
   - ACHTUNG: Auch die macOS-Version in `gemini-setup/settings.json` eintragen!
