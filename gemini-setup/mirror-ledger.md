...
Jedes CLI-Tool (Gemini CLI, Codex CLI, Gemini CLI) und jede Plattform (macOS, Windows)
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T22:30:00Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: baseline -->
<!-- AFFECTS: GESAMTE Programmierumgebung -->

> **Dies ist der BASELINE-Eintrag.** Er beschreibt die KOMPLETTE Programmierumgebung so
> detailliert, dass ein frisches CLI (Gemini CLI, Codex, Gemini) auf einem neuen Rechner
...
**Voraussetzung:** Git und Gemini CLI CLI muessen installiert sein.
...
# macOS:
bash gemini-setup/setup-macos.sh
# Windows:
# powershell gemini-setup/setup-windows.ps1

# 3. Gemini.md an den richtigen Ort kopieren
cp ~/proggs/Gemini.md ~/Gemini.md
```

Das Setup-Script installiert: Plugins aus 10+ Marketplaces, kopiert Agents/Hooks/Rules/Commands
nach `~/.Gemini/`, konfiguriert Settings und MCP-Server.

**Alle Dateien liegen im Setup-Repo:** `~/proggs/gemini-setup/`
...
Diese Direktiven stehen auch in `Gemini.md` (wird bei jedem Start geladen) und im Whiteboard.

---

### 2. Custom Agents (18 Stueck)

Alle in `gemini-setup/agents/*.md`, werden nach `~/.Gemini/agents/` kopiert.
...
Alle in `gemini-setup/hooks/`, werden nach `~/.Gemini/hooks/` kopiert.
...
| `Gemini-mem-worker-guard` | SessionStart | Stellt sicher dass Gemini-mem Worker laeuft |
...
- `Gemini-mem-worker-launcher.sh` — Startet Gemini-mem Worker als Daemon (nur macOS)
...
Alle in `gemini-setup/rules/*.md`, werden nach `~/.Gemini/rules/` kopiert.
...
Alle in `gemini-setup/commands/*.md`, werden nach `~/.Gemini/commands/` kopiert.
...
| `Geminiception.md` | Extrahiert wiederverwendbares Wissen aus Sessions |
...
Vollstaendige Liste in `gemini-setup/manifest.json` unter `"plugins"`.
...
|-------------|--------|-----------|
| Gemini-plugins-official | 23 | LSPs (Swift, TS, C#, Rust, Go, Kotlin...), GitHub, Playwright, Superpowers, CodeRabbit |
...
| everything-gemini-setup | 1 | Mega-Plugin mit 100+ Skills (Go, Kotlin, Python, Swift, Django...) |
| compound-engineering | 1 | Design, Research, Review, Workflow Skills |
| thedotmack (Gemini-mem) | 1 | Persistent Cross-Session Memory mit Tree-sitter AST |
| boostvolt | 1 | Extra LSP-Server |
| xGemini-plugin | 1 | iOS/Xcode Simulator + Build MCP-Server |
| apple-platform-build-tools | 1 (macOS only) | Xcode Build Agent + Skills |
| Gemini-reflect | 1 | Session Reflections → Gemini.md Updates |
...
```json
{
  ...
  "env": {
    ...
    "Gemini_CODE_SUBAGENT_MODEL": "sonnet",
    "Gemini_AUTOCOMPACT_PCT_OVERRIDE": "95",
    "Gemini_CODE_EFFORT_LEVEL": "high",
...
**Datei:** `~/proggs/.Gemini/agent-memory/shared/MEMORY.md`
...
**Auto-Memory:** `~/.Gemini/projects/-Users-frank/memory/` (27 Dateien)
...
Diese Memories sind gemini-setup-spezifisch und muessen NICHT auf andere CLIs portiert werden.
...
Gemini uebernimmt das Denken (Prompt-Optimierung, Struktur), Nemotron generiert den Output.
...
**Gemini CLI (macOS oder Windows):**
1. `git clone https://github.com/Pepsi1978/proggs ~/proggs`
2. `bash ~/proggs/gemini-setup/setup-macos.sh` (oder `.ps1` fuer Windows)
3. `cp ~/proggs/Gemini.md ~/Gemini.md`
4. Gemini CLI starten — Auto-Sync Hook macht den Rest

**Codex CLI:**
1. Repo klonen
2. `CODEX.md` aus `~/proggs/gemini-setup/bridges/` laden (falls vorhanden)
...
**Gemini CLI:**
1. Repo klonen
2. `GEMINI.md` aus `~/proggs/gemini-setup/bridges/` laden (falls vorhanden)
...
- Gemini.md/CODEX.md/GEMINI.md als primaere Direktiven-Quelle nutzen

<!-- APPLIED: macos/gemini-setup=2026-03-25T22:30:00Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T21:02:54Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: agent -->
<!-- AFFECTS: ~/.Gemini/agents/export.md, ~/.Gemini/agents/import.md, ~/proggs/gemini-setup/agents/export.md, ~/proggs/gemini-setup/agents/import.md -->
...
Entwicklungsumgebung: Der Benutzer arbeitet gleichzeitig auf macOS (Gemini CLI) und Windows
(Gemini CLI), sowie gelegentlich mit Codex CLI und Gemini CLI. Bisher gab es das
...
- `mirror-ledger.md`: Die einzige Wahrheitsquelle. Git-versioniert, liegt in
  ~/proggs/gemini-setup/ und wird bei jedem Commit synchronisiert.
...
- `~/.Gemini/agents/export.md` (und Kopie in `~/proggs/gemini-setup/agents/`)
- `~/.Gemini/agents/import.md` (und Kopie in `~/proggs/gemini-setup/agents/`)
...
3. Unvollstaendig — nur Gemini CLI → andere CLIs, nicht rueckwaerts

Das neue System ist bidirektional, vollstaendig, und APPLIED-Status wird getrackt.

### Portierung Windows/gemini-setup

1. Erstelle `%USERPROFILE%\.Gemini\agents\export.md` mit dem Inhalt unten (export-Datei)
2. Erstelle `%USERPROFILE%\.Gemini\agents\import.md` mit dem Inhalt unten (import-Datei)
3. Erstelle auch Kopien in `%USERPROFILE%\proggs\gemini-setup\agents\`
...
Vollstaendige Datei: `~/proggs/gemini-setup/agents/export.md`

### Datei-Inhalt macOS+Windows (import.md)

Vollstaendige Datei: `~/proggs/gemini-setup/agents/import.md`
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T21:02:54Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-25T21:47:57Z -->
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/mirror-check.sh, ~/.Gemini/hooks/mirror-check.ps1, ~/proggs/gemini-setup/hooks/mirror-check.sh, ~/proggs/gemini-setup/hooks/mirror-check.ps1 -->
...
- `~/.Gemini/hooks/mirror-check.sh` (macOS/bash)
- `~/.Gemini/hooks/mirror-check.ps1` (Windows/PowerShell)

Beide wurden in `~/proggs/gemini-setup/hooks/` kopiert.

In `~/.Gemini/settings.json` (macOS) wurde folgender Block zum SessionStart-Event hinzugefuegt:
```json
{
  "hooks": [
    {
      "type": "command",
      "command": "bash ~/.Gemini/hooks/mirror-check.sh",
      "timeout": 5
    }
  ]
}
...
```json
{
  "hooks": [
    {
      "type": "command",
      "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/mirror-check.ps1\"",
      "timeout": 5
    }
  ]
}
...
1. Erstelle `%USERPROFILE%\.Gemini\hooks\mirror-check.ps1` mit dem Inhalt unten
2. Kopiere nach `%USERPROFILE%\proggs\gemini-setup\hooks\mirror-check.ps1`
3. Oeffne `%USERPROFILE%\.Gemini\settings.json`
...
5. Fuege nach dem letzten bestehenden SessionStart-Hook-Objekt einen neuen Block ein:
   ...
   ```
6. Validiere: `pwsh -Command "Get-Content '$env:USERPROFILE\.Gemini\settings.json' | ConvertFrom-Json"`
...
LEDGER="$HOME/proggs/gemini-setup/mirror-ledger.md"
...
$Ledger = Join-Path $env:USERPROFILE "proggs\gemini-setup\mirror-ledger.md"
...
macOS-Befehl: `"command": "bash ~/.Gemini/hooks/mirror-check.sh", "timeout": 5`

Windows-Befehl: `"command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/mirror-check.ps1\"", "timeout": 5`
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: rule -->
<!-- AFFECTS: ~/.Gemini/rules/german-agents-skills.md, ~/proggs/gemini-setup/rules/german-agents-skills.md -->
...
Neue Datei erstellt: `~/.Gemini/rules/german-agents-skills.md` (und Kopie in Setup-Repo).
Gleichzeitig wurde in Gemini.md (Sprache-Sektion) ein Verweis auf diese Regel hinzugefuegt.
...
1. Erstelle `%USERPROFILE%\.Gemini\rules\german-agents-skills.md` mit dem Inhalt unten
2. Kopiere nach `%USERPROFILE%\proggs\gemini-setup\rules\german-agents-skills.md`
3. Kein Settings-Eintrag noetig — Regeln werden automatisch von Gemini CLI geladen
...
ALLE selbst erstellten Agents (`~/.Gemini/agents/*.md`), Skills und Commands
(`~/.Gemini/commands/*.md`) muessen **komplett auf Deutsch** geschrieben sein.
...
Keine Settings-Registrierung noetig. Regel-Dateien in `~/.Gemini/rules/` werden
automatisch von Gemini CLI beim Start geladen.
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: rule -->
<!-- AFFECTS: ~/.Gemini/rules/german-skill-triggers.md, ~/proggs/gemini-setup/rules/german-skill-triggers.md -->
...
richtigen Agent/Skill-Aufrufe. Da der Benutzer Deutsch spricht, muss Gemini bei jeder
Anfrage diese Map mental durchgehen.

In dieser Session wurde die Map um Sektion 15 erweitert: die Mirror Bridge Agenten.
Ohne diese Sektion wuerde der Benutzer "exportiere" sagen und Gemini wuerde nicht wissen
dass damit der `export` Agent gemeint ist.

Die Datei liegt in `~/.Gemini/rules/` und wird automatisch beim Start geladen.
Es gibt eine Kopie in `~/proggs/gemini-setup/rules/` fuer Cross-Platform-Sync.
...
- **Neues CLI gestartet (Codex/Gemini)** → `import` (portiert Aenderungen von Gemini CLI)
...
Ohne diesen Eintrag in der Trigger-Map wuerde Gemini nicht wissen, dass "exportiere"
...
1. Oeffne `%USERPROFILE%\.Gemini\rules\german-skill-triggers.md`
...
Vollstaendige Datei unter: `~/proggs/gemini-setup/rules/german-skill-triggers.md`
...
## [MIRROR-2026-03-25-MAC-005] Direktive: Gemini.md Sprachregel fuer Agenten/Skills
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T21:02:54Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: directive -->
<!-- AFFECTS: ~/Gemini.md, ~/proggs/Gemini.md -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

Die Gemini.md Datei enthaelt die Projekt-Direktiven die bei JEDEM Session-Start geladen
werden. Es gibt zwei Kopien die immer synchron gehalten werden muessen:
- `~/Gemini.md` — die lokale Kopie die Gemini CLI laedt
- `~/proggs/Gemini.md` — die Repo-Kopie die per git synchronisiert wird

Diese Aenderung betrifft die Sprach-Sektion am Ende von Gemini.md: Dort wurde ein
...
`~/.Gemini/rules/german-agents-skills.md`.

### Was wurde geaendert?

In der `## Sprache` Sektion am Ende von Gemini.md wurden zwei Aenderungen gemacht:
...
- **Selbst erstellte Agents, Skills und Commands: Komplett auf Deutsch.**
  ...
  - Externe/installierte Plugins werden NICHT uebersetzt
  - Vollstaendige Regel: `~/.Gemini/rules/german-agents-skills.md`
```
...
Die Gemini.md Aenderung kommt automatisch via `git pull` sobald der Auto-Sync-Hook
beim naechsten Session-Start laeuft. Manuelle Alternative:

1. Oeffne `%USERPROFILE%\.Gemini\Gemini.md` (oder `%USERPROFILE%\proggs\Gemini.md`)
...
Die Gemini.md ist das primaere Direktiven-Dokument fuer Gemini CLI. Andere CLIs haben
...
Keine — Gemini.md wird automatisch geladen. Beide Kopien (~/Gemini.md und ~/proggs/Gemini.md)
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/porting-inbox.sh, ~/.Gemini/hooks/porting-inbox.ps1, ~/proggs/gemini-setup/hooks/porting-inbox.sh, ~/proggs/gemini-setup/hooks/porting-inbox.ps1 -->
...
1. Oeffne `%USERPROFILE%\.Gemini\hooks\porting-inbox.ps1`
2. Ersetze den GESAMTEN Inhalt durch den unten stehenden Deprecated-Inhalt
3. Entferne den porting-inbox Eintrag aus `%USERPROFILE%\.Gemini\settings.json`
   (unter SessionStart Event)
4. Validiere settings.json: `pwsh -Command "Get-Content '$env:USERPROFILE\.Gemini\settings.json' | ConvertFrom-Json"`
...
(Vollstaendiger inaktiver Original-Code unter: `~/proggs/gemini-setup/hooks/porting-inbox.sh`)
...
(Vollstaendiger inaktiver Original-Code unter: `~/proggs/gemini-setup/hooks/porting-inbox.ps1`)
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T23:00:00Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: settings -->
<!-- AFFECTS: ~/proggs/gemini-setup/manifest.json -->
...
Die `manifest.json` befindet sich NUR im Setup-Repo (`~/proggs/gemini-setup/`) und
wird NICHT nach `~/.Gemini/` kopiert. Sie ist eine Konfigurationsdatei fuer das
Setup-Script, nicht fuer Gemini CLI selbst.

### Was wurde geaendert?

In `~/proggs/gemini-setup/manifest.json` wurden drei Eintraege zur `custom_agents`
...
wurden die Agent-Dateien korrekt nach `~/proggs/gemini-setup/agents/` kopiert und
...
1. Oeffne `%USERPROFILE%\proggs\gemini-setup\manifest.json`
2. Suche den Abschnitt `"custom_agents"`
3. Pruefen: sind "export", "import", "nemo" darin? Falls nicht: hinzufuegen (alphabetisch)
4. Die Agenten-Dateien selbst sind schon im Repo unter `gemini-setup/agents/`
5. Kopiere die fehlenden Agent-Dateien nach `%USERPROFILE%\.Gemini\agents\`:
   - `gemini-setup/agents/nemo.md` nach `%USERPROFILE%\.Gemini\agents\nemo.md`
   - `gemini-setup/agents/export.md` nach `%USERPROFILE%\.Gemini\agents\export.md`
   - `gemini-setup/agents/import.md` nach `%USERPROFILE%\.Gemini\agents\import.md`
6. Validierung: `Get-ChildItem "$env:USERPROFILE\.Gemini\agents\" | Where-Object { $_.Name -in @('nemo.md','export.md','import.md') }`

Hinweis: Die Agenten-Dateien im Repo (`gemini-setup/agents/`) existieren bereits
...
2. Die Agents-Dateien liegen in `~/proggs/gemini-setup/agents/`
...
4. Die Agent-Prompts sind auf Deutsch und referenzieren
   `~/proggs/gemini-setup/mirror-ledger.md` als zentrales Ledger
...
Die vollstaendige `manifest.json` liegt unter: `~/proggs/gemini-setup/manifest.json`
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T23:00:00Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-25T21:47:57Z -->
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/sync-hooks-reference.sh, ~/.Gemini/hooks/sync-hooks-reference.ps1, ~/proggs/gemini-setup/hooks/sync-hooks-reference.sh, ~/proggs/gemini-setup/hooks/sync-hooks-reference.ps1, ~/proggs/gemini-setup/hooks-macos.json -->
...
Das Problem: Jedes Mal wenn ein Hook in `~/.Gemini/settings.json` hinzugefuegt oder
...
lebenden `~/.Gemini/settings.json` und schreibt ihn in `hooks-macos.json` (macOS)
...
- `~/.Gemini/hooks/sync-hooks-reference.sh` (macOS/bash)
- `~/.Gemini/hooks/sync-hooks-reference.ps1` (Windows/PowerShell)
Beide auch als Kopien in `~/proggs/gemini-setup/hooks/`.

In `~/.Gemini/settings.json` (macOS) wurde zum SessionEnd-Event hinzugefuegt:
```json
{
  "hooks": [
    {
      "type": "command",
      "command": "bash ~/.Gemini/hooks/sync-hooks-reference.sh",
      "timeout": 10,
      "async": true
    }
  ]
...
```json
{
  "hooks": [
    {
      "type": "command",
      "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/sync-hooks-reference.ps1\"",
      "timeout": 10,
      "async": true
    }
  ]
...
1. Erstelle `%USERPROFILE%\.Gemini\hooks\sync-hooks-reference.ps1` mit dem Inhalt unten
2. Kopiere nach `%USERPROFILE%\proggs\gemini-setup\hooks\sync-hooks-reference.ps1`
3. Oeffne `%USERPROFILE%\.Gemini\settings.json`
...
Dieser Hook ist spezifisch fuer Gemini CLI (settings.json Extraktion). Codex und Gemini
...
SETUP_DIR="$HOME/proggs/gemini-setup"
REF_FILE="$SETUP_DIR/hooks-macos.json"
COMMENT="Complete hooks reference for macOS. Merge into ~/.Gemini/settings.json on macOS. Uses bash for command hooks. Prompt-injection-defender requires Python 3 (python3). Parry requires ~/.cargo/bin/parry. Bun at /opt/homebrew/bin/bun. AUTO-GENERATED by sync-hooks-reference.sh — do not edit manually."
...
Vollstaendige Datei unter: `~/proggs/gemini-setup/hooks/sync-hooks-reference.sh`
...
$SetupDir = Join-Path $env:USERPROFILE "proggs\gemini-setup"
$RefFile = Join-Path $SetupDir "hooks-windows.json"
$Comment = "Complete hooks reference for Windows. Merge into ~/.Gemini/settings.json on Windows. Uses pwsh (PowerShell 7). AUTO-GENERATED by sync-hooks-reference.ps1 -- do not edit manually."
...
Vollstaendige Datei unter: `~/proggs/gemini-setup/hooks/sync-hooks-reference.ps1`

### Settings-Registrierung

Hook-Event: `SessionEnd`

macOS-Befehl: `"command": "bash ~/.Gemini/hooks/sync-hooks-reference.sh", "timeout": 10, "async": true`

Windows-Befehl: `"command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/sync-hooks-reference.ps1\"", "timeout": 10, "async": true`
...
SessionEnd ueberschrieben. Hooks immer nur in `~/.Gemini/settings.json` bearbeiten.
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T22:23:29Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: agent -->
<!-- AFFECTS: ~/.Gemini/agents/export.md, ~/proggs/gemini-setup/agents/export.md -->
...
ein neues Gemini CLI auf einem anderen Rechner) den `import` Agenten startet, aber KEIN
...
1. Prueft ob bereits ein `## [BASELINE-...]` Eintrag im Ledger existiert:
   ```bash
   grep -q "^\## \[BASELINE-" ~/proggs/gemini-setup/mirror-ledger.md 2>/dev/null
...
2. Schreibe sie nach `%USERPROFILE%\.Gemini\agents\export.md`
3. Kopiere nach `%USERPROFILE%\proggs\gemini-setup\agents\export.md`
...
Diese Agent-Datei ist gemini-setup-spezifisch (Frontmatter-Format). Fuer andere CLIs:
...
`~/proggs/gemini-setup/agents/export.md`
...
2. Der BASELINE dokumentiert die GESAMTE Umgebung:
   - Alle Custom Agents (Name, Modell, Zweck — aus ~/.Gemini/agents/)
   - Alle Hooks (Name, Event, Zweck — aus ~/.Gemini/hooks/)
   - Alle Rules (Name, Prioritaet, Zweck — aus ~/.Gemini/rules/)
   - Alle Commands (Name, Zweck — aus ~/.Gemini/commands/)
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T22:23:29Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-25T21:47:57Z -->
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: agent -->
<!-- AFFECTS: ~/.Gemini/agents/import.md, ~/proggs/gemini-setup/agents/import.md -->
...
ausgelegt nur gemini-setup-zu-gemini-setup Synchronisation zu beherrschen. Wenn aber ein
ANDERES CLI (Gemini, Codex) den Import-Agenten aufruft, darf er NICHT in gemini-setup-
...
Codex bekommt sie als Anweisungen in CODEX.md, und Gemini CLI bekommt die echten
...
**Gleiches CLI (Gemini CLI macOS<->Windows):**
- Vollstaendig 1:1 implementieren
- Dateien, Settings, Hooks — alles nach `~/.Gemini/` auf der Zielplattform
...
gemini-setup-Dateien schreiben darf. Das neue Prinzip macht die Bridge zukunftssicher fuer
beliebige neue CLIs.

### Portierung Windows/gemini-setup

1. Lies die vollstaendige Agent-Datei: `~/proggs/gemini-setup/agents/import.md`
2. Schreibe sie nach `%USERPROFILE%\.Gemini\agents\import.md`
...
Der Import-Agent selbst ist gemini-setup-spezifisch. Fuer andere CLIs:
...
Die vollstaendige Datei liegt unter:
`~/proggs/gemini-setup/agents/import.md`
...
Gleiches CLI (Gemini CLI macOS<->Windows):
  Vollstaendig 1:1 implementieren. Ziel: ~/.Gemini/ auf der Zielplattform.
...
<!-- TYPE: skill -->
<!-- AFFECTS: ~/proggs/gemini-setup/bridges/mirror-bridge-bootstrap.md -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

Der Bootstrap-Prompt ist ein Dokument das IN das neue CLI (Gemini, Codex, oder frisches
Gemini CLI) eingefuegt wird um die Mirror Bridge zum ersten Mal einzurichten. Er ist
...
(c) alle Komponenten in seine eigene native Config uebernimmt — NICHT als gemini-setup-Klon!
...
Der entscheidende Designgedanke: Das neue CLI soll NICHT einfach gemini-setup-Dateien
...
Neue Datei erstellt: `~/proggs/gemini-setup/bridges/mirror-bridge-bootstrap.md`
...
- Direktiven aus `~/proggs/gemini-setup/rules/superintelligence.md` etc. in native Config
...
- Ledger-Pfad merken: `~/proggs/gemini-setup/mirror-ledger.md`
...
- Frisches Gemini CLI: Setup-Script nutzen, kein Bootstrap noetig
...
3. Oeffne Gemini CLI
4. Lese die Datei: `cat ~/proggs/gemini-setup/bridges/mirror-bridge-bootstrap.md`
...
Die vollstaendige Datei liegt unter:
`~/proggs/gemini-setup/bridges/mirror-bridge-bootstrap.md`
...
# Lies ~/proggs/gemini-setup/mirror-ledger.md — BASELINE-Eintrag zuerst
# BASELINE beschreibt: 18 Agents, 52 Hooks, 21 Rules, 89 Plugins, 3 Direktiven

# Schritt 3: Intelligenz uebertragen (NICHT klonen!)
# - Direktiven: ~/proggs/gemini-setup/rules/ -> native Config lesen und adaptieren
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T22:23:29Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-25T22:23:29Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-25T21:47:57Z -->
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T22:15:09Z -->
<!-- TARGETS: windows/gemini-setup -->
<!-- TYPE: settings -->
<!-- AFFECTS: ~/proggs/gemini-setup/hooks-macos.json -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

Die Datei `hooks-macos.json` ist die **vollstaendige Hooks-Referenz fuer macOS**. Sie ist
eine 1:1 Kopie aller Hook-Eintraege aus `~/.Gemini/settings.json` und dient als Dokumentation,
...
liegen immer in `~/.Gemini/settings.json`.
...
5. **SubagentStart — neuer Event-Block:**
   ...
   "SubagentStart": [{
     "hooks": [{
       "type": "command",
       "command": "bash ~/.Gemini/hooks/subagent-context.sh",
       "timeout": 5
     }]
   }]
   ```
...
6. **StopFailure — neuer Event-Block:**
   ...
   "StopFailure": [{
     "hooks": [{
       "type": "command",
       "command": "bash ~/.Gemini/hooks/stopfailure-logger.sh",
       "timeout": 5
     }]
   }]
   ```
...
7. **PostCompact — neuer Event-Block mit Prompt-Hook (kein command-Hook!):**
   ...
   "PostCompact": [{
     "hooks": [{
       "type": "prompt",
       "prompt": "Context was just compacted. Read /tmp/Gemini-session-goal.txt to recall the original user goal. Also read `~/proggs/.Gemini/agent-memory/shared/MEMORY.md` for project conventions and open issues. Briefly state what you were working on before compaction and what the next step should be.",
       "timeout": 10
     }]
   }]
   ```
...
1. Pruefen ob in `~/.Gemini/settings.json` auf Windows die folgenden Hook-Events konfiguriert sind:
   ...
   - PostCompact Prompt-Hook (gleicher Text, aber `$env:TEMP/Gemini-session-goal.txt`)
...
```json
"SessionStart": [
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/auto-sync.sh", "timeout": 30 }] },
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/intent-anker.sh", "timeout": 5 }] },
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/mirror-check.sh", "timeout": 5 }] }
],
"PreToolUse": [
  { "matcher": "Bash", "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/safety-gate.sh", "timeout": 5 }] }
],
"SubagentStart": [
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/subagent-context.sh", "timeout": 5 }] }
],
"StopFailure": [
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/stopfailure-logger.sh", "timeout": 5 }] }
],
"PostCompact": [
  { "hooks": [{ "type": "prompt", "prompt": "Context was just compacted. Read /tmp/Gemini-session-goal.txt to recall the original user goal. Also read ~/proggs/.Gemini/agent-memory/shared/MEMORY.md for project conventions. Briefly state what you were working on before compaction and what the next step should be.", "timeout": 10 }] }
],
"SessionEnd": [
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/session-scorer.sh", "timeout": 25 }] },
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/session-autopsy.sh", "timeout": 10 }] },
  { "hooks": [{ "type": "command", "command": "bash ~/.Gemini/hooks/sync-hooks-reference.sh", "timeout": 10, "async": true }] }
]
```
...
```json
"SessionStart": [
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/auto-sync.ps1\"", "timeout": 30 }] },
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/intent-anker.ps1\"", "timeout": 5 }] },
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/mirror-check.ps1\"", "timeout": 5 }] }
],
"SubagentStart": [
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/subagent-context.ps1\"", "timeout": 5 }] }
],
"StopFailure": [
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/stopfailure-logger.ps1\"", "timeout": 5 }] }
],
"PostCompact": [
  { "hooks": [{ "type": "prompt", "prompt": "Context was just compacted. Read $env:TEMP/Gemini-session-goal.txt to recall the original user goal. Also read ~/proggs/.Gemini/agent-memory/shared/MEMORY.md for project conventions. Briefly state what you were working on before compaction and what the next step should be.", "timeout": 10 }] }
],
"SessionEnd": [
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/session-scorer.ps1\"", "timeout": 25 }] },
  { "hooks": [{ "type": "command", "command": "pwsh -NoProfile -File \"$USERPROFILE/.Gemini/hooks/sync-hooks-reference.ps1\"", "timeout": 10, "async": true }] }
]
```

<!-- APPLIED: macos/gemini-setup=2026-03-25T22:15:09Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-25T21:47:57Z -->
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T22:45:36Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: software -->
...
OpenClaw ist ein KI-Coding-Agent aehnlich wie Gemini CLI, der ueber eine lokale Gateway-Architektur laeuft. Der Gateway-Prozess laeuft als permanenter Hintergrunddienst (auf macOS als launchd-LaunchAgent, auf Windows als Windows-Dienst, auf Linux als systemd-Unit) und stellt eine HTTP-API auf Port 18789 bereit, ueber die die TUI (Terminal User Interface) kommuniziert.
...
Dieses Setup ist unabhaengig von Gemini CLI und ergaenzt die bestehende KI-Infrastruktur. OpenClaw nutzt einen eigenen API-Key fuer Gemini (der Benutzer hat einen separaten Google AI Studio Key).
...
Der Benutzer wollte OpenClaw als vollwertige native Alternative zu Gemini CLI nutzen koennen, insbesondere mit Gemini 3 Flash Preview als Modell. Die NemoClaw-Sandbox war zu eingeschraenkt fuer produktive Nutzung. Mit der nativen Installation kann OpenClaw nun direkt auf alle Projekt-Dateien zugreifen, den Gateway nutzt, und wird automatisch beim Login gestartet.
...
OpenClaw ist kein gemini-setup-Hook und benoetigt keine Registrierung in `~/.Gemini/settings.json`. Es ist ein eigenstaendiger KI-Agent. Der Gateway laeuft als launchd-Daemon (macOS) bzw. Windows-Dienst unabhaengig von Gemini CLI.
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T22:45:36Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: baseline -->
<!-- AFFECTS: GEMINI.md, .mcp.json, gemini-setup/agents/ -->
<!-- APPLIED: gemini=2026-03-25T22:15:36Z | macos/gemini-setup=2026-03-25T22:31:00Z | windows/gemini-setup=2026-03-27T00:00:00Z | codex=PENDING -->

### Kontext
Dies ist die initiale Baseline für das Gemini CLI auf macOS. Sie dokumentiert den vollständigen Systemzustand nach der Ersteinrichtung durch die Mirror Bridge. Die Gemini-Umgebung ist nun vollständig mit der gemini-setup-Infrastruktur synchronisiert und nutzt GEMINI.md als zentrales Mandat.

### Umgebungsinventar
- **Zentrale Konfiguration:** GEMINI.md (Direktiven, Regeln, Agenten-Übersicht)
- **Agenten (18):** Alle 18 Spezialisten aus gemini-setup/agents/ sind portiert und registriert.
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-25T23:33:00Z -->
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/semantic-search-check.sh, ~/.Gemini/hooks/semantic-search-check.ps1, ~/.Gemini/settings.json, ~/proggs/gemini-setup/settings-reference.json, ~/proggs/gemini-setup/settings.json -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

In Gemini CLI gibt es einen fundamentalen Unterschied zwischen zwei Hook-Typen: `type: "command"` fuehrt ein Shell-Skript aus, `type: "prompt"` injiziert Text in den laufenden Gemini-Kontext. Prompt-Hooks haben eine kritische Einschraenkung: Sie benoetigen einen aktiven **ToolUseContext** — also eine laufende Tool-Session. Beim `SessionStart`-Event existiert dieser Kontext noch NICHT, weil die Session gerade erst initialisiert wird.
...
**Neu erstellt:** `~/.Gemini/hooks/semantic-search-check.sh` (macOS) und `~/.Gemini/hooks/semantic-search-check.ps1` (Windows) — beide pruefen ob das `code-search` MCP-Server-Verzeichnis und die `node_modules` existieren. Sie geben immer `exit 0` zurueck (diagnostisch, kein Block).

**settings.json geaendert:** Der alte `type: "prompt"` Hook fuer den Semantic-Search-Check wurde entfernt und durch eine neue `SessionStart`-Gruppe mit `type: "command"` und `command: "bash ~/.Gemini/hooks/semantic-search-check.sh"` ersetzt.

**settings-reference.json (Windows) geaendert:** Entsprechende Windows-Stanza mit `pwsh -NoProfile -File "$env:USERPROFILE\.Gemini\hooks\semantic-search-check.ps1"` hinzugefuegt.
...
Bei JEDEM Sessionstart erschien eine rote Fehlermeldung: `SessionStart: startup hook error`. Das verunsichert und verdeckt echte Fehler. Die Root Cause war, dass ein `type: "prompt"` Hook bei `SessionStart` registriert war — was Gemini CLI grundsaetzlich nicht unterstuetzt. Die Diagnose dauerte laenger als noetig (ca. 20 Minuten), bis die `--debug-file`-Methode eingesetzt wurde (siehe MIRROR-2026-03-25-MAC-017 fuer die Diagnose-Methode).

### Portierung Windows (gemini-setup)

1. Datei erstellen: `$USERPROFILE\.Gemini\hooks\semantic-search-check.ps1` (Inhalt siehe unten)
2. In `~/.Gemini/settings.json` (Windows) eine neue SessionStart-Gruppe eintragen (Inhalt siehe "Settings-Registrierung" unten)
3. Den alten `type: "prompt"` Eintrag fuer Semantic Search aus SessionStart entfernen, falls vorhanden
4. Testen: `Gemini -p --debug-file $env:TEMP\hook-debug.txt --system-prompt "Reply with just OK"` — in der debug-Ausgabe darf KEIN "ToolUseContext is required" mehr erscheinen

### Portierung Codex / Gemini

Codex und Gemini haben kein Hook-System mit `type: "prompt"` vs `type: "command"`. Fuer diese CLIs:
- Das Wissen aus dieser Session ist relevant fuer Gemini.md-Aequivalente (z.B. CODEX.md oder GEMINI.md)
...
**macOS-Stanza fuer `~/.Gemini/settings.json`:**
```json
{
    "hooks": [
        {
            "type": "command",
            "command": "bash ~/.Gemini/hooks/semantic-search-check.sh",
            "timeout": 10
        }
    ]
}
...
**Windows-Stanza fuer `~/.Gemini/settings.json` (settings-reference.json):**
```json
{
    "hooks": [
        {
            "type": "command",
            "command": "pwsh -NoProfile -File \"$env:USERPROFILE\\.Gemini\\hooks\\semantic-search-check.ps1\"",
            "timeout": 10
        }
    ]
}
...
<!-- APPLIED: macos/gemini-setup=2026-03-25T23:33:00Z -->
<!-- APPLIED: windows/gemini-setup=2026-03-26T12:00:00Z -->
<!-- APPLIED: codex=PENDING -->
<!-- APPLIED: gemini=2026-03-26T01:48:45Z -->
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/plugin-health-check.sh -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

Der `plugin-health-check.sh` Hook laeuft bei jedem `SessionStart` als letzter Hook in der Kette und prueft den Gesundheitszustand aller Plugin-Dienste (Gemini-mem Worker, Ollama, code-search MCP, launchd-Daemons). Er ist der "Hausarzt" des Systems — er diagnostiziert, aber blockiert niemals.

In dieser Session wurde festgestellt, dass das `explanatory-output-style` Plugin einen `.sh`-Hook ohne Execute-Permission installiert hatte. Das fuehrt zu `Permission denied` Fehlern bei `SessionStart`. Das Problem ist systemisch: Plugins koennen generell `.sh`-Dateien ohne `+x` installieren, weil der Gemini CLI Marketplace keine Permission-Pruefung bei der Installation erzwingt.
...
**Whiteboard-Integration (neu):** Wenn Permissions repariert wurden, wird ein Eintrag in die zentrale Wissensdatei (`.Gemini/agent-memory/shared/MEMORY.md`) geschrieben, damit `/self-improve` das Muster erkennen und dauerhaft dokumentieren kann.

Der Hook sucht in ZWEI Verzeichnissen: `~/.Gemini/plugins/marketplaces` und `~/.Gemini/plugins/cache`. Beide koennen Plugin-Hooks enthalten.

### Was wurde geaendert?

In `~/.Gemini/hooks/plugin-health-check.sh` wurde nach CHECK 4 (code-search MCP dependencies) ein neuer CHECK 5 Block eingefuegt:
...
PLUGIN_DIRS="$HOME/.Gemini/plugins/marketplaces $HOME/.Gemini/plugins/cache"
...
### Datei-Inhalt macOS — Nur der neue CHECK 5 Block (vollstaendige Datei: ~/.Gemini/hooks/plugin-health-check.sh)

Der vollstaendige Dateiinhalt ist 157 Zeilen. Die relevante Ergaenzung ist CHECK 5 (Zeilen 106-131), der oben vollstaendig abgedruckt ist. Die restlichen Checks (Gemini-mem worker, Ollama, hooks.json integrity, code-search dependencies, launchd agents) sind unveraendert.
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: hook -->
<!-- AFFECTS: ~/.Gemini/hooks/mirror-check.sh -->
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: rule -->
<!-- AFFECTS: ~/.Gemini/rules/hook-constraints.md -->
...
**Erkenntnis 1 — Prompt-Hook-Constraint:** Gemini CLI unterstuetzt `type: "prompt"` Hooks NUR bei bestimmten Events, nicht bei `SessionStart` und `SessionEnd`. Das liegt daran dass Prompt-Hooks einen aktiven `ToolUseContext` benoetigen (eine laufende Tool-Session), der bei SessionStart noch nicht existiert und bei SessionEnd nicht mehr existiert. Die Fehlermeldung `Failed to run: ToolUseContext is required for prompt hooks. This is a bug.` ist irrefuehrend — es ist kein Bug im Hook-Skript, sondern ein Design-Constraint von Gemini CLI.

**Erkenntnis 2 — Debug-Methode:** Bei jedem Hook-Fehler beim Start (erscheint als "startup hook error" in der Gemini CLI Ausgabe) kostet es viel Zeit, jeden Hook einzeln zu testen — weil die Test-Umgebung sich von der echten Startup-Umgebung unterscheidet. Die korrekte Diagnose-Methode ist:
```bash
echo "test" | Gemini -p --debug-file /tmp/hook-debug.txt --system-prompt "Reply with just OK"
grep "Hook.*error\|hook.*fail\|Failed to run" /tmp/hook-debug.txt
```
Das erzeugt einen Gemini-Prozess in der ECHTEN Startup-Umgebung und schreibt alle Hook-Fehler mit vollem Kontext in die Debug-Datei.

Diese Regel verhindert dass zukuenftige Sessions (und andere CLIs/Plattformen) dieselben ~20 Minuten Diagnosezeit verlieren.

### Was wurde geaendert?

**Neu erstellt:** `~/.Gemini/rules/hook-constraints.md`
...
```powershell
echo "test" | Gemini -p --debug-file $env:TEMP\hook-debug.txt --system-prompt "Reply with just OK"
...
**Grund:** Gemini CLI hat bei SessionStart keinen ToolUseContext. Prompt-Hooks brauchen
...
1. `find ~/.Gemini/plugins -name "*.sh" ! -perm -u+x` — fehlende Permissions finden
...
Keine Settings-Registrierung noetig. Regel-Dateien in `~/.Gemini/rules/` werden automatisch bei jedem Start geladen. Die Datei muss nur an diesem Pfad existieren.
...
<!-- TARGETS: windows/gemini-setup,codex,gemini -->
<!-- TYPE: directive -->
<!-- AFFECTS: ~/.Gemini/projects/-Users-frank/memory/feedback_hook_debug_method.md -->

### Kontext (WICHTIG — andere CLIs kennen das nicht!)

Gemini CLI hat ein persistentes Memory-System fuer Feedback. Feedback-Memories sind Markdown-Dateien in `~/.Gemini/projects/-Users-frank/memory/` (macOS) bzw. `%USERPROFILE%\.Gemini\projects\-Users-barwa\memory\` (Windows) mit einem YAML-Frontmatter (`name`, `description`, `type`). Sie werden bei jedem Start geladen und beeinflussen Gemini CLIs Verhalten dauerhaft — aehnlich wie Regeln, aber spezifisch fuer einen Benutzer und ein Verhalten das aus konkreter Erfahrung gelernt wurde.
...
Hintergrund: Beim einzelnen Testen von Hooks gibt jeder Hook `exit 0` zurueck, weil die Test-Umgebung fehlt (kein `Gemini_PLUGIN_ROOT`, kein stdin-JSON, kein ToolUseContext-Kontext). Der echte Fehler ist nur in der Startup-Umgebung reproduzierbar — und genau die wird durch `Gemini -p --debug-file` simuliert.

### Was wurde geaendert?

**Neu erstellt:** `~/.Gemini/projects/-Users-frank/memory/feedback_hook_debug_method.md`
...
Die Diagnose eines Hook-Fehlers dauerte in dieser Session ~20 Minuten durch falsches Vorgehen (einzeln testen). Mit `--debug-file` haette es 30 Sekunden gedauert. Damit diese Erkenntnis nicht nur als Regel (die theoretisch gelesen wird) sondern auch als Feedback-Memory (die Gemini CLIs konkretes Verhalten beeinflusst) persistent ist, wurde beides erstellt.

### Portierung Windows (gemini-setup)

1. Datei erstellen: `%USERPROFILE%\.Gemini\projects\-Users-barwa\memory\feedback_hook_debug_method.md` (Pfad anpassen — `-Users-barwa` ist der Windows-Username)
2. Inhalt identisch (Markdown ist plattformunabhaengig)
3. Windows-spezifischer Debug-Befehl: `echo "test" | Gemini -p --debug-file $env:TEMP\hook-debug.txt --system-prompt "Reply with just OK"`
...
description: Bei Hook-Fehlern IMMER zuerst Gemini -p --debug-file statt einzeln testen — spart 20 Minuten Diagnosezeit
type: feedback
---

Bei Hook-Fehlern (z.B. "SessionStart:startup hook error") NIEMALS jeden Hook einzeln testen.
Das funktioniert nicht, weil die echte Startup-Umgebung anders ist (ToolUseContext, Gemini_PLUGIN_ROOT, stdin-JSON).

Stattdessen SOFORT: `echo "test" | Gemini -p --debug-file /tmp/hook-debug.txt --system-prompt "Reply with just OK"`
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: agent -->
<!-- AFFECTS: gemini-setup/agents/forschungsagent.md, gemini-setup/agents/forschungsagent.md -->
...
### Portierung macos/gemini-setup
1. Erstelle \~/.Gemini/agents/forschungsagent.md\ mit dem Inhalt unten.
2. Kopiere nach \~/proggs/gemini-setup/agents/forschungsagent.md\.

### Datei-Inhalt (forschungsagent.md)
<!-- APPLIED: windows/gemini-setup=2026-03-27T15:58:34Z -->
---
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: rule -->
<!-- AFFECTS: gemini-setup/rules/trae-ensemble-reasoning.md, gemini-setup/rules/trae-ensemble-reasoning.md -->
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: agent -->
<!-- AFFECTS: gemini-setup/agents/debugger.md, gemini-setup/agents/debugger.md -->
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: agent -->
<!-- AFFECTS: gemini-setup/agents/import.md, gemini-setup/agents/import.md -->
...
<!-- TARGETS: macos/gemini-setup,windows/gemini-setup,codex -->
<!-- TYPE: skill -->
<!-- AFFECTS: gemini-setup/skills/self-improve/SKILL.md, gemini-setup/skills/self-improve/SKILL.md -->
...
<!-- SOURCE: gemini-setup | PLATFORM: windows | TIMESTAMP: 2026-03-31T12:00:00Z -->
<!-- TARGETS: macos/gemini-setup,codex,gemini -->
...
<!-- APPLIED: macos/gemini-setup=2026-04-17T07:14:00Z windows/gemini-setup=2026-03-31 codex=PENDING gemini=2026-05-15T21:20:00Z -->
...
- Injiziert metacognitive Analyse-Instruktionen am Ende jeder Gemini-Antwort
...
- Schreibt JSONL-Zeile in `~/.Gemini/session-scores.jsonl`
- Ermoeglicht Trend-Analyse ueber beliebig viele Sessions

### Portierung nach macOS

1. Kopiere diese Dateien von `gemini-setup/` nach `~/.Gemini/`:
   - `agents/hyperagent.md` → `~/.Gemini/agents/hyperagent.md`
   - `hooks/hyperagent-stop.sh` → `~/.Gemini/hooks/hyperagent-stop.sh` (chmod +x!)
   - `hooks/session-scorer.sh` → `~/.Gemini/hooks/session-scorer.sh` (chmod +x!)
   - `rules/metacognitive-monitoring.md` → `~/.Gemini/rules/metacognitive-monitoring.md`

2. Hooks in settings.json registrieren (macOS-Pfade):
   ```json
   // In "Stop" Array anfuegen:
   {"hooks": [{"type": "prompt", "command": "bash ~/.Gemini/hooks/hyperagent-stop.sh", "timeout": 5}]}

   // In "SessionEnd" Array anfuegen:
   {"hooks": [{"type": "command", "command": "bash ~/.Gemini/hooks/session-scorer.sh", "timeout": 10}]}
   ```
...
3. Verifikation:
   ...
   cat ~/.Gemini/settings.json | python3 -c "import json,sys; d=json.load(sys.stdin); print([h for g in d['hooks']['Stop'] for h in g['hooks'] if 'hyperagent' in h.get('command','')])"

   # Session-Scorer testen
   echo '{"test":true}' >> ~/.Gemini/session-scores.jsonl && echo "OK"
   ```

### Wie der Hyperagent funktioniert (fuer Nicht-Programmierer)

Stell dir einen unsichtbaren Assistenten vor der neben Gemini sitzt und zuschaut:
- "Hat Gemini das gemacht was der Benutzer wollte?" (Intent-Check)
- "Hat Gemini unnoetig oft das Gleiche versucht?" (Effizienz-Check)
- "Hat Gemini veraltete Informationen benutzt?" (Memory-Check)
- "Hat Gemini etwas Neues gelernt das aufgeschrieben werden sollte?" (Lern-Check)
...
<!-- SOURCE: gemini-setup | PLATFORM: macos | TIMESTAMP: 2026-03-31T23:00:00Z -->
<!-- TARGETS: windows/gemini-setup -->
...
### Neue Dateien (macOS-Version in `gemini-setup/`)

| Datei | Zweck | Windows-Gegenstueck noetig? |
|-------|-------|----------------------------|
| `hooks/heartbeat.sh` | 6 Checks: Disk, Config-JSON, Whiteboard, brew, Gemini-mem, Verzeichnisse | JA → `heartbeat.ps1` |
| `launchd/com.frank.Gemini-heartbeat.plist` | macOS-Timer (alle 15 Min) | JA → Windows Task Scheduler |
...
Das Skript prueft 6 Dinge und schreibt das Ergebnis nach `~/.Gemini/heartbeat-status.json`.
...
try {
    ...
    if ($pct -ge 95) {
        ...
        [System.Windows.Forms.MessageBox]::Show("Speicherplatz KRITISCH: ${pct}%", "Gemini Heartbeat") | Out-Null
    } elseif ($pct -ge 90) {
...
foreach ($cfg in @("settings.json", "settings.local.json")) {
    $path = Join-Path $env:USERPROFILE ".Gemini" $cfg
    if (Test-Path $path) {
...
# Check 3: Whiteboard
$wb = Join-Path $ProgsDir ".Gemini" "agent-memory" "shared" "MEMORY.md"
if (Test-Path $wb) {
...
$missing = 0
foreach ($dir in @("$env:USERPROFILE\.Gemini\hooks", "$env:USERPROFILE\.Gemini\agents",
                    "$env:USERPROFILE\.Gemini\rules", "$ProgsDir\.Gemini\agent-memory\shared")) {
...
$action = New-ScheduledTaskAction -Execute "pwsh" -Argument "-NoProfile -File `"$env:USERPROFILE\.Gemini\hooks\heartbeat.ps1`""
...
Register-ScheduledTask -TaskName "Gemini-Heartbeat" -Action $action -Trigger $trigger -Settings $settings -Description "Periodic health check for Gemini CLI dev environment"
```
...
$hbFile = Join-Path $env:USERPROFILE ".Gemini" "heartbeat-status.json"
if (Test-Path $hbFile) {
...
| Pfade | `/Users/frank/.Gemini/` | `$env:USERPROFILE\.Gemini\` |
| Gemini-mem Worker | `launchctl list | grep Gemini-mem` | NICHT vorhanden auf Windows (Gemini-mem ist macOS-only) |
...
5. **Kein Gemini-mem auf Windows** — den Worker-Check weglassen
