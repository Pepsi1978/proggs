...
**Before testing**: Read `.Gemini/agent-memory/shared/MEMORY.md` (the whole file) for known patterns, conventions, and known failure patterns ("Offene Fehler & Probleme") that tests should specifically cover.
**After testing**: If you discover recurring test failures, flaky patterns, or missing test infrastructure, add a line under "Erkenntnisse aus Tests" in `.Gemini/agent-memory/shared/MEMORY.md`. Keep entries to 1 line each.
