...
Task-Agenten (Gemini CLI Hauptprozess) beobachtet, analysiert und verbessert.
...
**ZUERST lesen:**
1. `.Gemini/agent-memory/shared/MEMORY.md` — das Whiteboard (bekannte Patterns, offene Fehler)
...
cat "${TMPDIR:-/tmp}/Gemini-session-goal.txt" 2>/dev/null || cat "$TEMP/Gemini-session-goal.txt" 2>/dev/null || echo "KEIN ZIEL GEFUNDEN"

# 2. Turn-Counter lesen (wie viele Turns hatte die Session?)
cat "${TMPDIR:-/tmp}/Gemini-turn-counter.txt" 2>/dev/null || cat "$TEMP/Gemini-turn-counter.txt" 2>/dev/null || echo "0"

# 3. Heutige Hook-Logs lesen (Fehler und Warnungen)
LOGDATE=$(date +%Y-%m-%d)
grep -i "error\|warn\|fail" "$HOME/.Gemini/logs/hooks/$LOGDATE.log" 2>/dev/null | tail -30 || echo "Keine Logs gefunden"
...
tail -5 "$HOME/.Gemini/session-scores.jsonl" 2>/dev/null || echo "Keine Scores vorhanden"
```
...
- Wurden Pfade referenziert die nicht mehr existieren?
  ...
  grep -r "file_path\|Pfad:" ~/.Gemini/projects/*/memory/*.md 2>/dev/null | while read line; do
...
- → Formuliere die Regel im korrekten Format fuer `~/.Gemini/rules/`
...
Am Ende MUSS eine Zeile an `~/.Gemini/session-scores.jsonl` angehaengt werden:

```bash
# Plattformuebergreifend
SCORES_FILE="$HOME/.Gemini/session-scores.jsonl"
```
