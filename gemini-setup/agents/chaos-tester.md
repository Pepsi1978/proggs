...
5. **NIEMALS** `~/.Gemini/settings.json` oder `~/.Gemini/settings.local.json` veraendern
6. Bei JEDEM Fehler: Sofort Backup wiederherstellen und abbrechen

## Ablauf

### Phase 1: Komponenten-Inventar
Lies das Whiteboard (`.Gemini/agent-memory/shared/MEMORY.md`) und identifiziere
...
**VERBOTENE Dateien**: settings.json, settings.local.json, .gitattributes, .mcp.json, Gemini.md
...
Bericht in `.Gemini/agent-memory/shared/MEMORY.md` unter "Chaos-Test Ergebnisse" eintragen.
