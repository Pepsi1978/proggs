...
cat "${TMPDIR:-/tmp}/Gemini-session-goal.txt" 2>/dev/null \
  || cat "$TEMP/Gemini-session-goal.txt" 2>/dev/null \
  || echo "KEIN ZIELDOKUMENT GEFUNDEN — Ziel aus Kontext rekonstruieren"

# 2. Letzte Session-Scores (Trend der letzten 5 Sessions)
tail -5 "$HOME/.Gemini/session-scores.jsonl" 2>/dev/null \
  || echo "Keine Session-Scores vorhanden"

# 3. Letzte 5 Experience-Eintraege (Strategie-Historie)
tail -5 "$HOME/../proggs/.Gemini/agent-memory/shared/experience-store.jsonl" 2>/dev/null \
  || tail -5 ~/proggs/.Gemini/agent-memory/shared/experience-store.jsonl 2>/dev/null \
  || echo "Kein Experience Store gefunden"

# 4. Letzte 5 Bug-Cases (bekannte Fallstricke)
tail -5 ~/proggs/.Gemini/agent-memory/shared/bug-cases.jsonl 2>/dev/null \
  || echo "Keine Bug-Cases gefunden"

# 5. Whiteboard lesen (offene Fehler, bewaehrte Muster)
head -80 ~/proggs/.Gemini/agent-memory/shared/MEMORY.md 2>/dev/null \
...
- Wurde nach Erlaubnis gefragt fuer Aktionen die laut Gemini.md autonom ausgefuehrt werden sollen?
