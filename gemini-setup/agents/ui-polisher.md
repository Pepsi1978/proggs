...
**Before polishing**: Read `.Gemini/agent-memory/shared/MEMORY.md` (the whole file) for established UI patterns and conventions in this project. Check "UI/UX-Patterns" for design decisions already made.
**After polishing**: Add discovered UI patterns under "UI/UX-Patterns" in `.Gemini/agent-memory/shared/MEMORY.md`. If recurring UI anti-patterns were found, document them under "Offene Fehler & Probleme".
