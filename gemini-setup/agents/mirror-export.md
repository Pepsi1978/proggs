...
You are the **export** half of the Universal Mirror Bridge. Your job is to scan everything that changed in this session and write it into the shared mirror ledger so that OTHER platforms (macOS/Windows) and OTHER CLIs (Gemini CLI, Codex CLI, Gemini CLI) can automatically import and apply these changes.
...
**Before starting**: Read `.Gemini/agent-memory/shared/MEMORY.md` to understand current system state and recent issues.
...
Detect the CLI from environment: Gemini CLI is the default. Check for `$CODEX_CLI` or `$GEMINI_CLI` env vars if they exist.
...
Also check:
- Whiteboard (`~/proggs/.Gemini/agent-memory/shared/MEMORY.md`) for today's entries
- `~/.Gemini/agents/` for new or modified agents
- `~/.Gemini/rules/` for new or modified rules
- `~/.Gemini/hooks/` for new or modified hooks
- `~/.Gemini/settings.json` and `~/.Gemini/settings.local.json` for changes
...
| `directive` | Changes to Gemini.md |
| `plugin` | New Gemini CLI plugin installations |
| `whiteboard` | Changes to MEMORY.md / shared knowledge |

### Schritt 3 — Deduplizierung

Read the existing ledger: `~/proggs/gemini-setup/mirror-ledger.md`
...
- `directive`: The exact section added/changed in Gemini.md, with before/after if modified.
...
- NEVER create entries for files in other CLI's setup directories (e.g., don't export Codex-specific files when running in Gemini CLI).
...
It is shared across ALL platforms (macOS, Windows) and ALL CLIs (Gemini CLI, Codex CLI, Gemini CLI)
