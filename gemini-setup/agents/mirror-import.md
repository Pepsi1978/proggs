...
**Before starting**: Read `.Gemini/agent-memory/shared/MEMORY.md` to understand current system state and avoid known issues.
...
uname -s
```
- `Darwin` → macOS, platform key: `macos/gemini-setup`
- `MINGW`/`MSYS`/`Windows` → Windows, platform key: `windows/gemini-setup`
...
Read the full file: `~/proggs/gemini-setup/mirror-ledger.md`
...
|---|-----|-----|--------|-------|-------------|
| 1 | MIRROR-2026-03-25-MAC-001 | rule | macOS/gemini-setup | 2026-03-25 | ... |

### Review noetig:
| # | ID | Typ | Quelle | Datum | Beschreibung |
|---|-----|-----|--------|-------|-------------|
| 1 | MIRROR-2026-03-25-MAC-002 | hook | macOS/gemini-setup | 2026-03-25 | ... |
```
...
2. Write it as a `.md` file in `~/.Gemini/rules/auto-learned/` (create directory if needed)
3. Copy to `~/proggs/gemini-setup/rules/auto-learned/`
→ **AUTO** (no user confirmation needed)

#### Type: `rule`
1. Extract the complete rule content from the entry
2. Write to `~/.Gemini/rules/{filename}.md`
3. Copy to `~/proggs/gemini-setup/rules/{filename}.md`
→ **AUTO**

#### Type: `directive`
1. Extract the Gemini.md change from the entry
2. Apply to BOTH copies: `~/Gemini.md` AND `~/proggs/Gemini.md`
...
2. Insert into `~/proggs/.Gemini/agent-memory/shared/MEMORY.md` under the correct section header
→ **AUTO**

#### Type: `hook`
1. Extract the correct platform variant (.sh for macOS, .ps1 for Windows) from the entry
2. Write the file to `~/.Gemini/hooks/{hookname}.{ext}`
3. On macOS: `chmod +x` the .sh file
4. Extract the settings registration JSON from the entry
5. Add the hook stanza to `~/.Gemini/settings.json`
6. Apply the **3-file rule**: Also update:
   - `~/proggs/gemini-setup/settings-reference.json` (Windows reference)
   - `~/proggs/gemini-setup/settings.json` (macOS reference)
   - Hook file copy to `~/proggs/gemini-setup/hooks/`
7. Validate JSON: `python3 -m json.tool ~/.Gemini/settings.json > /dev/null`
→ **SHOW DIFF** before applying

#### Type: `settings`
1. Extract the JSON stanza from the entry
2. Insert into `~/.Gemini/settings.json` at the correct location
3. Apply the **3-file rule** (update all 3 settings files in setup-repo)
4. Validate JSON: `python3 -m json.tool ~/.Gemini/settings.json > /dev/null`
→ **SHOW DIFF** before applying

#### Type: `agent`
1. Extract the complete agent file (frontmatter + prompt)
2. Write to `~/.Gemini/agents/{agentname}.md`
3. Copy to `~/proggs/gemini-setup/agents/{agentname}.md`
...
```bash
cd ~/proggs
git add gemini-setup/mirror-ledger.md
git add -A gemini-setup/  # Include any new files added to setup-repo
git commit -m "#NNN - import: apply N entries from mirror-ledger on {platform}

Co-Authored-By: Gemini Opus 4.6 (1M context) <noreply@anthropic.com>"
git push
```
...
| `/tmp/` | `$env:TEMP\` |
| `bash ~/.Gemini/hooks/X.sh` | `pwsh -NoProfile -File "$USERPROFILE/.Gemini/hooks/X.ps1"` |
