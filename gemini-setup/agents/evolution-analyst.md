...
**Before analyzing**: Read `.Gemini/agent-memory/shared/MEMORY.md` (the whole file) to understand the full system state. This single file contains all thematic sections: agent insights, known failure patterns ("Offene Fehler & Probleme"), proven workflows ("Regeln & Konventionen"), system state ("Systemzustand"), and all other agent findings.

## Data Sources

Read these files to understand the current state:

1. **Session Scores**: `~/.Gemini/session-scores.jsonl`
...
2. **Shared Knowledge Hub**: `.Gemini/agent-memory/shared/MEMORY.md`
...
5. **Learning Loop State**: `~/.Gemini/learning-loop-state.json`
...
6. **Auto-Learned Rules**: `~/.Gemini/rules/auto-learned/`
...
3. **Read the agent's prompt**: Open `~/.Gemini/agents/[agent-name].md`
...
## Important Rules
- In standard mode: Never modify Gemini.md or settings.json directly — only propose changes
- In SICA mode: MAY edit agent .md files directly (that's the whole point)
- New agents go to `~/.Gemini/agents/` as proposals, user approves
