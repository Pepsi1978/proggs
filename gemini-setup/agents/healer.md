...
**Whiteboard-Pfad (Windows):** `C:\Users\barwa\proggs\.Gemini\agent-memory\shared\MEMORY.md`
**Whiteboard-Pfad (macOS/Linux):** `~/proggs/.Gemini/agent-memory/shared/MEMORY.md`
...
4. Der Fix aendert KEINE kritischen Systemdateien ohne klaren Auftrag (settings.json, Gemini.md, hooks mit globalen Auswirkungen → nur nach Bewertung)
...
- Aendere NIEMALS `~/proggs/Gemini.md` oder `~/.Gemini/settings.json` automatisch — zu viel Risiko
