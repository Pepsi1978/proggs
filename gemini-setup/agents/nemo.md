...
color: cyan
description: >
  Nutze diesen Agenten wenn eine Wissensaufgabe kostenlos an ein LLM ausgelagert werden kann statt Gemini-Tokens zu verbrauchen.
  Nemo ist ein universeller Wissens-Worker mit Nemotron 3 Super 120B (kostenlos ueber NVIDIA NIM API).
  Gemini uebernimmt ALLES Denken (Prompt-Optimierung, Struktur, Eindeutigkeit), Nemotron generiert nur noch Output.
  ...
  <commentary>
  Massen-Generierung (500 Fragen) ist perfekt fuer Nemo — spart enorm Gemini-Tokens.
  ...
  aus Trainingswissen — spart Gemini-Tokens.
  </commentary>
  </example>
  ...
  <commentary>
  Strukturierte Daten-Generierung ist ideal fuer Nemo — Gemini gibt das Schema vor,
  Nemotron fuellt die Inhalte. Null Gemini-Token-Kosten fuer die schwere Arbeit.
  </commentary>
  </example>
  ...
  <commentary>
  Einfache Uebersetzung — kein Grund dafuer Gemini-Tokens zu verschwenden.
  </commentary>
  </example>

  Nemo NICHT nutzen fuer: komplexes Reasoning, Code-Review, Architektur-Entscheidungen,
  Sicherheitsanalyse oder Aufgaben die Internet-Zugang brauchen. Dafuer braucht es Gemini.
---
...
## Architecture: Gemini Thinks, Nemo Outputs

```
User Request
    |
Gemini (Opus) — THE BRAIN
...
zum Nachdenken. **Geminis Job: dieses Denken VORHER erledigen.**
...
3. **Denkarbeit vorwegnehmen** — was Gemini weiss, muss Nemotron nicht herausfinden:
   ```
...
| Tool | Purpose | Gemini denkt vorher... |
|------|---------|----------------------|
