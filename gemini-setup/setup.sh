...
echo ""
echo -e "${BLUE}=============================================${NC}"
echo -e "${BLUE}  Gemini CLI Setup v2.0 - macOS / Linux      ${NC}"
echo -e "${BLUE}=============================================${NC}"
echo ""
...
if ! command -v gemini &> /dev/null; then
    # Versuche alternativen Namen falls gemini nicht direkt gefunden wird
    if ! command -v gmini &> /dev/null; then
        MISSING="$MISSING gemini"
        echo -e "  ${RED}✗ Gemini CLI nicht gefunden${NC}"
    else
        GEMINI_CMD="gmini"
        echo -e "  ${GREEN}✓ Gemini CLI (als gmini) gefunden${NC}"
    fi
else
    GEMINI_CMD="gemini"
    echo -e "  ${GREEN}✓ Gemini CLI gefunden${NC}"
fi
...
if [ -f "$GEMINI_SETTINGS" ]; then
    if grep -q "superpowers-marketplace" "$GEMINI_SETTINGS" 2>/dev/null; then
...
for PLUGIN in swift-lsp typescript-lsp csharp-lsp clangd-lsp superpowers frontend-design context7 feature-dev code-simplifier commit-commands security-guidance gemini-md-management pr-review-toolkit gemini-cli-setup hookify playground ralph-loop code-review playwright coderabbit explanatory-output-style skill-creator plugin-dev agent-sdk-dev; do
    echo -ne "  $PLUGIN@gemini-plugins-official ... "
    if $GEMINI_CMD plugins install "$PLUGIN@gemini-plugins-official" 2>/dev/null; then
        echo -e "${GREEN}✓${NC}"
...
for PLUGIN in episodic-memory double-shot-latte superpowers-chrome gemini-session-driver superpowers-lab superpowers-developing-for-gemini-cli elements-of-style; do
    echo -ne "  $PLUGIN@superpowers-marketplace ... "
    if $GEMINI_CMD plugins install "$PLUGIN@superpowers-marketplace" 2>/dev/null; then
        echo -e "${GREEN}✓${NC}"
        INSTALLED=$((INSTALLED + 1))
    else
        echo -e "${YELLOW}(bereits installiert oder Fehler)${NC}"
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    for PLUGIN_SPEC in "apple-platform-build-tools@apple-platform-build-tools"; do
        echo -ne "  $PLUGIN_SPEC ... "
        if $GEMINI_CMD plugins install "$PLUGIN_SPEC" 2>/dev/null; then
            echo -e "${GREEN}✓${NC}"
            INSTALLED=$((INSTALLED + 1))
        else
            echo -e "${YELLOW}(bereits installiert oder Fehler)${NC}"
...
# Sequential Thinking
if $GEMINI_CMD mcp list 2>/dev/null | grep -q "sequential-thinking"; then
    echo -e "  ${GREEN}✓ sequential-thinking bereits konfiguriert${NC}"
else
    echo -ne "  sequential-thinking installieren ... "
    if $GEMINI_CMD mcp add --scope user sequential-thinking -- npx -y @modelcontextprotocol/server-sequential-thinking 2>/dev/null; then
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "${RED}✗${NC}"
    fi
...
# ---------------------------------------------------
echo -e "\n=== Deploying Settings ==="
SETTINGS_DST="$HOME/.gemini/settings.json"
...
echo "  Skills:          5 (auto-verify-iterate, cross-platform, undo-changes, android-clean-architecture, android-ninja)"
echo "  Hooks:          $hook_count Dateien deployed nach ~/.gemini/hooks/"
...
echo "  Marketplaces:    2 (gemini-plugins-official, superpowers-marketplace)"
echo ""
echo -e "  ${YELLOW}Hinweis: Starte Gemini CLI neu, damit alle Plugins aktiv werden:${NC}"
echo "  gemini"
echo ""
