...
In ALLEN deutschen Ausgaben + Programmierarbeiten echte Umlaute **ä/ö/ü/ß** — NIE "ae/oe/ue/ss". Gilt:
Chat, Code-Kommentare, UI-Strings, Doku, Memory, Commits, Logs, neue Agents/Skills/Commands. ASCII
erlaubt: Dateinamen/Pfade, Code-Variablen, URLs/Hashes/IDs, englische Texte + Commits, **bestehende
...
technische Bezeichner (MCP/API/JSON), Commits. Externe Plugins NICHT uebersetzen. Trigger-Map →
`german-skill-triggers.md`.
...
"Feature X nutze ich nicht" → nicht widersprechen, sondern anbieten zu entfernen oder direkt entfernen.

## 5. Links automatisch oeffnen

Empfohlene Webseite SOFORT oeffnen: tiefsten Link bauen → `start "URL"` (Windows) / `open "URL"` (macOS)
→ dann erklaeren. Direkt-Links: Google Cloud
