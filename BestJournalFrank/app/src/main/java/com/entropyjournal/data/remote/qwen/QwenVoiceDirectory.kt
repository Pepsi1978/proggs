package com.entropyjournal.data.remote.qwen
...
/** A voice that was cloned from a recording and lives in the user's Model Studio account. */
        ...
        try {
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    Log.e(TAG, "list failed ${response.code}: ${text.take(300)}")
            ...
            Log.e(TAG, "list failed: ${error.message}", error)
            emptyList()
        }
    }
