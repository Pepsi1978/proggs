...
constructor(
    ...
    suspend fun signIn(activityContext: Context): Result<UserProfile> {
        return try {
            // GetSignInWithGoogleOption forces the classic account picker UI instead of the
            // CredentialSelectorActivity bottom-sheet, which crashes on Samsung One UI / Android 16
            // with android.os.DeadObjectException during process init.
            val signInOption =
                GetSignInWithGoogleOption.Builder(Constants.GOOGLE_WEB_CLIENT_ID).build()
