...
Stand: 09.08.2026 · Stufe: v2 · Plattform(en): Android
...
| F-26 | Erscheinung umschalten | B-01, B-08 | Kern |
| **F-27** `NEU` | Zwischen den Hauptbildschirmen wischen | B-01, B-04, B-05, B-06, B-07 | Kern |
...
### F-27 — Zwischen den Hauptbildschirmen wischen · NEU

> **Neu aus dem Design.** In v1 waren die fünf Hauptbildschirme nur über die untere Leiste
> erreichbar. Der Designer hat zusätzlich eine Wisch-Navigation eingebaut.

- **Auslöser:** Waagerechte Wischbewegung auf B-01, B-04, B-05, B-06 oder B-07.
- **Ablauf:**
  1. Ein Wisch nach links wechselt zum nächsten Hauptbildschirm, ein Wisch nach rechts zum
     vorigen — in der Reihenfolge der unteren Leiste:
     **B-01 Heute → B-04 Ziele → B-05 Merkliste → B-06 Erkenntnisse → B-07 Logbuch**.
  2. Die untere Leiste zieht dabei mit: Das aktive Feld wandert (siehe Motion-Spec §5).
  3. An den Enden der Reihe passiert nichts — es wird **nicht** umlaufend gewechselt.
- **Daten:** Keine. Reine Navigation.
- **Ergebnis:** Derselbe Zustand, als wäre das Feld in der unteren Leiste angetippt worden.
- **Fehlerfall:** Keiner.
- **Regeln/Grenzen:**
  - Gilt **nur** für die fünf Hauptbildschirme. B-02 (Gespräch), B-03 (Auswertung),
    B-08 (Einstellungen) und B-09 (Selbstbild) sind über den Zurück-Pfeil zu verlassen und
    reagieren **nicht** auf Wischen.
  - Wischen darf nicht mit waagerechten Bewegungen **innerhalb** eines Bildschirms
    kollidieren — insbesondere nicht mit dem Wischen zum Löschen auf B-05 (F-19). Dort hat
    die Geste auf dem Listeneintrag Vorrang.
...
## Neu aus dem Design — beim Rückimport aufgelöst

Werft hat sieben Bedienelemente gemeldet, die es im Erst-Spec scheinbar nicht gab, und ihnen
`F-27` bis `F-33` gegeben. Der Abgleich gegen v1 zeigt: **sechs davon sind keine neuen
Funktionen.** Sie standen bereits im v1-UI-Spec, dort aber in Fließtext beschrieben und ohne
eigene `F-`Kennung — deshalb konnte Werft sie nicht zuordnen. Sie bekommen daher **keine**
neuen Kennungen, sondern werden an ihre bestehende Funktion gehängt.

Zusätzlich hatte Werft `F-27` **doppelt** vergeben (einmal für die Wisch-Navigation in der
Überblickstabelle, einmal für den Erscheinungs-Schnellschalter). Auch das ist hier aufgelöst.

| Werft-Kennung | Bildschirm | Element | Tatsächliches Ziel | Woher |
|---------------|------------|---------|--------------------|-------|
| F-27 *(Tabelle)* | B-01 Heute | Erscheinungs-Schnellschalter | **F-26** — Werft hat F-26 selbst bereits um diesen Schalter erweitert (siehe §2) | v1 §F-26 |
| F-28 | B-01 Heute | „Lieber tippen" | **F-01** — die Tastatur-Alternative zur Spracheingabe | v1 UI-Spec B-01, Zustand `LEER` |
| F-29 | B-03 Auswertung | Auswertungstext bearbeiten | **F-10** — das bearbeitbare Feld nach der Transkription | v1 §F-10 Schritt 2 |
| F-30 | B-03 Auswertung | „Überspringen" | **F-10** — führt zum nächsten offenen Experiment | v1 §F-10 Regeln/Grenzen |
| F-31 | B-07 Logbuch | Reiter „Letzte 15 Tage" | **B-07** — Ansichtswechsel innerhalb des Bildschirms | v1 UI-Spec B-07 |
| F-32 | B-07 Logbuch | Reiter „Langzeit" | **B-07** — Ansichtswechsel innerhalb des Bildschirms | v1 UI-Spec B-07 |
| F-33 | B-05 Merkliste | „Abbrechen" | **F-19** — der Nein-Zweig der Lösch-Rückfrage | aus dem Markup abgelesen |

**Zu `F-33` wurde nicht gefragt, weil das Design selbst die Antwort gibt:** Der Knopf steht
im `<dialog class="b05-dialog">` mit der Überschrift „Eintrag löschen?", direkt neben einem
zweiten Knopf, der `data-werft-funktion="F-19"` trägt. Er schließt den Dialog, ohne zu
löschen. Das ist abgelesen, nicht erfunden.

**Wirklich neu ist genau eine Funktion:** `F-27` — die **Wisch-Navigation** zwischen den fünf
Hauptbildschirmen (siehe §2). `F-28` bis `F-33` sind damit **nicht vergeben** und bleiben für
künftige Erweiterungen frei.
