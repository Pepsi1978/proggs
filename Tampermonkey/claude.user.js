...
// @downloadURL  https://raw.githubusercontent.com/Pepsi1978/proggs/main/Tampermonkey/claude.user.js
// @updateURL    https://raw.githubusercontent.com/Pepsi1978/proggs/main/Tampermonkey/claude.user.js
...
(() => {
	...
	const gmGetValue =
		typeof GM_getValue === "function" ? GM_getValue : GM?.getValue?.bind(GM);
	const gmSetValue =
		typeof GM_setValue === "function" ? GM_setValue : GM?.setValue?.bind(GM);
	const gmMenu =
	...
	async function setStoredApiKey(value) {
		...
		await Promise.resolve(
			gmSetValue(GEMINI_KEY_STORAGE, String(value || "").trim()),
		);
		return true;
	}
	...
	async function promptForApiKey() {
		...
		const next = window.prompt(
			"Bitte Gemini API-Key eingeben (lokal in Tampermonkey gespeichert):",
			current || "",
		);
	...
	if (gmMenu) {
		gmMenu("🔑 Gemini-Key setzen/ändern", async () => {
			const next = await promptForApiKey();
			if (next === null) return;
			showToast(next ? "✅ Gemini API-Key gespeichert." : "API-Key gelöscht.");
		});
		gmMenu("🧹 Gemini-Key löschen", async () => {
			await Promise.resolve(gmSetValue(GEMINI_KEY_STORAGE, ""));
			showToast("🧹 Gemini API-Key gelöscht.");
		});
		gmMenu("🎙️ Groq-Key setzen/ändern", () => {
			const key = (prompt("Neuen Groq API-Key eingeben:") || "").trim();
			if (key) {
				if (gmSetValue) gmSetValue("groqKey", key);
				alert("✅ Groq API-Key gespeichert.");
			} else {
				alert("⚠️ Kein Key eingegeben.");
			}
		});
		gmMenu("🗑️ Groq-Key löschen", () => {
			if (gmSetValue) gmSetValue("groqKey", "");
			alert("🧹 Groq API-Key gelöscht.");
		});
		gmMenu("🔄 Auto-Korrektur [AN/AUS]", async () => {
			CFG.autoGeminiCorrection = !CFG.autoGeminiCorrection;
			await Promise.resolve(
				gmSetValue("autoGeminiCorrection", CFG.autoGeminiCorrection),
			);
			showToast(
	...
	(async () => {
		if (gmGetValue) {
			const stored = await Promise.resolve(
				gmGetValue("autoGeminiCorrection", true),
			);
	...
	function isLikelyClaudePromptInput(el) {
		...
		if (id.includes("prompt-textarea") || dt.includes("prompt-textarea"))
			return true;
	...
	function findPrompt() {
		const direct = [
			...
			.filter(Boolean)
			.find((el) => isVisible(el) && isReasonableComposerSize(el));
	...
	function getUserTargetEditable() {
		...
		if (isEditableTarget(lastUserEditable) && isVisible(lastUserEditable))
	...
	function getProseMirrorRoot(el) {
		...
		if (closest && isVisible(closest) && isContentEditableLike(closest))
			return closest;
	...
	async function setViaPaste(el, text) {
		...
		return (
			normalizeForCompare(readPromptText(targetEl)) === targetCmp ||
			normalizeForCompare(readPromptText(targetEl)).length >=
				Math.min(20, targetCmp.length)
		);
	}
...
`.trim();
	...
	_domObserver = new MutationObserver(() => scheduleEnsureUI());
	try {
