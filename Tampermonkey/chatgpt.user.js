...
(() => {
	...
	function _tmGetValue(name, fallback) {
		try {
			...
			console.debug("[TM] _tmGetValue error:", e);
		}
		return fallback;
	}
	...
	function _tmSetValue(name, value) {
		try {
			...
			console.debug("[TM] _tmSetValue error:", e);
		}
	}
	...
	function getGeminiKey() {
		...
		if (!key) {
			key = (
				prompt(
					"Bitte Gemini API-Key eingeben (wird nur in Tampermonkey gespeichert):",
				) || ""
			).trim();
	...
	function getGroqKey() {
		...
		if (!key) {
			key = (
				prompt(
					"Bitte Groq API-Key eingeben (kostenlos auf groq.com, wird nur in Tampermonkey gespeichert):",
				) || ""
			).trim();
	...
	(function registerMenus() {
		try {
			...
			console.debug("[TM] registerMenus error:", e);
		}
	})();
	...
	const initialAutoGeminiCorrection =
		(typeof GM_getValue === "function"
			? GM_getValue("autoGeminiCorrection", true)
			: true) !== false;
	...
	function setGeminiModel() {
		const current =
			(_tmGetValue("geminiModel", "") || "").trim() || GEMINI_MODEL_DEFAULT;
		let model = (
			prompt("Gemini-Modell eingeben (aktuell: " + current + "):", current) ||
			""
		).trim();
		if (model) {
			_tmSetValue("geminiModel", model);
			alert(
				"✅ Gemini-Modell gespeichert: " +
					model +
	...
	function resetGeminiModel() {
		...
		alert(
			"✅ Gemini-Modell zurückgesetzt auf Standard:\n" +
	...
	const UI_IDS = {
		...
		autoEnter: "tm_chatgpt_btn_auto_enter",
		copy: "tm_chatgpt_btn_copy",
		paste: "tm_chatgpt_btn_paste",
	};
...
`.trim();
	...
	function showToast(msg, ms = 5500) {
		try {
			...
			console.debug("[TM] showToast error:", e);
		}
	}
	...
	function findComposerPrompt() {
		...
		const candidates = [
			...
			...document.querySelectorAll(
				"div#prompt-textarea[contenteditable='true'], div[data-testid='prompt-textarea'][contenteditable='true']",
			),
		].filter(isVisible);
	...
	function resolveEditable(el) {
		...
		if (isEditableTarget(el) && (isTextInput(el) || isContentEditableEl(el)))
			return el;
	...
	function getUserTargetEditable() {
		...
		if (resolveEditable(lastUserEditable) && isVisible(lastUserEditable))
			return lastUserEditable;
	...
	async function reactNudge(el) {
		...
		try {
			...
			console.debug("[TM] execCommand fallback error:", e);
		}
	}
	...
	function moveCaretToEnd(el) {
		try {
			...
			console.debug("[TM] moveCaretToEnd error:", e);
		}
	}
	...
	async function setViaPaste(el, text) {
		...
		if (isTextInput(el)) {
			...
			if (
				got.length >= Math.min(500, Math.floor(want.length * 0.5)) &&
				want.startsWith(got)
			) {
				showToast(
					`⚠️ Prompt vermutlich gekürzt.\nEingefügt: ${got.length} Zeichen von ${want.length}`,
					7000,
				);
				return true;
			}
	...
	function geminiGenerate(
		...
		if (!apiKey) {
			return Promise.reject(
				"API-Key fehlt. (Tampermonkey-Menü: „Gemini-Key setzen/ändern“)",
			);
		}
...
`.trim();
	...
	function applyButtonPosition(button) {
		...
		if (!vv || !Number.isFinite(vv.width) || !Number.isFinite(vv.height)) {
			setUiStyle(
				button,
				"right",
				`calc(env(safe-area-inset-right, 0px) + ${rightPx}px)`,
			);
			setUiStyle(
				button,
				"bottom",
				`calc(env(safe-area-inset-bottom, 0px) + ${bottomPx}px)`,
			);
		...
		setUiStyle(
			button,
			"left",
			`${Math.max(UI_MIN_EDGE_GAP, Math.round(left))}px`,
		);
		setUiStyle(
			button,
			"top",
			`${Math.max(UI_MIN_EDGE_GAP, Math.round(top))}px`,
		);
	...
	function startListening() {
		...
		navigator.mediaDevices
			...
			.catch((err) => {
				...
				showToast(
					"\u274c Mikrofon-Zugriff fehlgeschlagen:\n" + String(err),
					8000,
				);
				setTimeout(() => setMicState("idle"), 3000);
			});
	...
	async function runMemoryPrompt() {
		...
		try {
			...
			if (!ok) {
				...
				showToast(
					"❌ Eingabefeld hat den Memory-Prompt nicht übernommen.",
					6500,
				);
				setTimeout(() => setMemBtnState("idle"), 2500);
				return;
	...
	function mountOrRepairUI() {
		...
		if (!micBtn.isConnected || micBtn.parentNode !== mountNode)
		...
		if (!enterBtn.isConnected || enterBtn.parentNode !== mountNode)
			mountNode.appendChild(enterBtn);

		// Paste (über Enter)
		pasteBtn = getOrCreateButton(UI_IDS.paste);
		styleRoundButton(pasteBtn, 0, 104);
		preventFocusSteal(pasteBtn);
		...
		if (!pasteBtn.isConnected || pasteBtn.parentNode !== mountNode)
			mountNode.appendChild(pasteBtn);

		// Copy (über Paste)
		copyBtn = getOrCreateButton(UI_IDS.copy);
		styleRoundButton(copyBtn, 0, 156);
		preventFocusSteal(copyBtn);
		...
		if (!copyBtn.isConnected || copyBtn.parentNode !== mountNode)
			mountNode.appendChild(copyBtn);

		// Gemini Toggle (über Copy)
		...
		if (
			!geminiToggleBtn.isConnected ||
			geminiToggleBtn.parentNode !== mountNode
		)
			mountNode.appendChild(geminiToggleBtn);
		...
		if (!clearBtn.isConnected || clearBtn.parentNode !== mountNode)
			mountNode.appendChild(clearBtn);

		// Prompt-Builder (Frank) über Clear
		...
		if (!promptBtn.isConnected || promptBtn.parentNode !== mountNode)
		...
		if (!promptBtn2.isConnected || promptBtn2.parentNode !== mountNode)
		...
		if (!memBtn.isConnected || memBtn.parentNode !== mountNode)
	...
	function startUiWatchdog() {
		...
		try {
			...
			console.warn("[TM] Observer setup failed:", e);
		}
		...
		try {
			...
			console.warn("[TM] History hooks setup failed:", e);
		}
	...
	function boot() {
		...
		showToast(
			"✅ Script aktiv. 💾 + ❌ + 🎙️ + G + ✨ + 🪄 unten rechts.\nTipp: erst ins Ziel-Eingabefeld klicken, dann Button drücken.",
			3200,
		);
