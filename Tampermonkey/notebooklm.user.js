...
(() => {
	...
	function setGeminiKey() {
		...
		if (key) {
			try {
				GM_setValue(GEMINI_API_KEY_STORAGE, key);
			} catch {}
	...
	function getGeminiApiKey() {
		const storedKey =
			typeof GM_getValue === "function"
				? GM_getValue(GEMINI_API_KEY_STORAGE, "")
				: "";
		...
		if (trimmed) {
			if (typeof GM_setValue === "function")
				GM_setValue(GEMINI_API_KEY_STORAGE, trimmed);
...
`.trim();
	...
	function startUiWatchdog() {
		...
		try {
			...
			_domObserver = new MutationObserver(() => {
				if (
					!document.getElementById("tm-notebooklm-mic") ||
	...
	function boot() {
		...
		try {
			...
			console.error("[STT-notebooklm] mountOrRepairUI:", e);
		}
		try {
			startUiWatchdog();
		} catch (e) {
			console.error("[STT-notebooklm] startUiWatchdog:", e);
		}
		showToast(
