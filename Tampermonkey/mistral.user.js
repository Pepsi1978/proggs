...
// @updateURL    https://raw.githubusercontent.com/Pepsi1978/proggs/main/Tampermonkey/mistral.user.js
...
(() => {
	...
	function clearGeminiKey() {
		try {
			GM_setValue(STORAGE_KEYS.geminiApiKey, "");
		} catch {}
	...
	function pickEditableFromEvent(e) {
		...
		if (Array.isArray(path)) {
			for (const node of path) {
				...
				if (resolved && isEditableTarget(resolved) && isVisible(resolved))
					return resolved;
			}
		}
	...
	async function setViaPaste(el, text) {
		...
		if (isTextInput(el)) {
			...
			await sleep(40);
			return (
				equalTextStructureAware(readPromptText(el), target) ||
				equalTextLoose(readPromptText(el), target)
			);
		}
		...
		try {
			if (navigator.clipboard?.writeText) {
				await navigator.clipboard.writeText(target);
				try {
					changed = !!document.execCommand("paste");
				} catch {}
			}
		} catch {}
	...
	async function resolveGeminiApiKey() {
		...
		if (!key.trim()) {
			const entered = window.prompt(
				"Bitte gib deinen Gemini API-Key ein (wird lokal in Tampermonkey gespeichert):",
			);
...
`.trim();
	...
	async function runPromptBuilder() {
		...
		try {
			...
			if (!ok) {
				...
				setPromptBtnState("idle");
				showToast(
					"⚠️ Automatisches Einfügen wurde blockiert/uneindeutig erkannt.\nPrompt liegt in der Zwischenablage → bitte manuell einfügen (Strg+V).",
					9000,
				);
				return;
			}
	...
	try {
		...
		_domObserver = new MutationObserver(() => ensureUI());
	...
	_uiInterval = setInterval(ensureUI, 1200);
})();
