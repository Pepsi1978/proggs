    ...
    suspend fun backupCustomAnalyses(): Result<Unit> {
        return try {
            ...
            if (jsonResult.isSuccess) {
                ...
                Log.d(
                    "SyncDebug",
                    "Custom analyses backed up to Drive (${list.size} entries)",
                )
            } else {
                Log.e(
                    "SyncDebug",
                    "Custom analyses backup failed: ${jsonResult.exceptionOrNull()?.message}",
                )
            }
            jsonResult
        } catch (e: Exception) {
            Log.e("SyncDebug", "Custom analyses backup exception: ${e.message}", e)
            Result.failure(e)
        }
    }
