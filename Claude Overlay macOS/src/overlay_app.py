...
from claude_window import (
    clear_input,
    is_claude_running,
    is_target_app_frontmost,
    paste_text,
)
from config import Settings

# ---------------------------------------------------------------------------
# Logging (sicher fuer Hintergrundprozesse, wo sys.stderr = None sein kann)
# ---------------------------------------------------------------------------
...
class ClaudeOverlayApp:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # macOS: Fenster nicht im Dock anzeigen
        # (wird durch overrideredirect(True) bereits erreicht)
        ...
        # macOS-kompatible Emoji-Schriftart
        ...
        gemini_font = tkfont.Font(family="Helvetica Neue", size=7, weight="bold")
        ...
        close_font = tkfont.Font(family="Helvetica Neue", size=7, weight="bold")
        self.close_text = self.canvas.create_text(
            close_cx, close_cy, text="\u2715", font=close_font, fill="#888888",
        )

        # ----- Statustext -----
        status_font = tkfont.Font(family="Helvetica Neue", size=7)
        ...
        # macOS: Rechtsklick kann auch Ctrl+Klick sein (Button-2 auf macOS)
        ...
        # Vordergrund-Tracking starten
        self.root.after(500, self._track_foreground)

        # Periodisch topmost sicherstellen
    ...
    def _process_audio_pipeline(self, audio_path) -> None:
        transcript = None
        try:
    ...
    def _paste_and_finish(self, text: str) -> None:
        ...
        try:
            paste_text(text, settings=self.settings, tk_root=self.root)
    ...
    # ------------------------------------------------------------------
    # Besen-Button: Eingabefeld leeren
    # ------------------------------------------------------------------
    def _clear_input(self) -> None:
        try:
            clear_input(settings=self.settings)
    ...
    # ------------------------------------------------------------------
    # Vordergrund-Fenster-Tracking (macOS via AppleScript)
    # ------------------------------------------------------------------
    def _track_foreground(self) -> None:
        """Blendet Overlay bei Fokuswechsel aus/ein."""
        if is_target_app_frontmost(self.settings):
            self._show_overlay()
        else:
            self._hide_overlay()
        self.root.after(500, self._track_foreground)
