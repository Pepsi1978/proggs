...
class WhisperHallucinationFilter {
    fun filter(result: GroqTranscriptionResponse, analysis: SpeechAnalysis?): String {
        ...
        val aligned = if (analysis == null) confident else confident.filter { segment ->
        ...
        return blockIfFloskel(kept.joinToString(" ") { it.text?.trim().orEmpty() }.trim(), analysis)
    }
    ...
    private fun isBlocklistedFloskel(text: String, analysis: SpeechAnalysis?): Boolean {
        ...
        val normalized = buildString(text.length) {
            text.lowercase().forEach { append(if (it.isLetter()) it else ' ') }
        }.split(' ').filter(String::isNotEmpty).joinToString(" ")
        return normalized.isNotEmpty() &&
            normalized.split(' ').size <= FLOSKEL_MAX_WORDS &&
            normalized in FLOSKEL_BLOCKLIST &&
            analysis.voicedMs < SILENCE_CONTEXT_MAX_VOICED_MS
    }

    private companion object {
        const val NO_SPEECH_THRESHOLD = 0.6
        const val AVG_LOGPROB_THRESHOLD = -1.0
        const val COMPRESSION_RATIO_THRESHOLD = 2.4
        const val MINI_NOISE_MAX_SECONDS = 0.4
        const val FLOSKEL_MAX_WORDS = 8
        const val FLOSKEL_MAX_CHARS = 64
        const val SILENCE_CONTEXT_MAX_VOICED_MS = 600
        val FLOSKEL_BLOCKLIST = setOf(
            "vielen dank",
