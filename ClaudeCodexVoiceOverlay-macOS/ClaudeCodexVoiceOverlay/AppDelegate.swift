...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NSLog("ClaudeCodexVoiceOverlay started")
        ...
        //    InputController.clearLine() aufgerufen. Das hat jeden Auftrag
        ...
        panel.onXMouseDown = { [weak self] in
            ...
            // Erstes Loeschen mit voller Maschinerie: aktiviert die Ziel-App +
            // wartet 150 ms + sendet Cmd+A + Backspace. Danach ist die App aktiv
            // und wir koennen direkt nachfeuern.
            self.clearLine()
            // Repeat-Timer: alle 100 ms direkt Cmd+A + Backspace senden, OHNE
            // Queue-Umweg. Vor jedem Tick pruefen wir ob die linke Maustaste noch
            // gedrueckt ist — wenn nicht, sofort stoppen. Kein Backlog moeglich
            // weil sendKeyCombo synchron innerhalb von Mikrosekunden zurueckkehrt.
            // Bei Electron-Apps (einzelnes Eingabefeld) markiert Cmd+A den
            // gesamten Inhalt und Backspace loescht ihn — Wiederholung ist ein
            // harmloser No-Op wenn das Feld bereits leer ist.
            ...
            self.xRepeatTimer = Timer.scheduledTimer(withTimeInterval: 0.10, repeats: true) { [weak self] _ in
                ...
                // Activation. 0x00 = 'a' (Cmd+A markiert alles), 0x33 = Backspace.
                InputController.sendKeyCombo(keyCode: 0x00, flags: .maskCommand) // Cmd+A
                InputController.sendKeyCombo(keyCode: 0x33, flags: [])           // Backspace
            }
        }
    ...
    private func handleInputSubmit(_ middleText: String) {
        ...
        InputController.pasteText(final, autoEnter: autoEnterEnabled)
    ...
    private func ensurePromptBoardInstance() {
        ...
        p.onInsertText = { [weak self] text in
            ...
            InputController.pasteText(text, autoEnter: self.autoEnterEnabled)
        }
            ...
            DispatchQueue.main.async {
                ...
                case .success(let corrected):
                    ...
                    DispatchQueue.global(qos: .userInitiated).async {
                        ...
                        usleep(120_000)
                        InputController.pasteText(finalText, autoEnter: shouldAutoEnter)
                    }
    ...
    private func insertLastScreenshot() {
        ...
        InputController.pasteText(toPaste, autoEnter: false)
    ...
    private func pastePromptByLetter(_ letter: Character) {
        do {
            ...
            InputController.pasteText(text, autoEnter: autoEnterEnabled)
        } catch {
