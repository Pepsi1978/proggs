...
///
/// Vollstaendige Portierung von ClaudeVoiceOverlay-Windows/Services/AppController.cs
/// bzw. TerminalVoiceOverlay-Windows/Services/TerminalController.cs.
...
final class TerminalController {
    ...
    static func clearLine(completion: ((Bool) -> Void)? = nil) {
        sendQueue.async {
            ...
            guard sendKeyCombo(keyCode: 0x00, flags: .maskCommand) else {
                report(completion, false, "clearLine: Cmd+A fehlgeschlagen"); return
            }
            usleep(50_000)
            // … Backspace (0x33) deletes the selection.
            report(completion, sendKeyCombo(keyCode: 0x33, flags: []), nil)
        }
    }
    ...
    static func clearAllInput(completion: ((Bool) -> Void)? = nil) {
        sendQueue.async {
            ...
            for _ in 0..<2 {
                if !sendKeyCombo(keyCode: 0x00, flags: .maskCommand) { // Cmd+A
                    report(completion, false, "clearAllInput: Cmd+A fehlgeschlagen")
                    return
                }
                usleep(50_000)
                if !sendKeyCombo(keyCode: 0x33, flags: []) {           // Backspace
                    report(completion, false, "clearAllInput: Backspace fehlgeschlagen")
                    return
                }
                usleep(50_000)
            }
